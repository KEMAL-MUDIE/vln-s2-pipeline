#!/usr/bin/env python3
"""
Gate 4 Visual v32 — No Midpoints for Goal Segment (reduce "through the" from 39.5%)

IMPROVEMENTS OVER v31:
  v31 reduced "walk past the" from 26.9% to 18.7% (good) but "through the" increased
  from 33.3% to 39.5% (bad). Root cause: removing goal [pass:] kept goal [thru:],
  which still adds "through the X" near the stop phrase.

  This version: ELIMINATE ALL MIDPOINTS for goal segment (neither [thru:] nor [pass:]).
  Turn midpoints are unchanged (strict whitelist, [thru:] and [pass:] for distinctive).
  Goal approach: VLN model uses stop phrase directly for final navigation.

  Expected:
    "through the": ~32-35% (down from 39.5%, back to v30 level)
    "walk past the": ~18-19% (unchanged from v31)
    Combined improvement: closest to GT 27.2%/8.7% of any version so far.

  All v31 innovations retained:
  ...
  GT analysis reference (v31 actuals):
    "through the":   GT=27.2%, v31=39.5%  → v32 target: ~32-35%
    "walk past the": GT=8.7%,  v31=18.7%  → v32: ~18-19% (unchanged)
    "wooden":        GT=3.3%,  v31=0.1%   ✓
    "continue":      GT=10.4%, v31=10.0%  ✓
    GT-match:        v31=0.952 ✓

  Phase 2 checkpoint: outputs/gate4_v32_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v32.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v32.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v32_phase2_checkpoint.json"
...
async def main():
    ...
    print("=== Gate 4 v32: No Goal-Segment Midpoints (reduce through-the from 39.5%) ===")
    ...
    print(f"  v32 key change: eliminate ALL goal-segment midpoints (both [thru:] and [pass:])."
    ...
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v32 should be <1%")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v31=0.952, v26=0.955)")
    ...
    print(f"  [Note: v32 no goal-segment midpoints (both [thru:] and [pass:] suppressed)]")
