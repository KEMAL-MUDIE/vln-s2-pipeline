#!/usr/bin/env python3
"""
Gate 4 Visual v121 — Fix massive motion-word overcorrection discovered in v120 analysis.
v120 results: through_the=28.8%, walk_past=7.5%, continue=9.8%, avg_words=26.9w (excellent).
BUT: "go forward"=80.3% (GT=1.7%), "keep going"=37.7% (GT~0%), "walk forward"=40.7% (GT=6.5%).
     "walk into"=35.9% (GT=7.7%). HUGE vocabulary gaps larger than through_the/walk_past!
GT prefers: "walk straight"=9.5%, "go straight"=9.0%, "go to"=8.6%, "walk to"=5.9%.

Fix 1 — MOTION WORDS: Remove "go forward"/"walk forward"/"keep going"; use "walk straight"/"go straight".
Fix 2 — DISTANCE rule: Use "walk straight"/"go straight" not "go forward"/"keep going".
Fix 3 — (→ room) notation: Reduce frequency 72%→30%; vary away from "walk into" → "go to"/"walk to".
Fix 4 — clean(): "go forward"→"walk straight", "keep going"→"go straight" post-processing.

Expected: "go forward"→~5%, "keep going"→~2%, "walk forward"→~7%, "walk straight"→~9%, "go straight"→~9%.
Keep: [pass: X] P=0.70, P(door→room)=28%, P(through_hallway)=16%, no had_pass suppression.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v121.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v121_phase2_checkpoint.json"  # v121 — fix motion-word overcorrection
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    ...
                    # v121: 28% doorway notation; 30% room entry (was 72% → "walk into" 35.9% vs GT=7.7%)
                    # Reducing (→ rm) from 72% to 30% brings "walk into" closer to GT=7.7%.
                    r = rng.random()
                    if r < 0.28:
                        parts.append(f"(door→ {rm})")
                    elif r < 0.58:  # additional 30% → (→ rm) with varied phrase in SYSTEM_INTRO
                        parts.append(f"(→ {rm})")
                    else:
                        parts.append(f"turn {direction}" if direction else "")
            else:
                parts.append(f"turn {direction}")
            turn_idx += 1
        elif t == "elevation":
...
- When you see (→ room) in the route: describe entering that room naturally — vary the phrase: "go to the [room]", "head to the [room]", "walk to the [room]", "go into the [room]". Do NOT always use "walk into". Do NOT write "enter the [room]". Keep it brief (3-5 words).
...
- MOTION WORDS: Prefer "walk straight", "go straight", "walk to the X", "go to the X", "walk down", "head toward". AVOID overusing "go forward", "walk forward", "keep going" — these are unnatural. Do NOT use "continue" or "proceed".
- DISTANCE: When the route shows a 'straight Xm' segment — NEVER write metric numbers. Use natural language: "take a few steps" (≤2m), "walk straight" or "go to the [next landmark]" (3–5m), "walk straight ahead" or "walk down the hallway" (6–9m), "go straight" (≥10m).
...
def clean(raw, sv):
    ...
    _ws_variants = [
        (r'\bwalk straight for a long while\b', ['go straight a long way', 'walk forward a long way']),
        (r'\bwalk straight for a while\b',      ['go straight for a while', 'walk forward for a while']),
        (r'\bwalk straight a few\b',             ['go straight a few', 'walk a few']),
        (r'\bwalk straight\b',                   ['go straight', 'walk forward']),
    ]
    ...
    result = re.sub(r'\bcontinue\s+to\s+the\b', 'walk to the', result)
    result = re.sub(r'\bContinue\s+for\b', 'Go straight for', result)
    result = re.sub(r'\bcontinue\s+for\b', 'go straight for', result)
...
async def main():
    ...
    print(f"  v121 KEY FIX: motion-word overcorrection (go_forward=80.3%→~5%, keep_going=37.7%→~2%)")
    print(f"  v121: _ws_variants removes go_forward/keep_going; clean() converts residual keep_going→go_straight")
