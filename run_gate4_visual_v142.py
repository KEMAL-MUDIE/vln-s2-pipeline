#!/usr/bin/env python3
"""
Gate 4 Visual v142 — "go straight" → "walk forward" (safer than v140's "walk ahead"/"walk a bit").

v140 ACTUAL: through_the=27.6%, hallway=20.6%, into_hallway=4.5%, continue=10.4%(PERFECT!),
             walk_past=8.6%, avg_words=25.5w, vocab_gap=0.8pp → pred_SR=63.40%

v140 PROBLEM: "walk ahead"=62.64% (GT=0.16%, 387x overuse!) + "walk a bit"=48.83% (GT=0.05%).
  Replacing OOD "go straight" (81%, GT=7.94%, 10x overuse) with even MORE OOD phrases is worse.
  "walk forward" (GT=6.09%) is a much better replacement: less OOD, more natural.

v141 FAILED: FRESH gen with Phase2 prompt changes → continue=22.1% (catastrophic), through_the=22.6%,
  avg_words=23.3w. Prompt changes caused cascading failures across multiple metrics.
  pred_SR=51.7% (20pp vocab_gap) → NOT deployed, NOT evaluated.

v142 STRATEGY: Same as v140 but convert "go straight" → "walk forward" instead of "walk ahead"/"walk a bit".
  "walk forward" is GT=6.09% (plausible) vs "walk ahead" (GT=0.16%) and "walk a bit" (GT=0.05%).
  Still overused vs GT (60%+ of episodes will have "walk forward"), but less extreme.
  Reuse v139's P2 checkpoint — no new LLM calls.

Expected v142:
  - "go straight": 81% → ~0% (converted by post-proc)
  - "walk forward": 11.69% → ~60%+ (replaces go straight; still overuse vs GT=6.09%)
  - 5-metric: same as v140 (through_the≈27.6%, hallway≈20.6%, continue=10.4%, walk_past=8.6%)
  - vocab_gap: ~0.8pp → pred_SR≈63.40%
  - actual SR: may improve over v140 since "walk forward" is less OOD than "walk ahead"/"walk a bit"

Fix 1 — Phase 2 checkpoint: REUSE v139 (gate4_v139_phase2_checkpoint.json)
Fix 2 — Post-proc: "go straight ahead"→"walk forward", "go straight"→"walk forward" (AFTER cont restoration)
Fix 3 — All P_ parameters: UNCHANGED from v139/v140
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v142.json.gz"
...
async def main():
    ...
    print('=== Gate 4 v142: go_straight → walk_forward (safer than v140 walk_ahead/walk_a_bit, reuse v139 P2) ===')
    ...
    for ep in episodes:
        ...
        # v142: Convert "go straight" → "walk forward" (GT=6.09%, more natural than v140's "walk ahead"=0.16%).
        # Must run AFTER continue restoration so "Walk forward" here is NOT restored to "Continue forward".
        # v140 lesson: "walk ahead"=62% and "walk a bit"=49% were more OOD than "go straight" itself.
