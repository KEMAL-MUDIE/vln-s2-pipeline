#!/usr/bin/env python3
"""
Gate 4 Visual v53 — v51 base + soft through_the quality nudge (distinctive over generic)

v51 RESULTS (base):
  GT-match: 0.961 (best!), avg_words: 26.2 (GT=26.8), through_the: 32.5% (GT=27.2%)
  hallway: 27.6% (GT=20.4%), walk_past: 15.0% (GT=8.7%), turns: 0.71 (GT=0.59)

v52 TESTED: Blanket prohibition "write through_the ONLY from markers" — REGRESSION.
  GT-match dropped to 0.952, through_the dropped to 13.1% (too low, went below GT=27.2%).
  Root cause: prohibition was too strong, LLM avoided the phrase even when [thru:] was present.

v53 FIX: Replace blanket prohibition with quality nudge:
  - Keep [thru:] rule but add guidance: prefer DISTINCTIVE descriptions over "through the doorway"
  - Examples: "through the arched entrance", "through the wooden doorframe", "through the double doors"
  - Reserve generic "through the doorway" only when description is specifically a plain doorway
  - This should reduce "through the doorway" (23.5% eps) while preserving distinctive variants (9%)
  - Expected: through_the 32.5% → ~27-28% (closer to GT=27.2%)

All v51 improvements retained (word budgets, hidden rm, rm turn sampling, hallway suppression).

Phase 2 checkpoint: outputs/gate4_v53_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v53.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v53.json.gz"
...
async def main():
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v51=0.71, v47=0.99) ← v53: v51+soft thru nudge")
