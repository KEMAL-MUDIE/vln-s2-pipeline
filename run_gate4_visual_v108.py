#!/usr/bin/env python3
"""
Gate 4 Visual v108 — Enable spontaneous "passing the" for straight segments + fix stop phrases.

v107 ANALYSIS (actual results):
  - "passing the": 28.9% (STILL LOW! v24=79.7%)  ← Root cause: only 39.4% of eps have
                                                    distinctive midpoints; also prompt BLOCKS
                                                    spontaneous LLM "passing the" generation!
  - "continue":    43.7% (IMPROVED from 20.1%)  ← v107 fix worked!
  - "straight":    145.6% (overused — too many per ep)
  - "walk into":   73.9% (too high, v24=52.6%)
  - "hallway":     49.6% (good)
  - "turn left":   45.7% (close, v24=51.6%)

ROOT CAUSE OF LOW "passing the":
  1. Only 39.4% of episodes have DISTINCTIVE midpoints (beds, windows, plants are generic).
     Even with P_PASS_MARKER=0.80: 39.4% × 0.80 = ~31.5% max from [pass:] markers.
  2. Visual context header EXPLICITLY BLOCKS spontaneous LLM generation:
     "Visual context (background only — do NOT use these to add landmarks...)"
     But v24 achieved 79.7% SPONTANEOUSLY from visual context!
  3. The system prompt only permits "passing the" when route has [pass:] markers.
     So episodes without distinctive midpoints get 0% "passing the".

ROOT CAUSE OF STOP PHRASE FAILURES:
  v95_gpu1 analysis: 136 episodes had OS=1.0 (robot WAS at goal) but NE=3-6m → failed!
  These are pure STOP PHRASE failures. Robot couldn't recognize the goal landmark.
  Current stop phrase: from APPROACH VIEW (1-2 steps before goal) → may reference wrong object.

v108 CHANGES FROM v107:
  1. Enable SPONTANEOUS "passing the" for straight segments:
     - Change visual context header to allow midpoint landmark use in straight sections
     - Add system prompt rule: "In long straight segments, optionally add 'passing the X'"
     - Expected: "passing the" rises from 28.9% → ~65-75% (v24=79.7%)
  2. Add midpoint visual context to the prompt for straight-segment midpoints:
     - Include ALL midpoint objects (not just distinctive) in visual narrative for straight segs
     - Tagged as "(midpoint: X)" so LLM can use it for "passing the X" phrases
     - Expected: LLM sees more objects → generates more "passing the X"
  3. Improve stop phrase generation:
     - Add system prompt rule: never use wall/floor/ceiling as stop anchor
     - Prefer specific furniture, fixtures, or doorways as stop landmarks
     - Keep "Wait there" for truly unspecific endings

  Expected stats:
    passing the:  28.9% → ~65-75%  (v24=79.7%)
    continue:     43.7% → ~50-55%  (maintain v107 improvement)
    SR target: ~45-50%+ (better vocabulary + stop phrase accuracy)

Phase 2 checkpoint: outputs/gate4_v108_phase2_checkpoint.json (NEW -- fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v108.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v108.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v108_phase2_checkpoint.json"  # v108 — enable spontaneous "passing the"
...
P_PASS_MARKER = 0.80   # v106: RESTORE pass markers — but prompt forces "passing the X" not "walk past the X"
                       # v24 had "passing the"=79.7%! We removed this in v103 but it's HELPFUL (motion confirmation)
P_THRU_MARKER = 0.00   # v103: remove thru markers (v24 analysis: through_the HURTS SR)   # v89: increased from 0.90 → 0.95 to push through_the toward GT 27.2%
P_TURN_UNANCHORED = 0.90  # v105: HIGH-TURN — include most unanchored turns (v24 had many turns, high SR)
...
_GENERIC_WPT_OBJ_RE = re.compile(
    ...
    r'long|short|sectional|reclining|upholstered|folding|'  # v77: folding added
    r'kitchen|dining|living|coffee|end|console|'
    r'display|storage|filing|support|decorative|'
    r'framed|carved|ornamental)\s+)*'
    r'(?:table|walls?|sofa|chair|seat|bench|couch|bed|mattress|'
    r'cabinet|cabinet\s+door|dresser|drawer|wardrobe|closet|'
    r'armchairs?|'  # v77: armchair/armchairs explicitly
    ...
    r'light\s+switch(?:es)?|switch(?:es)?|outlet|socket|'  # v76: small fixtures
...
# v107: PARTIAL ROLLBACK — v106 suppressed "Continue straight/forward" which killed "continue" (20%→target 57%).
# v24 analysis: "continue straight"=379 uses, "continue forward"=225 uses — these are GT-like patterns!
# v107: Only suppress "Continue walking" and "Continue moving" (non-GT filler phrases).
# KEEP: "Continue straight", "Continue forward", "Continue ahead" — v24 uses these extensively.
_CONTINUE_MOTION_RE = re.compile(
    # v107: NO-OP — allow "continue straight/forward/ahead" to pass through unchanged.
    # v24 used "continue straight" 379x, "continue forward" 225x — these are GT-like patterns.
    # "Continue walking/moving" is handled by _CONTINUE_FRAG_RE below (→ "Walk straight").
    # Use impossible pattern so _replace_continue_motion never fires.
    r'(?!x)x',
    re.IGNORECASE
)
        ...
        if t == "straight":
            ...
            if seg_midpoint and turn_label in seg_midpoint:
                ...
                if action == "thru":
                    ...
                    if rng.random() < P_PASS_MARKER:  # target ~10% walk-past (GT=8.7%)
                        parts.append(f"[{action}: {mid_desc}]")
                        had_pass_markers = True  # v77: track that this episode got [pass:]
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v106: Restore hallway context for FIRST hallway per episode.
                # v103-v105 suppressed all hallway room labels → "hallway"=23.4%, into_hall=4.2%.
                # v24 had hallway=51.8%, into_hall=33.7% — hallway context helps model navigate.
                # Fix: Allow first hallway transition to include "(→ hallway)" label.
                # Subsequent hallways → just "turn {direction}" (prevent repetition).
                is_hallway = "hallway" in rm.lower() or "corridor" in rm.lower()
                if is_hallway:
                    if hallway_count == 0:
                        # First hallway: include room label for context
                        hallway_count += 1
                        if direction:
                            parts.append(f"(→ hallway) turn {direction}")
                        else:
                            parts.append(f"(→ hallway)")
                    else:
                        # Subsequent hallways: just turn direction (no repetition)
                        if direction:
                            parts.append(f"turn {direction}")
                        # else: no direction and already mentioned hallway — skip
                else:
                    parts.append(f"(→ {rm})")
            else:
                parts.append(f"turn {direction}")
...
- When a turn has → [object ahead] after the arrow: add a BRIEF "walk to the X" phrase (3-5 words) like "walk to the bookshelf" or "continue to the painting" — keep it very short. Do NOT write "walk past the" for → [object ahead]; use "walk to the" or "continue to the" only
- When route has [pass: description]: the robot moves alongside an object — write "passing the X" (2-4 words, gerund form). Example: "passing the brown armchair", "passing the wooden table". Do NOT write "walk past the" or "passing by" — write ONLY "passing the X". Keep it very brief.
- MOTION LANDMARKS: When walking in a straight segment, you MAY add ONE brief "passing the X" phrase for a distinctive object you see along the way (sofa, armchair, painting, staircase, fireplace, mirror, dining table, refrigerator, etc.). Use natural gerund form: "continue straight, passing the grey sofa" or "walk straight, passing the oak dining table". ONLY for physically distinctive objects — NOT for walls, floors, ceilings, or room names. Keep it very brief (3-5 words).
- When you see (→ room) in the route: write "walk into the [room]" — describe entering that room. Do NOT write "enter the [room]". Do NOT write "turn left" or "turn right" for these transitions. Keep it brief (3-5 words).
- Turns with no brackets/arrows: write ONLY "turn left" or "turn right" — no objects, no room names after the turn direction
- CRITICAL: Do NOT add "turn left" or "turn right" unless the route explicitly shows "turn left" or "turn right". If the route has no turn, do NOT write one. Write "walk straight" or "go straight" instead — NOT "walk toward the [room name]".
...
- ROOM TRANSITIONS: When you see (→ room) in the route, briefly describe entering: "walk into the bedroom", "walk into the kitchen", "walk into the bathroom". Do NOT use "enter the" — always use "walk into the". Do NOT write "turn left" or "turn right" for room entries. Use the room name once, briefly (3-5 words). The route will only show meaningful non-hallway rooms — if you see no (→ room) marker, do NOT invent a room name.
- DISTANCE: When the route shows a 'straight Xm' segment — NEVER write metric numbers. Use natural language: "a few steps" (≤2m), "walk straight" (3–5m), "walk straight for a while" (6–9m), "continue straight along the path" (≥10m). R2R annotators never write "3 meters" or "5m" — keep it conversational.
- STYLE: Prefer "walk straight" and "go straight" over "walk forward". Keep instructions clean and turn-focused. Do NOT add "through the X" — only write what is explicitly shown in the route.
- STOP PHRASE: For the final destination, use a SPECIFIC visual landmark (sofa, armchair, counter, plant, doorway, etc.). Do NOT use "wall", "floor", or "ceiling" as stop references — always name a specific object or feature the robot can recognize. Keep stop phrase brief: "Stop near the [X]." or "Wait at the [X]."
...
def build_visual_narrative(lm_info: Dict, sel_turns: List[Dict], vision_map: Dict,
    ...
    if not lines:
        return ""
    # v108: Allow LLM to use visual context for "passing the X" in straight segments.
    # Previously blocked spontaneous landmark use — but v24 achieved 79.7% "passing the"
    # via spontaneous visual context use. This is the key fix for "passing the" coverage.
    header = ("Visual context — use to describe objects passed in straight segments ('passing the X'):\n")
    return header + "\n".join(lines)
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    if n_bracketed > 0 and n_room_trans > 0:
        ...
        anchor_note = (f"Route has {n_bracketed} bracketed landmark(s) and {n_room_trans} "
                       f"room transition(s) marked (→ room) — use landmarks for turn references; "
                       f"for (→ room) entries write 'walk into the [room]' only — do NOT use 'enter the' "
                       f"(no turn direction for room transitions). Use each room name AT MOST ONCE.")
    elif n_bracketed > 0:
        anchor_note = f"Route has {n_bracketed} bracketed landmark(s) — use them naturally."
    elif n_room_trans > 0:
        # v95: restored room-entry language — LLM should mention room transitions naturally
        anchor_note = (f"Route has {n_room_trans} room transition(s) marked (→ room) — "
                       f"for each, briefly describe entering: 'walk into the [room]'. "
                       f"Do NOT use 'enter the' — always use 'walk into the'. "
                       f"Do NOT write 'turn left' or 'turn right' for room entries. "
    ...
    if n_pass == 1:
        anchor_note += (f" Route has 1 midpoint [pass:] marker — write 'passing the X' (gerund, NOT 'walk past the X')."
                        f" Keep it brief: 'passing the brown armchair', 'passing the wooden table'.")
    elif n_pass > 1:
        anchor_note += (f" Route has {n_pass} midpoint [pass:] markers — for each, write 'passing the X' "
                        f"(gerund, NOT 'walk past the X'). Keep each brief: 'passing the X'.")
...
def clean(raw, sv):
    ...
    # Apply BEFORE v106 walk_past→passing conversion to properly suppress generic hallucinations.
    # Generic objects (table, wall, sofa...) → "walk straight"; specific objects → "passing the X"
    raw = _GENERIC_WPT_OBJ_RE.sub('walk straight', raw)
    # v106: Convert remaining "walk past the X" → "passing the X" (v24-style motion confirmation)
    # After _GENERIC_WPT_OBJ_RE suppresses generic objects, what's left = specific/distinctive objects.
    # v24 had "passing the"=79.7% — motion confirmation helps agent stay on path without stop confusion.
    raw = re.sub(r'\bwalk\s+past\s+the\b', 'passing the', raw, flags=re.IGNORECASE)
    raw = re.sub(r'\bgo\s+past\s+the\b', 'passing the', raw, flags=re.IGNORECASE)
    raw = re.sub(r'\bpassed\s+the\b', 'passing the', raw, flags=re.IGNORECASE)
...
def quality_ok(text):
    ...
    if len(words) < 12: return False, "navigation_too_brief"
...
async def main():
    ...
    print('=== Gate 4 v108: Blacklist filter for [pass:] (39%→90% eligible) + allow spontaneous "passing the" ===')
    print(f"  KEY FIX: choose_stop() recalibrated — Stop near: 34.5% → ~5% (GT=1.3%)")
    print(f"  Stop distribution: at=35%, in front of=25%, next to=20%, by=15%, near=5%")
    print(f"  Wait distribution: at=30%, near=22%, by=18%, there=16%, next to=14%")
    print(f"  Also fixes apply_goal_approach_stop() which had same 'near' overuse")
    print(f"  RETAIN: all v83 fixes (continue relax, hallway, clean() fixes, distance rule)")
    print(f"  P_PASS_MARKER={P_PASS_MARKER} (unchanged from v80)")
    ...
    print(f"  KEY v76 FIX: P_PASS_MARKER=0.30 (was 0.50) → walk-past ~10% (was 16.9%, GT=8.7%)")
    ...
    for ep in episodes:
        ...
        task_meta[eid] = {
            ...
            "stop_phrase": stop_phrase,
            "stop_type":   stop_type,
            "had_pass":    had_pass,  # v77: deterministic, from separate build_route_desc call
            "thru_descs":  thru_descs,  # v92: for through_the injection
        }
    ...
    for ep in episodes:
        ...
        if "through the doorway" in text.lower() and not _has_thru_midpoint(eid):
            text = _THRU_DOORWAY_RE.sub("walk straight", text)
            # Clean up "walk straight walk straight" duplicates
            text = re.sub(r'\bwalk straight\s+walk straight\b', 'walk straight', text, flags=re.I)
            # Fix duplicates
            text = re.sub(r'\bwalk straight\s+walk straight\b', 'walk straight', text, flags=re.I)

        # v79: Removed binary had_pass suppression (v77/v78 overcorrected → WP only 1.8-2.7%).
        # Rely on prompt fix alone ([→ X] = "walk to the" only, [pass:] = "walk past the").
        # WPT_OBJ_RE still removes hallucinated generic WP (table, sofa, wall, etc.).
        # Expected WP% ~8-12% (close to GT 8.7%, better than v76 14.8%).

        # v95: Hallway suppression REMOVED — room-entry language now allowed by prompt.
        # LLM will write "walk into the hallway" naturally for (→ hallway) route turns.
        # No filtering or sampling needed.

        # v95: Hallway injection REMOVED — LLM will generate "walk into the hallway" naturally
        # via (→ hallway) route markers and the restored room-entry rule in the prompt.
        # v94 injection was a workaround for the ROOM NAMES ban; now that ban is removed.
        ...
        if cont_rng.random() < 0.30 and _WALK_FWD_CAP_RE.search(text):
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v62=2.0%) ← KEY METRIC")
    print(f"  hallway:       {100*hallway_n/n:.1f}%  (GT=20.4%, v62=19.2%) ← KEY METRIC")
    ...
    print(f"  [v103 targets: no landmarks; through_the → ~1%; walk_past → ~2%; eps_with_turn ~50%; SR target ≥40%]")
