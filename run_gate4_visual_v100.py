#!/usr/bin/env python3
"""
Gate 4 Visual v100 — v99 + hallway suppression: only mention hallway when it is a distinctive transition.
...
Phase 2 checkpoint: outputs/gate4_v100_phase2_checkpoint.json (NEW — fresh LLM calls needed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v100.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v100.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v100_phase2_checkpoint.json"  # NEW — fresh P2 for room-entry prompt
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v99: Remove explicit turn direction from room transitions.
                # GT analysis: "turn * into X" = 3.2%; "walk into X" without turn = 14.2%.
                # v95-v98: "turn {direction} (→ rm)" → LLM writes "turn left into hallway" (41%).
                # This distribution mismatch hurt SR. v99: just "(→ rm)" → "walk into hallway".
                parts.append(f"(→ {rm})")
            else:
...
- When you see (→ room) in the route: write "walk into the [room]" or "enter the [room]" — describe entering that room. Do NOT write "turn left" or "turn right" for these transitions. For HALLWAYS: if you have already mentioned "hallway" earlier in the instruction, write "continue forward" or "keep going" instead of mentioning hallway again. For non-hallway rooms (bedroom, kitchen, bathroom, living room, etc.): always mention the room name once. Keep it brief (3-5 words).
...
- ROOM TRANSITIONS: When you see (→ room) in the route, briefly describe entering: "walk into the bedroom", "enter the kitchen", "go to the bathroom". Do NOT write "turn left" or "turn right" for room entries. Use each room name AT MOST ONCE. For HALLWAYS specifically: describe entering only if it is the first hallway in the route — if the route has multiple (→ hallway) transitions, only mention it once (e.g., "walk down the hallway"), then for subsequent hallway entries just use "continue forward" or "walk further". Brief (3-5 words) room-entry is enough.
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    if n_bracketed > 0 and n_room_trans > 0:
        ...
        anchor_note = (f"Route has {n_bracketed} bracketed landmark(s) and {n_room_trans} "
                       ...
                       f"for (→ room) entries write 'walk into the [room]' or 'enter the [room]' "
        ...
        anchor_note = (f"Route has {n_room_trans} room transition(s) marked (→ room) — "
                       f"for each, briefly describe entering: 'walk into the [room]' or 'enter the [room]'. "
...
async def main():
    ...
    print(f"  [v100 targets: hallway suppressed when repeated; hallway% → ~25% (vs v99 50.1%, GT 20.4%)]")
