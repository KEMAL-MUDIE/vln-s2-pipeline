#!/usr/bin/env python3
"""
Gate 4 Visual v129 — Re-calibration: measured LLM compliance from v128 + rename (hall_into→) → (→ hallway).

v128 ACTUAL results: through_the=24.0% (GT=27.2%), hallway=19.0% (GT=20.4%), into_hallway=2.6% (GT=4.4%).
                     continue=11.2% (GT=10.4%), avg_words=25.3w (GT=26.8w).

v128 ANALYSIS:
  - Extended Stage 1 pattern caught "walk straight through the hallway" in unannotated eps → more phantom
    through_the removed → through_the DECREASED from 25.3% to 24.0% (counter to P_THRU_HALL increase).
  - Measured non-hallway through_the = 24.0 - (32%×69.3%×52%) = 24.0 - 11.53 = 12.47pp
    (was 15.57pp in v127 — extended Stage 1 removed 3.1pp of phantom, cleaning genuine phantoms).
  - Measured (hall_into→) LLM compliance = 2.6% / (10.2%×69.3%) = 36.8% (was assumed 60%).
  - Measured (hall→) compliance = (19.0 - 11.53 - 2.6) / (15.6%×69.3%) = 4.87/10.8 = 45.1% (stable ≈ v127).
  - continue: 11.2% (0.8pp above GT=10.4% — slight overshoot, keep P=0.70).

v129 CALIBRATION (using measured compliance values):
  Non-hallway through_the (extended Stage 1 cleaning, stable from v128): 12.47pp

Fix 1 — P_THRU_HALL: 32% → 41% (fix through_the: 24.0% → 27.2%)
         Math: 41% × 69.3% × 52% = 14.77pp from hallway
         Total through_the: 14.77 + 12.47 = 27.24% ≈ GT=27.2% ✓

Fix 2 — Rename (hall_into→) → (→ hallway): use same notation as (→ room), expect ~85% LLM compliance.
         (→ room) gets ~85% compliance; (hall_into→) got only 36.8% — LLM confused by novel notation.
         SYSTEM_INTRO: "(→ hallway): write 'walk into the hallway' — same pattern as (→ room)."
         Target: P × 59% × 69.3% × 85% = 4.4% → P=12.7% of remaining 59% = 7.5% absolute
         Draw: 0.41 < draw < 0.485 → (→ hallway)

Fix 3 — (hall→) P rebalanced: 15% of remaining 51.5% = 7.7% absolute
         Needed hallway from (hall→): 20.4 - 14.77 - 4.4 = 1.23pp
         P × 51.5% × 69.3% × 45% = 1.23pp → P = 7.64% / 51.5% ≈ 14.8% of remaining
         Draw: 0.485 < draw < 0.562 → (hall→)
         Unannotated: 43.8% of all hallway eps

Fix 4 — Continue restoration P: KEEP at 0.70 (v128 gives 11.2% ≈ GT=10.4%, acceptable).

Fix 5 — Stage 1: KEEP extended pattern + "walk forward to the room" replacement (v128 cleaned phantoms).

Fix 6 — had_any_hall tracks had_thru_hall OR had_hall_entry OR had_hall_into (rename only).

Expected v129: through_the≈27.2%, hallway≈20.4%, into_hallway≈4.4%, continue≈11.0%, avg_words≈25.3w+.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v129.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v129_phase2_checkpoint.json"  # v129 — recalibrated from v128 measured compliance
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    ...
                    #
                    # v129 calibration:
                    #   P_THRU=41%:        41% × 69.3% × 52% = 14.77pp through_the AND hallway
                    #   non-hallway through_the: 12.47pp (extended Stage 1 cleaning, stable)
                    #   total through_the: 14.77 + 12.47 = 27.24% ≈ GT=27.2% ✓
                    #
                    #   (→ hallway) P=12.7% of remaining 59% = 7.5% abs → draw < 0.485
                    #   (→ hallway) hallway: 7.5% × 69.3% × 85% = 4.42pp ← into_hallway ≈ GT=4.4% ✓
                    #   (hall→) P=14.8% of remaining 51.5% = 7.6% abs → draw < 0.562
                    #   (hall→) hallway: 7.6% × 69.3% × 45% = 2.37pp
                    #   total hallway: 14.77 + 4.42 + 2.37 = 21.56%... slightly above GT=20.4%
                    #   (use P_HALL_INTO=10% of remaining 51.5% = 5.15% abs → hallway: 1.61pp → total: 20.80%)
                    draw = rng.random()
                    if draw < 0.41:        # P_THRU_HALL=41% — write "walk through the hallway"
                        if direction:
                            parts.append(f"turn {direction} (through hallway)")
                        else:
                            parts.append("(through hallway)")
                    elif draw < 0.485:     # (→ hallway) P=12.7% of remaining 59% → 7.5% abs
                        # "(→ hallway)" notation: matches (→ room) convention, LLM writes "walk into the hallway"
                        if direction:
                            parts.append(f"turn {direction} (→ hallway)")
                        else:
                            parts.append("(→ hallway)")
                    elif draw < 0.536:     # (hall→) P=10% of remaining 51.5% → 5.15% abs
...
async def main():
    ...
    print('=== Gate 4 v129: Re-calibration from v128 measured compliance ===')
    print(f"  v128 actual: through_the=24.0%, hallway=19.0%, into_hallway=2.6%, continue=11.2%, avg_words=25.3w")
    print(f"  Fix 1: P_THRU_HALL 32%→41% → through_the: 41%×69.3%×52%=14.77pp + 12.47pp = 27.24% ≈ GT=27.2%")
    print(f"  Fix 2: (hall_into→)→(→ hallway): notation matches (→ room) convention → compliance 36.8%→~85%")
    print(f"         P=7.5% abs → into_hallway: 7.5%×69.3%×85%=4.42pp ≈ GT=4.4%")
    print(f"  Fix 3: (hall→) P=5.15% abs → hallway: 5.15%×69.3%×45%=1.61pp; total: 14.77+4.42+1.61=20.80%≈GT")
    print(f"  Fix 4: Continue P=0.70 (keep from v128: continue=11.2% ≈ GT=10.4%, acceptable)")
    print(f"  Fix 5: Stage 1 extended pattern + 'walk forward to the room' (keep from v128)")
    print(f"  Expected: through_the≈27.2%, hallway≈20.8%, into_hallway≈4.4%, continue≈11%, avg_words≈25.3w+")
