#!/usr/bin/env python3
"""
Gate 4 Visual v56 — v55 base + walk_past reduction via → rule example change

v55 RESULTS (base):
  GT-match: 0.959, toward_the: 3.0% (GT=3.8% — near perfect!), avg_words: 26.0
  TRADEOFF: walk_past 17.5% (was 15.3% in v54fix, GT=8.7%) — LLM shifted from "toward" to "walk past"

REMAINING GAPS (v55 vs GT):
  walk_past:  17.5% vs GT=8.7% — increased vs v54fix, LLM overcompensating with "walk past"
  hallway:    25.1% vs GT=20.4% — 5pp over
  toward_the: 3.0% vs GT=3.8% — slightly below GT (OK, less concerning than 10%)
  avg_turns:  0.74 vs GT=0.59  — medium gap

v56 ROOT CAUSE ANALYSIS (walk_past):
  In v55, the → rule example "walk past the grey sofa" is the ONLY natural walking example.
  LLM compensates for loss of "toward the" by overusing "walk past" pattern.
  Both examples in v55 are "passing" actions: "walk past the grey sofa" AND "pass by the bookshelf".
  Fix: Replace one of the pass examples with a neutral approach: "walk to the bookshelf"
  This gives LLM vocabulary that is neither "toward the" nor "walk past the".

v56 FIX:
  - Change → rule examples: "walk past the grey sofa" or "pass by the bookshelf"
    → "walk to the bookshelf" or "walk past the grey sofa"
  - "walk to the X" is neutral (no toward_the, less walk_past)
  - [pass:] markers STILL drive walk_past (own rule), but → rule now splits between "walk to" and "walk past"
  - Fresh Phase 2 LLM generation (RULES changed)
  - Retain v54's surgical post-processing for "through the doorway"
  Expected: walk_past 17.5% → ~13-15%, toward_the stays ~3-5% (GT=3.8%), GT-match ~0.960

Phase 2 checkpoint: outputs/gate4_v56_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v56.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v56.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v56_phase2_checkpoint.json"
...
async def main():
    ...
    print('=== Gate 4 v56: walk_past fix — → rule now uses "walk to the X" as primary example ===')
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v55=0.74) ← v56: walk_past fix")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v55=3.0%, v54fix=10.1%) ← KEY METRIC")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v55=0.959, v54fix=0.961, v53=0.961)")
