#!/usr/bin/env python3
"""
Gate 4 Visual v155 — Fix walk_straight assembly injection + reduce turn-anchor.

v154 ACTUAL: GT-match=0.864, toward_the=0.7%✓, avg_words=26.7w✓, walk_straight=19.6%(GT=8.8%), turn-anchor=23.0%
  Root causes in v154:
  1. walk_straight=19.6%: assembly post-proc injects "walk straight" at line 1556:
     _GENERIC_WPT_OBJ_RE.sub('walk straight') → fires when Phase 2 generates "walk past the hallway/wall"
     Also _CONTINUE_FRAG_RE converts "Continue walking/moving" → "walk straight"
  2. turn-anchor=23.0% (GT=16.5%): TURN LANDMARKS rule causes Phase 2 to anchor turns when visual
     context mentions "doorway" — even neutral "A plain doorway leads into the hallway." triggers anchoring.
  3. avg_turns=0.95 (GT=0.59): too many turns; P_ANCHOR_EPISODE too high for actual anchor generation.

v155 CHANGES from v154:
  1. Assembly fix: _GENERIC_WPT_OBJ_RE.sub('walk') instead of 'walk straight' — major walk_straight reducer.
  2. Assembly fix: _CONTINUE_FRAG_RE.sub('walk') instead of 'walk straight'.
  3. Remove TURN LANDMARKS rule from SYSTEM_INTRO (causes Phase 2 to anchor turns from visual context).
  4. Reduce P_ANCHOR_EPISODE 0.231 → 0.18 (reduce anchors toward GT 16.5%; compensate for spontaneous adds).
  5. Keep all v154 improvements: toward_the=0.7%, extract_scene_from_desc, take_few=0.3%, avg_words=26.7.

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C)
Phase 2 checkpoint: outputs/gate4_v155_phase2_checkpoint.json (FRESH)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v155.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v155.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v155: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v155_phase2_checkpoint.json"  # v155: FRESH — walk_straight fix + turn-anchor reduction
...
async def main():
    ...
    print('=== Gate 4 v155: Fix walk_straight assembly injection + reduce turn-anchor ===')
    print(f"  v154 METRICS: GT-match=0.864, toward_the=0.7%✓, avg_words=26.7✓, walk_straight=19.6%, turn-anchor=23.0%")
    print(f"  v155 FIX 1: _GENERIC_WPT_OBJ_RE.sub('walk') not 'walk straight' — main walk_straight reducer")
    print(f"  v155 FIX 2: _CONTINUE_FRAG_RE → 'walk' (not 'walk straight')")
    print(f"  v155 FIX 3: Removed TURN LANDMARKS rule (was causing Phase 2 to anchor turns from visual context)")
    print(f"  v155 FIX 4: P_ANCHOR_EPISODE 0.231→0.18 (aim for 14-15% to hit 16.5% with spontaneous adds)")
    print(f"  GT: walk_straight=8.8%, turn-anchor=16.5%, avg_turns=0.59, toward_the=3.8%")
    print(f"  Expected: walk_straight~8%(GT), turn-anchor~17%, toward_the~1%, GT-match>0.95")
    ...
    print(f"  [v155 targets: walk_straight~8%(GT), turn-anchor~17%(GT=16.5%), toward_the~1%, GT-match>0.95, SR target ≥42%]")
