...
OUTPUT     = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v20.json.gz"
P1_CKPT    = ROOT / "outputs" / "gate4_v19_phase1_checkpoint.json"  # reuse v19 Phase 1!
P2_CKPT    = ROOT / "outputs" / "gate4_v20_phase2_checkpoint.json"
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v20: show ahead ONLY for anchored turns (preserves 17% anchor rate)
...
- [object] and [object ahead] are SEPARATE: [object] = where to turn, [object ahead] = what to pass while walking after the turn. Do NOT confuse them.
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    # Count turns that have BOTH a landmark AND an ahead (only anchored turns show ahead)
...
async def main():
    ...
    print("=== Gate 4 v20: Ahead-Enriched Instructions ===")
    ...
    print(f"  v20 key change: extract ahead noun from turn 2nd sentence → [X ahead] in route")
    print()
