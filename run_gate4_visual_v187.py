#!/usr/bin/env python3
"""
Gate 4 Visual v187 -- terminal stop/wait injection + walk_through_hallway → walk_down_hallway.

CHANGES FROM v186:
  v186 actual stats (PYTHONHASHSEED=42): through_the=35.0%, enter=15.3%, walk_ahead=9.8%,
                                          go_straight=25.2%, 2-sent=42.7%, avg_words=27.5
  GT actual measured (val_unseen):        through_the=27.0%, enter=10.9%, walk_ahead=0.1%,
                                          go_straight=3.2%, stop=29.2%, wait=17.5%

  CRITICAL FINDING: v186 has stop=6.7%, wait=5.1% vs GT stop=29.2%, wait=17.5%.
  CMA was trained on GT where ~40% of instructions contain stop/wait cues.
  Without stop/wait in our instructions, the model doesn't know when to terminate -> kills SR.

FIX 1: Terminal stop/wait injection (P=0.30)
  When instruction lacks stop/wait, append "Stop there." or "Wait there." with P=0.30.
  Expected: stop/wait coverage 12% -> 12% + 0.30*88% = 38.4% (approx GT 40% combined)
  This is the highest-impact SR fix: teaches model WHEN to stop.

FIX 2: walk/go through hallway/corridor -> walk/go down hallway/corridor (P=0.65)
  "walk through the hallway" -> "walk down the hallway" -- natural GT-style language.
  v186: walk_through_hallway=10.6% (195 episodes).
  At P=0.65: 127 conversions / 1839 = 6.9pp reduction in through_the.
  Expected: through_the 35.0% -> ~28.1% (close to GT 27.0%)

INHERITED from v186 (all):
  - through->enter P=0.25 (expanded regex incl. room/area/foyer/lobby)
  - walk_forward->go_straight P=0.30
  - walk_ahead->go_straight P=0.20
  - P_STOP_MERGE=0.30
  - skip rate 25%, P_MERGE=0.60
  - PYTHONHASHSEED=42 (deterministic hash)

Expected v187:
  through_the approx 28% (from 35.0%) -- MAJOR improvement toward GT=27%
  stop approx 38% (from 6.7%) -- CRITICAL improvement toward GT=29.2%
  wait approx 17% (from 5.1%) -- improvement toward GT=17.5%
  2-sent approx 42.7% (same)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v187_phase2_checkpoint.json (REUSE v180 P2 results -- assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v187.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v187.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v187_phase2_checkpoint.json"  # v187: REUSE v180 P2 results -- assembly-only
