#!/usr/bin/env python3
"""
Gate 4 Visual v184 -- through_the→enter conversion + walk_forward→go_straight redistribution.

CHANGES FROM v183:
  v183 final stats: through_the=39.9%, walk_forward=13.3%, enter=11.4%, 2-sent=47.4%
  GT reference:     through_the=27.2%, walk_forward=6.1%,  enter=17%,   2-sent=42%

  KEY FINDING: through_the=39.9% vs GT=27.2% — 12.7pp OVERUSE not fixed since v157.
  Source: primarily LLM-generated "walk through the X" (not midpoint injection which is already reduced).
  P_THRU_MARKER reduction alone won't fix it — need post-processing on LLM text.

FIX 1: "walk/go through the [room]" → "enter the [room]" at P=0.15
  Semantically valid: "walk through the kitchen" ↔ "enter the kitchen"
  Two-for-one: reduces through_the AND increases enter (both toward GT)
  Expected: through_the 39.9%→33.9%, enter 11.4%→17.4% (≈ GT=17%!)

FIX 2: soft walk_forward → go_straight at P=0.30
  walk_forward=13.3% vs GT=6.1% — 2.2× GT. LLM generates naturally but too high.
  Redistribute to go_straight (v24-style) rather than eliminate.
  Expected: walk_forward 13.3%→9.3%, go_straight 19.8%→23.8% (≈ v24's 23%)

FIX 3: Reduce P_STOP_MERGE: 0.45 → 0.30
  Further reduce 2-sent from 47.4% toward GT=42%.
  v181's 2-sent increase was caused by walk_ahead→walk_forward interaction (now REMOVED in v182/v183).
  Expected: 2-sent 47.4%→~43-44%

INHERITED from v183 (all):
  - skip rate 25% (was 50% in v182)
  - P_MERGE=0.60 (was 0.75 in v182)
  - REMOVED go_straight→walk_ahead conversion (v182 fundamental fix)
  - replace_go_straight fallback = "go straight" (v182)
  - go_start P=0.70, enter P=0.75 for walk_to
  - walk_ahead/fwd prep reduction (v177/v178)
  - sentence merger s1≤12, combined≤20 (v178)

Expected v184:
  avg_words ≈ 27.5w
  through_the ≈ 34% (from 39.9% — toward GT=27.2%)
  enter ≈ 17% (from 11.4% — matching GT=17%!)
  walk_forward ≈ 9.3% (from 13.3% — toward GT=6.1%)
  go_straight ≈ 23.8% (from 19.8% — staying near v24's 23%)
  2-sent ≈ 43-44% (from 47.4% — approaching GT=42%)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v184_phase2_checkpoint.json (REUSE v180 P2 results — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v184.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v184.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v184_phase2_checkpoint.json"  # v184: FRESH (use v180 P2 results)
...
async def main():
    ...
    print('=== Gate 4 v184: through_the→enter + walk_forward→go_straight + P_STOP_MERGE 0.45→0.30 ===')
    print(f"  v183 stats: through_the=39.9%, enter=11.4%, walk_forward=13.3%, go_straight=19.8%, 2-sent=47.4%")
    print(f"  GT reference: through_the=27.2%, enter=17%, walk_forward=6.1%, go_straight=7.9%, 2-sent=42%")
    print(f"  v184 CHANGES:")
    print(f"    FIX1: 'walk/go through the X'→'enter the X' P=0.15 → through_the:39.9%→34%, enter:11.4%→17%!")
    print(f"    FIX2: walk_forward→go_straight P=0.30 → walk_forward:13.3%→9.3%, go_straight:19.8%→23.8%")
    print(f"    FIX3: P_STOP_MERGE 0.45→0.30 → 2-sent:47.4%→~43%")
    print(f"  INHERITED FROM v183: skip=25%, P_MERGE=0.60, REMOVED go_straight→walk_ahead")
    ...
    for ep in episodes:
        ...
        _THROUGH_ENTER_RE = re.compile(
            ...
            r'|media room|library|sitting room|hallway|corridor|doorway|archway|opening|entryway))\b',
            re.IGNORECASE
        )
        _thru_enter_rng = random.Random(eid ^ 0x3A4B_5C6D)
        def _convert_through_to_enter(m):
            if _thru_enter_rng.random() < 0.15:
                cap = m.group(0)[0].isupper()
                noun = m.group(2)
