#!/usr/bin/env python3
"""
Gate 4 Visual v81 — v80 + hallway language restore + clean() fixes + distance calibration

v80 ISSUE (post-analysis):
  "down the hallway": 0.7% (GT=13.9%) — severely suppressed by double ban:
    1. SYSTEM_INTRO ROOM NAMES rule explicitly bans "walk down the hallway"
    2. _MIDROUTE_HALL_RE assembly post-processing removes remaining hallway mentions
  "walk forward art": 15 garbled episodes (VLM drops "past the" before artwork nouns)
  starts-with-period: 13 episodes with truncated first sentence
  "walk forward": 2832 occurrences = 1.54/ep (too generic, replaces all hallway language)

v81 FIX:
  1. Prompt: Remove "walk down the hallway" from ROOM NAMES prohibition.
     Allow directional "walk down the hallway" (GT style) vs room-entry "walk into the hallway" (banned).
  2. Assembly: Relax _MIDROUTE_HALL_RE to only suppress room-entry patterns (into/toward/to),
     NOT directional patterns (down/along). This restores hallway% from 0.7% toward GT 13.9%.
  3. clean(): Fix "walk forward art/artwork/painting" → "walk past the art" (garbling fix).
  4. clean(): Strip leading period artifacts (". Wait near X." → "Wait near X.").
  5. Prompt: Add distance calibration rule — convert 'straight Xm' to relative language.
  6. P_PASS_MARKER=0.10 (unchanged from v80 — best GT-match=0.994).
  7. New seed: 0x8145_5300.

Phase 2 checkpoint: outputs/gate4_v81_phase2_checkpoint.json (FRESH — prompt changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v81.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v81.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v81_phase2_checkpoint.json"  # FRESH — prompt changed
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8145_5300)  # v81: new seed
...
- ROOM NAMES: Do NOT write "walk into the hallway", "walk toward the hallway", "walk through the hallway", "go through the hallway", "walk into the living room", or "walk toward the bedroom" as mid-route steps. However, "walk down the hallway" or "walk along the hallway" IS allowed when describing continuous forward movement along an existing corridor — this is natural R2R style. Only name other rooms (bedroom, kitchen, bathroom) when they are the FINAL destination labeled in "Goal:".
- DISTANCE: When the route shows a 'straight Xm' segment — NEVER write metric numbers. Use natural language: "a few steps" (≤2m), "walk forward" (3–5m), "walk forward for a while" (6–9m), "walk down the hallway" if in a corridor context (≥8m). R2R annotators never write "3 meters" or "5m" — keep it conversational.
...
async def main():
    ...
    print('=== Gate 4 v81: v80 + hallway restore + clean() fixes + distance calibration ===')
    print(f"  KEY FIX: hallway% restored (ban lifted from 'walk down the hallway', was 0.7% → target ~13.9% GT)")
    print(f"  KEY FIX: 'walk forward art' garbling fixed in clean() via _FWDOBJ_FIX_RE")
    print(f"  KEY FIX: leading period artifacts stripped in clean()")
    print(f"  KEY FIX: distance calibration rule added to prompt (straight Xm → relative language)")
    print(f"  P_PASS_MARKER={P_PASS_MARKER} (unchanged from v80)")
    print(f"  Seed: 0x8145_5300")
    ...
    for ep in episodes:
        ...
        # v81: RELAXED — only suppress room-entry patterns (into/toward/to the hallway),
        # NOT directional patterns (down/along the hallway). GT uses "walk down the hallway"
        # in 13.9% of episodes as path language; v80 over-suppressed it to 0.7%.
        # Keep: "walk down the hallway", "walk along the hallway" (GT-style directional)
        # Remove: "walk into the hallway", "walk toward the hallway" (room-entry hallucinations)
