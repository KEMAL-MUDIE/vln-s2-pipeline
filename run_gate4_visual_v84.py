#!/usr/bin/env python3
"""
Gate 4 Visual v84 — v83 + GT-calibrated stop phrases

v83 ISSUE (post-analysis):
  "Stop near" usage: 34.5% (GT=1.3%) — massively overused (26x more than GT).
  Root cause: choose_stop() patterns include "Stop near X." as 1/3 of stop options.
  GT stop prepositions (from analysis of 748 stop sentences):
    stop at: 8.8%, stop in the: 7.5%, stop in front of: 6.1%, stop next to: 3.1%,
    stop by: 2.1%, stop near: 1.3%
  GT wait prepositions:
    wait at: 7.9%, wait near: 6.6%, wait by: 5.1%, wait there: 4.7%
  Current choose_stop: "Stop near/Stop at/Stop in front of" each 33% → "near" overdominates.

v84 FIX:
  1. Recalibrate choose_stop() to match GT distribution:
     Stop patterns (normalized, excluding "stop on" which needs surface context):
       "Stop at X" → 35%, "Stop in front of X" → 25%, "Stop next to X" → 20%,
       "Stop by X" → 15%, "Stop near X" → 5%
     Wait patterns (calibrated):
       "Wait at X" → 30%, "Wait near X" → 22%, "Wait by X" → 18%,
       "Wait there" → 16%, "Wait next to X" → 14%
     Room-goal stops ("stop in the bedroom"):
       "Stop in the X" → added when goal_room is known
  2. Expected: "Stop near" drops from 34.5% → ~5%, "at/in front of/next to/by" increase.
  3. Keep all v83 improvements: continue fix, hallway, clean() fixes.
  4. Seed: 0x8445_5300. P_PASS_MARKER=0.10 (unchanged).

Phase 2 checkpoint: outputs/gate4_v84_phase2_checkpoint.json (FRESH — stop phrases changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v84.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v84.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v84_phase2_checkpoint.json"  # FRESH — stop phrases changed
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8445_5300)  # v84: new seed
    ...
    for ep in episodes:
        ...
        if "hallway" in text.lower():
            ...
            if _HALLWAY_DIR_RE.search(text):
                ep_rng = random.Random(eid ^ 0x8445_5300)
