#!/usr/bin/env python3
"""
Gate 4 Visual v37 — Fix v36 Regressions (dup-stop + "continue" + WPT substitution)

v36 RESULTS:
  proceed: 43.8% → 1.7% ← MAJOR WIN (GT=0.8%)
  through the: 27.5% (≈v35 27.2%) ✓
  dup-stop: 71 ← CRITICAL BUG from 4-sentence cap expansion
  GT-match: 0.956 ← dropped from v35=0.961 (stop%=52.5% due to 4-sentence cap)
  continue: 18.5% (unchanged from v35=18.1%, GT=10.4%)

IMPROVEMENTS OVER v36:
  v37 changes:
    1. Revert 4→3 sentence cap: fixes dup-stop=71 (4-sentence allowed both stop+wait phrases)
    2. WPT substitution: "continue forward" → "walk forward"
       Reduces "continue" overuse since _GENERIC_WPT_OBJ_RE fires ~228 times
       Expected: "continue" 18.5% → ~14-15% (closer to GT 10.4%)
    3. All v36 "proceed" elimination retained (1.7% → target ≤1%)

  All v35 innovations retained:
    - through_the hallway→down, [room]→into
    - P_PASS=0.50, P_THRU=0.85 midpoint sampling
    - Generic WPT suppression + through-hallway + through-room
    - Material filter, GT-match metric
    - Fragment removal (Take a., Head.)
    - Duplicate motion collapse

  Expected:
    "proceed":       ≤1.7% (keep v36 fix, GT=0.8%)
    "continue":      ~14-15% (WPT→walk forward, GT=10.4%)
    "through the":   ~27.5% (≈GT 27.2%) ✓
    "walk past the": ~10-11%
    dup-stop:        0 (3-sentence cap fix)
    GT-match:        ≥ 0.961 (v35=0.961, v36=0.956)

  GT analysis reference (v36 actuals):
    "proceed":       GT=0.8%,  v36=1.7%   → v37 target: ≤1.7% ✓
    "continue":      GT=10.4%, v36=18.5%  → v37 target: ~14%
    "through the":   GT=27.2%, v36=27.5%  ← KEEP ✓
    "walk past the": GT=8.7%,  v36=10.3%  → v37 same
    dup-stop:        GT=0,     v36=71     → v37 target: 0 ✓
    GT-match:        v36=0.956 → v37 target: ≥0.961

  Phase 2 checkpoint: outputs/gate4_v37_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v37.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v37.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v37_phase2_checkpoint.json"
...
def clean(raw, sv):
    ...
    raw = re.sub(r'\s+', ' ', raw).strip()
    # v37: revert to 3-sentence cap (4-sentence caused dup-stop=71 in v36)
    sents = re.split(r'(?<=[.!?])\s+', raw)
...
async def main():
    ...
    print('=== Gate 4 v37: Fix v36 Regressions (dup-stop 71→0, continue↓, 3-sentence cap) ===')
    ...
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v37 should be <1%")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v36=0.956, v35=0.961, v34=0.968)")
    ...
    print(f"  [Note: v37: v36 proceed fix + 3-sentence cap + walk forward WPT substitution]")
