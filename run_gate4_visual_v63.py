#!/usr/bin/env python3
"""
Gate 4 Visual v63 — Calibrated P_ANCHOR_EPISODE + P_STOP for GT-match improvement

v62 RESULTS (base):
  GT-match: 0.951, avg_turns: 0.59 (GT=0.59), hallway: 19.2% (GT=20.4%)
  anchor_err: |15.0% - 16.5%| = 1.5pp → wastes 0.030 on GT-match
  stop_err:   |52.2% - 50.8%| = 1.4pp → wastes 0.014 on GT-match
  Combined loss: 0.044 → if fixed: GT-match ≈ 0.951 + 0.044 = ~0.98

KEY FINDING:
  LLM consistently over-generates stop phrases (+1.4pp) and under-generates turn anchors (-1.5pp)
  Calibrating P_ANCHOR_EPISODE and P_STOP can compensate for these systematic biases.

Math:
  v62: P_ANCHOR_EPISODE=0.21 → route_anchor_rate=17.8%, output_anchor=15.0%
       LLM_anchor_compliance = 15.0/17.8 = 0.843
       To target output_anchor=16.5%: route_anchor = 16.5/0.843 = 19.6%
       Eligible fraction ≈ 17.8/0.21 = 84.8% → P_ANCHOR = 19.6/84.8 = 0.231

  v62: P_STOP=0.508 → actual_stop=52.2% (over by 1.4pp consistently)
       Target: actual_stop=50.8% → P_STOP = 0.508 - 0.014 = 0.494

v63 CHANGES:
  - P_ANCHOR_EPISODE: 0.21 → 0.231 (target output_anchor ~16.5% = GT)
  - P_STOP: 0.508 → 0.494 (compensate for LLM +1.4pp over-stop)
  - P_TURN_UNANCHORED: 0.42 (keep — proven avg_turns=0.59)
  - All v62 post-processing retained (doorway, walk_past, hallway surgical)

Expected:
  - anchor_err: 1.5pp → ~0 (saves 0.030 on GT-match)
  - stop_err: 1.4pp → ~0 (saves 0.014 on GT-match)
  - GT-match: 0.951 → ~0.965-0.980 (LLM variance remains)
  - avg_turns: ~0.59, hallway: ~19-21%, all other metrics similar to v62

Phase 2 checkpoint: outputs/gate4_v63_phase2_checkpoint.json (FRESH — P_ANCHOR_EPISODE changes route)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v63.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v63.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v63_phase2_checkpoint.json"  # FRESH — calibrated anchor+stop!
