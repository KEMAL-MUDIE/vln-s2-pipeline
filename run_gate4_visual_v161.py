#!/usr/bin/env python3
"""
Gate 4 Visual v161 — v159 + GT-calibrated continue (10.4%) via reduced P_continue.

v159 ACTUAL: GT-match=0.978, go_straight=24.9%≈v24✓, walk_straight=8.6%≈GT✓, avg_turns=0.67
  Remaining gaps: continue=23.7%(GT=10.4%), through_the=33.7%(GT=27.2%)

v161 HYPOTHESIS: Does reducing continue from 24% → 10% (GT level) improve SR?
  v24 had continue=61% (very high) → SR=40.24%
  v159 has continue=23.7% → SR=?
  v161 targets GT continue=10.4%: P_continue: 0.655 → 0.28

  Note: v24's very high continue DID NOT prevent 40.24% SR. This test checks if
  LOWER continue (GT-matched) also works — or if high continue is beneficial.

v161 CHANGES from v159:
  1. P_continue: 0.655 → 0.28 → continue ≈ 0.655*(0.28/0.655)*23.7% ≈ 10.1% ≈ GT=10.4%
     Effect: fewer "Walk forward" → "Continue forward" conversions
     Side effect: more "Walk forward" stays as "Walk forward" (not "Continue")
  2. Everything else identical to v159 (50% skip → go_straight≈24.9%, P_TURN=0.09, P_THRU=0.60)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C)
Phase 2 checkpoint: outputs/gate4_v158_phase2_checkpoint.json (REUSE v158 checkpoint, same params)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v161.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v161.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v161: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v158_phase2_checkpoint.json"  # v161: REUSE v158 (same P_TURN=0.09, P_THRU=0.60)
...
async def main():
    ...
    print('=== Gate 4 v161: v159 + GT continue (P=0.28) → continue~10.4% + go_straight~24.9% ===')
    print(f"  v159 METRICS: GT-match=0.978✓, go_straight=24.9%≈v24✓, walk_straight=8.6%✓, continue=23.7%")
    print(f"  v161 CHANGE: P_continue 0.655→0.28 → continue ≈ 10.1% ≈ GT=10.4%")
    print(f"    v24 had continue=61% (high) AND got 40.24% SR. This tests if LOW continue (GT level) also works.")
    print(f"    If v161 SR > v159 SR: lower continue helps. If similar: continue doesn't matter for SR.")
    ...
    for ep in episodes:
        ...
        # v161: P_continue 0.655→0.28 to reduce continue from 23.7%→~10.1% (GT=10.4%)
        # Scaling: 0.655*(0.28/0.655) = 0.28 → continue 23.7%*(0.28/0.655) ≈ 10.1% ≈ GT=10.4%
        # Note: v24 had continue=61% but still got 40.24% SR. This tests if GT-matched continue helps.
        if cont_rng.random() < 0.28 and _WALK_FWD_CAP_RE.search(text):
            text = _WALK_FWD_CAP_RE.sub(r'Continue \1', text, count=1)

        # v161 (from v159): replace_go_straight_with_landmarks skipped 50% → go_straight ≈ 23.75% ≈ v24=23%.
        # SR HYPOTHESIS: v24 (go_straight=23%) → SR=40.24% (BEST non-GT).
        # v158 (100% skip) → go_straight=47.5% — too extreme. v159 targets v24's sweet spot.
        # Skip rate: v156=8%→3.9%, v157=15%→6.8%, v158=100%→47.5%, v159=50%→~23.75%
        # Calculation: 50% episodes skip × 47.5% base rate ≈ 23.75% go_straight (matches v24=23%)
    ...
    print(f"  go_straight:   {100*go_straight_n/n:.1f}%  (GT=7.9%, v159=24.9%) ← v161: same 50% skip as v159")
    print(f"  walk_straight: {100*wlk_straight_n/n:.1f}%  (GT=8.8%, v159=8.6%) ← v161: unchanged")
