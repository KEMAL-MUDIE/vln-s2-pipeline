#!/usr/bin/env python3
"""
Gate 4 Visual v151 — Fix "take a few steps" + go_straight overuse from v150.

v150 ACTUAL (projected): ~32-36% SR. Root causes identified:
  - "take a few steps" in 29.1% of eps (GT=0.2%!) — from DISTANCE rule mapping ≤2m → "take a few steps"
  - "go straight" in 80.7% of eps (GT=7.9%) — from MOTION WORDS rule + post-processing
  - "continue" banned in prompt → only 1.1% raw, forced up by post-processing
  v24 (40.24% SR) has: take_few=0.3%, go_straight=23%, walk_straight=27%, continue natural

v151 CHANGES from v150:
  1. SYSTEM_INTRO DISTANCE rule: removed "take a few steps" — for short segs, skip or chain.
     New: "For ≤2m: skip or chain ('turn left and walk to the X'). For 3-8m: 'walk' or 'continue'.
     For >8m: 'walk straight' or 'go straight' sparingly."
  2. SYSTEM_INTRO MOTION WORDS: allow "continue" (GT=10.4%), allow "toward the X" (helps direction).
     Remove "go straight" preference — use it only for long featureless corridors.
  3. Reuse v150's direction-aware Phase C checkpoint (no re-annotation needed).
  4. Fresh Phase 2 generation with new SYSTEM_INTRO.
  5. Assembly: same as v150c.

Phase 1 checkpoint: outputs/gate4_v150_phase1_checkpoint.json (REUSED: direction-aware, 3192 frames)
Phase 2 checkpoint: outputs/gate4_v151_phase2_checkpoint.json (FRESH — new SYSTEM_INTRO)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v151.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v151.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v150_phase1_checkpoint.json"  # v151: REUSE v150 direction-aware Phase C (no re-annotation)
...
P2_CKPT       = ROOT / "outputs" / "gate4_v151_phase2_checkpoint.json"  # v151: FRESH — new SYSTEM_INTRO (no take_a_few, no go_straight overuse)
...
- MOTION WORDS: Use natural walking verbs: "walk", "go", "continue". Chain movements naturally: "turn left and walk into the kitchen". For landmarks: "walk toward the X", "walk to the X". AVOID "proceed", "go forward", "walk forward", "keep going", "head toward". Use "continue" freely — it is natural indoor navigation language. Use "go straight" or "walk straight" ONLY for long featureless corridors (>8m with no landmarks); otherwise just "walk" or "continue".
- DISTANCE: The route shows 'straight Xm' entries — NEVER write metric numbers or "take a few steps". For short segments (≤2m): skip them or chain the adjacent movements (e.g. "turn left and walk to the X"). For medium segments (3-8m): use "walk" or "continue" naturally. For long segments (>8m): use "walk straight" or "go straight" ONCE and briefly.
...
async def main():
    ...
    print('=== Gate 4 v151: Fix take_a_few+go_straight overuse. REUSE v150 direction-aware Phase C + fresh Phase 2 ===')
    print(f"  v150 PROJECTED: ~32-36% SR. take_few=29.1%(GT=0.2%), go_straight=80.7%(GT=7.9%)")
    print(f"  v24 ACTUAL: 40.24% SR. take_few=0.3%, go_straight=23%, hallway=51.8%")
    print(f"  GT: take_few=0.2%, go_straight=7.9%, walk_straight=8.8%, toward_the=3.8%, hallway=20.5%")
    print(f"  v151 ROOT CAUSE FIX: DISTANCE rule removed 'take a few steps' (was ≤2m mapping)")
    print(f"  v151 MOTION WORDS FIX: allow 'continue' (was banned), allow 'toward the X'")
    print(f"  v151 MOTION WORDS FIX: 'go straight'/'walk straight' only for long featureless >8m segments")
    print(f"  v151 Phase C: REUSE v150 direction-aware checkpoint (3192 direction-aware turn descriptions)")
    print(f"  Expected raw output: take_few<3%, go_straight<35%, continue>10%, toward>5%")
    ...
    print(f"  Phase 1: REUSED v150 direction-aware Phase C (LEFT/RIGHT turn prompts)")
    ...
    print(f"  --- v151 KEY FIX METRICS ---")
    print(f"  take_few:      {100*take_few_n/n:.1f}%  (GT=0.2%, v150=29.1%) ← MUST be <3%")
    print(f"  go_straight:   {100*go_straight_n/n:.1f}%  (GT=7.9%, v150=80.7%) ← MUST be <35%")
    print(f"  walk_straight: {100*wlk_straight_n/n:.1f}%  (GT=8.8%, v150=12.9%)")
    ...
    print(f"  [v151 targets: take_few<3%(GT=0.2%), go_straight<35%(GT=7.9%), continue>10%(GT=10.4%), SR target ≥40%]")
