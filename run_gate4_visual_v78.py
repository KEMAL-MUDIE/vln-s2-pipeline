#!/usr/bin/env python3
"""
Gate 4 Visual v78 — v77 + prompt fix: [→ X] markers must NOT use "walk past the"

v77 ISSUE:
  "walk past the": 1.8% (GT=8.7%) — OVERCORRECTED (too low).
  Root cause of regression: LLM generates "walk past the X" for [→ X] markers
  (midpoint-ahead, turn approach), not just [pass:] markers. With had_pass=False
  for ~29% of episodes, suppression removes 18.5pp → only 1.8% survive.
  GT-match dropped 0.984 (was 0.993) — seeded rng also changing thru/turn decisions.

v78 FIX:
  1. Prompt fix: Remove "walk past the" from [→ object ahead] examples.
     Only "walk to the" is allowed for [→ X] markers.
     "walk past the" is ONLY valid when route has [pass:] markers.
  2. P_PASS_MARKER: 0.30 → 0.50 (more episodes get had_pass=True, more WP survive)
  3. Keep seeded rng (eid ^ 0x7845_5300) for pass/thru decisions + had_pass tracking
  4. Expect: walk-past ~8-10%, GT-match ~0.990+

Phase 2 checkpoint: outputs/gate4_v78_phase2_checkpoint.json (FRESH — prompt changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v78.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v78.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v78_phase2_checkpoint.json"  # FRESH — prompt changed
...
P_PASS_MARKER = 0.50   # v78: back to 0.50 (v77=0.30 gave had_pass=T 71% but WP only 1.8%)
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x7845_5300)  # v78: episode-specific seed (different from v77)
...
async def main():
    ...
    print('=== Gate 4 v76: v75 + P_PASS_MARKER=0.30 + material strip fix + walls regex fix ===')
    ...
    for ep in episodes:
        ...
        # v58/v77: Surgical walk_past post-processing.
        # v77 FIX: Use route-level had_pass flag (seeded deterministic rng) instead of
        # _has_pass_midpoint(eid). With v75 midpoints (42.4% artwork = pass type), the old
        # _has_pass_midpoint is True for ALL episodes → suppression never fires.
        # Now: only episodes where build_route_desc() actually added a [pass:] marker
        # are allowed to keep "walk past the X". Others → "walk to the X".
        had_pass = meta.get("had_pass", False)
        if re.search(r'\bwalk\s+past\s+the\b', text, re.I) and not had_pass:
