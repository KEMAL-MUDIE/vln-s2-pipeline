#!/usr/bin/env python3
"""
Gate 4 Visual v124 — Fix through_the, avg_words, continue based on v123 analysis.
v123 results: through_the=32.7% (GT=27.2%), avg_words=25.2w (GT=26.8w), continue=7.0% (GT=10.4%).
              toward_the=1.3% ✓ (FIXED), hallway=33.3% (same as v120).

Fix 1 — P(door→ room): 28% → 22% to reduce through_the from 32.7% → ~26-27%.
Fix 2 — quality_ok min_words: 12 → 8 (v123 had 110 failures vs v120's 67; fallback reduces avg_words).
Fix 3 — Continue suppression: remove "continue for" and "continue to the" suppressions
         (too aggressive: continue dropped from 9.8% to 7.0% vs GT=10.4%).
Keep all v123 changes: no "head toward" in MOTION WORDS, no "walk down the hallway" in DISTANCE.

Expected v124: through_the~26%, avg_words~26.5w, continue~9-10%, walk_past~8%, toward_the~2-4%.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v124.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v124_phase2_checkpoint.json"  # v124 — fix through_the/avg_words/continue from v123 analysis
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    ...
                    # v124: reduce P(door→ room) from 28% to 22%.
                    # v123 through_the=32.7% (GT=27.2%) due to motion-word reduction freeing budget for doorway.
                    # At P=22%: expected through_the~26-27% (closer to GT).
                    if rng.random() < 0.22:
...
def clean(raw, sv):
    ...
    # v124: Remove "continue for" and "continue to the" suppressions — too aggressive in v123 (continue=7.0%).
    #       Only suppress "continue forward" and "continue straight" (direct overuse targets).
    #       Allow "continue for a while", "continue to the X" to survive → continue → ~9-10%.
...
async def main():
    ...
    print(f"  v124: P(door→room)=22% (was 28%), quality_ok min_words=8 (was 12), remove continue_for suppression")
    print(f"  v124: expected through_the~26%, avg_words~26.5w, continue~9-10%")
