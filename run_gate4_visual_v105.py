#!/usr/bin/env python3
"""
Gate 4 Visual v105 — v104 + HIGH-TURN: include ALL turns (no angle filter), P_TURN_UNANCHORED=0.90.

KEY FINDING (v24 turn analysis):
  v24 (40.24% SR): turn left=51.6%, turn right=53.6% → Many episodes have BOTH turns
  v103 (??% SR):   turn left=24.5%, turn right=27.4% → Only ~48% eps have any turn

  v24 had MANY MORE explicit turns per episode. Currently we skip:
  1. Turns with angle < 70° and no landmark (line 1149: angle < 70 skip)
  2. Unanchored turns at P=0.42 (line 1156: P_TURN_UNANCHORED=0.42)

  These filters reduce turns to match GT avg (0.59/ep). But v24 had 1.0-1.5 turns/ep
  and still got 40%! The model follows step-by-step guidance better with more explicit turns.

v105 CHANGES FROM v104:
  1. Remove angle < 70° skip: include ALL turns regardless of angle
  2. P_TURN_UNANCHORED = 0.90 (was 0.42) → more unanchored turns included
  3. Expected: turn left ~40%, turn right ~45%, eps_with_turn ~70-80%
  4. Keep all v104 settings: no landmarks, "straight" vocab, "walk into", P_THRU=0, P_PASS=0

  Hypothesis: More explicit turn guidance → agent doesn't miss turns → higher SR
  Risk: Too many turns might overload the agent. But v24's high turn count worked.

  Expected effects:
    turn left%:     24.5% → ~40-50%  (v24=51.6%)
    turn right%:    27.4% → ~45-55%  (v24=53.6%)
    eps_with_turn:  48.1% → ~75-85%  (v24 winning eps=90.7%)
    avg_turns/ep:    0.57 → ~1.0-1.2  (v24 winning eps much higher)
    SR target: > 40% (test if more turns = better SR like v24)

Phase 2 checkpoint: outputs/gate4_v105_phase2_checkpoint.json (NEW — fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v105.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v105.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v105_phase2_checkpoint.json"  # v105 — fresh P2 (high-turn: all turns included)
...
P_PASS_MARKER = 0.00   # v103: remove pass markers (v24 analysis: walk_past HURTS SR)   # v80: reduced to 0.10 → fewer [pass:] markers → less LLM WP
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v103: Keep v101 hallway suppression; restore P_HALLWAY_TURN=1.0.
                # v102 had 0.65 sampling to reduce eps_with_turn → but v24 analysis shows
                # HIGH turns HELP SR (v24: 90.7% has_turn). Don't undersample turns.
                # Focus: remove [thru:]/[pass:] landmarks instead (key SR lever from v24).
                is_hallway = "hallway" in rm.lower() or "corridor" in rm.lower()
                if is_hallway:
                    if direction:
                        # v103: Always include hallway turn (P_HALLWAY_TURN=1.0)
                        parts.append(f"turn {direction}")
                    # else: no direction — skip (silent straight through hallway)
                else:
                    parts.append(f"(→ {rm})")
            else:
                parts.append(f"turn {direction}")
...
- When you see (→ room) in the route: write "walk into the [room]" — describe entering that room. Do NOT write "enter the [room]". Do NOT write "turn left" or "turn right" for these transitions. Keep it brief (3-5 words). These are only non-hallway rooms (bedroom, kitchen, bathroom, living room, etc.).
...
- STYLE: Prefer "walk straight" and "go straight" over "walk forward". Keep instructions clean and turn-focused. Do NOT add "through the X" or "walk past the X" — only write what is explicitly shown in the route.
...
def clean(raw, sv):
    ...
    # v104: use "walk straight" instead of "walk forward" to match v24 vocabulary
