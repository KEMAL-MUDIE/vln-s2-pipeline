#!/usr/bin/env python3
"""
Gate 4 Visual v176 -- avg_words restoration + stop-room context + sentence merger refinement.

ROOT CAUSE (v174/v175 avg_words regression):
  v173: avg_words=24.6 (walk_to=46.7%, natural LLM sentences)
  v174: avg_words=21.4 → SENTENCE STRUCTURE "PREFER 2 sent" + DISTANCE "use walk ahead/just walk"
  v175: avg_words=22.2 → DISTANCE rule still causes walk_to=23% (was 46.7%), LLM generates "walk"

  The DISTANCE rule "use walk ahead or just walk" causes LLM to replace "walk to the [dest]"
  (4-7w) with "walk ahead" (2w) or bare "walk" (1w). walk_to dropped 46.7%→23.0% = -23.7%.
  "PREFER 2 sentences" also compressed instructions.

FIX 1: REVERT SENTENCE STRUCTURE to v173 style
  v175: "PREFER 2 sentences. Chain ALL consecutive actions with 'and'"
  v176: "Write 1-3 sentences maximum. Chain consecutive actions with 'and'..." (v173 style)
  Expected: LLM generates natural 3-sent instructions → walk_to restored to ~46%

FIX 2: REVERT DISTANCE RULE to v173 style
  v175: "AVOID go straight, walk straight, walk forward — use walk ahead or just walk instead"
  v176: "use walk straight/go straight ONCE and briefly" for long segments (v173 style)
  Expected: walk_to restored → avg_words +1.8w (from 22.2 toward 24.6)

FIX 3: RAISE LENGTH BUDGETS +4w per tier
  v173 avg=24.6, GT=26.8 → 2.2w gap. Raising budgets adds ~0.5-1w.
  path_len<=4: 13-21 → 16-24; path_len=5: 24-32 → 27-36; path_len=6: 27-35 → 30-39; >=7: 33-46 → 37-50

FIX 4: STOP PHRASE ROOM CONTEXT (P_STOP_ROOM=0.15)
  GT: 12.6% of stop phrases have "in the [room]" ("Stop near the couch in the living room.")
  v176: P=0.15 → inject "in the [goal_room]" before final period of stop phrase
  +3-4w to 15% of episodes → +0.45w avg. goal_room already in scope during post-processing.

FIX 5: REMOVE walk_forward from walk_ahead replacement
  GT uses walk_forward=6.1% — keep it. Only replace go_straight (GT=0.2%) and walk_straight (GT=0.6%).

FIX 6: SOFTEN SENTENCE MERGER (P_MERGE=0.60)
  v175: always merge → 2-sent=44.9% (too high vs GT=33.8%)
  v176: P_MERGE=0.60 → ~60% of eligible pairs merge → 2-sent≈38%, 3-sent≈45%

Expected v176:
  avg_words ≈ 25.5w (restored 24.6 + 0.5 LENGTH + 0.45 stop_room)
  2-sent ≈ 38%, 3-sent ≈ 45%, on_dir ≈ 4-5%, color ≈ 9%, enter ≈ 7%, couch ≈ 7%

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v176_phase2_checkpoint.json (FRESH)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v176.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v176.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v176_phase2_checkpoint.json"  # v176: FRESH — revert DISTANCE/SENTENCE rules, raise LENGTH budgets, stop-room context
...
async def main():
    ...
    print('=== Gate 4 v176: avg_words RESTORED + stop-room context + softer merger ===')
    print(f"  ROOT CAUSE v174/v175: DISTANCE 'use walk ahead/just walk' + 'PREFER 2 sent'")
    print(f"    → walk_to dropped 46.7%→23% → avg_words 24.6→22.2")
    print(f"  v176 FIXES:")
    print(f"    FIX1: SENTENCE STRUCTURE reverted to '1-3 sentences max' (v173 style)")
    print(f"    FIX2: DISTANCE rule reverted — allows 'walk straight/go straight' for long segments")
    print(f"    FIX3: LENGTH budgets raised +4w per tier → ~+0.5w avg")
    print(f"    FIX4: Stop-phrase room context P=0.15 → 'in the [goal_room]' → +0.45w avg")
    print(f"    FIX5: walk_forward KEPT (GT=6.1%) — only go_straight/walk_straight replaced")
    print(f"    FIX6: Sentence merger P=0.60 (was always) → 2-sent≈38% (GT=33.8%)")
    print(f"  KEEPS ALL v174 STRUCTURAL IMPROVEMENTS:")
    print(f"    - 'door on the left/right': on_left≈4.5%, on_right≈4.5% ✓")
    print(f"    - color_stop: ~9%≈GT=10.9% ✓, enter≈7%, couch≈7% ✓")
    print(f"  Expected: avg_words≈25.5, sent(2)=38%, on_left/right≈4-5%, walk_fwd≈25%")
    ...
    for ep in episodes:
        ...
        # v176: walk_forward/go_straight REDUCTION — only replace rare GT forms (go_straight, walk_straight)
        # v175 replaced walk_forward→walk_ahead but GT uses walk_forward=6.1% — KEEP walk_forward.
        # GT: go_straight=0.2%, walk_straight=0.6% → very rare, replace with walk_ahead (2w→2w neutral)
        # GT: walk_forward=6.1% → KEEP (don't replace)
        ...
        # goal_room is defined earlier in this episode's processing scope
        ...
        if (goal_room and stop_type == "stop" and _stop_room_rng.random() < 0.15
            ...
            # Insert "in the [room]" before the final period/end of text
            if text.rstrip().endswith('.'):
                text = text.rstrip()[:-1] + _room_injection + '.'
            else:
                text = text.rstrip() + _room_injection + '.'

        # v174/v176: SENTENCE MERGER — merge consecutive short action sentences toward GT 2-sent style
        # GT: 2-sent=33.8% peak, v173: 3-sent=63.2% dominant.
        # v176: P_MERGE=0.60 (softened from v175's always-merge → less over-compression)
        # v175 always-merged → 2-sent=44.9% (too high vs GT=33.8%); P=0.60 → ~38% target
        _P_MERGE = 0.60
            ...
            while i < len(sents):
                ...
                if i + 1 < len(sents):
                    ...
                    can_merge = (
                        ...
                        _merge_rng.random() < _P_MERGE  # v176: 60% chance to merge (was 100%)
                    )
                    if can_merge:
