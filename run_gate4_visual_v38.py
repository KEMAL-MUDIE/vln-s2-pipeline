#!/usr/bin/env python3
"""
Gate 4 Visual v38 — Fix Remaining dup-stop=62 in v37

v37 RESULTS:
  GT-match: 0.957, through_the: 27.1%, walk_past: 10.5%, continue: 9.0%, proceed: 1.6%
  dup-stop: 62 ← STILL PRESENT despite 3-sentence cap
  Root cause: LLM generates "and stop at X" in NON-TERMINAL sentences.
  Analysis (45/62 cases): "and stop at/in/near X" appears mid-instruction, then
  final "Stop near Y." appears as the terminal sentence → two stop occurrences.

IMPROVEMENTS OVER v37:
  v38 changes:
    1. In clean(), before 3-sentence join: scan non-terminal sentences for
       "and stop at/in/near/by X" → replace with "and walk to X"
       "and wait at/in/near/by X" → replace with "and walk to X"
       Effect: removes 45+ dup-stop cases where "stop/wait" appears mid-instruction

  All v37 innovations retained:
    - proceed elimination (43.8%→1.6%)
    - WPT substitution "walk forward" (continue 18.5%→9.0%)
  ...
  Expected:
    dup-stop:        ~15-20 (45 direct fixes, ~17 residual stop+stop from other causes)
    GT-match:        ≥ 0.957 (v37=0.957)
    proceed:         ≤1.6% ✓
    continue:        ~9.0% ✓
    through the:     ~27.1% ✓

  Phase 2 checkpoint: outputs/gate4_v38_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v38.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v38.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v38_phase2_checkpoint.json"
...
def clean(raw, sv):
    ...
    raw = re.sub(r'\s+', ' ', raw).strip()
    sents = re.split(r'(?<=[.!?])\s+', raw)
    # v38: fix dup-stop — in non-terminal sentences, replace "and stop at/in/near X" with "and walk to X"
    # Analysis: 45/62 dup-stop cases have "and stop" in non-terminal sentence (LLM puts terminal action mid-instruction)
    _MID_STOP_RE = re.compile(r'\band\s+stop\s+(at|in|near|by|on|outside|beside)\b', re.IGNORECASE)
    _MID_WAIT_RE = re.compile(r'\band\s+wait\s+(at|in|near|by|on|outside|beside)\b', re.IGNORECASE)
    cleaned_sents = []
    for i, s in enumerate(sents):
        if i < len(sents) - 1:
            s = _MID_STOP_RE.sub(r'and walk to', s)
            s = _MID_WAIT_RE.sub(r'and walk to', s)
        cleaned_sents.append(s)
    result = " ".join(cleaned_sents[:3]).strip()
...
async def main():
    ...
    print('=== Gate 4 v38: Fix dup-stop=62 ("and stop at X" mid-instruction → "and walk to X") ===')
    ...
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v38 should be <1%")
    ...
    print(f"  [Note: v38: v37 proceed/continue fix + mid-instruction 'and stop→walk to' dup-stop fix]")
