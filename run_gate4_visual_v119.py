#!/usr/bin/env python3
"""
Gate 4 Visual v119 — Route injection for walk_past: write "walk past the X" DIRECTLY in route.
v118: walk_past=3.5% despite removing had_pass suppression. Root cause: LLM ignores [pass: X] markers.
Fix: replace "[pass: X]" with literal "walk past the X" as a route step (direct injection).
P_PASS_MARKER=0.10 (lower to compensate for near-100% LLM compliance with direct phrase).
Also: P(door->room)=28%, P(through_hallway)=16% (calibrated for through_the~27%, hallway~20%).
Restore had_pass suppression to suppress spontaneous walk_past outside of route-injected episodes.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v119.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v119_phase2_checkpoint.json"  # v119 — direct walk_past injection in route
...
P_PASS_MARKER = 0.10   # v119: lower to 0.10 (direct phrase injection → near-100% LLM compliance → target ~8-10% walk_past)
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if seg_midpoint and turn_label in seg_midpoint:
                ...
                if action == "thru":
                    ...
                    if rng.random() < P_PASS_MARKER:  # target ~10% walk-past (GT=8.7%)
                        # v119: inject "walk past the X" DIRECTLY in route (LLM ignores [pass: X] marker)
                        parts.append(f"walk past the {mid_desc}")
                        had_pass_markers = True  # v77: track that this episode got walk_past phrase in route
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    # v119: Allow hallway with "through" notation 16% of the time (v118=20% → hallway=35.1% too high; target ~20%)
                    if rng.random() < 0.16:
                        if direction:
                    ...
                    # v119: 28% chance of doorway notation (v118=35% → through_the=33.3% too high; target ~27%)
                    if rng.random() < 0.28:
...
- When route has "walk past the X" as a step: include this phrase in the instruction — the robot moves alongside that object. Keep it brief (3-5 words). Do NOT change to "passing the".
...
- When you see (through hallway): write "walk through the hallway" or "go through the hallway" — do NOT write "walk into the hallway". Hallway transitions always pass through, not into.
...
- MOTION WORDS: Use "go straight", "walk forward", "head forward", "go forward", "keep going". Do NOT use "continue" or "continue forward" — use "keep going" or "go forward" instead.
- DISTANCE: When the route shows a 'straight Xm' segment — NEVER write metric numbers. Use natural language: "a few steps" (≤2m), "go forward" (3–5m), "walk forward for a while" (6–9m), "keep going" (≥10m).
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    # v119: count direct "walk past the" phrases injected in route (v119: no [pass:] markers, direct injection)
    n_pass = len(re.findall(r'\bwalk past the\b', route, re.IGNORECASE))
    ...
    elif n_room_trans > 0 or n_door_trans > 0 or n_thru_hall > 0:
    ...
    # v119: n_pass counts "walk past the" steps directly injected in route (no [pass:] markers)
    # Anchor note: emphasize the direct phrase in the route
    if n_pass > 0:
        anchor_note += (f" Route includes {n_pass} 'walk past the X' step(s) — include these phrases in the instruction."
...
def clean(raw, sv):
    ...
    # v117: Fix "walk into the hallway" → "walk through the hallway" (GT: through, not into).
    # "walk into the hallway" → "walk through the hallway" / "go through the hallway"
    _hallway_into_re = re.compile(r'\bwalk\s+into\s+the\s+(?:hallway|corridor)\b', re.IGNORECASE)
    def _fix_hallway_into(m):
        verb = 'Walk' if m.group(0)[0].isupper() else 'walk'
        return f'{verb} through the hallway'
    result = _hallway_into_re.sub(_fix_hallway_into, result)

    # v117: Convert "continue [X]" → "keep going [X]" or "go forward [X]" (GT: continue=10.4%, ours=22%)
    # "continue forward" → "keep going" | "continue down" → "keep going" | standalone "continue" → "go forward"
    result = re.sub(r'\bContinue\s+forward\b', 'Keep going', result)
    result = re.sub(r'\bcontinue\s+forward\b', 'keep going', result)
    result = re.sub(r'\bContinue\s+straight\b', 'Go straight', result)
    result = re.sub(r'\bcontinue\s+straight\b', 'go straight', result)
    result = re.sub(r'\bContinue\s+down\b', 'Go down', result)
    result = re.sub(r'\bcontinue\s+down\b', 'go down', result)
    result = re.sub(r'\bContinue\s+through\b', 'Walk through', result)
    result = re.sub(r'\bcontinue\s+through\b', 'walk through', result)
    result = re.sub(r'\bContinue\s+to\s+the\b', 'Walk to the', result)
    result = re.sub(r'\bcontinue\s+to\s+the\b', 'walk to the', result)
    result = re.sub(r'\bContinue\s+for\b', 'Keep going for', result)
    result = re.sub(r'\bcontinue\s+for\b', 'keep going for', result)
...
async def main():
    ...
    for ep in episodes:
        ...
        # v119: Restore had_pass suppression — walk_past only from route-injected "walk past the X" steps.
        # LLM ignores [pass: X] markers (v118=3.5% despite no suppression). Now route injection is direct.
        # Suppress spontaneous walk_past from non-route-injected episodes.
