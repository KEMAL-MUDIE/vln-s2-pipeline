#!/usr/bin/env python3
"""
Gate 4 Visual v106 — v24 RECONSTRUCTION: restore "passing the X" (79.7% in v24) + hallway context.

CRITICAL DISCOVERY (vocabulary analysis v24 vs v104):
  v24 (40.24% SR): "passing the"=79.7%, "walk into"=52.6%, "hallway"=51.8%, "into the hall"=33.7%
  v104 (??% SR):   "passing the"=0.0%,  "walk into"=50.7%, "hallway"=24.1%, "into the hall"=4.2%

  v24 used "passing the X" in 79.7% of episodes — motion confirmation while navigating.
  "Passing the X" (gerund = continuous motion) != "walk past the X" (can trigger stop).
  Our v103+ set P_PASS_MARKER=0.0 which removed ALL landmark motion confirmations.
  We measured "walk past the" (dropping) but missed "passing the" (which is fine/helpful).

  Root cause: [pass: X] -> LLM can write either "walk past the X" OR "passing the X".
  Fix: Restore P_PASS_MARKER but FORCE "passing the X" style via prompt rule change.

v106 CHANGES FROM v105:
  1. P_PASS_MARKER = 0.80: Restore midpoint landmarks for motion confirmation
     - Prompt rule: "[pass: X] -> write 'passing the X' (NOT 'walk past the X')"
     - Expected: "passing the"=~60-70% (v24=79.7%)
  2. Restore "(-> hallway)" in route for first hallway per episode (max 1 hallway label)
     - Previously: hallway turns -> just "turn {direction}" (suppressed room context)
     - v106: first hallway transition -> "(-> hallway) turn {direction}" or "(-> hallway)"
     - Subsequent hallways -> still just "turn {direction}" (prevent repetition)
     - Expected: "into the hall"=~20-25% (v24=33.7%), hallway%=~35-40% (v24=51.8%)
  3. Keep v105 high-turn: P_TURN_UNANCHORED=0.90, angle<30deg
  4. Keep v104 vocabulary: "walk straight", "walk into the X", no "enter the X"
  5. Keep P_THRU_MARKER=0.0 (no "through the X" markers)

  Expected effects:
    passing the:    0.0% -> ~65%  (v24=79.7%)
    turn left:     ~40% -> ~45-50%  (v24=51.6%)
    walk into:     ~50% -> ~50%  (v24=52.6%)
    into the hall:  4.2% -> ~20-25%  (v24=33.7%)
    hallway%:      24.1% -> ~35-40%  (v24=51.8%)
    SR target: ~40%+ (full v24 reconstruction, should match or exceed v24's 40.24%)

Phase 2 checkpoint: outputs/gate4_v106_phase2_checkpoint.json (NEW -- fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v106.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v106.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v106_phase2_checkpoint.json"  # v106 — fresh P2 (v24 reconstruction: passing the X + hallway)
...
_CONTINUE_MOTION_RE = re.compile(
    # v83: only suppress bare motion-only continues (forward/straight/ahead/walking/moving).
    # KEEP: through/into/along/toward/past/down/up/across/in (GT-style continuation phrases).
    # GT uses "continue toward the X", "continue through the arch", "continue up the stairs"
    # which are natural R2R patterns; banning them caused continue% = 1.2% (GT=10.4%).
    r'\bContinue\s+(forward|straight|ahead|walking|moving)\b',
    re.IGNORECASE
)
...
async def main():
    ...
    print('=== Gate 4 v84: v83 + GT-calibrated stop phrases ===')
