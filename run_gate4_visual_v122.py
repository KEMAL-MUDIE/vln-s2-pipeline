#!/usr/bin/env python3
"""
Gate 4 Visual v122 — Keep v121's good motion-word fixes; revert bad (→ room) reduction.
v121 results: toward_the=17.0% (GT=3.8%), hallway=44.3% (GT=20.4%) — HUGE regressions!
Root cause:
  1. (→ room) frequency 72%→30%: LLM fills in "hallway" when no room context → hallway jumps.
  2. (→ room) rule "go to/head to/walk to": "head toward" creates toward_the=17%.
Good v121 fixes to KEEP:
  - _ws_variants removed "go forward"/"keep going" → reduces go_forward/keep_going overcorrection
  - clean() "keep going"→"go straight": reduces 37.7% to ~2%
  - MOTION WORDS/DISTANCE rule: prefer "walk straight"/"go straight"
Bad v121 fixes to REVERT:
  - (→ room) 30% frequency → REVERT to 100% (all non-door room transitions get room label)
  - (→ room) "go to/head to" rule → REVERT to simple "walk into" (accepts 35.9% walk_into but prevents toward_the explosion)

Expected v122: go_forward~10%, keep_going~2%, walk_straight~10%, walk_past~8%, toward_the~5%.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v122.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v122_phase2_checkpoint.json"  # v122 — good v121 fixes + revert (→ room) reduction
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    ...
                    # v122: REVERTED (→ rm) frequency to 100% (was 30% in v121 → hallway=44.3%, toward_the=17%).
                    # Keeping all non-door room transitions with "(→ rm)" prevents LLM hallway hallucination.
                    if rng.random() < 0.28:
...
async def main():
    ...
    print(f"  v122: keep v121 motion-word fixes (no go_forward/keep_going in _ws_variants)")
    print(f"  v122: REVERT (→ room) 30% reduction → 100% (was causing hallway=44%, toward_the=17%)")
