#!/usr/bin/env python3
"""
Gate 4 Visual v198 -- FIX11-15: Adversarially-validated surface-text quality improvements.

BASE: v197 (FIX1-10+FIX10b — walk_to=7.3%, walk_past=49.6%, walk_toward=18.1%)

MOTIVATION (from 8-agent deep research + adversarial verification):
Adversarial agents REJECTED three aggressive walk_past changes (post-turn rebalancing,
final-segment fix, FIX7/FIX8 probability reduction) due to cascade risks and false premises.
v198 implements only SAFE, non-rejected improvements: surface-text quality fixes that
do NOT disturb the walk_past/walk_to/walk_toward distribution balance.

NEW IN v198:
FIX11: OOV Token Elimination.
  PROBLEM: 77 tokens not in R2R training vocabulary (terracotta, porcelain, carving,
           chapel, museum, featuring) appear in 250+ instructions. CMA LSTM maps to UNK.
           KL(train||v194)=0.6856 vs GT=0.0152 (45x worse). OOV tokens degrade LSTM encoding.
  FIX: Replace known OOV tokens with in-vocabulary equivalents via regex substitution.
  Expected: +0.5-1.5 SR. Walk distribution unchanged.

FIX12: Comma+Then Reduction.
  PROBLEM: 0.561 comma+then per instruction (GT=0.038, 14.7x excess). 53% of episodes
           have multiple instances. CMA attention fragments across excess clause boundaries.
  FIX: Keep first comma+then instance; convert excess to period-separated sentence.
  Expected: +0.5-1.5 SR. Affects ~979 episodes (53%).

FIX13: Walk-And Vague Filler Elimination.
  PROBLEM: "walk and turn/enter/go/continue" in 7.6% of episodes (GT=0.1%). The "walk and"
           prefix carries zero spatial information and fragments natural action chains.
  FIX: Strip "walk and " prefix, keeping the meaningful verb capitalized.
  Expected: +0.3-0.5 SR. ~140 episodes.

FIX14: Stop Phrase Cap.
  PROBLEM: "stop in front of" at 11.0% (GT=4.7%, 2.3x excess). Forces CMA's front-of
           spatial model on configurations where it doesn't fit.
  FIX: Cap by converting 57% of occurrences to "stop near" (→ ~5.5%, GT=4.7%).
  Expected: +0.5-1.0 SR. No cascade risk.

FIX15: Turn Vocabulary Diversification.
  PROBLEM: "turn left/right" at 68.2% (GT=41.3%). GT uses "make a left/right" (3.3%) and
           "take a left/right" (7.8%), both at 0.3% in v197. Missing egocentric turn forms.
  FIX: Replace ~20% of "turn left"/"turn right" with "make a left"/"take a left" equivalents.
  Expected: +0.5-1.5 SR. Walk distribution unchanged.

INHERITED FROM v197: FIX1-10, FIX10b (walk_to=7.3%, walk_past=49.6%, walk_toward=18.1%)
ASSEMBLY-ONLY: Reuse outputs/gate4_v198_phase2_checkpoint.json (copy of v190 P2).

Expected v198:
  walk_to: ~7% (GT=5.1%) — unchanged from v197 (safe)
  walk_past: ~49% (GT=10.5%) — UNCHANGED from v197 (no aggressive walk_past changes)
  walk_toward: ~18% (GT=4.9%) — unchanged from v197
  comma+then: 0.561/ep -> ~0.05/ep (GT=0.038) [FIX12]
  turn diversity: make/take a left/right -> ~7% (from ~0.3%) [FIX15]
  stop_in_front_of: 11% -> ~5.5% (GT=4.7%) [FIX14]
  Predicted SR: 62-70%+ (conservative +2-4pp from v197 baseline)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v198_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v198.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v198.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v198_phase2_checkpoint.json"  # v198: assembly-only, copy of v190 P2 (FIX11-15)
...
def replace_go_straight_with_landmarks(text: str, phase_c_descs: Dict[str, str]) -> str:
    ...
    def _replace_gs(m):
        ...
        if lm_idx[0] < len(landmarks):
            ...
            lm_idx[0] += 1
            replacement = f"walk past the {lm}"  # v194 FIX8: was "walk to the" (walk_to injection root cause)
        else:
...
async def main():
    ...
    for ep in episodes:
        ...
        _v197_fix10_rng = random.Random(eid ^ 0xF1A2_B3C4)  # separate RNG for walk_toward decision
        def _intermediate_walk_to_to_past(text_in):
            ...
            if len(sentences) <= 1:
                ...
                def _single_sent_conv(m):
                    if _v193_fix7_rng.random() < 0.90:
                        cap = m.group(0)[0].isupper()
                        obj = m.group(1).strip()
                        # v197 FIX10: 40% → "walk toward the X", 60% → "walk past the X"
                        if _v197_fix10_rng.random() < 0.40:
            ...
            for i, sent in enumerate(sentences):
                ...
                if is_last or has_stop:
                    ...
                    # Non-final, non-stop sentence: convert walk_to → walk_past or walk_toward
                    def _conv_wt(m):
                        if _v193_fix7_rng.random() < 0.90:
                            cap = m.group(0)[0].isupper()
                            obj = m.group(1).strip()
