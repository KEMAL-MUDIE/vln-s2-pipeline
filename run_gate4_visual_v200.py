#!/usr/bin/env python3
"""
Gate 4 Visual v200 -- FIX17: FIX7 Output Diversification (GT vocabulary expansion beyond walk_past/walk_toward).

BASE: v199 (FIX1-16 — walk_past=37.3%, walk_toward=17.6%, avg_words=26.7, GT-match=0.842)

MOTIVATION:
GT train analysis reveals: walk_past (11.6%) is just one of several "passing" verbs in GT.
GT also uses: go_past (2.8%), pass_the (5.3%), continue_past (0.7%), head_toward (1.4%).
FIX7 currently converts walk_to→ exclusively walk_past (60%) or walk_toward (40%) — ignoring
this rich passing vocabulary that CMA was trained on. Expanding to match GT distribution
reduces walk_past inflation while maintaining semantic correctness.

GT passing vocabulary total: walk_past(11.6%) + go_past(2.8%) + pass_the(5.3%)
                              + continue_past(0.7%) + head_toward(1.4%) = ~21.8%
GT toward vocabulary: walk_toward(6.9%) + head_toward(1.4%) + go_toward(1.3%) = ~9.6%
GT ratio: 69% pass-by / 31% toward.

NEW IN v200:
FIX17: FIX7 Output Diversification — expand replacement vocabulary from 2 to 6 phrases.
  PROBLEM: FIX7 (with FIX10) uses only walk_past (60%) and walk_toward (40%).
           walk_past= 37.3% in v199 (GT=11.6%). FIX7 contributes ~13pp of this excess.
  FIX: Replace the 2-phrase mix with a 6-phrase GT-native vocabulary:
       P=0.35: "walk past the X"    (walk_past — main GT intermediate phrase)
       P=0.24: "walk toward the X"  (walk_toward — GT=6.9%)
       P=0.18: "go past the X"      (go_past — GT=2.8%, natural navigation verb)
       P=0.12: "pass the X"         (pass_the — GT=5.3%, compact and natural)
       P=0.05: "continue past the X"(continue_past — GT=0.7%, sentence variety)
       P=0.06: "head toward the X"  (head_toward — GT=1.4%, alternative directional)
  EXPECTED: walk_past ~32% (−5.5pp from v199=37.3%, FIX7 walk_past fraction: 60%→35%)
            go_past ~2.4% (new, GT=2.8%)
            pass_the ~1.6% (new, GT=5.3%)
            walk_toward ~16% (slight decrease; total toward stays ~22%)
  Seed: _v197_fix10_rng = random.Random(eid ^ 0xF1A2_B3C4) — REUSE existing FIX10 RNG

INHERITED FROM v199: FIX1-16 (FIX8 diversification + surface quality + OOV + turn)
INHERITED FROM v197: FIX1-10+FIX10b (walk_to=7.3% base)
ASSEMBLY-ONLY: Reuse outputs/gate4_v200_phase2_checkpoint.json (copy of v190 P2).

Expected v200:
  walk_past: ~32% (GT=11.6%) — −5.5pp from v199 [FIX17 expands FIX7 vocabulary]
  go_past: ~2.4% (GT=2.8%) — new metric, close to GT!
  pass_the: ~1.6% (GT=5.3%) — new metric, partial GT match
  walk_toward: ~16% (GT=6.9%) — slight improvement
  All v199 fixes (FIX11-16) inherited unchanged.
  Predicted SR: 64-73% (conservative +1-2pp from v199 baseline)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v200_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v200.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v200.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v200_phase2_checkpoint.json"  # v200: assembly-only, copy of v190 P2 (FIX17)
...
async def main():
    ...
    for ep in episodes:
        ...
        def _pick_passing_replacement(cap, obj):
            # v200 FIX17: 6-phrase GT-native distribution (GT ratios: 69% pass-by / 31% toward)
            # walk_past(35%) | walk_toward(24%) | go_past(18%) | pass_the(12%) | continue_past(5%) | head_toward(6%)
            # GT evidence: go_past=2.8%, pass_the=5.3%, continue_past=0.7%, head_toward=1.4%
            ...
            if r < 0.35:
                ...
                return ('Continue past the ' if cap else 'continue past the ') + obj
            else:
