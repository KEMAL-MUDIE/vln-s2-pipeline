#!/usr/bin/env python3
"""
Gate 4 Visual v49 — Best of v47+v48: hidden rm (v47) + rm turn sampling (v48)

v47 RESULTS:
  hallway: 22.6% (best), through_the: 28.0% (good), turns: 0.99 (regression)
v48 RESULTS:
  turns: 0.70 (best!), hallway: 27.7% (good), through_the: 32.0% (regression from → rm)

v49 FIX: Hide rm from route (v47) AND apply P_TURN_UNANCHORED to rm turns (v48).
  - rm turns sampled at P_TURN_UNANCHORED=0.55 (same as v48)
  - For kept rm turns: NO room name in route (same as v47 — "turn {direction}" only)
  - Prevents → rm from inflating through_the via "through the hallway" LLM behavior
  - All v47/v48 prompt improvements retained

Expected BEST of both:
  avg_turns:    0.99 → ~0.68-0.72 (rm turn sampling keeps turns low)
  hallway:      27.7% → ~22-24% (hidden rm keeps hallway low, like v47)
  through_the:  32.0% → ~27-28% (hidden rm prevents inflation, like v47)
  toward_the:    8.8% → ~8-9% (maintained)
  GT-match:     0.955 → ~0.955-0.960 (similar, turns don't affect formula directly)

Phase 2 checkpoint: outputs/gate4_v49_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v49.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v49.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v49_phase2_checkpoint.json"
...
async def main():
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v48=0.70, v47=0.99) ← v49 KEY FIX")
