#!/usr/bin/env python3
"""
Gate 4 Visual v60 — P_TURN_UNANCHORED 0.55→0.45 to match GT avg_turns (0.59)

v59 RESULTS (base):
  GT-match: 0.958, toward_the: 2.8%, through_the: 28.0%, walk_past: 13.3%, hallway: 25.1%
  avg_turns: 0.71 (GT=0.59) — persistent 0.12/ep overcount

KEY FINDING on avg_turns:
  P_TURN_UNANCHORED controls sampling of bare unanchored turns (no landmark, no room name).
  Reducing 0.55→0.45: for each unanchored turn, 45% kept vs 55% before (~18% fewer unanchored turns).
  Requires FRESH Phase 2 generation (route changes → different LLM prompts).

v60 FIX:
  - P_TURN_UNANCHORED: 0.55 → 0.45 (target avg_turns ~0.58-0.60 matching GT=0.59)
  - Fresh Phase 2 checkpoint (route structure changes with turn sampling)
  - Retain v59 post-processing: doorway, walk_past, hallway surgical fixes

Math:
  Current: P=0.55, avg_turns=0.71
  Estimated: P=0.45 → avg_turns ≈ 0.71 × (0.45/0.55) ≈ 0.58 (GT=0.59) — near perfect!
  eps_with_turn: 55.6% → ~46% (GT=41.3%)

Phase 2 checkpoint: outputs/gate4_v60_phase2_checkpoint.json (FRESH — turn sampling changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v60.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v60.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v60_phase2_checkpoint.json"  # FRESH — turn sampling changed!
...
P_TURN_UNANCHORED = 0.45  # v60: reduced from 0.55 → target avg_turns ~0.58 (GT=0.59)
...
async def main():
    ...
    print('=== Gate 4 v60: P_TURN_UNANCHORED 0.55→0.45 to match GT avg_turns (0.59) ===')
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v59=0.71) ← v60: P_TURN_UNANCHORED 0.55→0.45")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v59=2.8%) ← KEY METRIC")
    print(f"  hallway:       {100*hallway_n/n:.1f}%  (GT=20.4%, v59=25.1%) ← KEY METRIC")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v59=0.958, v54fix=0.961, v53=0.961)")
