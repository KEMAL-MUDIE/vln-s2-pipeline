#!/usr/bin/env python3
"""
Gate 4 Visual v14 — Selective Landmark Provision (GT-Distribution Match)

THE ROOT PROBLEM WITH v11/v12/v13:
  The LLM uses ANY landmark context provided, regardless of "optional" labels.
  v11: showed ALL landmarks as required → 87.5% anchor rate
  v12: showed ALL landmarks as required → 95.0% anchor rate
  v13: showed ALL landmarks as "optional" → 83.1% anchor rate (still OOD)

  GT turn-anchor rate = 16.5%. No amount of prompt engineering fixes this
  when we provide landmark context for every turn.

THE FIX (v14):
  SELECTIVE LANDMARK PROVISION:
  - For each turn, with probability p_anchor (= 0.20): provide the landmark
  - For 80% of turns: provide NO landmark context (just "turn left" or "turn right")
  - The LLM will naturally generate anchor phrases ONLY for the ~20% of turns
    where we provided the landmark
  - Result: ~18-22% turn-anchor rate (matching GT's 16.5%)

STOP/WAIT FIX:
  Post-process to enforce GT distribution: 50% stop / 30% wait / 17% none
  Use regex replacement after LLM generation to normalize stop/wait words.

EXPECTED OUTCOME:
  - Turn-anchor rate: ~18-22% (GT = 16.5%)
  - Stop%: ~50%, Wait%: ~30%, Neither%: ~17%
  - avg_words: 22-28 (close to GT = 26.8)
  - SR: should approach or exceed GT baseline (63.89%) + synergy with cache

Output: outputs/datasets/val_unseen_generated_gemma_visual_v14.json.gz
"""
import asyncio
import gzip
import json
...
OUTPUT      = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v14.json.gz"
CKPT        = ROOT / "outputs" / "gate4_visual_v14_checkpoint.json"
...
# ── Constants (GT-derived) ────────────────────────────────────────────────────

# Probability of providing landmark context for each turn
# Tuned to produce ~16-20% turn-anchor rate (matching GT's 16.5%)
P_ANCHOR_TURN = 0.22

# GT stop/wait distribution
P_STOP    = 0.508  # GT: 50.8%
P_WAIT    = 0.305  # GT: 30.5%
# P_NEITHER = 1 - P_STOP - P_WAIT = 18.7% (close to GT's 16.5%)
...
class ScenePathIndex:
    """Pure path-similarity index — no anchor priority (v14)."""
    def __init__(self, all_eps):
        self.by_scene = defaultdict(list)
        n_anchor = 0
        for ep in all_eps:
            sc    = ep["scene_id"].split("/")[-2]
            instr = ep.get("instruction", {}).get("instruction_text", "") if isinstance(ep.get("instruction"), dict) else ""
            if not instr.strip(): continue
            anchored = has_turn_anchor(instr)
            if anchored: n_anchor += 1
        ...
        print(f"ScenePathIndex: {len(self.by_scene)} scenes, {total} eps, {n_anchor} anchored ({100*n_anchor/max(total,1):.1f}%)")

    def top_k(self, ep, ep_feat, k=8):
        sc   = ep["scene_id"].split("/")[-2]
        pool = [e for e in self.by_scene.get(sc, []) if e["episode_id"] != ep["episode_id"]]
        if not pool: return []
        scored = sorted([(path_similarity(ep_feat, e["feat"]), e) for e in pool], key=lambda x: -x[0])
...
def get_landmark_info(ep, pf_map, old_map, primitives):
    ...
    if pf:
        ...
        for i, td in enumerate(pf_turns):
            ...
            result["turns"].append({"direction":d,"landmark":best,"room":td.get("room",""),
                                    "room_trans":td.get("room_transition","")})
        for i in range(len(pf_turns), len(turn_dirs)):
            result["turns"].append({"direction":turn_dirs[i],"landmark":"","room":"","room_trans":""})
        goal = pf.get("goal") or {}
        result["goal_landmark"] = goal.get("stop_landmark","") or goal.get("main_landmark","the destination")
        ...
        result["goal_landmark"] = gl.get("stop_landmark") or sc.get("stop_landmark") or "the destination"
...
# ── Selective landmark provision (KEY v14 innovation) ─────────────────────────

def selective_turns(lm_info: Dict, eid: int) -> List[Dict]:
    """
    For each turn, decide whether to provide the landmark based on P_ANCHOR_TURN.
    Uses eid-seeded RNG for determinism. Returns turns with landmark optionally cleared.
    """
    rng = random.Random(eid ^ 0xF4A7)
    turns = lm_info.get("turns", [])
    result = []
    for t in turns:
        show_lm = rng.random() < P_ANCHOR_TURN
        result.append({
            "direction": t["direction"],
            "landmark":  t["landmark"] if (show_lm and t["landmark"]) else "",
            "room":      t["room"] if show_lm else "",
            "room_trans": t["room_trans"] if show_lm else "",
        })
    return result


# ── Stop/wait choice — GT distribution ───────────────────────────────────────

def choose_stop(goal_lm: str, goal_room: str, eid: int) -> Tuple[str, str]:
    """Returns (phrase, type). GT: stop=50.8%, wait=30.5%, neither=18.7%."""
    ...
    if r < P_STOP:
        ...
        patterns = [f"Wait near {lm}.", f"Wait by {lm}.", "Wait there."]
        return rng.choice(patterns), "wait"
    else:
        return "", "none"
...
# ── Route description for prompt (selective landmarks) ────────────────────────
...
Style: Be natural and brief (18-30 words). Write like a human R2R annotator:
- Use landmarks in the route description ONLY if they are shown in brackets
- For turns without a landmark: just say "turn left" or "turn right"
- Use natural R2R patterns: "walk past the X", "turn left into the hallway", "walk through the door"
- Endings vary: "Stop near X.", "Wait near X.", "Wait there." or no explicit ending
- Start verb is given — use it"""


def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns, stop_phrase, stop_type):
    ...
    if stop_type == "stop":
        ending_req = f'End: "{stop_phrase}"'
    elif stop_type == "wait":
        ending_req = f'End: "{stop_phrase}"'
    else:
        ending_req = "No explicit stop/wait needed"

    # Only count turns with landmarks shown
    n_shown_lms = sum(1 for t in sel_turns if t.get("landmark"))
...
async def generate(tasks, concurrency=12):
    ...
    results = {}
    for eid, result in await asyncio.gather(*[generate_one(client,t,sem,done,total,t0) for t in tasks]):
        results[eid] = result
    return results
...
async def main():
    ...
    ap.add_argument("--p-anchor", type=float, default=P_ANCHOR_TURN,
                    help=f"Probability of showing landmark per turn (default {P_ANCHOR_TURN})")
    args = ap.parse_args()

    print("=== Gate 4 v14: Selective Landmark Provision (GT-Distribution) ===")
    print(f"  p_anchor_per_turn: {args.p_anchor} → expected anchor rate ~{args.p_anchor*100:.0f}%")
    print(f"  stop/wait: {P_STOP*100:.0f}%/{P_WAIT*100:.0f}%/{(1-P_STOP-P_WAIT)*100:.0f}% (GT=50.8/30.5/16.5)")
    ...
    gt_map = {ep["episode_id"]: ep.get("instruction",{}).get("instruction_text","")
    ...
    for ep in episodes:
        ...
        # KEY: selective landmark provision
        sel_turns  = selective_turns(lm_info, eid)
        stop_phrase, stop_type = choose_stop(lm_info["goal_landmark"], lm_info["goal_room"], eid)

        if str(eid) in ckpt:
            skipped += 1
            task_meta[eid] = {"sv": gt_start_verb(gt_instr), "goal_lm": lm_info["goal_landmark"],
                              "stop_phrase": stop_phrase, "stop_type": stop_type}
            continue

        examples = sc_idx.top_k(ep, ep_feat, k=args.k_similar)
        prompt, sv = build_prompt(ep, gt_instr, examples, lm_info, prims, sel_turns, stop_phrase, stop_type)
    ...
    # Verify selective landmark stats
    total_turns = sum(len(ep.get("reference_path",[]))-1 for ep in episodes if len(ep.get("reference_path",[]))>1)
    shown_turns = 0
    for ep in episodes:
        eid = ep["episode_id"]
        prims = get_primitives(ep)
        lm_info = get_landmark_info(ep, pf_map, old_map, prims)
        sel = selective_turns(lm_info, eid)
        shown_turns += sum(1 for t in sel if t.get("landmark"))
    print(f"  Turns with landmark shown: {shown_turns}/{total_turns} = {100*shown_turns/max(total_turns,1):.1f}%")
    ...
    for ep in episodes:
        ...
        # POST-PROCESS: enforce stop/wait distribution
    ...
    out_data = {"episodes": episodes_out, "instruction_vocab": gt_data.get("instruction_vocab",{})}
