#!/usr/bin/env python3
"""
R2R Val_seen Split Annotation — Gemma text-only mode.
Annotates all 778 val_seen episodes with auto-generated instructions.
No GPU required — uses Gemma 4 31B AWQ API only (path geometry → instruction).

Output: outputs/datasets/val_seen_generated_gemma_textonly.json.gz
Checkpoint: outputs/val_seen_gate4_checkpoint.json
"""
import asyncio
import gzip
import json
...
VAL_SEEN_PATH = "/mnt/nvme0/vln_habitat/habitat_data/datasets/vln/mp3d/r2r/v1/val_seen/val_seen_patched.json.gz"
OUTPUT_PATH = ROOT / "outputs" / "datasets" / "val_seen_generated_gemma_textonly.json.gz"
CHECKPOINT_PATH = ROOT / "outputs" / "val_seen_gate4_checkpoint.json"
...
async def main():
    ...
    print("=== R2R Val_seen Split Auto-Annotation (Text-Only Mode) ===")
    print(f"Model:    Gemma 4 31B AWQ @ 10.77.32.231:8000")
    print(f"Input:    {VAL_SEEN_PATH}")
    print(f"Output:   {OUTPUT_PATH}")

    with gzip.open(VAL_SEEN_PATH, "rt") as f:
        data = json.load(f)
    episodes = data["episodes"]
    print(f"  {len(episodes)} val_seen episodes loaded")
    ...
    print("\n[Gate 2] Analyzing val_seen paths...")
    tasks = []
    for ep in episodes:
    ...
    print("\n[Samples] First 3 val_seen instructions:")
    ...
    print("[Gate 6] Assembling val_seen dataset...")
    assembled = []
    for ep in episodes:
        eid = ep["episode_id"]
