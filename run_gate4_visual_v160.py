#!/usr/bin/env python3
"""
Gate 4 Visual v160 — "walk forward" substitution: replace go_straight with neutral motion, not visual landmarks.

ANALYSIS OF SR GAP:
  v24 (go_straight=23%, simple cmds) → SR=40.24% (BEST non-GT).
  v157 (replace_gs→visual landmarks: "walk to the grey sofa") → SR=~34%.

  Key hypothesis: replace_go_straight_with_landmarks converts "go straight" → "walk to the [visual object]".
  When Phase C landmark is CORRECT, this adds useful context. When INCORRECT/MISLEADING (model can't find
  the object or it's in the wrong direction), it actively hurts navigation.

  v158/v159 test: disable replace_gs → keep raw "go straight" → SR=?
  v160 test: replace "go straight" → "walk forward" (neutral, no visual confusion)
    Implementation: pass empty phase_c_descs={} to replace_go_straight_with_landmarks
    → always uses fallback "walk forward" instead of "walk to the [visual landmark]"

  Expected v160: go_straight=0%, "walk forward" high%, GT-match≈v157=0.974
  If SR improves over v157: visual landmarks are the problem (not "go straight" vs "walk forward")
  Compare v158/v159: if they also improve SR, then ANY motion command beats visual landmarks.

v157 ACTUAL: GT-match=0.974, walk_straight=9.0%, go_straight=6.8%, avg_turns=0.80

v160 CHANGES from v157:
  1. replace_go_straight_with_landmarks: pass empty vision_map → always uses "walk forward" fallback
     Expected: go_straight=0%, "walk forward" appears in ~45% of episodes (raw rate)
  2. P_TURN_UNANCHORED: 0.12→0.09 (same as v158: target avg_turns≈0.60 ≈ GT=0.59)
  3. Everything else identical to v157

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C for TURNS)
Phase 2 checkpoint: outputs/gate4_v160_phase2_checkpoint.json (FRESH: new P_TURN=0.09, same as v158)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v160.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v160.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v160: REUSE v152 room-context Phase C for turns
...
P2_CKPT       = ROOT / "outputs" / "gate4_v160_phase2_checkpoint.json"  # v160: FRESH — P_TURN=0.09, P_THRU=0.60 (same as v158/159)
...
P_ANCHOR_EPISODE = 0.18   # v155: reduced 0.231→0.18 to push turn-anchor toward GT 16.5% (v154 was 23% with spontaneous additions)
...
P_TURN_UNANCHORED = 0.09  # v160: 0.12→0.09 (same as v158, target avg_turns≈0.60 ≈ GT=0.59)
...
async def main():
    ...
    print('=== Gate 4 v160: walk_forward substitution (no visual landmarks in straight segments) ===')
    print(f"  v156 METRICS: GT-match=0.975✓, turn-anchor=17.5%✓, toward_the=0.9%✓, take_few=0.2%✓")
    print(f"  v156 GAPS: walk_straight=0.0%(GT=8.8%), go_straight=3.9%(GT=7.9%), avg_turns=0.86(GT=0.59)")
    print(f"  v157 FIX 1: _ws_variants restores 'walk straight' as 1/4 alt (25% → ~8.75% walk_straight)")
    print(f"  v157 FIX 2: go_straight skip 8%→15% (v156=3.9%, target GT=7.9%)")
    print(f"  v157 FIX 3: P_TURN_UNANCHORED 0.18→0.12 (avg_turns 0.86→~0.57 ≈ GT=0.59)")
    print(f"  v157 FIX 4: P_THRU_MARKER 0.75→0.60 (through_the 33.7%→~27.0% ≈ GT=27.2%)")
    print(f"  GT: walk_straight=8.8%, go_straight=7.9%, avg_turns=0.59, turn-anchor=16.5%")
    print(f"  Expected: walk_straight~9%, go_straight~7%, avg_turns~0.57, through_the~27%, GT-match≥0.97")
    ...
    print(f"  v105: HIGH-TURN — P_TURN_UNANCHORED=0.12 (v157 reduced from 0.18)")
    ...
    for ep in episodes:
        ...
        # v160: replace_go_straight_with_landmarks with EMPTY vision_map → always uses "walk forward" fallback.
        # HYPOTHESIS: the visual landmark substitution ("walk to the grey sofa") confuses the model
        # when the described object is not exactly at the navigation point. Using "walk forward" instead
        # gives a neutral, non-misleading motion command without visual reference confusion.
        # v157: 15% skip → 6.8% go_straight, 85% get "walk to the X" (visual landmarks)
        # v160: 0% skip, empty vision_map → 0% go_straight, ~45% get "walk forward" instead
        # Compare with v158 (100% skip → go_straight stays) and v157 (walk to the X) for SR ranking.
        text = replace_go_straight_with_landmarks(text, {})  # empty dict → always fallback "walk forward"
    ...
    print(f"  go_straight:   {100*go_straight_n/n:.1f}%  (GT=7.9%, v157=6.8%) ← v160: should be ~0% (replaced with walk forward)")
    print(f"  walk_straight: {100*wlk_straight_n/n:.1f}%  (GT=8.8%, v157=9.0%) ← v160: unchanged")
    ...
    print(f"  [v157 targets: walk_straight~9%(GT=8.8%), go_straight~7%(GT=7.9%), avg_turns~0.57(GT=0.59), through_the~27%(GT=27.2%), GT-match≥0.97]")
