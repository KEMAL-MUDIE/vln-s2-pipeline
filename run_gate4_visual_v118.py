#!/usr/bin/env python3
"""
Gate 4 Visual v118 — Calibrate doorway/hallway notation: v117 over-corrected both.
v117 results: through_the=44.8% (GT=27.2%), hallway=39.1% (GT=20.4%) — both too high.
Fixes: P(door->room) 60%->35%, P(through_hallway) 40%->20%.
Also: remove had_pass suppression (over-suppressed walk_past to 3.4% GT=8.7%).
Rely on _wpt_counter + _GENERIC_WPT_OBJ_RE only (v114 style). Target walk_past ~10-15%.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v118.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v118_phase2_checkpoint.json"  # v118 — calibrated doorway probabilities
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    # v118: Allow hallway with "through" notation 20% of the time (v117=40% → hallway=39.1% too high)
                    if rng.random() < 0.20:
                        if direction:
                    ...
                    # v118: 35% chance of doorway notation (v117=60% → through_the=44.8% too high; target ~27%)
                    if rng.random() < 0.35:
...
async def main():
    ...
    for ep in episodes:
        ...
        # v118: REMOVED had_pass suppression (over-suppressed walk_past to 3.4% in v115-v117).
        # Revert to v114-style: rely on _wpt_counter cap (1 per instruction) + _GENERIC_WPT_OBJ_RE.
        # Expected walk_past: 10-15% (v114 had 30.6% — cap reduces it; target GT=8.7%).
