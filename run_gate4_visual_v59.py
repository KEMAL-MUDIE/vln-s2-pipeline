#!/usr/bin/env python3
"""
Gate 4 Visual v59 — v56 checkpoint + walk_past + hallway surgical post-processing

v58 RESULTS (base):
  GT-match: 0.958, toward_the: 3.3%, through_the: 28.0%, walk_past: 13.3%, hallway: 26.0%
  walk_past reduction from v56 only 0.3pp (13.6→13.3%) — most episodes HAVE pass-type midpoints

KEY FINDING from v58 (walk_past post-processing):
  Only ~6 episodes had NO pass-type midpoints — walk_past is at its natural floor (~13.3%).
  The LLM generates "walk past the X" for episodes with valid pass-type objects in visual context.
  walk_past CANNOT be reduced below ~13% without prohibitions or removing visual context.

v59 NEW TARGET: hallway reduction (26.0% vs GT=20.4%, 5.6pp over)
  Root cause: LLM uses "walk through the hallway" / "walk down the hallway" despite ROOM NAMES rule.
  These patterns are NOT covered by the current prohibition (only "walk into the hallway" is).
  Surgical fix: detect mid-route hallway mentions (goal is NOT a hallway) → replace with "walk forward"

v59 FIX:
  - Reuse v56 Phase 2 checkpoint (RULES unchanged)
  - Retain v58's walk_past post-processing (removes ~6 hallucinated cases)
  - NEW: hallway post-processing:
    - Check stop_phrase for "hallway" → if stop mentions hallway, goal IS a hallway → keep all mentions
    - If stop does NOT mention hallway: mid-route patterns like "walk through the hallway",
      "walk down the hallway" are hallucinated → replace with "walk forward"
  Expected: hallway 26.0% → ~21-23% (GT=20.4%), GT-match maintained ~0.958

Phase 2 checkpoint: outputs/gate4_v56_phase2_checkpoint.json (REUSE — post-processing only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v59.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v59.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v56_phase2_checkpoint.json"  # REUSE v56 checkpoint!
...
async def main():
    ...
    print('=== Gate 4 v59: hallway surgical fix — remove mid-route hallway when goal is not hallway ===')
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v56=0.71) ← v59: hallway surgical post-proc")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v56=3.3%, v58=3.3%) ← KEY METRIC")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v58=0.958, v54fix=0.961, v53=0.961)")
