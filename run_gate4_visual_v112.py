#!/usr/bin/env python3
"""
Gate 4 Visual v112 — Fix over-suppression: move _GENERIC_WPT_OBJ_RE BEFORE "passing the" conversion.

v111 ACTUAL RESULTS:
  - walk_past:    12.6% (GT=8.7%) ← HUGE WIN! was 0.0% in v110
  - passing_the:  13.0% (combined; pure passing_the≈0%) ← conversion working
  - into_hallway: 1.0%  (GT=4.4%) ← maintained
  - hallway:      5.4%  (GT=20.4%)
  - continue:     15.8% (GT=10.4%)
  - avg_words:    22.3  (GT=26.8) ← PROBLEM: 3.5 words below GT!

ROOT CAUSE OF avg_words DROP:
  v111 order: "passing the" → "walk past the" THEN _GENERIC_WPT_OBJ_RE
  _GENERIC_WPT_OBJ_RE is too broad — matches furniture (armchair, sofa, table, chair, etc.)
  "passing the armchair" → "walk past the armchair" → "walk straight" (WRONG: removes 4 words!)
  This stripped ~3.5 words/instruction on average.

v112 FIX:
  Swap the order: _GENERIC_WPT_OBJ_RE FIRST, then "passing the" → "walk past the"
  - _GENERIC_WPT_OBJ_RE suppresses only [pass:] marker hallucinations (structural/wrong objects)
  - "passing the X" from LLM (with specific observed objects) → "walk past the X" — NOT suppressed
  - Expected: avg_words goes back to ~25-26w (GT=26.8w)
  - "walk past the" may rise to ~15-20% but with specific objects that exist in the scene

  Expected vocabulary targets:
    walk_past:    12.6% → ~15-20% (higher but specific objects from LLM observation)
    avg_words:    22.3w → ~25-26w (GT=26.8w, restored)
    continue:     15.8% → ~14%   (near GT=10.4%)
    into_hallway: 1.0%  → ~2%    (GT=4.4%, still slight under)
    SR target: 55-65%+ (better word count alignment with GT)

Phase 2 checkpoint: outputs/gate4_v112_phase2_checkpoint.json (NEW -- fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v112.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v112.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v112_phase2_checkpoint.json"  # v112 — fix over-suppression: _GENERIC_WPT_OBJ_RE before "passing the" conversion
