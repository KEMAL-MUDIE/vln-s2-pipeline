#!/usr/bin/env python3
"""
Gate 4 Visual v194 -- ROOT CAUSE FIX: replace_go_straight_with_landmarks injects walk_to.

ROOT CAUSE ANALYSIS:
  v190 actual: walk_to=44.8%(GT=5.1%), walk_past=11.3%(GT=8.7%)
  v193b (FIX7 P=0.90 + FIX7b P=0.80): walk_to=17.2%, walk_past=38.3%

  Root cause of walk_to=44.8% in v190 (vs 20.1% in raw P2 checkpoint):
    The assembly function replace_go_straight_with_landmarks() replaces EVERY "go straight"
    in the LLM output with "walk to the {landmark}" using Phase C visual data.
    Measurement: 59/200 episodes had walk_to ADDED by assembly (29.5%)!
    So of the 44.8% walk_to: ~20.1% from LLM, ~24.7pp INJECTED by assembly.

FIX 8 (KEY NEW FIX — ROOT CAUSE): Change replace_go_straight_with_landmarks replacement.
  OLD: "walk to the {lm}"  (causes model to treat landmark as DESTINATION → premature stop)
  NEW: "walk past the {lm}" (correct GT vocabulary for intermediate pass-through landmarks)
  This is assembly-only — no new LLM generation needed.
  Expected: walk_to drops from 17.2% → ~8% (close to GT=5.1%), walk_past → ~48%

FIX 7 (from v193b): Sentence-level walk_to → walk_past for NON-FINAL sentences (P=0.90).
  Handles remaining walk_to from LLM P2 output (source A).

FIX 7b (from v193b): "walk to the [room]" → "enter the [room]" at P=0.80.

INHERITED FROM v190/v193b: FIX 1-6 + FIX 7 + FIX 7b.
ASSEMBLY-ONLY: Reuse outputs/gate4_v194_phase2_checkpoint.json (copy of v190 P2).

Expected v194:
  walk_to: ~7-9% (from v193b=17.2%, GT=5.1%) — ROOT CAUSE FIXED
  walk_past: ~45-50% (high but semantically correct GT vocabulary)
  continue: ~17.7% (unchanged)
  exit: ~13-14% (unchanged)
  for_a_bit: 0% (unchanged)
  Predicted SR: 60-65%+ (root cause elimination + sentence-level post-processing)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v194_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v194.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v194.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v194_phase2_checkpoint.json"  # v194: assembly-only, copy of v190 P2 (root cause fix)
