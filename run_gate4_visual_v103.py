#!/usr/bin/env python3
"""
Gate 4 Visual v103 — v102 + REMOVE [thru:] and [pass:] landmarks (low-landmark, high-turns variant).

KEY HYPOTHESIS (from v24 wins analysis):
  v24 beat v62 in episodes where: through_the=0.9%, walk_past=0.3%, has_turn=90.7%
  v62 lost in those episodes: through_the=24.6%, walk_past=10.7%, has_turn=52.2%
  CONCLUSION: [thru:] and [pass:] LANDMARK INJECTIONS HURT SR in many episodes.
  The injected landmarks are INACCURATE (vision errors) → confuse the VLN model.
  v24 had no landmark injection → cleaner turn-based instructions → better model behavior.

v103 CHANGES FROM v102:
  1. P_THRU_MARKER = 0.0: Remove all [thru: description] from route
     (was 0.95; caused through_the=13.9%; v24-wins had 0.9% → landmark hurts)
  2. P_PASS_MARKER = 0.0: Remove all [pass: description] from route
     (was 0.10; caused walk_past=19.1%; v24-wins had 0.3% → landmark hurts)
  3. Remove [thru:] and [pass:] prompt rules (LLM won't see them)
  4. Restore P_HALLWAY_TURN=1.0 (no sampling → more explicit turns)
     Give the model clear directional instructions without confusing landmarks.

  Expected effects:
    through_the%:  13.9% → ~1-2% (only natural LLM generation, no route markers)
    walk_past%:    19.1% → ~2-3% (same)
    eps_with_turn: ~50% (restored, no 0.65 sampling)
    avg_words:     ~23-25w (shorter without landmarks)
    SR target: ≥ 40% (test if removing landmarks helps like v24)

Phase 2 checkpoint: outputs/gate4_v103_phase2_checkpoint.json (NEW — fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v103.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v103.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v103_phase2_checkpoint.json"  # v103 — fresh P2 (no landmarks, turn-heavy)
...
- STYLE: Avoid starting sentences with "Continue straight" — use action verbs instead ("Walk forward"). Keep instructions clean and turn-focused. Do NOT add "through the X" or "walk past the X" — only write what is explicitly shown in the route.
