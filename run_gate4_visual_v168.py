...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v168.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v168_phase2_checkpoint.json"  # v168: FRESH — P_DIR=0.12 + nav-verb ahead filter
...
P_TURN_UNANCHORED = 0.30  # v166: 0.55→0.30 — reduce avg_turns (v165=1.28 vs GT=0.59). GT unanchored/turn≈0.425
P_DIR_ROOM_TRANS  = 0.10  # v168: recalibrated from v167 measurement
                           # v167 (P_DIR=0.12) gave anchor%=19.8% (still 3.3pp above GT=16.5%)
                           # v164 (0.00)=6.1%, v166 (0.20)=23.2%, v167 (0.12)=19.8%
                           # v168 (0.10): anchor%≈17-18% (approaching GT=16.5%)
                           # ROOT FIX: extract_landmark_brief rejects nav-verb landmarks (Turn right → None)
                           # This prevents "walk to the Turn right" from appearing in instructions
...
async def main():
    ...
    print('=== Gate 4 v167: Calibrate anchor%=16.5% + fix nav-verb ahead labels ===')
    print(f"  v166 MEASURED: anchor%=23.2% (GT=16.5%), GT-match=0.8607 — still overcorrected")
    print(f"  FIX 1: P_DIR_ROOM_TRANS=0.12 (calibrated: 6.1+10.3=16.4% anchor%)")
    print(f"         v164(0.00)=6.1%, v166(0.20)=23.2% → v167(0.12)≈16.5% target")
    print(f"  FIX 2: Nav-verb ahead filter — 'Turn right' as ahead label → '' (prevents 'walk to Turn right')")
    print(f"  FIX 3 (kept): Furniture filtered from [pass:] — prevents armchair/sofa confusion")
    print(f"  v167: P_ANCHOR=0.03 + P_DIR=0.12 + P_TURN_UNANCHORED=0.30 + P_continue=0.28 + FRESH")
    print(f"    Expected: anchor%≈16.5%(GT!), avg_turns≈0.7-0.9, GT-match≥0.97")
