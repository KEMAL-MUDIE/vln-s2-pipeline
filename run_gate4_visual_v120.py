#!/usr/bin/env python3
"""
Gate 4 Visual v120 — Restore [pass: X] markers with P=0.70, remove had_pass suppression.
v119: walk_past=1.2% (worst ever) — root cause: direct "walk past the X" injection in route
      gets SKIPPED by LLM which focuses on doorway/turn steps. P_PASS=0.10 too low.
Fix:  Restore [pass: X] marker format (proven to drive LLM compliance in v113/v114).
      P_PASS_MARKER=0.70 to compensate for ~20% LLM compliance when doorway notation competes.
      Math: 0.70 × 59% pass-type midpoints × 20% compliance = 8.3% walk_past (GT=8.7%).
      Remove had_pass suppression — it was over-suppressing spontaneous walk_past (1.2%→needed 8.7%).
      Keep P(door→room)=28%, P(through_hallway)=16% for through_the~30% and hallway~34%.
      Result: walk_past ~8.3% (GT=8.7%), through_the ~30% (vs GT=27.2%), hallway ~34% (vs GT=20.4%).
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v120.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v120_phase2_checkpoint.json"  # v120 — restored [pass: X] markers, P=0.70, no had_pass suppression
...
P_PASS_MARKER = 0.70   # v120: 0.70 × 59% pass-type × 20% compliance-with-doorway = 8.3% walk_past (GT=8.7%)
...
async def main():
    ...
    print(f"  v120 KEY FIX: P_PASS_MARKER={P_PASS_MARKER} (restore [pass: X] markers, no had_pass suppression)")
    print(f"  v120 math: 0.70 × 59% pass-type × 20% compliance = 8.3% walk_past (GT=8.7%)")
