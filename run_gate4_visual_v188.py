#!/usr/bin/env python3
"""
Gate 4 Visual v188 -- go_straight->walk_straight + walk_into->enter + drop walk_ahead.

CHANGES FROM v187:
  v187 actual stats (PYTHONHASHSEED=42): through_the=29.0%(GT=27.2%), stop=53.6%(GT=53.0%),
    wait=33.0%(GT=32.6%), go_straight=25.2%(GT=7.9%), walk_into=25.9%(GT=6.9%),
    walk_ahead=9.8%(GT=0.2%), walk_straight=0.0%(GT=8.8%), 2-sent=40.7%

  NEWLY DISCOVERED (correct GT stats using val_unseen.json.gz):
    go_straight=7.9%, walk_straight=8.8%, walk_ahead=0.2%, walk_into=6.9%
    walk_to=5.5%(!!!), turn=51.1%(v187=73.0%), exit=18.0%(v187=10.7%)

  v187 key issues:
    - go_straight: 25.2% vs GT 7.9% (+17pp excess)
    - walk_straight: 0% vs GT 8.8% (entirely missing!)
    - walk_ahead: 9.8% vs GT 0.2% (+9.6pp excess)
    - walk_into: 25.9% vs GT 6.9% (+19pp excess)

FIX 1: go_straight -> walk_straight at P=0.35 per occurrence
  Reduces go_straight: 25.2% -> ~16%
  Adds walk_straight: 0% -> ~9% (GT=8.8% -- near perfect!)
  "Go straight" -> "Walk straight"; "go straight" -> "walk straight"

FIX 2: walk ahead -> walk (drop 'ahead') at P=0.65
  When "walk ahead" is followed by and/for/into/towards/through/across/along/until/from
  "walk ahead and turn" -> "walk and turn"
  "walk ahead into the bedroom" -> "walk into the bedroom"
  Reduces walk_ahead from 9.8% to ~3.4%
  Does NOT convert to go_straight (that adds to an already-high category)

FIX 3: walk into the [room_type] -> enter the [room_type] at P=0.30
  Reduces walk_into from 25.9% to ~18%
  Boosts enter from 11.5% toward GT=16%
  Room types: bedroom|kitchen|bathroom|dining room|living room|office|study|hallway|foyer|room

INHERITED from v187 (all):
  - walk/go through hallway -> walk/go down hallway P=0.65
  - terminal stop/wait injection P=0.30
  - through->enter P=0.25 (expanded regex)
  - walk_forward->go_straight P=0.30
  - walk_ahead->go_straight P=0.20 (still run; FIX2 adds to it)
  - P_STOP_MERGE=0.30, P_MERGE=0.60
  - PYTHONHASHSEED=42

Expected v188:
  go_straight: 25.2% -> ~16.4% (down 8.8pp)
  walk_straight: 0% -> ~8.8% (up 8.8pp -- matches GT!)
  walk_ahead: 9.8% -> ~3.4% (down 6.4pp)
  walk_into: 25.9% -> ~18.1% (down 7.8pp)
  enter: 11.5% -> ~17.5% (up 6pp -- close to GT=16%)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v188_phase2_checkpoint.json (REUSE v180 P2 results -- assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v188.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v188.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v188_phase2_checkpoint.json"  # v188: REUSE v180 P2 results -- assembly-only
...
async def main():
    ...
    print('=== Gate 4 v186: expand through→enter regex + soft walk_ahead→go_straight P=0.20 ===')
    print(f"  v185 stats (PYTHONHASHSEED=42): through_the=35.2%, enter=15.2%, walk_ahead=11.6%, 2-sent=42.7%")
    print(f"  GT reference: through_the=27.2%, enter=17%, walk_ahead=6.1%, 2-sent=42%")
    print(f"  v186 CHANGES:")
    print(f"    FIX1: expand regex with room/area/foyer → through_the:35.2%→~33%")
    print(f"    FIX2: walk_ahead→go_straight P=0.20 → walk_ahead:11.6%→9.3%, go_straight:23.4%→25.7%")
    ...
    for ep in episodes:
        ...
        # v188 FIX1: go_straight → walk_straight at P=0.35 per occurrence
        # go_straight=25.2% (GT=7.9%) — way overused. walk_straight=0% (GT=8.8%) — entirely missing!
        # Converting 35% of "go straight" to "walk straight" gives:
        #   go_straight: 25.2%→~16.4%, walk_straight: 0%→~8.8% (exactly GT target!)
        ...
        # v188 FIX3: walk into the [room] → enter the [room] at P=0.30
        # walk_into=25.9% (GT=6.9%) — 19pp excess. enter=11.5% (GT=16%) — 4.5pp below GT.
        # Converting 30% of walk_into gives: walk_into→18.1%, enter→17.5% (close to GT=16%)
        ...
        # v184 FIX2: soft walk_forward → go_straight at P=0.30
        # walk_forward=13.3% (GT=6.1%) — 2.2× GT. LLM generates naturally but too high.
        # Redistribute to go_straight (v24-style: 23%) rather than eliminate.
        # Expected: walk_forward 13.3%→9.3%, go_straight 19.8%→23.8% (≈ v24's 23%)
        # ONLY standalone "walk forward" (not "walk forward to/through/etc." — already handled by prep reduction)
        _WF_STANDALONE_RE = re.compile(
            r'\bwalk forward(?!\s+(to|past|through|into|toward|towards|along|up|down)\b)',
            re.IGNORECASE
        )
        _wf_gs_rng = random.Random(eid ^ 0xB2C3_D4E5)
        def _walk_forward_to_go_straight(m):
            if _wf_gs_rng.random() < 0.30:
                cap = m.group(0)[0].isupper()
                return 'Go straight' if cap else 'go straight'
            return m.group(0)
        text = _WF_STANDALONE_RE.sub(_walk_forward_to_go_straight, text)
        ...
        # v186 FIX2: soft walk_ahead → go_straight at P=0.20
        # walk_ahead=11.6% (GT=6.1%) — 5.5pp above GT.
        # Convert 20% of standalone walk_ahead to go_straight (v24-style vocabulary).
        # Expected: walk_ahead 11.6%→9.3%, go_straight 23.4%→25.7%
        # ONLY standalone "walk ahead" (not "walk ahead to/through/etc." — already handled by prep reduction)
        _WA_STANDALONE_RE = re.compile(
            r'\bwalk ahead(?!\s+(to|past|through|into|toward|towards|along|up|down)\b)',
            re.IGNORECASE
        )
        _wa_gs_rng = random.Random(eid ^ 0xC3D4_E5F6)
        def _walk_ahead_to_go_straight(m):
            if _wa_gs_rng.random() < 0.20:
                cap = m.group(0)[0].isupper()
                return 'Go straight' if cap else 'go straight'
            return m.group(0)
        text = _WA_STANDALONE_RE.sub(_walk_ahead_to_go_straight, text)
        # v182 note: previously removed walk_ahead→walk_forward (v181 FIX1) — that redistributed to wrong word
        # v186: redirecting to go_straight instead (v24-style)
