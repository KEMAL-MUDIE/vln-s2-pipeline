#!/usr/bin/env python3
"""
Gate 4 Visual v42 — Improved system prompt: reduce hallway/room overuse, add positional cues
...
v40 FIXES:
  ...
  Phase 2 checkpoint: outputs/gate4_v42_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v42.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v42.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v42_phase2_checkpoint.json"
...
- ROOM NAMES: Do NOT write "walk into the hallway" or "walk into the living room" as a waypoint — write "walk forward" instead. Only use room names (hallway, living room, bedroom, kitchen) when they are the FINAL destination OR when they provide essential disambiguation (e.g., "enter the bedroom on the left").
...
async def main():
    ...
    print('=== Gate 4 v42: Fix dup-stop=62 ("and stop at X" mid-instruction → "and walk to X") ===')
    ...
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v42 should be <1%")
    ...
    print(f"  [Note: v42: improved prompt (hallway suppress, positional cues) + fresh Phase 2]")
