#!/usr/bin/env python3
"""
Gate 4 Visual v96 — Fix v95 overshoots: separate turn+room, diversify straight language.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v96.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v96_phase2_checkpoint.json"  # NEW — fresh P2 for room-entry + distance fixes
...
- DISTANCE: When the route shows a 'straight Xm' segment — NEVER write metric numbers. Vary your language naturally: short (≤3m)="a few steps" or "take a couple steps" or omit; medium (4–7m)="walk toward the [next landmark]" or "continue to the [goal area]" or "keep going"; long (≥8m)="walk for a while" or "continue along the hallway" or "walk down the corridor". Avoid overusing "walk forward" — only use it if no better description fits. R2R annotators vary their language.
...
async def main():
    ...
    print(f"  [v96 targets: room-entry separate clause, varied distance language, SR > v95]")
