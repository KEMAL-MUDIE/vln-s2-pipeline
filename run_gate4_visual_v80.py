#!/usr/bin/env python3
"""
Gate 4 Visual v80 — v79 + P_PASS_MARKER=0.10 → reduce LLM WP generation

v79 ISSUE:
  "walk past the": 16.5% (GT=8.7%) — too high (same as v76, prompt fix had no effect).
  Root cause: LLM generates WP from [pass:] markers. P_PASS_MARKER=0.30 creates
  too many [pass:] markers → LLM generates WP ~16-17% before WPT_OBJ_RE.

v80 FIX:
  1. P_PASS_MARKER=0.10 (was 0.30). With 3.87 midpoints/ep, P(had_pass=T)=33%.
     Fewer [pass:] markers → LLM generates WP less often.
  2. No had_pass suppression (v77/v78 showed it overcorrects).
  3. Keep v78/v79 prompt fix ([→ X] = "walk to the" only).
  4. WPT_OBJ_RE removes generic WP (table, sofa, wall etc.).
  5. Expect: walk-past ~8-10% (close to GT 8.7%), GT-match ~0.990+

Phase 2 checkpoint: outputs/gate4_v80_phase2_checkpoint.json (FRESH — P_PASS_MARKER changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v80.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v80.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v80_phase2_checkpoint.json"  # FRESH — P_PASS_MARKER changed
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8045_5300)  # v80: new seed
