#!/usr/bin/env python3
"""
Gate 4 Visual v39 — Fix dup-stop=58 root cause (v38 partial fix had logic bug)

v38 RESULTS:
  GT-match: 0.964, through_the: 27.0%, walk_past: 10.5%, continue: 9.1%, proceed: 1.7%
  dup-stop: 58 ← STILL PRESENT (root cause now understood)

ROOT CAUSE ANALYSIS (v38 bug):
  v38 fix used `if i < len(sents) - 1:` (skip last LLM sentence).
  BUT: the "and stop near X." sentence ends up AS the last LLM sentence -> guard skips it.
  The template stop_phrase is appended AFTER clean(), so:
    LLM last sentence: "and stop near X."  <- skipped by guard, stays in output
    Template appended: "Stop near X."      <- creates dup-stop
  All 58 remaining patterns confirmed "and stop/wait near/in/at X" at sentence[-1].

v39 FIX:
  Remove the `if i < len(sents) - 1:` guard entirely.
  Apply _MID_STOP_RE / _MID_WAIT_RE to ALL sentences.
  The regex requires `\band\s+stop\b` (continuation marker "and"),
  NEVER matches "Stop near X." (capital S, no "and") -> safe to apply everywhere.
  Also: delete entire "and stop/wait X" fragment via broader regex for cleaner output:
    "Walk past the table and stop near X." -> "Walk past the table."

  All v38 innovations retained:
  ...
  Expected:
    dup-stop:        ~0 (complete fix -- all 58 cases were "and stop/wait" pattern)
    GT-match:        ~0.964 (maintained from v38)
    proceed:         ~1.7%
    continue:        ~9.1%
    through the:     ~27.0%

  Phase 2 checkpoint: outputs/gate4_v39_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v39.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v39.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v39_phase2_checkpoint.json"
...
async def main():
    ...
    print('=== Gate 4 v39: Fix dup-stop=62 ("and stop at X" mid-instruction → "and walk to X") ===')
    ...
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v39 should be <1%")
    ...
    print(f"  [Note: v39: v38 + proper dup-stop fix (delete 'and stop/wait X' fragment from ALL sents)]")
