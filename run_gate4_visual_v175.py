...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v175.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v175_phase2_checkpoint.json"  # v175: FRESH — preserve word count (walk_ahead) + all v174 structural improvements
...
- DISTANCE: The route shows 'straight Xm' entries — NEVER write metric numbers or "take a few steps". For short segments (≤2m): skip them or chain adjacent movements (e.g. "turn left into the kitchen"). For medium segments (3-8m): use "walk" or "walk ahead" briefly — one phrase. For long segments (>8m): use "walk ahead" or "continue" toward the next landmark — one phrase. AVOID "go straight", "walk straight", "walk forward" — use "walk ahead" or just "walk" instead.
...
async def main():
    ...
    print('=== Gate 4 v175: v174 FIXES + WORD-COUNT-PRESERVING walk_ahead substitution ===')
    print(f"  v174 MEASURED: sent(2)=42.0%(GT=33.8%)✓, on_left=4.5%, on_right=4.5%(GT=12.9%/13.3%)")
    print(f"    door_on=9.0%(GT=5.5%)✓, couch=7.2%(GT=7.2%)✓, color=7.9%(GT~10.9%)✓")
    print(f"  v174 BUG: avg_words=21.4 (was 24.6 in v173, GT=26.8) — walk_forward→continue -1w each!")
    print(f"    ROOT CAUSE: 'walk forward'→'continue'(-1w), 'go straight'→'continue'(-1w)")
    print(f"    3+ occurrences/ep × -1w = -3.2w average drop")
    print(f"  v175 FIX: WORD-COUNT-PRESERVING substitution")
    print(f"    'walk forward' → 'walk ahead' (2w→2w, preserves count)")
    print(f"    'go straight' → 'walk ahead' (2w→2w)")
    print(f"    'walk straight' → 'walk ahead' (2w→2w)")
    print(f"    Expected: avg_words restored to ~24.8-25.0 (v173 level + merger/on_dir additions)")
    print(f"  KEEPS ALL v174 STRUCTURAL IMPROVEMENTS:")
    print(f"    - sentence merger: 2-sent=42%(GT=33.8%) ✓")
    print(f"    - 'door on the left/right': on_left≈4.5%, on_right≈4.5% ✓")
    print(f"    - color_stop: 7.9%≈GT=10.9% ✓")
    print(f"    - enter=7.3%(GT=5.4%), couch=7.2%(GT=7.2%) ✓")
    print(f"  GT: walk_straight=8.8%, go_straight=7.9%, avg_turns=0.59, turn-anchor=16.5%")
    print(f"  Expected: walk_ahead≈25%, avg_words≈24.8, sent(2)=40%, on_left/right≈4-5%")
    ...
    for ep in episodes:
        ...
        # v175: walk_forward/go_straight REDUCTION — WORD-COUNT PRESERVING version of v174
        # v174's approach reduced avg_words from 24.6→21.4 by removing "forward"/"straight" (-1w each)
        # v175 FIX: replace "forward"/"straight" with "ahead" (same word count, 2w→2w)
        # GT: walk_forward=6.1%, go_straight=7.9%, walk_straight=8.8% (not banned — just less common)
        # "walk forward" (2w) → "walk ahead" (2w): preserves word count
        # "go straight" (2w) → "walk ahead" (2w): preserves word count
        # "walk straight" (2w) → "walk ahead" (2w): preserves word count
        # This changes vocabulary without losing word count — avg_words stays at ~24.6-25.0
        text = re.sub(r'\bwalk forward\b', 'walk ahead', text, flags=re.IGNORECASE)
        text = re.sub(r'\bgo straight\b', 'walk ahead', text, flags=re.IGNORECASE)
