#!/usr/bin/env python3
"""
Gate 4 Visual v79 — v78 + REMOVE binary had_pass suppression

v78 ISSUE:
  "walk past the": 2.7% (GT=8.7%) — still too low after had_pass suppression.
  Root cause: had_pass binary suppression is too aggressive.
  Even with 93% had_pass=True, LLM only generates WP in ~2.9% of those episodes.
  Result: 93% × 2.9% = 2.7% vs GT 8.7%.

v79 FIX:
  1. REMOVE had_pass binary suppression at assembly time.
     Rely on the v78 prompt fix alone: [→ X] = "walk to the" only,
     [pass:] = "walk past the". Expect raw WP ~10-12% (close to GT 8.7%).
  2. Keep seeded rng + had_pass tracking (for analysis, not suppression).
  3. P_PASS_MARKER=0.30 (fewer [pass:] markers → less LLM WP → closer to GT).
  4. WPT_OBJ_RE still removes generic WP (table, sofa, wall etc.).
  5. Expect: walk-past ~8-12%, GT-match ~0.990+

Phase 2 checkpoint: outputs/gate4_v79_phase2_checkpoint.json (FRESH — suppression removed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v79.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v79.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v79_phase2_checkpoint.json"  # FRESH — suppression removed
...
P_PASS_MARKER = 0.30   # v79: back to 0.30 (fewer [pass:] markers → less LLM WP → closer to GT)
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x7945_5300)  # v79: episode-specific seed (different from v78)
