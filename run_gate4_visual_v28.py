#!/usr/bin/env python3
"""
Gate 4 Visual v28 — Generic-Object Filter for Midpoint [pass:] Markers

IMPROVEMENTS OVER v27:
  1. GENERIC OBJECT FILTER for [pass:] midpoints:
     Skip "walk past the X" when X is generic furniture (sofa, table, chair, bed, wall, cabinet).
     Only emit [pass:] for distinctive objects (fireplace, piano, art, refrigerator, staircase, etc.)
     GT "walk past the": 8.7%. v27: 45.9% (too high). Target: 10-15%.
  2. All v27 innovations retained:
     - [thru: X] → "through the X" for doorways (target ~27%)
     - [pass: X] → "walk past the X" for distinctive objects only
     - Turn threshold 70°, straight threshold 2m
     - Anti-loop, path-budget, door landmarks

  GT analysis reference:
    "through the":   GT=27.2%, v27=23.9% ✓
    "walk past the": GT=8.7%,  v27=45.9% → v28 target: 10-15%
    "passing the":   GT=5.1%,  v27=0.6%  ✓
    "continue":      GT=10.4%, v27=8.8%  ✓
    turn left/right: GT=48.9%, v27=~52%  ✓

  Phase 2 checkpoint: outputs/gate4_v28_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v28.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v28.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v28_phase2_checkpoint.json"
...
async def main():
    ...
    print("=== Gate 4 v28: Generic-Object Filter for Midpoint [pass:] Markers ===")
    ...
    print(f"  v28 key changes: generic filter skips [pass:] for sofa/table/chair/wall/etc."
          f" Only distinctive objects (fireplace, piano, art) get 'walk past the X'")
    print()
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v27-corrected=0.928, v26=0.955)")
