...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v169.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v169_phase2_checkpoint.json"  # v169: FRESH — P_TURN_UNANCHORED=1.0 + P_ANCHOR=0.13 + P_DIR=0.05
...
P_ANCHOR_EPISODE = 0.13   # v169: restored from 0.03→0.13 (v24 used 0.21 → SR=40.24% best). With P_TURN=1.0, more turns = need more anchors for 16.5% anchor%
...
async def main():
    ...
    print('=== Gate 4 v169: ALL TURNS + COLOR STRIPPING + MORE DOORS — Target SR>40% ===')
    print(f"  DEEP RESEARCH FINDINGS (v168 failure analysis):")
    print(f"    - has_color%: GT=15.2% vs v168=74.0% (OVERUSE 5x!) — model trained on GT can't ground rich colors")
    print(f"    - door%: GT=48% vs v168=26.9% (UNDERUSE 2x!) — doors are critical indoor nav landmarks")
    print(f"    - avg_turns: v24(1.53→SR=40.24%) vs v168(0.64→low SR) — more turns = better guidance")
    print(f"  FIX 1: P_TURN_UNANCHORED=1.0 (RESTORE ALL TURNS like v24: avg_turns→1.4-1.5)")
    print(f"  FIX 2: COLOR STRIPPING in route landmarks (dark brown → ∅, white marble → marble)")
    print(f"  FIX 3: (door→ room) fraction 0.21→0.45 (door mentions GT=48% → target)")
    print(f"  FIX 4: P_ANCHOR_EPISODE=0.13 (was 0.03, v24=0.21 for 16.5% anchor%)")
    print(f"  FIX 5: P_DIR_ROOM_TRANS=0.05 (was 0.10, reduced to compensate more P_ANCHOR turns)")
    print(f"  KEPT: nav-verb rejection in extract_landmark_brief (Turn right → None)")
    print(f"  v169: Expected anchor%≈15-17%, avg_turns≈1.4-1.5, has_color≈20-25%, door%≈35-40%")
