#!/usr/bin/env python3
"""
Gate 4 Visual v50 — Recalibrate P_THRU + P_PASS after turn-sampling changes

v49 RESULTS:
  hallway:      23.1% (GT=20.4%) ← NEAR-PERFECT
  avg_turns:     0.71 (GT=0.59)  ← excellent (from v47=0.99)
  through_the:  31.8% (GT=27.2%) ← REGRESSION: +4.6pp
  walkpast:     13.2% (GT=8.7%)  ← REGRESSION: +4.5pp
  toward_the:    8.6% (GT=3.8%)  ← improved from v45=26.5%
  GT-match:     0.959             ← OK

ROOT CAUSE of through_the + walkpast regression (v49):
  When rm turns are sampled (45% filtered), the route has fewer direction changes.
  The LLM fills absent navigation context with more [thru:] markers (→ "through the doorway")
  and [pass:] markers (→ "walk past the X"). Both markers fire at their calibrated rates
  (P_THRU=0.85, P_PASS=0.60) but now represent a higher proportion of the shorter route.

v50 FIX: Recalibrate sampling rates to match GT targets under new turn-sampling regime.
  P_THRU_MARKER: 0.85 → 0.73 (target: through_the=27.2% from current 31.8%)
    Scale: 0.85 × (27.2/31.8) ≈ 0.727 → 0.73
  P_PASS_MARKER: 0.60 → 0.40 (target: walkpast=8.7% from current 13.2%)
    Scale: 0.60 × (8.7/13.2) ≈ 0.395 → 0.40

All v49 route/prompt settings retained (hidden rm, rm turn sampling, turn suppression).

Expected:
  through_the:  31.8% → ~27% (recalibrated P_THRU=0.73)
  walkpast:     13.2% → ~9% (recalibrated P_PASS=0.40)
  hallway:      23.1% → ~22% (maintained)
  avg_turns:     0.71 → ~0.71 (unchanged: rm turn sampling)
  GT-match:     0.959 → ~0.960-0.965 (minor improvement)

Phase 2 checkpoint: outputs/gate4_v50_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v50.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v50.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v50_phase2_checkpoint.json"
...
P_PASS_MARKER = 0.33   # v50: recalibrated from v49's 0.50 → 13.2% → 0.50×(8.7/13.2)≈0.33 (target GT=8.7%)
P_THRU_MARKER = 0.73   # v50: recalibrated from v49's 0.85 → 31.8% → 0.85×(27.2/31.8)≈0.73 (target GT=27.2%)
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if seg_midpoint and turn_label in seg_midpoint:
                ...
                if action == "thru":
                    if random.random() < P_THRU_MARKER:  # v50: 0.73 → ~27% through (GT=27.2%)
                        parts.append(f"[{action}: {mid_desc}]")
                elif not is_generic_pass_object(mid_desc):
                    if random.random() < P_PASS_MARKER:  # v50: 0.33 → ~9% walk-past (GT=8.7%)
                        parts.append(f"[{action}: {mid_desc}]")
...
async def main():
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v49=0.71, v47=0.99) ← maintained")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v49=8.6%) ← maintained")
    print(f"  hallway:       {100*hallway_n/n:.1f}%  (GT=20.4%, v49=23.1%) ← maintained")
    print(f"  into hallway:  {100*into_hall_n/n:.1f}%  (GT=4.4%, v49=1.1%) ← maintained")
    print(f"  spatial:")
