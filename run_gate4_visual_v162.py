#!/usr/bin/env python3
"""
Gate 4 Visual v162 — Fix LLM "walk to the X" source: → [object ahead] → "walk forward".

TWO SOURCES OF "walk to the X":
  Source 1 (LLM): SYSTEM_INTRO rule for → [object ahead]: "add a BRIEF 'walk to the X' phrase"
  Source 2 (post): replace_go_straight_with_landmarks converts "go straight" → "walk to X"

v157-v161 only fixed Source 2 (via replace_gs skip rate).
v162 fixes Source 1: change Phase 2 prompt so → [object ahead] → "walk forward" (not "walk to X").

HYPOTHESIS: LLM-generated "walk to the X" in turn descriptions may confuse the agent.
  If agent cannot see the object or misidentifies it, "walk to X" is worse than "walk forward".
  v162 removes ALL "walk to X" from LLM output for → [object ahead] markers.

v159 ACTUAL: GT-match=0.978, go_straight=24.9%≈v24=23%, walk_straight=8.6%≈GT✓, avg_turns=0.67

v162 CHANGES from v159:
  1. SYSTEM_INTRO rule for → [object ahead]: "walk to the X" → "walk forward"
     OLD: "add a BRIEF 'walk to the X' phrase (3-5 words) — keep it very short."
     NEW: "just write 'walk forward' — do NOT reference the object."
  2. Remove "walk forward" from MOTION WORDS AVOID list (it's now preferred for → [object ahead])
  3. P2_CKPT: FRESH (gate4_v162_phase2_checkpoint.json) — prompt changed, must regenerate
  4. replace_go_straight_with_landmarks: keep 50% skip (same as v159)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C)
Phase 2 checkpoint: outputs/gate4_v162_phase2_checkpoint.json (FRESH — prompt changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v162.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v162.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v162: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v162_phase2_checkpoint.json"  # v162: FRESH — prompt changed (→ [object ahead] → "walk forward"), must regenerate
...
P_TURN_UNANCHORED = 0.09  # v158: 0.12→0.09 to reduce avg_turns from 0.80 toward GT=0.59 (scaling: 0.80*0.09/0.12=0.60)
...
- When a turn has → [object ahead] after the arrow: just write "walk forward" — do NOT reference the object. Keep straight sections simple.
...
- MOTION WORDS: Use natural walking verbs: "walk", "go". Chain movements naturally: "turn left into the kitchen". For landmarks: "walk to the X". AVOID "proceed", "go forward", "keep going", "head toward", "walk toward". Use "continue" ONLY when no new visual landmark is present — prefer "walk" or "go" as primary verbs. "walk forward" is acceptable for straight sections with no landmark.
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    if n_ahead > 0:
        anchor_note += (f" Route also has {n_ahead} [X ahead] marker(s) — after turning at"
                        f" [landmark], just write 'walk forward' — do NOT reference the object ahead.")
...
async def main():
    ...
    print('=== Gate 4 v162: fix LLM "walk to X" source (→[obj] → "walk forward") + 50% gs skip ===')
    print(f"  v159 METRICS: GT-match=0.978✓, avg_turns=0.67, walk_straight=8.6%✓, go_straight=24.9%≈v24=23%")
    print(f"  v162 CHANGES: (1) SYSTEM_INTRO → [object ahead] rule: 'walk to the X' → 'walk forward'")
    print(f"                (2) 'walk forward' removed from MOTION WORDS AVOID list")
    print(f"                (3) FRESH Phase 2 checkpoint (gate4_v162_phase2_checkpoint.json)")
    print(f"    Source 1 fix: LLM no longer writes 'walk to the X' for → [object ahead] markers.")
    ...
    for ep in episodes:
        ...
        # v162: same 50% skip as v159 → go_straight ≈ 23.75% ≈ v24=23%.
        # v162 additionally fixes Source 1 (LLM prompt): → [object ahead] now → "walk forward" not "walk to X".
        # Source 2 (post-process): still 50% skip, same as v159.
    ...
    print(f"  go_straight:   {100*go_straight_n/n:.1f}%  (GT=7.9%, v159=24.9%) ← v162: 50% skip (same as v159)")
    print(f"  walk_straight: {100*wlk_straight_n/n:.1f}%  (GT=8.8%, v159=8.6%) ← v162: unchanged")
    print(f"  walk_forward:  {100*walk_forward_n/n:.1f}%  (v160=57.8%, v159=?) ← v162: LLM now uses for → obj ahead")
    ...
    print(f"  [v162 change: → [object ahead] → 'walk forward' (not 'walk to X'). 50% gs skip same as v159.]")
    print(f"  [Expected: go_straight≈24.9% (same as v159), walk_forward↑ (LLM now uses it for → obj ahead)]")
