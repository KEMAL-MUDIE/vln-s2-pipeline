#!/usr/bin/env python3
"""
Gate 4 Visual v25 — Loop-Free + Path-Budget + Turn-Side Landmarks

IMPROVEMENTS OVER v24:
  1. ANTI-LOOP: System prompt adds "Do NOT repeat any motion phrase" rule.
     Post-generation truncation at first repeated 5-gram + stop phrase recovery.
     Targets the 9.8% of episodes where v24 generated looping instructions.
  2. PATH-LENGTH WORD BUDGET: Prompt now includes target word count based on
     path_len (4→15-22w, 5→18-26w, 6→22-30w, 7+→26-34w). Fixes short-path
     over-generation: 66.3% of GT≤15w episodes were over-generated by >10w.
  3. All v24 innovations retained: turn-side landmarks, dual-segment midpoints.
  ...
  Phase 2 — Instruction generation with turn-side + dual-midpoint landmarks:
    ...
    Checkpoint: outputs/gate4_v25_phase2_checkpoint.json
    Output: outputs/datasets/val_unseen_generated_gemma_visual_v25.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v25.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v25_phase2_checkpoint.json"
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    # v25: path-length-aware word budget to prevent over-generation on short paths
    path_len = len(ep.get("reference_path", []))
    if path_len <= 4:
        anchor_note += " LENGTH: Write 12-20 words total. Very short path — be brief."
    elif path_len == 5:
        anchor_note += " LENGTH: Write 18-26 words total."
    elif path_len == 6:
        anchor_note += " LENGTH: Write 22-30 words total."
    else:
        anchor_note += " LENGTH: Write 26-35 words total."
...
async def main():
    ...
    print("=== Gate 4 v25: Loop-Free + Path-Budget + Turn-Side Landmarks ===")
    ...
    print(f"  v25 key changes: anti-loop system prompt, path-length word budget, loop truncation")
    print()
