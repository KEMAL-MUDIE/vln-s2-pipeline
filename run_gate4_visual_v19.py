#!/usr/bin/env python3
"""
Gate 4 Visual v19 — Richer Vision Narrative Instructions

IMPROVEMENT OVER v18:
  v18 Phase 1 asked for 3-8 word descriptions — too terse to capture what an
  agent PASSES along the path, not just what it turns at. GT instructions like
  "Walk straight passing the gray couch and stop near the rug" reference
  intermediate objects the agent sees while walking — not just at turn points.

THE FIX (v19): RICHER PHASE 1 + VISUAL NARRATIVE IN PHASE 2
  Phase 1 — Rich visual narrative extraction:
    Ask Gemma to describe in 1-2 sentences: the most prominent landmark AND
    what's visible in the direction the robot moves. Includes objects the agent
    will PASS, not just the pivot point. max_tokens=80 (vs 30 in v18).
    Checkpoint: outputs/gate4_v19_phase1_checkpoint.json

  Phase 2 — Instruction generation with visual narrative context:
    SAME v17 calibration (P_ANCHOR=0.21, stop/wait GT distribution),
    BUT adds a "Visual scene context" block to the prompt showing the full
    visual narrative of the path. The LLM can now write "walk past the couch"
    because it sees that context from Phase 1.
    Checkpoint: outputs/gate4_v19_phase2_checkpoint.json
    Output: outputs/datasets/val_unseen_generated_gemma_visual_v19.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT     = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v19.json.gz"
P1_CKPT    = ROOT / "outputs" / "gate4_v19_phase1_checkpoint.json"
P2_CKPT    = ROOT / "outputs" / "gate4_v19_phase2_checkpoint.json"
...
def get_landmark_info_v18(ep, pf_map, old_map, primitives, vision_map: Dict) -> Dict:
    ...
    # Turn landmarks: extract core noun phrase from richer description
...
async def main():
    ...
    print("=== Gate 4 v19: Richer Vision Narrative Instructions ===")
    ...
    print(f"  v19 key changes: richer Phase 1 (1-2 sentences), visual narrative in Phase 2 prompt")
    print()
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v18=0.942, v17=0.948)")
