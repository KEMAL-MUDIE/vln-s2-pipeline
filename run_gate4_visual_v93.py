#!/usr/bin/env python3
"""
Gate 4 Visual v93 — v92 + remove hallway directional filter (P_allow: 0.69 → 1.0)

v92 RESULTS:
  through_the: 27.1% (GT=27.2%) ← FIXED via assembly injection ✓
  hallway:     14.8% (GT=20.4%) ← REMAINING GAP: -5.6pp
  toward_the:  3.4% (GT=3.8%) ← small gap, acceptable
  avg_words:   26.8 (GT=26.8) ✓
  GT-match:    0.992 ✓

v93 ROOT CAUSE ANALYSIS — hallway gap:
  The hallway directional filter (P_allow=0.69) was calibrated for v82 when the LLM
  generated very high hallway% (v81=56.1%) due to weaker ROOM NAMES ban. Now with the
  current strict ban ("Do NOT write walk down/into/through/toward the hallway"), the LLM
  generates only ~15.6% directional hallway phrases pre-filter. The filter then reduces
  this to 15.6% × 0.69 = 10.8% directional contribution.

  Total current hallway: ~4% non-directional + 10.8% directional = 14.8%
  With P_allow=1.0 (no filter): ~4% + 15.6% = 19.6% ≈ GT 20.4%

v93 FIX — remove hallway dir filter (P_allow: 0.69 → 1.0):
  Simply remove the `if ep_rng.random() > 0.69: suppress` block for directional hallway.
  All hallway directional phrases generated by LLM under the current ban pass through.
  Expected: hallway 14.8% → ~19.5-20% (GT=20.4%)

  NOTE: Entry hallway (walk into the hallway) remains at 0.5% — structural gap (LLM ban).
  into_hallway: 0.5% vs 4.4% GT — not addressed in v93, would require LLM prompt change.

Phase 2 checkpoint: outputs/gate4_v91_phase2_checkpoint.json (REUSED — no new LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v93.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v93.json.gz"
