#!/usr/bin/env python3
"""
Gate 4 Visual v102 — v101 + quality fixes (truncated instruction detection, toward_the fix).

KEY CHANGES FROM v101:
  v101 results: hallway=23.4% (GT=20.4%) ✓, avg_turns=0.61 (GT=0.59) ✓
                eps_with_turn=50.9% (GT=41.3%) slightly high
                28/1839 truncated (starts '.', < 8 words); toward_the=7.1% (GT=3.8%)

v102 FIXES:
  1. quality_ok: also reject if starts with '.' OR < 12 words (catches truncated outputs)
  2. P_TOWARD = 0.0: disable random toward_the conversion (was 0.25; caused 7.1% vs GT 3.8%)
     Natural toward_the from LLM is ~2-3%; GT=3.8% — close enough without injection.
  3. Hallway turns: apply P_TURN_UNANCHORED with probability to reduce eps_with_turn
     from 50.9% toward GT 41.3%. Use P_HALLWAY_TURN=0.65 to sample hallway turn events.

Phase 2 checkpoint: outputs/gate4_v102_phase2_checkpoint.json (NEW — fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v102.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v102.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v102_phase2_checkpoint.json"  # v102 — fresh P2 (quality fixes + toward_the fix)
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v102: Hallway suppression (from v101) + P_HALLWAY_TURN sampling.
                # v101 result: eps_with_turn=50.9% (GT=41.3%) — hallway turns over-fired.
                # v102 fix: sample hallway turns at P_HALLWAY_TURN=0.65 → reduces by ~30%.
                # Expected: eps_with_turn 50.9%×0.65 + non-hallway turns ≈ 42-44% (→ GT 41.3%).
                is_hallway = "hallway" in rm.lower() or "corridor" in rm.lower()
                if is_hallway:
                    if direction:
                        # v102: Only include hallway turn 65% of the time (to reduce eps_with_turn)
                        if rng.random() < 0.65:
                            parts.append(f"turn {direction}")
                        # else: skip this hallway turn entirely (silent straight through)
                    # else: no direction — skip entirely (no hallway mention)
                else:
                    parts.append(f"(→ {rm})")
            else:
                parts.append(f"turn {direction}")
...
async def main():
    ...
    print(f"  [v102 targets: quality fixes; truncated → 0; toward_the → ~3%; eps_with_turn → ~42%]")
