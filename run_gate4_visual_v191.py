#!/usr/bin/env python3
"""
Gate 4 Visual v191 -- NEW Phase 2 LLM generation with v24-style "passing" prompt (no walk_to).

CHANGES FROM v189:
  v189 actual stats: go_straight=11.3%(GT=7.9%), walk_straight=10.9%(GT=8.8%),
    walk_ahead=2.0%(GT=0.2%), walk_to=50.2%(GT=5.1%), continue=19.5%(GT=10.4%),
    exit=10.7%(GT=18.0%), walk_past=7.3%(GT=10.5%), for_a_bit=15.2%(GT=0%)

  PROBLEM 1: walk_to_the 50.2% vs GT=5.1% — 10x overuse. Structural LLM artifact.
  PROBLEM 2: "for a bit/while/moment" filler 15.2% — GT=0% (never appears in GT!).
  PROBLEM 3: continue=19.5% vs GT=10.4% — 9.1pp excess from walk_ahead->continue P=0.70.
  PROBLEM 4: exit=10.7% vs GT=18.0% — 7.3pp deficit. GT uses "exit" for room transitions.

FIX 1 (NEW): "walk to the X for a bit/while/moment" -> "walk past the X" at P=0.75
  Filler signals intermediate waypoint -> walk_past is semantically correct.
  Walk_to: -3.9pp. Walk_past: 7.3%+3.9%=11.2% (GT=10.5% -- nearly perfect!).
  Must run BEFORE filler removal (fix 3) to detect the pattern.

FIX 2 (NEW): "walk to the X and/comma turn [dir]" -> "turn [dir] at the X" at P=0.80
  GT landmark style. Walk_to: -3.9pp for 4.9% of instructions.

FIX 3 (NEW): Remove remaining "for a bit/while/moment" filler.
  GT=0% -- completely eliminates non-GT artifact from 15.2% of instructions.

FIX 4 (NEW): "walk out of the X" -> "exit the X" at P=0.70 (+1.4pp exit).
FIX 5 (NEW): "leave the [room]" -> "exit the [room]" at P=0.40 (+1.5pp exit).
  Exit combined: 10.7% -> ~13.6% (toward GT=18.0%).

FIX 6 (MODIFY): Reduce walk_ahead->continue P=0.70->0.45
  continue: 19.5% -> ~17.0% (less overshoot vs GT=10.4%).

INHERITED from v189 (all unchanged).

Expected v190:
  walk_to: ~42.4% (still high but -7.8pp from 50.2%)
  walk_past: ~11.2% (GT=10.5% -- near perfect!)
  for_a_bit: 0% (GT=0%)
  exit: ~13.6% (GT=18.0%, +2.9pp improvement)
  continue: ~17.0% (GT=10.4%, -2.5pp improvement)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v191_phase2_checkpoint.json (NEW LLM generation -- v24-style passing prompt)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v191.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v191.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v191_phase2_checkpoint.json"  # v191: NEW LLM generation -- v24-style passing prompt
...
v191 KEY CHANGE: Use "passing the X" (gerund) for intermediate landmarks — NOT "walk to the X".
Real R2R annotators describe paths holistically: "Walk straight, passing the couch. Turn left. Stop near the bookshelf."
...
- When a turn has → [object ahead] after the arrow: describe the object as something you PASS — write "passing the X" or "continuing past the X" (3-5 words gerund form). FORBIDDEN: "walk to the X", "go to the X". Use gerund "passing the X" naturally: "Walk straight, passing the couch." or "Continue, passing the table."
- When route has [pass: X]: the robot moves alongside that object — write ONLY "passing the X" or "continuing past the X" (3-5 words). FORBIDDEN: "walk to the X", "walk past the X", "go past the X". Use ONLY gerund form "passing the X".
...
- When you see (hall→): write "continue through the hallway" or "go through the hallway" (3-4 words max). Do NOT use "walk to the hallway".
...
- SENTENCE STRUCTURE: Write 1-3 sentences maximum. Chain consecutive actions with "and" or comma+then: "turn left and walk through the hallway", "Walk straight, passing the sofa, then turn right". NEVER write one action per sentence. Real R2R instructions: "Exit the room and turn left. Walk straight, passing the sofa. Stop near the bookshelf." (2-3 sentences).
...
- CRITICAL: NEVER write "walk to the X" or "go to the X" for intermediate objects. Always use "passing the X" or "walk past the X" for intermediate objects. "walk to the X" is ONLY allowed for the final stop destination.
- MOTION WORDS: Use natural walking verbs: "walk", "go", "continue". For intermediate landmarks: "passing the X", "walk past the X". For final destination: "Stop near the X", "Wait at the X". AVOID "proceed", "go forward", "walk forward", "keep going", "head toward". Use "continue" and "go straight" freely for forward movement.
- VOCABULARY: Use "couch" (not "sofa") — this matches how real R2R annotators write. Use "passing the X" (gerund form) for passing alongside an object — NOT "walk past the X".
- DISTANCE: The route shows 'straight Xm' entries — NEVER write metric numbers or "take a few steps". For short segments (≤2m): skip them or chain adjacent movements. For medium segments (3-8m): use "walk" or "continue" briefly — one phrase. For long segments (>8m): use "walk straight" or "go straight" ONCE briefly.
