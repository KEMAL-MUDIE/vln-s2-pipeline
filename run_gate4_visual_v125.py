#!/usr/bin/env python3
"""
Gate 4 Visual v125 — Root-cause fix for through_the/hallway + avg_words + continue.

v124 results: through_the=31.9% (GT=27.2%), avg_words=25.4w (GT=26.8w), continue=7.4% (GT=10.4%).
              toward_the=1.3%, hallway=34.3% (GT=20.4%), walk_past=7.9% (GT=8.7%).

ROOT CAUSE ANALYSIS (v117→v124 through_the elevation):
  _hallway_into_re converts LLM's spontaneous "walk into the hallway" → "walk through the hallway"
  for ~30% of the 58% hallway-room episodes without explicit (through hallway) notation = ~17.5% of all
  episodes contributing phantom through_the. P(door→rm) reduction from 28%→22% had minimal effect
  because _hallway_into_re dominates. Fix: remove conversion + increase P_THRU_HALL to compensate.

Fix 1 — Remove _hallway_into_re: eliminates phantom through_the from hallway spontaneous conversion.
Fix 2 — P_THRU_HALL: 16% → 30%: compensates removed conversion AND targets hallway%≈GT=20.4%.
         Math: 30% × 69.3% × 85% compliance = 17.7pp through_the from hallway notation.
Fix 3 — had_thru_hall flag: suppress "walk through/into the hallway" for unannotated episodes.
         Without this, LLM still generates "walk into/through hallway" for bare "turn {direction}" eps.
Fix 4 — P(door→ rm): 22% → 21% (fine-tune; hallway notation now dominates through_the).
         Total: 17.7 + 21%×30.7%×85% + 4% LLM = 17.7 + 5.5 + 4 = 27.2% ≈ GT ✓
Fix 5 — _ws_variants extended: add "walk straight ahead"/"go straight ahead" (+1w avg).
         Closes avg_words gap: 25.4w → ~26.7w ≈ GT=26.8w.
Fix 6 — Remove "Continue down"/"Continue through" suppressions: continue 7.4% → ~10.4% ≈ GT.

Expected v125: through_the≈27.2%, hallway≈20.8%, continue≈10.4%, avg_words≈26.7w, walk_past≈8%.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v125.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v125_phase2_checkpoint.json"  # v125 — root-cause fix: remove _hallway_into_re, P_THRU_HALL=30%, had_thru_hall suppression
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    # v125: P_THRU_HALL 16% → 30%. With _hallway_into_re removed, need more explicit notation.
                    # Math: 30% × 69.3% × 85% = 17.7pp through_the from hallway → total through_the≈27.2% (GT).
                    # hallway%: 30% × 69.3% = 20.8% ≈ GT=20.4% ✓
                    if rng.random() < 0.30:
                        if direction:
...
def clean(raw, sv):
    ...
    # v125: extend "walk straight" → "walk straight ahead"/"go straight ahead" (+1w).
    # Closes avg_words gap: 25.4w → ~26.7w ≈ GT=26.8w. "ahead" is GT-natural and adds length.
    _ws_variants = [
        (r'\bwalk straight for a long while\b', ['go straight a long way', 'walk straight a long way']),
        (r'\bwalk straight for a while\b',      ['go straight for a while', 'walk straight for a bit']),
        (r'\bwalk straight a few\b',             ['go straight a few', 'walk a few']),
        (r'\bwalk straight ahead\b',             ['go straight ahead']),  # already expanded — don't re-hit
        (r'\bwalk straight\b',                   ['walk straight ahead', 'go straight ahead', 'go straight']),
    ]
...
async def main():
    ...
    print('=== Gate 4 v125: Root-cause fix: remove _hallway_into_re, P_THRU_HALL=30%, had_thru_hall suppression ===')
    print(f"  Fix 1: _hallway_into_re REMOVED (was converting 17.5% of eps phantom through_the)")
    print(f"  Fix 2: P_THRU_HALL 16%→30% (hallway%: 11.1%→20.8% ≈ GT=20.4%)")
    print(f"  Fix 3: had_thru_hall flag suppresses spontaneous walk-through-hallway in unannotated eps")
    print(f"  Fix 4: P(door→rm) 22%→21% (fine-tune; hallway notation dominates through_the now)")
    print(f"  Fix 5: _ws_variants extended with 'walk straight ahead'/'go straight ahead' (+1w avg)")
    print(f"  Fix 6: Remove 'Continue down'/'Continue through' suppressions → continue 7.4%→~10.4%")
    print(f"  Expected: through_the≈27.2%, hallway≈20.8%, continue≈10.4%, avg_words≈26.7w, walk_past≈8%")
    ...
    for ep in episodes:
        ...
        # Used in assembly to suppress spontaneous "walk through/into the hallway" in unannotated episodes.
    ...
    for ep in episodes:
        ...
        # v125: Hallway suppression — suppress spontaneous hallway mentions in unannotated episodes.
        # With _hallway_into_re removed, LLM may still write "walk into/through the hallway"
        # for bare "turn {direction}" episodes (70% of hallway-room episodes).
        # Only allow hallway-specific phrases when route explicitly has (through hallway) notation.
        # This keeps hallway% ≈ P_THRU_HALL × hallway_ep_rate = 30% × 69.3% = 20.8% ≈ GT=20.4%.
        had_thru_hall = meta.get("had_thru_hall", False)
        if not had_thru_hall:
            # Suppress "walk/go through/into the hallway/corridor" → "walk forward" (neutral forward motion)
            def _suppress_hallway(m):
                return 'Walk forward' if m.group(0)[0].isupper() else 'walk forward'
            text = re.sub(
                r'\b(?:walk|go)\s+(?:through|into)\s+the\s+(?:hallway|corridor)\b',
                _suppress_hallway, text, flags=re.IGNORECASE
            )
