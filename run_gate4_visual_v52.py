#!/usr/bin/env python3
"""
Gate 4 Visual v52 — v51 base + marker-discipline rules for through_the and walk_past

v51 RESULTS (base):
  GT-match: 0.961, hallway: 27.6%, through_the: 32.5%, walk_past: 15.0%, turns: 0.71
  avg_words: 26.2 (GT=26.8) — word count fixed!

ROOT CAUSE ANALYSIS (v52):
  through_the: 32.5% (GT=27.2%) — 23.5% from "through the doorway", 9% from other variants.
    LLM generates "through the doorway" independently of [thru:] markers in ~16% of eps.
    v50 proved marker rate reduction has minimal effect (31.8%→31.5%).
    MUST use prompt prohibition.
  walk_past: 15.0% (GT=8.7%) — LLM adds "walk past the [color] [object]" independently.
    Same root cause: marker rate doesn't control LLM-generated instances.

v52 FIX: Add strict marker-discipline rules to RULES section:
  1. "Write 'through the X' ONLY when [thru:] marker appears in the route for that segment."
  2. "Write 'walk past the X' ONLY when [pass:] marker appears in the route for that segment."
  Expected: through_the 32.5%→~27% (eliminates ~16% LLM-independent instances)
             walk_past 15.0%→~9% (same mechanism)
  Also: Generalize hallway prohibition to cover more hallway patterns (27.6%→~22%).

All v51 improvements retained (word budgets, hidden rm, rm turn sampling, etc.).

Phase 2 checkpoint: outputs/gate4_v52_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v52.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v52.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v52_phase2_checkpoint.json"
...
- When route has [thru: description]: the robot passes through an architectural opening — write "through the X" (2-4 words). Example: "through the doorway", "through the arched entrance". Do NOT write "passing through" or "passing the" — write "through the X" only. CRITICAL: Do NOT write "through the doorway", "through the arched entrance", or any "through the X" phrase unless [thru:] explicitly appears in the route. If there is no [thru:] marker, write "walk forward" instead — never add your own "through the doorway".
- When route has [pass: description]: the robot walks by an object — write "walk past the X" (3-5 words). Example: "walk past the sofa", "walk past the staircase". Do NOT write "passing the" — write "walk past the X" only. CRITICAL: Do NOT write "walk past the X" unless [pass: X description] explicitly appears in the route. If there is no [pass:] marker, do NOT add "walk past the" — write "walk forward" instead.
...
- ROOM NAMES: Do NOT name rooms (hallway, corridor, bedroom, kitchen, living room, bathroom, dining room) as mid-route movement. Specifically forbidden mid-route: "walk into the hallway", "walk through the hallway", "walk down the hallway", "walk toward the hallway", "continue down the hallway", "walk into the living room", "walk toward the bedroom". Write "walk forward" or "turn [direction] and walk forward" instead. Only name a room when it is the FINAL destination labeled in "Goal:" or when "on the left/right" provides essential disambiguation for a landmark near the path.
...
async def main():
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v51=0.71, v47=0.99) ← v52: v51+marker_discipline")
