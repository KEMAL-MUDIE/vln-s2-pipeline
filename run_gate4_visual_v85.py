#!/usr/bin/env python3
"""
Gate 4 Visual v85 — v84 + through/toward/continue/hallway calibration

v84 STATS (post-analysis):
  through the:  21.7% (GT=27.2%) — -5.5pp gap: ROOM NAMES ban + assembly double-suppression
  toward the:   0.7%  (GT=3.8%)  — -3.1pp gap: prompt restricts → [ahead] to "walk to the" only
  continue:     6.1%  (GT=10.4%) — -4.3pp gap: LLM under-uses continue; no explicit examples
  hallway:      13.8% (GT=20.4%) — -6.6pp gap: probabilistic allow=36% too low
  into hallway: 0.5%  (GT=4.4%)  — -3.9pp gap: entirely banned in ROOM NAMES rule

v85 FIXES:
  1. "through the" (+5.5pp target):
     - Increase P_THRU_MARKER from 0.85 → 0.93 to generate more [thru:] markers
     - Add prompt clarification: [thru:] ALWAYS produces "through the X", even near hallways
     - The ROOM NAMES ban applies only to un-marked room traversals, NOT [thru:] markers
  2. "toward the" (+3.1pp target):
     - Expand → [object ahead] rule from "walk to the X / continue to the X" to also allow
       "walk toward the X" — this matches GT approach language ("walk toward the window")
  3. "continue" (+4.3pp target):
     - Add explicit "continue toward/through" examples to the prompt
     - v83 relaxed _CONTINUE_MOTION_RE to allow "continue toward/through" — just need examples
  4. "hallway" (+6.6pp target):
     - Increase probabilistic allow from P=0.36 → P=0.50 (allow 50% of directional hallway)
     - Expected: 13.8% × (50/36) ≈ 19.2% — close to GT 20.4%
  5. "into hallway" (+3.9pp target, partial):
     - Allow "walk into the hallway" with P=0.20 in assembly (20% of cases)
     - Complement to directional hallway allow (entering vs traversing)
  Seed: 0x8545_5300. Fresh checkpoint.

Phase 2 checkpoint: outputs/gate4_v85_phase2_checkpoint.json (FRESH — prompt changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v85.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v85.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v85_phase2_checkpoint.json"  # FRESH — prompt changed
...
P_THRU_MARKER = 0.93   # v85: increased from 0.85 → compensate for assembly hallway/room conversions
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8545_5300)  # v84: new seed
...
- When a turn has → [object ahead] after the arrow: add a BRIEF approach phrase (3-5 words) like "walk to the bookshelf", "walk toward the window", or "continue toward the painting" — keep it very short. Do NOT write "walk past the" for → [object ahead]; use "walk to the X", "walk toward the X", or "continue toward the X" only
- When route has [thru: description]: the robot passes through an architectural opening — ALWAYS write "through the X" (2-4 words), regardless of surrounding context. Example: "through the arched entrance", "through the double doors", "through the doorway". This rule overrides room-name restrictions — if the [thru:] description mentions a doorway near a hallway, still write "through the doorway" (not "walk forward"). Prefer distinctive: "through the arched entry" over "through the doorway" when the description allows. Do NOT write "passing through" or "passing the".
...
- STYLE: Avoid starting sentences with "Continue straight" — use action verbs instead ("Walk forward"). Avoid overusing "wooden" for generic surfaces — mention material only for distinctive objects (marble fireplace, glass door are fine). Use "continue toward the X" or "continue through the X" naturally when moving toward or through landmarks — these are valid GT-style phrases (e.g., "continue toward the desk", "continue through the arch").
...
async def main():
    ...
    for ep in episodes:
        ...
        if "hallway" in text.lower():
            ...
            if _HALLWAY_ENTRY_RE.search(text):
                # v85: allow "walk into the hallway" P=0.20 (GT=4.4%, was 0% - entirely banned)
                entry_rng = random.Random(eid ^ 0x8545_E300)
                if entry_rng.random() > 0.20:  # suppress 80% → allow 20% → target ~4% (GT=4.4%)
                    text = _HALLWAY_ENTRY_RE.sub("walk forward", text)
                    text = re.sub(r'\bwalk forward\s+walk forward\b', 'walk forward', text, flags=re.I)
            if _HALLWAY_DIR_RE.search(text):
                ep_rng = random.Random(eid ^ 0x8545_5300)
                if ep_rng.random() > 0.50:  # v85: suppress 50% → allow 50% → target ~19% (GT=20.4%)
    ...
    print(f"  [v85 targets: through_the ~27%, toward_the ~3%, continue ~10%, hallway ~20%]")
