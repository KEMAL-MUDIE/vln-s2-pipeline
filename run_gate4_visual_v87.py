#!/usr/bin/env python3
"""
Gate 4 Visual v87 — v86 + ROOM NAMES relaxation for hallway + toward/through tuning

v86 STATS (post-analysis):
  through the:  22.2% (GT=27.2%) — still -5pp gap
  toward the:   2.9%  (GT=3.8%)  — close, increase P_TOWARD slightly
  hallway:      14.4% (GT=20.4%) — -6pp gap: LLM not generating; ROOM NAMES ban blocks it
  into hallway: 0.5%  (GT=4.4%)  — v86 P=0.20 allow didn't help (LLM never generates it)
  continue:     5.4%  (GT=10.4%) — persistent gap; add "continue" prompt examples

v87 FIXES:
  1. "hallway" (+6pp target): RELAX ROOM NAMES rule for directional "walk down the hallway"
     — keep ban on "walk into/toward the hallway" but ALLOW "walk down/along the hallway".
     Let assembly probabilistic filter (P=0.48 suppress) calibrate to ~18-20%.
     Remove the directional ban from prompt ROOM NAMES line.
  2. "toward the" (+0.9pp): Increase P_TOWARD from 0.20 → 0.25. Expected 2.9% → 3.5-4%.
  3. "through the" (+5pp): Increase P_THRU_MARKER from 0.90 → 0.95 for maximum marker density.
  4. "continue" (+5pp): Add "Walk X and continue through/toward Y" examples to prompt.
     These are GT-style multi-clause instructions that include "continue".
  Seed: 0x8745_5300.

Phase 2 checkpoint: outputs/gate4_v87_phase2_checkpoint.json (FRESH — prompt + seed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v87.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v87.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v87_phase2_checkpoint.json"  # FRESH — prompt + seed
...
P_THRU_MARKER = 0.95   # v87: increased from 0.90 → 0.95 for maximum through_the density
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8745_5300)  # v84: new seed
...
- ROOM NAMES: Do NOT write "walk into the hallway", "walk toward the hallway", "walk through the hallway", "walk into the living room", or "walk toward the bedroom" as mid-route steps. However, "walk down the hallway" or "walk along the hallway" ARE allowed when the robot is traversing a long corridor — use them naturally (GT annotators write "walk down the hallway" frequently). Write "walk forward" or "turn [direction] and walk forward" for short corridor segments. Only name bedroom/bathroom/living room/kitchen when it is the FINAL destination labeled in "Goal:" or when "on the left/right" provides essential disambiguation.
...
- STYLE: Avoid starting sentences with "Continue straight" — use action verbs instead ("Walk forward"). Avoid overusing "wooden" for generic surfaces — mention material only for distinctive objects (marble fireplace, glass door are fine). For multi-step navigation: "Walk [first action] and continue [second action]" is natural GT-style phrasing — e.g. "Turn right and continue through the arch", "Walk forward and continue past the table".
...
async def main():
    ...
    for ep in episodes:
        ...
        if "hallway" in text.lower():
            ...
            if _HALLWAY_ENTRY_RE.search(text):
                # v86: allow "walk into the hallway" P=0.20 (GT=4.4%; was always suppressed)
                entry_rng = random.Random(eid ^ 0x8745_E300)
            ...
            if _HALLWAY_DIR_RE.search(text):
                ep_rng = random.Random(eid ^ 0x8745_5300)
                if ep_rng.random() > 0.48:  # v86: suppress 52% → allow 48% → target ~18-20% (GT=20.4%)
        ...
        # P_TOWARD=0.20: ~20% of episodes that have "walk to the X" get "toward" instead.
        ...
        if _WALK_TO_THE_RE.search(text):
            toward_rng = random.Random(eid ^ 0x8745_A300)
            if toward_rng.random() < 0.25:  # v87: 0.20 → 0.25 to hit GT 3.8% (was 2.9%)
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v86=0.991, v84=0.992)")
    print(f"  [v87 targets: through_the ~25%+, toward_the ~3.5%, hallway ~18-22%, continue ~8%]")
