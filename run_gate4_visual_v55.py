#!/usr/bin/env python3
"""
Gate 4 Visual v55 — v54fix base + toward_the reduction via → rule example change

v54fix RESULTS (base):
  GT-match: 0.961, through_the: 27.5% (GT=27.2% — nearly perfect!), avg_words: 26.1

REMAINING GAPS (v54fix vs GT):
  toward_the: 10.1% vs GT=3.8% — 2.7× GT, biggest remaining metric gap
  hallway:    26.4% vs GT=20.4% — 6pp over
  walk_past:  15.3% vs GT=8.7% — mostly structural (→ rule), lower priority
  avg_turns:  0.72 vs GT=0.59  — medium gap

v55 ROOT CAUSE ANALYSIS (toward_the):
  Primary source: → rule example "continue toward the television" teaches LLM to use "toward the"
  for walking phrases after turns. GT rarely uses "toward the" (3.8% vs 10.1%).
  Fix: Remove "continue toward the television" → replace with "pass by the bookshelf"
  This eliminates direct teaching of "toward the" in → context without adding a blanket prohibition.

v55 FIX:
  - Change → rule example: "continue toward the television" → "pass by the bookshelf"
  - Fresh Phase 2 LLM generation (RULES changed)
  - Retain v54's surgical post-processing for "through the doorway" (proven safe)
  Expected: toward_the 10.1% → ~5-7% (closer to GT=3.8%)
  GT-match: ~0.961 (same rules structure, no new prohibitions)

Phase 2 checkpoint: outputs/gate4_v55_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v55.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v55.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v55_phase2_checkpoint.json"
...
- When a turn has → [object ahead] after the arrow: add a BRIEF walking phrase (3-5 words) like "walk past the grey sofa" or "pass by the bookshelf" — keep it very short
...
async def main():
    ...
    print('=== Gate 4 v55: toward_the fix — removed "continue toward the" example from → rule ===')
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v54fix=0.72) ← v55: toward_the fix")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v54fix=10.1%) ← KEY METRIC (v55 target: ~5-7%)")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v54fix=0.961, v53=0.961, v51=0.961)")
