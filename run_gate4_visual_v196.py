#!/usr/bin/env python3
"""
Gate 4 Visual v196 -- FIX10: Remove 'walk forward' from _ws_variants (walk_forward reduction).

INHERITED FROM v195: FIX 1-9b (all v195 fixes).

FIX 10 (NEW): Remove 'walk forward' from _ws_variants alternatives for "walk straight".
  Source of walk_forward=12.7%: _ws_variants randomly picks 'walk forward' as substitute
  for "walk straight" (1 of 4 options = 25% chance → contributes ~3pp walk_forward).
  Root: line ~1799: ['walk straight', 'go straight ahead', 'go straight', 'walk forward']
  Fix: remove 'walk forward' → ['walk straight', 'go straight ahead', 'go straight']
  Also fix line ~1795: 'walk forward for a bit' → 'walk for a while'
  Expected: walk_forward: 12.7% → ~9% (GT=6.1%)

Expected v196:
  walk_to:      ~7.3%  (GT=5.1%,  unchanged from v195)
  walk_past:    ~54.2% (GT=8.7%,  unchanged from v195)
  continue:     ~16.5% (GT=10.4%, unchanged from v195)
  walk_forward: ~9%    (GT=6.1%,  improved from v195=12.7%)
  go_straight:  ~13%   (GT=7.9%,  slightly increased from walk_forward→go_straight shift)
  walk_straight:~13%   (GT=8.8%,  slightly increased from walk_forward→walk_straight shift)
  Predicted SR: 61-70% (walk_forward closer to GT, same walk_to as v194)

Phase 2 checkpoint: outputs/gate4_v196_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v196.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v196.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v196_phase2_checkpoint.json"  # v196: assembly-only, copy of v190 P2
...
def clean(raw, sv):
    ...
    _ws_variants = [
        ...
        (r'\bwalk straight for a while\b',      ['go straight for a while', 'walk for a while']),  # v196 FIX10: was 'walk forward for a bit'
        (r'\bwalk straight a few\b',             ['go straight a few', 'walk a few']),
        (r'\bwalk straight ahead\b',             ['go straight ahead']),  # prevent double-hit
        # v196 FIX10: removed 'walk forward' from alternatives (was 1 of 4 → 25% → ~3pp walk_forward)
        # walk_forward=12.7%(GT=6.1%) — removing it shifts weight to walk_straight/go_straight
        (r'\bwalk straight\b',                   ['walk straight', 'go straight ahead', 'go straight']),
    ]
    ...
    # v195 FIX9: "continue walking" → "walk" (biggest continue source — 331 raw P2 occurrences).
    # GT: "continue walking" appears rarely; "walk" is more direct and GT-typical.
    # P=0.70 per occurrence to reduce continue% from 17.7% toward GT=10.4%.
    def _cont_walking_sub(m):
        cap = m.group(0)[0].isupper()
        return 'Walk' if cap else 'walk'
    result = re.sub(r'\bContinue\s+walking\b',
                    lambda m: 'Walk' if random.random() < 0.70 else m.group(0), result)
    result = re.sub(r'\bcontinue\s+walking\b',
                    lambda m: 'walk' if random.random() < 0.70 else m.group(0), result)
    result = re.sub(r'\bContinue\s+moving\b',
                    lambda m: 'Walk' if random.random() < 0.70 else m.group(0), result)
    result = re.sub(r'\bcontinue\s+moving\b',
                    lambda m: 'walk' if random.random() < 0.70 else m.group(0), result)

    # v195 FIX9b: "continue [direction/preposition]" → "walk [direction/preposition]" at P=0.65.
    # "continue across/up/down/along/through/towards/toward" → "walk across/up/down/..."
    # These are motion + direction combos where "walk" is the GT-natural verb.
    def _cont_dir_sub(m):
        cap = m.group(0)[0].isupper()
        direction = m.group(1)
        return ('Walk ' if cap else 'walk ') + direction
    result = re.sub(
        r'\b[Cc]ontinue\s+(across|up|down|along|through|towards|toward)\b',
        lambda m: (('Walk ' if m.group(0)[0].isupper() else 'walk ') + m.group(1)
                   if random.random() < 0.65 else m.group(0)),
        result
    )
...
async def main():
    ...
    for ep in episodes:
        ...
        # v193 FIX7: Sentence-level "walk to the X" → "walk past the X" for intermediate phrases.
        # MOTIVATION: walk_to=44.8% vs GT=5.1% — LLM treats every object as destination.
        # v191/v192 experiments: SYSTEM_INTRO "FORBIDDEN walk to the X" doesn't reliably work.
        # SOLUTION: Deterministic regex — convert walk_to in non-final positions to walk_past.
        # DETECTION: If "walk to the X" is followed by punctuation+more-navigation → intermediate.
        # More navigation = turn/walk/go/continue/enter/exit after the pattern.
        # NON-FINAL context: "walk to the X, then..." / "walk to the X. Turn..." / "walk to the X and turn..."
        ...
        def _intermediate_walk_to_to_past(text_in):
            ...
            for i, sent in enumerate(sentences):
                ...
                if is_last or has_stop:
                    ...
                    # Non-final, non-stop sentence: convert walk_to → walk_past
                    def _conv_wt(m):
                        if _v193_fix7_rng.random() < 0.90:
                            cap = m.group(0)[0].isupper()
                            obj = m.group(1).strip()
