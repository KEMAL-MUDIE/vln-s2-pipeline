#!/usr/bin/env python3
"""
Gate 4 Visual v146 — Improved go_straight post-processing with clean landmark extraction.

v145 ACTUAL: through_the=27.08%, hallway=20.23%, into_hallway=4.35%, continue=9.68%, walk_past=8.37%
             avg_words=29.8w (+3w over GT!), vocab_gap=1.39pp → pred_SR=63.04%
v145 PROBLEMS: extract_landmark_brief() produced truncated/messy phrases:
  - "large rectangular framed portrait of" — truncated at preposition
  - "dark grey accent wall featuring ahead" — trailing filler words
  - "wooden" at 25.6% (GT=3.3%) — material adjectives leaking in
  - avg_words=29.8w (GT=26.8w) — +3w because multi-word replacements

v146 STRATEGY: Same post-processing approach as v145 but with improved extraction:
  - Phase 1: SAME as v143 (gate4_v143_phase1_checkpoint.json)
  - Phase 2: REUSE v145 Phase 2 checkpoint (no new generation needed, faster)
  - IMPROVED extract_landmark_brief():
    * Strip trailing filler words (ahead, with, of, featuring, positioned, etc.)
    * Strip leading color/material adjectives (wooden, grey, brown, etc.)
    * Limit to 2 core words max
  - FALLBACK: "walk straight" instead of keeping "go straight" when no clean landmark
  - Expected: go_straight→~3-8%, avg_words→~27w, no wooden/material OOD

Phase 1 checkpoint: outputs/gate4_v143_phase1_checkpoint.json (same as v143/v145)
Phase 2 checkpoint: outputs/gate4_v145_phase2_checkpoint.json (REUSED from v145)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v146.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v146.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v143_phase1_checkpoint.json"  # SAME as v143/v145: auto-annotator Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v145_phase2_checkpoint.json"  # v146: REUSE v145 Phase 2 (same generation, better post-proc)
...
def extract_landmark_brief(desc: str) -> Optional[str]:
    ...
    if not desc:
        return None
    text = desc.strip()
    # Turn point: "Turn at the white kitchen island. Ahead, ..." → extract noun phrase
...
def replace_go_straight_with_landmarks(text: str, phase_c_descs: Dict[str, str]) -> str:
    """v146: Replace 'go straight' with Phase C landmarks (clean extraction) or 'walk straight' fallback.
    v145 lesson: truncated messy landmarks + material adjectives from Phase C leak in → use fallback.
    """
    if not text:
        return text
    # Build ordered list of clean destination landmarks from Phase C
    landmarks = []
    if phase_c_descs:
        for i in range(1, 15):
            label = f"turn_{i}"
    ...
    # Replace "go straight" occurrences in route order
    lm_idx = [0]
    def _replace_gs(m):
        ...
        if lm_idx[0] < len(landmarks):
            ...
            replacement = "walk straight"  # safe fallback: GT≈6% (less OOD than go_straight=80.91%)
...
async def main():
    ...
    print('=== Gate 4 v146: Improved go_straight post-proc: clean landmarks + walk_straight fallback ===')
