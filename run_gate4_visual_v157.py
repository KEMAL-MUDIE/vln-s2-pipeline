#!/usr/bin/env python3
"""
Gate 4 Visual v157 — Balance walk_straight + go_straight + reduce avg_turns further.

v156 ACTUAL: GT-match=0.975✓, turn-anchor=17.5%✓, toward_the=0.9%✓, take_few=0.2%✓
  Remaining gaps (not in GT-match formula but important for SR):
  1. walk_straight=0.0% (GT=8.8%): overcorrected — _ws_variants removed ALL walk_straight
  2. go_straight=3.9% (GT=7.9%): partial improvement, need ~8%
  3. avg_turns=0.86 (GT=0.59): P_TURN_UNANCHORED=0.18 not low enough; need 0.12
     Scaling: 0.86*(0.12/0.18)=0.57 → P=0.12 should reach ~0.57 (close to GT=0.59)
  4. through_the=33.7% (GT=27.2%): P_THRU_MARKER=0.75 too high → 0.60 gives ~27.3%

v157 CHANGES from v156:
  1. _ws_variants: restore 'walk straight' as one of 4 alts (was 'walk ahead') → ~25% chance
     Expected: ~35% episodes with walk_straight from LLM × 25% survive = 8.75% ≈ GT=8.8%
  2. go_straight skip: 8%→15% → more go_straight survives (GT=7.9%, v156=3.9%)
  3. P_TURN_UNANCHORED: 0.18→0.12 (avg_turns 0.86→~0.57 ≈ GT=0.59)
  4. P_THRU_MARKER: 0.75→0.60 (through_the 33.7%→~27.3% ≈ GT=27.2%)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C)
Phase 2 checkpoint: outputs/gate4_v157_phase2_checkpoint.json (FRESH: new P_TURN_UNANCHORED+P_THRU)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v157.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v157.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v157: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v157_phase2_checkpoint.json"  # v157: FRESH — balanced walk_straight + lower avg_turns
...
P_TURN_UNANCHORED = 0.12  # v157: 0.18→0.12 to reduce avg_turns from 0.86 toward GT=0.59 (scaling: 0.86*0.12/0.18=0.57)
...
async def main():
    ...
    print('=== Gate 4 v157: Balance walk_straight + go_straight + reduce avg_turns further ===')
    ...
    for ep in episodes:
        ...
        # v156: P% of episodes skip replacement → allows "go straight" to survive (GT go_straight=7.9%).
        # v157: increased 8%→15% (v156 gave 3.9%, GT=7.9% — need ~2x more skip).
        # Seed derived from eid to be deterministic per episode.
        _gs_skip_rng = random.Random(eid ^ 0x60_5700)
        if _gs_skip_rng.random() >= 0.15:
    ...
    print(f"  go_straight:   {100*go_straight_n/n:.1f}%  (GT=7.9%, v156=3.9%) ← v157 target ~8%")
    print(f"  walk_straight: {100*wlk_straight_n/n:.1f}%  (GT=8.8%, v156=0.0%) ← v157 target ~9%")
