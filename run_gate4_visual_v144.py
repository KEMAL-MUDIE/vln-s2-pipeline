#!/usr/bin/env python3
"""
Gate 4 Visual v144 — Phase C landmark injection to reduce go_straight overuse.

v143 ACTUAL: through_the=27.41%, hallway=20.77%, into_hallway=4.51%, continue=10.49%, walk_past=8.70%
             avg_words=25.6w, vocab_gap=0.79pp → pred_SR=63.41%
v143 EXTENDED: go_straight=80.91% (GT=7.94%, +72.97pp!) — structural issue from DISTANCE rule

v144 STRATEGY: Reduce go_straight by injecting Phase C destination landmarks into route_summary.
  - Phase 1: SAME as v143 (gate4_v143_phase1_checkpoint.json from auto_annotator Phase C)
  - Phase 2: FRESH generation with modified route_summary + DISTANCE rule
  - Route change: "straight 15m" → "straight 15m [→ kitchen island]" when Phase C has context
  - DISTANCE rule: use "walk toward the X" / "walk to the X" when [→ X] context available
  - Post-proc change: remove "go straight" from _ws_variants (walk straight diversification)

Expected improvements:
  - go_straight: 80.91% → ~20-35% (huge reduction via landmark context + post-proc fix)
  - toward_the: 1.09% → ~3-5% (GT=3.75%, closer to GT via "walk toward the X")
  - 5-metric vocab: unchanged (P_ params same as v139/v143)
  - vocab_gap: ~0.8pp → pred_SR≈63.4%
  - Actual SR: potentially higher — less OOD vocabulary + more semantically precise instructions

Phase 1 checkpoint: outputs/gate4_v143_phase1_checkpoint.json (same as v143)
Phase 2 checkpoint: outputs/gate4_v144_phase2_checkpoint.json (FRESH generation)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v144.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v144.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v143_phase1_checkpoint.json"  # SAME as v143: auto-annotator Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v144_phase2_checkpoint.json"  # v144: FRESH generation (go_straight fix)
...
def extract_landmark_brief(desc: str) -> Optional[str]:
    """Extract a 2-5 word landmark noun phrase from a Phase C vision description.
    Used by build_route_desc to inject destination context into straight segments.
    """
    if not desc:
        return None
    text = desc.strip()
    # Turn point: "Turn at the white kitchen island. Ahead, ..." → "white kitchen island"
    m = re.match(r'Turn at (?:the )?(.+?)\.', text)
    if m:
        np = m.group(1).strip()
        words = np.split()[:5]
        return ' '.join(words)
    # "Ahead, a living room..." → "living room"
    m = re.match(r'Ahead[,\s]+(?:a |an |the )?(.+?)(?:\s+(?:with|is|are|and)|\.|$)', text)
    if m:
        np = m.group(1).strip()
        words = np.split()[:4]
        return ' '.join(words)
    # Goal/start: "The grey lounge chair positioned..." → "grey lounge chair"
    m = re.match(r'(?:The |A |An )([A-Za-z].+?)(?:\s+(?:positioned|located|is|are|against|and|with)|\.|$)', text)
    if m:
        np = m.group(1).strip()
        words = np.split()[:4]
        return ' '.join(words)
    # Fallback: take first 3 content words
    words = [w for w in text.split() if len(w) > 2]
    return ' '.join(words[:3]) if len(words) >= 2 else None


def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
                     path_len: int = 0, eid: int = 0, phase_c_descs: Optional[Dict[str, str]] = None):
    ...
    phase_c_descs: {label: desc} from Phase C vision — injects destination landmarks for straights (v144).
    ...
    for prim_i, p in enumerate(primitives):
        t = p["type"]
        if t == "straight":
            d = p.get("distance_m", 0)
            if d > straight_min_dist:
                # v144: inject Phase C destination landmark for this straight segment
                landmark_ctx = None
                if phase_c_descs:
                    # Find destination: the next turn or goal after this straight segment
                    dest_label = "goal"
                    for j in range(prim_i + 1, len(primitives)):
                        pj_type = primitives[j]["type"]
                        if pj_type in ("left_turn", "right_turn"):
                            dest_label = f"turn_{turn_idx + 1}"
                            break
                    raw_desc = phase_c_descs.get(dest_label, "")
                    if raw_desc:
                        landmark_ctx = extract_landmark_brief(raw_desc)
                if landmark_ctx:
                    parts.append(f"straight {d:.0f}m [→ {landmark_ctx}]")
                else:
...
- MOTION WORDS: Prefer "walk to the X", "walk toward the X", "walk straight". AVOID "go forward", "walk forward", "keep going", "go straight" — these are generic and unhelpful. Do NOT use "continue", "proceed".
- DISTANCE: When the route shows a 'straight Xm [→ landmark]' segment — say "walk toward the landmark" or "walk to the landmark" (NEVER say "go straight" when a destination is known). When the route shows 'straight Xm' with no destination: "take a few steps" (≤2m), "walk straight" (3–5m), "walk straight ahead" (≥6m). NEVER write metric numbers.
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    # v144: pass Phase C vision descriptions for landmark injection into straight segments
    _phase_c = vision_map.get(str(eid), {})
    route, had_pass_markers = build_route_desc(primitives, sel_turns, seg_midpoint,
                                               path_len=path_len, eid=eid,
                                               phase_c_descs=_phase_c)
...
def clean(raw, sv):
    ...
    # v144: Remove "go straight" from _ws_variants — landmark injection now handles most straights.
    # Remaining "walk straight" fallbacks (no landmark context) stay as "walk straight ahead".
    # This eliminates post-proc's contribution (~20-25pp) to go_straight overuse.
    _ws_variants = [
        (r'\bwalk straight for a long while\b', ['walk straight a long way', 'walk ahead a long way']),
        (r'\bwalk straight for a while\b',      ['walk straight ahead for a bit', 'walk a little further']),
        (r'\bwalk straight a few\b',             ['walk a few', 'take a few steps']),
        (r'\bwalk straight ahead\b',             ['walk straight ahead']),  # prevent double-hit
        (r'\bwalk straight\b',                   ['walk straight ahead', 'walk ahead']),
    ]
    ...
    # v144: Convert "keep going" → "walk straight ahead" (avoid go_straight).
    result = re.sub(r'\bKeep going\b', 'Walk straight ahead', result)
    result = re.sub(r'\bkeep going\b', 'walk straight ahead', result)
    ...
    # v144: "continue forward/straight" → "walk straight ahead" (avoid go_straight).
    result = re.sub(r'\bContinue\s+forward\b', 'Walk straight ahead', result)
    result = re.sub(r'\bcontinue\s+forward\b', 'walk straight ahead', result)
    result = re.sub(r'\bContinue\s+straight\b', 'Walk straight ahead', result)
    result = re.sub(r'\bcontinue\s+straight\b', 'walk straight ahead', result)
...
async def main():
    ...
    print('=== Gate 4 v144: Phase C landmark injection for straight segments (go_straight fix) ===')
    ...
    print(f"  v140 actual: go_straight→0% ✓ but walk_ahead=62%(GT=0.16%) + walk_a_bit=49%(GT=0.05%) — new OOD!")
    print(f"  v141 FAILED: continue=22.1%, through_the=22.6%, avg_words=23.3w — Phase2 prompt change too aggressive")
    print(f"  v142: REUSE v139 P2 ckpt + post-proc 'go straight'→'walk forward' (GT=6.09%, much less OOD)")
    ...
    print(f"  Phase 1: NEW auto-annotator Phase C checkpoint (v143_phase1)")
    ...
    for ep in episodes:
        ...
        # v143: NO go_straight post-proc — fresh generation, testing whether Phase 1 descriptions
        # affect go_straight% naturally (v139 had go_straight=81.35% without post-proc).
        # This is an intentional A/B test: v143 vs v139 with ONLY Phase 1 changed.
