#!/usr/bin/env python3
"""
Gate 4 Visual v62 — P_TURN_UNANCHORED 0.42, REVERTED room names rule (v61 over-corrected)

v61 RESULTS (base, FAILED):
  GT-match: 0.949, toward_the: 3.5%, through_the: 25.9%, walk_past: 14.4%, hallway: 6.6%
  avg_turns: 0.58 (GT=0.59) — PERFECT avg_turns!
  BUT hallway massively over-corrected: 6.6% vs GT=20.4% (SAME error as v52 blanket prohibition)
  Blanket "NEVER use hallway" rule suppresses even valid stop-phrase uses

KEY LEARNING (v61 = repeat of v52 anti-pattern):
  Blanket prohibitions cause over-correction. LLM stops mentioning hallway ANYWHERE.
  Need: specific-example rule (not NEVER), plus surgical post-processing for mid-route patterns.

v62 FIX:
  - REVERT ROOM NAMES rule to v59's specific-example version (not blanket prohibition)
  - Keep P_TURN_UNANCHORED=0.42 (proven to give avg_turns=0.58, GT=0.59)
  - Keep all v59 post-processing (doorway, walk_past, hallway surgical)
  - The surgical hallway post-processing already catches the patterns the prompt missed

v62 ROOM NAMES rule (REVERTED from v61):
  "Do NOT write 'walk into the hallway', 'walk toward the hallway', 'walk through the hallway',
   'walk down the hallway', 'go through the hallway', 'head down the hallway' as mid-route steps.
   Write 'walk forward' instead. Only name a room when it is the FINAL destination in Goal:."

Expected:
  - avg_turns: ~0.58 (GT=0.59) — same P_TURN_UNANCHORED as v61
  - hallway: ~22-25% (LLM generates normally, surgical post-proc removes mid-route)
  - GT-match: ~0.952-0.958 (LLM variance, similar to v60)
  - avg_words: ~26.0-26.2 (restored from v61's 25.6)

Phase 2 checkpoint: outputs/gate4_v62_phase2_checkpoint.json (FRESH — different prompt from v61)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v62.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v62.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v62_phase2_checkpoint.json"  # FRESH — reverted room names rule!
...
async def main():
    ...
    print('=== Gate 4 v62: P_TURN_UNANCHORED 0.42 + reverted room names rule (v61 over-corrected) ===')
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v61=0.58) ← v62: P_TURN_UNANCHORED 0.42")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v61=3.5%) ← KEY METRIC")
    print(f"  hallway:       {100*hallway_n/n:.1f}%  (GT=20.4%, v61=6.6%!) ← KEY METRIC (v61 over-corrected!)")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v61=0.949, v60=0.952, v59=0.958)")
