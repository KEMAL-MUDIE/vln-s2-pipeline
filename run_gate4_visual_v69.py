#!/usr/bin/env python3
"""
Gate 4 Visual v67 — v63 base + lower straight threshold for long paths + floor-stop fix

v66 RESULTS (v63 + floor-stop fix):
  GT-match: 0.984, through_the: 27.8% (GT=27.2%), hallway: 19.4% (GT=20.4%)
  avg_words: 25.4 (GT=26.8) — 1.4w gap; plen=6 worst: 27.2w vs GT=30.5w (-3.3w!)

ROOT CAUSE of word gap:
  plen=6 episodes (552/1839 = 30%): GT avg=30.5w, v66=27.2w (-3.3w)
  Budget "31-40 words" but LLM writes 27.2w — LLM runs out of route content.
  Straight segments 1.0-2.0m (avg 1.46m) are currently DROPPED (threshold=2.0m).
  Adding them: avg 0.94 extra markers/ep → ~1.9w gain per plen=6 episode.

v67 CHANGES (all from v63):
  1. build_route_desc: path_len parameter — for path_len>=7: straight threshold 2.0→1.0m
     (adds "straight 1m" markers → LLM writes "walk forward" or "continue" +2w each)
  2. Budget hint: path_len>=7 → "33-46 words" (was "31-40 words")
  3. Floor-stop fix: inline in Phase 2 assembly (0% bad floor stops, same as v65/v66)

Expected:
  - avg_words for plen=6: 27.2 + ~2.0 + ~0.5 = ~29.7w (gap -3.3 → ~-0.8)
  - Overall avg_words: 25.4 + ~0.6 = ~26.0w (gap 1.4 → ~0.8)
  - through_the%: ~27.8% (v63 base, unchanged)
  - hallway%: ~19.4% (v63 base, unchanged)
  - floor-stop%: 0.0%
  - GT-match: ~0.984 (same as v66)

Phase 2 checkpoint: outputs/gate4_v67_phase2_checkpoint.json (FRESH — route changes require new P2)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v67.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v67.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v67_phase2_checkpoint.json"  # FRESH — route changes require new P2
