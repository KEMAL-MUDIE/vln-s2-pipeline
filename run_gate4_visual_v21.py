#!/usr/bin/env python3
"""
Gate 4 Visual v20 — Ahead-Enriched Instructions

IMPROVEMENT OVER v19:
  v19 generated richer 1-2 sentence Phase 1 descriptions capturing what's
  visible AHEAD after each turn. But Phase 2 only used the FIRST sentence
  (turn pivot landmark). The second sentence ("Ahead, a living room with a
  grey sofa is visible") was completely discarded.

  GT instructions like "Walk straight passing the gray couch and stop near
  the rug" reference objects the agent PASSES while walking — not just at
  turn points. The v19 Phase 1 second sentences capture exactly this!

THE FIX (v20): AHEAD-ENRICHED PHASE 2 (reuses v19 Phase 1 checkpoint)
  Phase 1 — REUSED from v19 (no new API calls).
    Checkpoint: outputs/gate4_v19_phase1_checkpoint.json

  Phase 2 — Instruction generation WITH ahead context:
    Extract core noun phrase from the 2nd sentence of each turn description.
    Include in route notation as: turn left at [kitchen island] → [grey sofa ahead]
    New SYSTEM rule: "turn with [X ahead]: add brief 'walk past/toward X' phrase"
    This gives agents intermediate visual confirmation during walking.
    Checkpoint: outputs/gate4_v20_phase2_checkpoint.json
    Output: outputs/datasets/val_unseen_generated_gemma_visual_v20.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT     = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v21.json.gz"
P1_CKPT    = ROOT / "outputs" / "gate4_v19_phase1_checkpoint.json"  # reuse v19 Phase 1!
P2_CKPT    = ROOT / "outputs" / "gate4_v21_phase2_checkpoint.json"
...
PREPOSITION_RE = re.compile(
    r'\s+(positioned|against|near|by|in|with|on|at|of|to|for|leading|facing|containing|next)\b',
    re.IGNORECASE
)
...
def get_landmark_info_v18(ep, pf_map, old_map, primitives, vision_map: Dict) -> Dict:
    """
    Build landmark info for v19: uses vision descriptions (richer 1-2 sentences).
    - goal_landmark: core noun phrase (4-6 words) for stop phrase
    - goal_full: full first sentence shown in Goal: line
    - turn landmarks: noun phrase from turn description (for the anchoring system)
    Room names always from gate3_perframe.
    """
    eid = ep["episode_id"]
    eid_str = str(eid)
    vis = vision_map.get(eid_str, {})  # {label: rich_desc_or_None}
    ...
    # Turn landmarks: extract pivot (1st sentence) and ahead (2nd sentence)
...
def episode_selective_turns(lm_info: Dict, eid: int, p_anchor: float = P_ANCHOR_EPISODE) -> List[Dict]:
    ...
    for t in turns:
        ...
        result.append({
            ...
            # v21: pass ahead for ALL turns (both anchored and non-anchored)
            "ahead": t.get("ahead", ""),
        })
        if t_room:
            prev_room = t_room
    ...
    if rng.random() < p_anchor:
        ...
        if turns_with_lm:
            ...
            # ahead is already set for all turns from the loop above
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v21: anchored turn gets [X ahead] (agent may write "walk past/toward X")
                base = f"turn {direction} at [{lm}]"
                parts.append(f"{base} → [{ahead} ahead]" if ahead else base)
            elif rm:
                # v21: room transition — also add (toward: X) for non-anchored ahead
                base = f"turn {direction} → {rm}"
                parts.append(f"{base} (toward: {ahead})" if ahead else base)
            else:
                # v21: plain turn — add (toward: X) hint if we have ahead info
                if ahead:
                    parts.append(f"turn {direction} (toward: {ahead})")
                else:
...
RULES:
- [object] in brackets = turn pivot landmark: write naturally "turn left at the grey pillar" or "turn right past the table"
- [object ahead] after → arrow = what you pass AFTER the bracketed turn: add a brief "walk past/toward X" (3-5 words) after turning
- (toward: object) in parentheses = what you'll see while walking after a plain turn: add "head toward X" or "walk toward X" (3-5 words) — use 'toward', NOT 'past'
- (→ room) after arrow = room transition after a plain turn: mention it naturally as "walk into the living room"
- Plain turns with no brackets or parentheses: write ONLY "turn left" or "turn right"
- Do NOT invent objects not shown in the route
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
                 stop_phrase, stop_type, vision_map: Dict):
    ...
    n_ahead_anch = sum(1 for t in sel_turns if t.get("landmark") and t.get("ahead"))
    n_toward = sum(1 for t in sel_turns if not t.get("landmark") and t.get("ahead"))
    ...
    if n_ahead_anch > 0:
        anchor_note += (f" {n_ahead_anch} [X ahead] marker(s): after the bracketed turn,"
                        f" add brief 'walk past/toward X' (3-5 words).")
    if n_toward > 0:
        anchor_note += (f" {n_toward} (toward: X) marker(s): after a plain turn,"
                        f" add 'head toward X' or 'walk toward X' (3-5 words, NOT 'past').")
...
async def main():
    ...
    print("=== Gate 4 v21: All-Turns Ahead-Enriched Instructions ===")
    ...
    print(f"  v21 key change: show (toward: X) for ALL non-anchored turns with ahead info")
    print()
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v20=0.937, v19=0.965, v18=0.958)")
