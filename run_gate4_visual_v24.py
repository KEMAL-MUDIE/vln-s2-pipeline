#!/usr/bin/env python3
"""
Gate 4 Visual v24 — Turn-Side Landmarks + Dual-Segment Midpoints

IMPROVEMENTS OVER v23:
  1. TURN-SIDE LANDMARKS (new): Rendered TURN-FACING views (90° to turning side)
     capture what GT annotators see ("turn left at the pink bench"). These are
     used as turn landmarks instead of approach-direction Phase 1 descriptions.
     89.4% of turn-side frames have useful descriptions; 42.7% give better
     landmarks than approach-direction Phase 1.
     Checkpoint: gate4_v24_turn_sides_p1_checkpoint.json

  2. DUAL-SEGMENT MIDPOINTS (from v23): First AND last segment get [pass: X].
     88.6% coverage (vs 68.7% in v22).
  ...
  Phase 2 — Instruction generation with turn-side + dual-midpoint landmarks:
    ...
    Checkpoint: outputs/gate4_v24_phase2_checkpoint.json
    Output: outputs/datasets/val_unseen_generated_gemma_visual_v24.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v24.json.gz"
...
P1_TS_CKPT    = ROOT / "outputs" / "gate4_v24_turn_sides_p1_checkpoint.json"  # v24 turn-sides
P2_CKPT       = ROOT / "outputs" / "gate4_v24_phase2_checkpoint.json"
...
_GENERIC_TURN_WORDS = {
    'wall', 'walls', 'floor', 'ceiling', 'door', 'doors', 'hallway', 'corridor',
    'space', 'room', 'area', 'window', 'opening', 'entrance', 'exit', 'passage',
    'corner', 'junction',
}
