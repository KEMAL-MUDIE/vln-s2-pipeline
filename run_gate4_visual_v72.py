#!/usr/bin/env python3
"""
Gate 4 Visual v72 — v70 + direction-aware Phase 1 turn landmarks

v70 BASELINE:
  GT-match: 0.987, avg_words: 26.4 (GT=26.8)
  Approach-view rendering (from path[i-1] facing path[i]): 89.9% good landmark rate
  Landmarks described WITHOUT knowing the turn direction → VLM sometimes picks wrong side.

v72 FIX:
  Replace approach-views Phase 1 checkpoint (gate4_v70_approach_views_p1_checkpoint.json)
  with direction-aware Phase 1 (gate4_v72_approach_views_dir_p1_checkpoint.json).
  - Same approach-view rendering geometry as v70 (correct angle)
  - VLM prompt now says "Robot will turn LEFT/RIGHT — look for landmark on LEFT/RIGHT side"
  - Good rate: 97.9% (vs 89.9% in v70) — improve=375, regress=52 vs non-directional
  - Uses turn_dir field from approach_views.json to select L/R prompt
  Everything else identical to v70 (GT-match statistics preserved).

Phase 2 checkpoint: outputs/gate4_v72_phase2_checkpoint.json (FRESH — P1 data changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v72.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v72.json.gz"
...
P1_TS_CKPT    = ROOT / "outputs" / "gate4_v72_approach_views_dir_p1_checkpoint.json"  # v72: direction-aware approach-view landmarks
P2_CKPT       = ROOT / "outputs" / "gate4_v72_phase2_checkpoint.json"  # FRESH — P1 data changed
