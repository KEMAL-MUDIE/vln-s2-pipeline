...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v171.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v171_phase2_checkpoint.json"  # v171: FRESH — sentence chaining + enter verb + anchor% fix
...
- SENTENCE STRUCTURE: Write 2-3 sentences total — NOT one sentence per action. Chain consecutive actions with "and": "turn left and walk through the hallway" NOT "Turn left. Walk through the hallway." Chain the final stop: "walk to the X and stop near the Y" or "walk into the room and wait there." This is how real R2R annotators write — they combine related steps into one sentence.
...
async def main():
    ...
    print('=== Gate 4 v171: SENTENCE CHAINING + ENTER VERB + ANCHOR% RECAL ===')
    print(f"  v170 MEASURED: anchor%=25.1%(GT=16.5%), avg_turns=1.05, GT-match=0.822")
    print(f"  DEEP RESEARCH FINDINGS:")
    print(f"    - Sentence count: GT=1-2 sent (53%), v169/v170=3-4 sent (80%) — BIGGEST GAP!")
    print(f"    - 'enter' verb: GT=17.0%, v169=1.3% — model expects 'enter the room' not 'into the'")
    print(f"    - Action bigrams: (go,walk)=7.46x GT, (walk,walk)=2.25x GT — too repetitive")
    print(f"  FIX 1 (HIGHEST IMPACT): SENTENCE STRUCTURE rule in system prompt")
    print(f"         'Write 2-3 sentences total. Chain with and: turn left and walk through.'")
    print(f"  FIX 2: ENTER VERB rule — '(door→room)' → 'enter the room' (preferred) or 'walk through doorway into'")
    print(f"  FIX 3: P_ANCHOR_EPISODE=0.06 + P_HALLWAY_POSITIONAL=0.30 (was 100%→25.1%)")
    print(f"         Hallway injection was main cause of anchor% overshoot 22.5%→25.1%")
    print(f"  KEPT: ANTI-COLOR prompt, P_TURN_UNANCHORED=1.0, (door→room)=45%, nav-verb fixes")
    print(f"  v171: Expected anchor%≈15-17%, sent_count=2-3(target!), enter%≈8-12%, GT-match≥0.90")
