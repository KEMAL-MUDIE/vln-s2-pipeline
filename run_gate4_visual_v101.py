#!/usr/bin/env python3
"""
Gate 4 Visual v101 — v100 + hallway suppression in route code + turn direction restoration.

KEY CHANGES FROM v100:
  v100 problem: hallway% = 49.4% (GT=20.4%), eps_with_turn = 28.1% (GT=41.3%)
  v100 hallway suppression via PROMPT was ineffective — 69.3% of eps have (→ hallway) in route
  and LLM still mentions it 49.4% of the time even with dedup rule.

v101 FIX — in build_route_desc (code change, not just prompt):
  For HALLWAY room transitions:
    - Suppress "(→ hallway)" from route entirely
    - If the transition has a turn direction: emit "turn {direction}" (keep turn, lose room)
    - If no turn direction: skip entirely (silent straight into hallway)
  For NON-HALLWAY room transitions:
    - Keep v99/v100 style: "(→ {room})" — LLM writes "walk into the bedroom" naturally

  Expected effects:
    hallway%:      49.4% → ~5-10% (LLM only mentions hallway if it generates naturally)
    eps_with_turn: 28.1% → ~35-40% (hallway turns now emit "turn {direction}" explicitly)

v101 PROMPT CLEANUP:
  Remove hallway-specific dedup rules from ROOM TRANSITIONS and (→ room) sections
  since the suppression is now handled in the route code.

Phase 2 checkpoint: outputs/gate4_v101_phase2_checkpoint.json (NEW — fresh LLM calls needed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v101.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v101.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v101_phase2_checkpoint.json"  # v101 — fresh P2 (hallway suppression in route)
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v101: Hallway suppression in route code (v100 prompt-only approach failed).
                # Problem: 69.3% of eps have (→ hallway); prompt dedup got only 49.4% → GT=20.4%.
                # Root cause: room labels are 34-40% accurate; "hallway" is massively over-applied.
                # Fix: For HALLWAY transitions → emit "turn {direction}" (keep turn, drop room name).
                #      For NON-HALLWAY transitions → keep v99 style "(→ room)" (no turn dir).
                # Expected: hallway% 49% → ~5-10%, eps_with_turn 28% → ~35-40%.
                is_hallway = "hallway" in rm.lower() or "corridor" in rm.lower()
                if is_hallway:
                    if direction:
                        parts.append(f"turn {direction}")
                    # else: straight into hallway — skip entirely (no hallway mention)
                else:
                    parts.append(f"(→ {rm})")
            else:
                parts.append(f"turn {direction}")
...
- When you see (→ room) in the route: write "walk into the [room]" or "enter the [room]" — describe entering that room. Do NOT write "turn left" or "turn right" for these transitions. Keep it brief (3-5 words). These are only non-hallway rooms (bedroom, kitchen, bathroom, living room, etc.).
...
- ROOM TRANSITIONS: When you see (→ room) in the route, briefly describe entering: "walk into the bedroom", "enter the kitchen", "go to the bathroom". Do NOT write "turn left" or "turn right" for room entries. Use the room name once, briefly (3-5 words). The route will only show meaningful non-hallway rooms — if you see no (→ room) marker, do NOT invent a room name.
...
async def main():
    ...
    print(f"  [v101 targets: hallway suppressed in route code; hallway% → ~5-10%; eps_with_turn → ~35-40%]")
