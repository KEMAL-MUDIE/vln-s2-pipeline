#!/usr/bin/env python3
"""
Gate 4 Visual v71 — v70 + more explicit turn directions + hybrid landmark selector

v70 ANALYSIS (partial v62 results as proxy):
  Turn-path episodes WITH explicit "turn left/right": SR=37.3% (134 eps)
  Turn-path episodes WITHOUT explicit "turn left/right": SR=22.4% (174 eps)
  Gap: 14.9pp — caused by agent not knowing WHEN to turn

ROOT CAUSE of 22.4% SR for unanchored turn-path episodes:
  P_TURN_UNANCHORED=0.42 → only 42% of unanchored turns get explicit "turn left/right".
  58% of turn-path episodes just say "walk forward" with no directional cue.
  Agent walks past the turn without changing direction → navigation failure.

v71 CHANGES (from v70):
  1. P_TURN_UNANCHORED: 0.42 → 0.85 (85% of unanchored turns get explicit direction)
     - Converts ~174 eps from 22.4% SR to ~37% SR bracket
     - Expected gain: +4-6pp overall SR
  2. Hybrid landmark selector: approach-view (v70) preferred, turn-side (v24) as fallback
     - If av_desc is None/generic → use ts_desc from v24 checkpoint
     - Maximizes landmark coverage: both sources complement each other
     - Expected: +1-2pp from better landmark matching

Phase 2 checkpoint: outputs/gate4_v71_phase2_checkpoint.json (FRESH)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v71.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v71.json.gz"
...
P1_TS_CKPT    = ROOT / "outputs" / "gate4_v24_turn_sides_p1_checkpoint.json"  # v71: turn-side (fallback)
P1_AV_CKPT    = ROOT / "outputs" / "gate4_v70_approach_views_p1_checkpoint.json"  # v71: approach-view (preferred)
P2_CKPT       = ROOT / "outputs" / "gate4_v71_phase2_checkpoint.json"  # FRESH
...
P_TURN_UNANCHORED = 0.85  # v71: raised from 0.42 → explicit turns for 85% of unanchored paths (SR gain from 22.4%→37%)
...
def load_turn_sides_checkpoint() -> Dict:
    """v71: Load HYBRID turn landmark map — approach-view preferred, turn-side as fallback.

    Format: {eid_str: {turn_N: best_desc}}
    - If approach-view (P1_AV_CKPT) has a good description → use it
    - If not → fall back to turn-side (P1_TS_CKPT)
    - Good = not None and len(words) >= 2
    """
    av = json.load(open(P1_AV_CKPT)) if P1_AV_CKPT.exists() else {}
    ts = json.load(open(P1_TS_CKPT)) if P1_TS_CKPT.exists() else {}

    all_eids = set(av.keys()) | set(ts.keys())
    merged = {}
    for eid in all_eids:
        av_ep = av.get(eid, {})
        ts_ep = ts.get(eid, {})
        all_labels = set(list(av_ep.keys()) + list(ts_ep.keys()))
        ep_merged = {}
        for label in all_labels:
            av_d = av_ep.get(label)
            ts_d = ts_ep.get(label)
            # Prefer approach-view if good (≥2 words, not None)
            av_good = av_d and len(av_d.split()) >= 2
            ts_good = ts_d and len(ts_d.split()) >= 2
            if av_good:
                ep_merged[label] = av_d
            elif ts_good:
                ep_merged[label] = ts_d
            else:
                ep_merged[label] = av_d or ts_d  # whichever is non-None
        merged[eid] = ep_merged
    return merged
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    route = build_route_desc(primitives, sel_turns, seg_midpoint, path_len=path_len)
...
async def main():
    ...
    print(f"\nPhase 1 hybrid checkpoint (av+ts): {len(ts_vision_map)} episodes")
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v62=0.59) ← v71: P_TURN_UNANCHORED 0.85")
