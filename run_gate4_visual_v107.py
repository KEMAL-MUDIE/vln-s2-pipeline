#!/usr/bin/env python3
"""
Gate 4 Visual v107 — Fix "passing the" (28%→70%) + restore "continue straight".

v106 ANALYSIS (actual results vs expected):
  - "passing the": 28.2% (expected 65%, v24=79.7%) ← CRITICAL MISS
  - "straight":    145%  (many per episode, too many)
  - "continue":    20.1% (expected 57%, v24=57.1%) ← CRITICAL MISS
  - "hallway":     49.0% (good, v24=51.8%)
  - "turn left":   45.4% (close, v24=51.6%)
  - "walk into":   73.8% (too high, v24=52.6%)

ROOT CAUSE 1 — "passing the" miss:
  _DISTINCTIVE_OBJECTS_RE whitelist is TOO RESTRICTIVE.
  v24's top "passing the X" objects: sofa (21+), dining table (16+), armchair (12+),
  doors/doorways (12+), bookshelves (6+), kitchen island (8+), kitchen cabinets (8+).
  ALL of these are in _GENERIC_OBJECTS_RE or missing from _DISTINCTIVE_OBJECTS_RE.
  Fix: Add v24's actual "passing the" objects to the distinctive list.

ROOT CAUSE 2 — "continue" miss:
  _CONTINUE_MOTION_RE converts "Continue straight" → "Walk straight".
  v24 used "continue straight": 379 times, "continue forward": 225 times.
  Our postprocessing strips ALL "continue straight/forward/ahead" → kills "continue".
  Fix: Remove "straight" and "forward" from _CONTINUE_MOTION_RE; keep only "walking/moving".

v107 CHANGES FROM v106:
  1. Expand _DISTINCTIVE_OBJECTS_RE: add sofa/couch/sectional, armchair, dining table,
     kitchen island/cabinet, door/doorway/archway, bookshelf/bookshelves, sideboard.
     Expected: "passing the" rises from 28% → ~65-70% (v24=79.7%).
  2. Fix _CONTINUE_MOTION_RE: only suppress "Continue walking/moving" (NOT straight/forward/ahead).
     Expected: "continue" rises from 20% → ~50-57% (v24=57.1%).
  3. All other settings unchanged from v106.

  Expected stats (targeting v24 match):
    passing the:  28.2% → ~65%+  (v24=79.7%)
    continue:     20.1% → ~50-57% (v24=57.1%)
    turn left:    45.4% → ~45-50%  (v24=51.6%)
    walk into:    73.8% → ~50-60%  (v24=52.6%)
    hallway:      49.0% → ~50%+    (v24=51.8%)
    SR target: ~45%+ (should improve over v24's 40.24% by closer vocabulary match)

Phase 2 checkpoint: outputs/gate4_v107_phase2_checkpoint.json (NEW -- fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v107.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v107.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v107_phase2_checkpoint.json"  # v107 — fix passing-the + continue
...
_DISTINCTIVE_OBJECTS_RE = re.compile(
    ...
    r'kitchen\s+cabinet|sideboard|console|'          # cabinetry landmarks
    r'door(?:way)?|double\s+doors?|archway|arch|'    # architectural passages
    r'bookshelf|bookshelves|bookshelf|'               # shelving
...
def is_generic_pass_object(desc: str) -> bool:
    """v107: EXPANDED WHITELIST — includes v24's actual "passing the" objects.
    v31 whitelist was too strict: blocked sofas, armchairs, dining tables, doors, bookshelves.
    v24 analysis shows these ARE the primary "passing the" objects (sofas=21, tables=16,
    doors=12, armchairs=12, kitchen island=8, bookshelves=6). v107 expands whitelist to match.
    Expected: "passing the" rises from 28% (v106) → ~65-70% (v24=79.7%).
    """
    if not desc:
        return True
...
async def main():
    ...
    print('=== Gate 4 v107: Fix "passing the" (28%→70%) + restore "continue straight" ===')
