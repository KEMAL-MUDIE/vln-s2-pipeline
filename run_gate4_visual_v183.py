#!/usr/bin/env python3
"""
Gate 4 Visual v183 -- go_straight reduction via skip rate 50%→25% + P_MERGE 0.75→0.60.

CHANGES FROM v182:
  v182 final stats: go_straight=31.4%, walk_ahead=11.9%, walk_forward=12.4%, 2-sent=50.8%, go_start=22.8%
  GT reference:     go_straight=7.9%,  walk_ahead=6.1%,  walk_forward=6.1%,  2-sent=42%,   go_start=18.5%
  v24 (BEST SR):    go_straight=23% → SR=40.24%

FIX 1: Reduce go_straight skip rate: 50% → 25%
  Skip group means episodes keep go_straight unchanged (no landmark replacement).
  Reducing skip means 75% of episodes now run replace_go_straight_with_landmarks (was 50%).
  More episodes convert go_straight → walk_to_X (when landmark available) or keep as "go straight" (fallback).
  Estimated landmark coverage ≈60% → net: go_straight drops by ~21%
  Expected: go_straight 31.4% → ~24.7% (approaching v24's 23%)

FIX 2: Reduce P_MERGE: 0.75 → 0.60
  Forward sentence merger was creating too many 2-sentence instructions.
  v182 removed walk_ahead→walk_forward conversion, so interaction that caused v181's 2-sent increase is gone.
  Reducing P_MERGE cleanly reduces 2-sent without unexpected interactions.
  Expected: 2-sent 50.8% → ~44% (closer to GT=42%)

INHERITED from v182 (all):
  ...
  - enter P=0.75 for walk_to
  - stop-phrase merger P=0.45
...
Expected v183:
  avg_words ≈ 27.0w
  go_straight ≈ 24.7% (from 31.4% — approaching v24's 23%)
  walk_ahead ≈ 11.9% (same)
  walk_forward ≈ 13.5% (slight increase from more landmark-run episodes' no-landmark fallback)
  2-sent ≈ 44% (from 50.8% — closer to GT=42%)
  go_start ≈ 22.8% (same)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v183_phase2_checkpoint.json (REUSE v180 P2 results — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v183.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v183.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v183_phase2_checkpoint.json"  # v183: FRESH (use v180 P2 results)
...
async def main():
    ...
    print('=== Gate 4 v183: go_straight skip 50%→25% + P_MERGE 0.75→0.60 ===')
    print(f"  v182 stats: go_straight=31.4%, walk_ahead=11.9%, walk_forward=12.4%, 2-sent=50.8%")
    print(f"  GT reference: go_straight=7.9%, walk_ahead=6.1%, walk_forward=6.1%, 2-sent=42%")
    print(f"  v24 BEST (SR=40.24%): go_straight=23% kept naturally")
    print(f"  v183 CHANGES:")
    print(f"    FIX1: go_straight skip rate 50%→25% → go_straight 31.4%→~24.7% (toward v24's 23%)")
    print(f"    FIX2: P_MERGE 0.75→0.60 → 2-sent 50.8%→~44% (toward GT=42%)")
    print(f"  INHERITED FROM v182: REMOVED go_straight→walk_ahead, go_start P=0.70, enter P=0.75, stop-merge P=0.45")
    print(f"  Expected: go_straight≈24.7%, walk_ahead≈11.9%, walk_forward≈13.5%, 2-sent≈44%, avg_words≈27.0")
