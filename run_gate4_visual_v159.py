#!/usr/bin/env python3
"""
Gate 4 Visual v159 — Match v24 go_straight=23% via 50% skip of replace_go_straight_with_landmarks.

SR HYPOTHESIS REFINEMENT:
  v158: replace_gs DISABLED (100% skip) → go_straight=47.5% → SR=? (extreme)
  v159: 50% skip → go_straight≈23.75% ≈ v24's go_straight=23% → SR=? (target v24's 40.24%)
  v157: 15% skip → go_straight=6.8%(≈GT=7.9%) → SR=~34% (expected)
  v24 (BEST non-GT): go_straight=23% → SR=40.24%

  v159 targets EXACTLY the go_straight rate that gave v24 its best-ever SR (40.24%).
  If the SR hypothesis is correct (higher go_straight = better SR), v159 should match v24.

v158 ACTUAL: GT-match=0.981✓, avg_turns=0.54≈GT✓, walk_straight=8.9%≈GT✓, go_straight=47.5%

v159 CHANGES from v158:
  1. replace_go_straight_with_landmarks skip: 100%→50% → go_straight ≈ 23.75% ≈ v24=23%
     Calculation: 50% episodes skip → 0.50 × 47.5%_base_rate ≈ 23.75% go_straight
     This specifically matches v24's best-SR go_straight rate.
  2. Everything else identical to v158 (P_TURN_UNANCHORED=0.09, P_THRU=0.60, etc.)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C)
Phase 2 checkpoint: outputs/gate4_v159_phase2_checkpoint.json (REUSE v158 phase2, same P_TURN=0.09)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v159.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v159.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v159: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v158_phase2_checkpoint.json"  # v159: REUSE v158 (same P_TURN=0.09, P_THRU=0.60 — only skip rate differs)
...
async def main():
    ...
    print('=== Gate 4 v159: 50% skip replace_go_straight_with_landmarks → match v24 go_straight ===')
    print(f"  v158 METRICS: GT-match=0.981✓, avg_turns=0.54✓, walk_straight=8.9%✓, go_straight=47.5%")
    print(f"  v159 CHANGE: replace_gs skip 100%→50% → go_straight ≈ 23.75% ≈ v24=23%")
    print(f"    v24 (go_straight=23%) achieved SR=40.24% (BEST non-GT). v159 targets this exact rate.")
    print(f"    Hypothesis: if SR improves from ~34% → ~40%, go_straight is the key variable.")
    ...
    for ep in episodes:
        ...
        # v159: replace_go_straight_with_landmarks skipped 50% → go_straight ≈ 23.75% ≈ v24=23%.
    ...
    print(f"  go_straight:   {100*go_straight_n/n:.1f}%  (GT=7.9%, v158=47.5%) ← v159: 50% skip → target ~23.75% ≈ v24=23%")
    print(f"  walk_straight: {100*wlk_straight_n/n:.1f}%  (GT=8.8%, v158=8.9%) ← v159: unchanged")
