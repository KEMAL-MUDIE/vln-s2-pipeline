#!/usr/bin/env python3
"""
Gate 4 Visual v70 — v67 base + approach-direction turn landmarks (Phase 1 fix)

v68 RESULTS (best so far):
  GT-match: 0.991, avg_words: 26.7 (GT=26.8) — essentially perfect statistics
  SR (expected): ~40% — plateau caused by SEMANTIC quality, not statistics

ROOT CAUSE of SR=40% plateau:
  v24-v68 all use turn-side Phase 1: renders at ±90° from approach heading.
  GT annotators describe landmarks they see APPROACHING the turn (~30-45° ahead-left/right).
  This angular mismatch → wrong/non-visible landmarks in turn instructions.
  SR by path type: straight=54.5%, single_turn=38.9%, winding=36.0% — turns are the failure.

v70 FIX:
  Replace turn-side Phase 1 checkpoint (gate4_v24_turn_sides_p1_checkpoint.json)
  with approach-direction Phase 1 (gate4_v70_approach_views_p1_checkpoint.json).
  - Rendered from path[i-1] facing path[i]: landmark appears ahead-left or ahead-right
  - HFOV=79° (matching eval sensor, vs old renderer 90°)
  - Total: 4017 frames across 1839 episodes (avg ~2.2 turn frames/ep)
  Everything else identical to v67 (GT-match=0.991 statistics preserved).

Phase 2 checkpoint: outputs/gate4_v70_phase2_checkpoint.json (FRESH — P1 data changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v70.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v70.json.gz"
...
P1_TS_CKPT    = ROOT / "outputs" / "gate4_v70_approach_views_p1_checkpoint.json"  # v70: approach-direction (replacing turn-sides)
P2_CKPT       = ROOT / "outputs" / "gate4_v70_phase2_checkpoint.json"  # FRESH — P1 data changed
