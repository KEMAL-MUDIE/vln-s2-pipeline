#!/usr/bin/env python3
"""
Gate 4 Visual v194b -- ROOT CAUSE FIX with MODERATE FIX7 (P=0.50) for walk_past balance.

MOTIVATION:
  v194: walk_to=7.3%, walk_past=54.2% — ROOT CAUSE FIXED but walk_past very high (6× GT=8.7%)
  v194b: Reduce FIX7 from P=0.90 → P=0.50 to lower walk_past while keeping FIX8 (root cause fix)

KEY CHANGES FROM v194:
  FIX 7 P: 0.90 → 0.50 (less aggressive sentence-level walk_to→walk_past conversion)
  FIX 7b P: 0.80 → 0.60 (less aggressive room conversion)
  FIX 8: KEPT (walk past the {lm} replacement in replace_go_straight_with_landmarks)

Expected v194b:
  walk_to: ~11-13% (less FIX7 conversion, but still benefits from FIX8 root cause fix)
  walk_past: ~42-46% (lower than v194=54.2% — may be safer for CMA navigation)
  Predicted SR: 58-65% (between v193b and v194)

COMPARISON:
  v193b: walk_to=17.2%, walk_past=38.3%, predicted 55-60% SR
  v194:  walk_to=7.3%,  walk_past=54.2%, predicted 60-68% SR
  v194b: walk_to=~12%,  walk_past=~44%,  predicted 58-65% SR (MODERATE)

Phase 2 checkpoint: outputs/gate4_v194b_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v194b.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v194b.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v194b_phase2_checkpoint.json"  # v194b: assembly-only, copy of v190 P2 (moderate FIX7)
...
async def main():
    ...
    for ep in episodes:
        ...
        def _intermediate_walk_to_to_past(text_in):
            ...
            if len(sentences) <= 1:
                ...
                def _single_sent_conv(m):
                    if _v193_fix7_rng.random() < 0.50:  # v194b: P=0.50 (was 0.90 in v194)
                        cap = m.group(0)[0].isupper()
                        obj = m.group(1).strip()
                        return ('Walk past the ' if cap else 'walk past the ') + obj
                    return m.group(0)
        ...
        def _walk_to_room_to_enter(m):
            if _v193_fix7b_rng.random() < 0.60:  # v194b: P=0.60 (was 0.80 in v194)
                cap = m.group(0)[0].isupper()
