#!/usr/bin/env python3
"""
Gate 4 Visual v104 — v103 + VOCABULARY FIX: "forward"→"straight", remove "enter", restore "continue".

KEY FINDING (vocabulary analysis v24 vs v103):
  v24 (40.24% SR): straight=68.5%, turn left/right=51-53%, continue=57.1%, walk into=52.6%
  v103 (?? SR):    forward=90.5%, turn left/right=24-27%, continue=9.1%,   enter=42.4%

  v103 has massive "forward" overuse (90.5% vs GT=11%). v24 uses "straight" (68.5% vs GT=22.5%).
  The InternVLA-N1 model was trained on GT; "go straight" is 2x more GT-like than "walk forward".
  "enter the X" (v103=42.4%) diverges from v24=4.4% and GT=17% — replace with "walk into".

v104 CHANGES FROM v103:
  1. Replace default route segment "walk forward" → "go straight"
  2. SYSTEM_INTRO: prefer "walk straight" over "walk forward"
  3. Remove "enter the [room]" option; use only "walk into the [room]"
  4. clean(): replace all "walk forward" substitutions → "walk straight"
  5. Continue restoration: P=0.08 → P=0.30 (target continue%=~20%, v24=57.1%, GT=10.4%)
  6. Keep all v103 settings: P_THRU=0.0, P_PASS=0.0, P_HALLWAY_TURN=1.0

  Expected effects:
    straight%:   17.9% → ~50%+  (v24=68.5%, GT=22.5%)
    forward%:    90.5% → ~15%   (v24=23.5%, GT=11%)
    enter%:      42.4% → ~10%   (v24=4.4%, GT=17%)
    walk into%:  10.2% → ~40%+  (v24=52.6%, GT=6.9%)
    continue%:    9.1% → ~20%   (v24=57.1%, GT=10.4%)
    SR target: > 40% (vocabulary alignment with v24 style)

Phase 2 checkpoint: outputs/gate4_v104_phase2_checkpoint.json (NEW — fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v104.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v104.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v104_phase2_checkpoint.json"  # v104 — fresh P2 (vocabulary fix: straight not forward)
