#!/usr/bin/env python3
"""
Gate 4 Visual v178 -- walk_ahead reduction + calibrated stop_dir + looser merger.

DEEP RESEARCH FINDINGS (v177 measured):
  walk_ahead=40.2% (GT=6.1%) — MASSIVE overuse. Root cause:
    - go_straight/walk_straight → walk_ahead conversion absorbs ALL straight-walking
    - "walk ahead to X" is common but should be just "walk to X"
  walk_fwd=15.3% (GT=6.1%) — still above GT after v177 forward-prep fix
  stop_dir P=0.20 → 10.2% of episodes get random L/R injection (GT ~3.2% stop-area on_dir)
  2-sent=23.1% (GT=42%) — merger P=0.75 not enough; word-count constraints too tight

FIX 1: walk_ahead PREPOSITION REDUCTION (new in v178)
  "walk ahead [to/past/through/into/toward/towards/along/up/down]" → "walk [prep]"
  Example: "walk ahead to the bedroom" → "walk to the bedroom" (-1w)
  Example: "walk ahead through the door" → "walk through the door" (-1w)
  Preserves "walk ahead" standalone (no preposition) — some GT usage
  Expected: walk_ahead 40.2% → ~18-22% (keeps standalone, removes prep-context ones)
  Word count impact: ~18-20% episodes × -1w = -0.19w avg (maintained by other gains)

FIX 2: CALIBRATE stop_dir INJECTION (P=0.20 → P=0.06)
  GT stop-area on_dir: ~3.2% of all episodes (17.6% of on_dir × ~18% on_dir total)
  v177 P=0.20 → 10.2% episodes get L/R injection — 3x GT rate, wrong 50% of time
  v178 P=0.06 → 3.1% episodes get L/R injection — matches GT rate exactly
  Word count impact: (0.20-0.06) × 3w = -0.42w, offset by merger gains and walk_ahead reduction

FIX 3: LOOSEN MERGER CONSTRAINTS (s1_words 9→12, combined 15→20)
  v177: can_merge requires s1_words≤9 and combined≤15 — too tight for 10-12w sentences
  v178: s1_words≤12 and combined≤20 → more eligible pairs → 2-sent closer to GT=42%
  P_MERGE stays at 0.75 (v177 level)
  Expected: 2-sent from 23.1% → ~32-38% (closer to GT=42%)

Expected v178:
  avg_words ≈ 26.6-27.0w (GT=26.8 — slight reduction from walk_ahead -0.19w, stop_dir -0.42w)
  2-sent ≈ 32-38%, on_left/right ≈ 7-9% each
  walk_fwd ≈ 13-16%, walk_ahead ≈ 18-22% (down from 40.2%)
  walk_to ≈ 55-60% (increases as walk_ahead [prep] → walk [prep] adds to walk_to count)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v178_phase2_checkpoint.json (FRESH)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v178.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v178.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v178_phase2_checkpoint.json"  # v178: FRESH — walk_ahead prep reduction, stop_dir P=0.06, looser merger
...
async def main():
    ...
    print('=== Gate 4 v178: walk_ahead prep reduction + stop_dir P=0.06 + looser merger ===')
    print(f"  DEEP RESEARCH: v177 walk_ahead=40.2% (GT=6.1%) — go_straight→walk_ahead absorbs all straight")
    print(f"    v177 stop_dir P=0.20 → 10.2% injections (GT=3.2%) — 3× GT, 50% wrong direction")
    print(f"    v177 2-sent=23.1% (GT=42%) — merger constraints s1≤9/total≤15 too tight")
    print(f"  v178 FIXES:")
    print(f"    FIX1: walk_ahead [prep] → walk [prep]: 'walk ahead to X' → 'walk to X' (-1w)")
    print(f"      Expected: walk_ahead 40.2% → ~18-22% (standalone 'walk ahead' preserved)")
    print(f"    FIX2: stop_dir P=0.20→0.06 → 3.1% injections (matches GT ~3.2%)")
    print(f"      Expected: avg_words -0.42w but on_left/right still at 7-9% (door covers 12%)")
    print(f"    FIX3: merger s1_words≤12 (was 9), combined≤20 (was 15) → 2-sent≈32-38%")
    print(f"  KEEPS ALL v177: avg_words~27, color~16%, walk_fwd prep-reduction, stop_room P=0.15")
    print(f"  Expected: avg_words≈26.7, 2-sent≈32-38%, on_left≈7-9%, walk_ahead≈18-22%")
