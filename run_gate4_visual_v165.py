#!/usr/bin/env python3
"""
Gate 4 Visual v165 — Two architectural fixes for SR gap vs v24.

ROOT CAUSE ANALYSIS (v164 → v165):
  1. FURNITURE IN [pass:] MARKERS: v130 ep10 FAILS with "past the brown armchair and beige sofa"
     v24 ep10 SUCCEEDS with "passing the arched wooden doorway" (architectural, not furniture)
     Agent trained on GT-style R2R knows room navigation, NOT furniture-as-waypoints
     → Fix: Add common furniture to [pass:] blacklist (is_generic_pass_object)

  2. ANCHOR% CALIBRATION (v164 failure: anchor%=6.1% vs GT=16.5%):
     GT's 16.5% anchor% comes MAINLY from "turn left into the bedroom/hallway" (room transitions)
     NOT from furniture anchors. TURN_ANCHOR_RE matches: "turn left INTO the bedroom" ✓
     But our route uses "(→ bedroom)" without direction → LLM writes "walk into the bedroom"
     "walk into the bedroom" does NOT match TURN_ANCHOR_RE (no "turn left/right" prefix)!
     → Fix: Add turn direction to non-hallway room transitions: "(→ bedroom)" → "turn left (→ bedroom)"
     This generates "turn left into the bedroom" which MATCHES TURN_ANCHOR_RE → anchor%→16-18%

KEY METRICS:
  v164 anchor%=6.1% (too low; target 16.5%) — root cause: no direction in (→ room) markers
  v24 (SR=40.24%): walk_into=52.6%, avg_turns=1.53, unanchored=60.4%
  GT  (SR=63.30%): walk_into=12.2%, avg_turns=0.59, anchor=16.5%

v165 DESIGN:
  1. Furniture filter in [pass:] — _STRUCTURAL_OBJECTS_RE extended with common furniture
     sofa, armchair, chair, table, bed, cabinet, shelf, lamp, curtain, plant → filtered out
     Only distinctive objects remain: piano, fireplace, pool table, mosaic, chandelier, etc.
  2. Turn direction in non-hallway room transitions — "turn left (→ bedroom)" instead of "(→ bedroom)"
     LLM writes "turn left into the bedroom" → TURN_ANCHOR_RE matches → anchor%→~16-18%
  3. P_TURN_UNANCHORED: 0.28 → 0.55 → compensate word loss from fewer [pass:] markers
     More explicit turn directions → avg_turns↑ → closer to v24's style
  4. P_ANCHOR_EPISODE: 0.03 (keep from v164 — room transitions now provide bulk of anchor%)
  5. FRESH Phase 2 checkpoint (route descriptions change: direction in room transitions)

Expected: anchor%≈16-18% (GT=16.5%), avg_turns≈1.0-1.3, walk_past_the≈2-3%, avg_words≈25-27w

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v165_phase2_checkpoint.json (FRESH — route descriptions change)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v165.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v165.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v165: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v165_phase2_checkpoint.json"  # v165: FRESH — route changes (direction in room transitions)
...
P_ANCHOR_EPISODE = 0.03   # v164: drastically reduced 0.18→0.03 to match GT anchored_turn=3.2% (Phase C furniture anchors OOD)
...
P_TURN_UNANCHORED = 0.55  # v165: 0.28→0.55 → compensate word loss from furniture filter in [pass:]; more turn guidance
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    ...
                    # v165: Include turn direction in non-hallway room transitions.
                    # Old: "(→ bedroom)" → LLM writes "walk into the bedroom" → NO TURN_ANCHOR_RE match
                    # New: "turn left (→ bedroom)" → LLM writes "turn left into the bedroom" → MATCHES TURN_ANCHOR_RE
                    # Root cause of v164 anchor%=6.1%: room transitions had no direction → LLM used "walk into"
                    # Fix: adding direction → "turn left into the bedroom" → anchor%→~16-18% (GT=16.5%)
                    if direction:
                        if rng.random() < 0.21:
...
- When you see turn left/right (door→ room): write "turn left/right through the doorway into the [room]" — combine the turn with the doorway transition. ALWAYS mention the doorway.
- When you see (door→ room) alone (no turn prefix): write "walk through the doorway into the [room]" or "go through the door into the [room]".
...
- When you see turn left/right (→ room): write "turn left/right into the [room]" — combine the turn direction with entering the room. This is the most natural form. Do NOT write "enter the [room]". Keep it brief (4-6 words).
- When you see (→ room) alone (no turn prefix): write "walk into the [room]" — describe entering that room. Keep it brief (3-5 words).
...
- MOTION WORDS: Use natural walking verbs: "walk", "go". Chain movements naturally: "turn left into the kitchen". For landmarks: "walk to the X". AVOID "proceed", "go forward", "walk forward", "keep going", "head toward", "walk toward". Use "continue" ONLY when no new visual landmark is present — prefer "walk" or "go" as primary verbs.
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    n_turn_door_trans = len(re.findall(r'turn\s+(?:left|right)\s+\(door→', route))
...
async def main():
    ...
    print('=== Gate 4 v165: Furniture filter in [pass:] + turn direction in room transitions (anchor% fix) ===')
    print(f"  FIX 1: Furniture filtered from [pass:] — 'walk past the brown armchair' confuses agent")
    print(f"  FIX 2: Turn direction in non-hallway room transitions — 'turn left (→ bedroom)'")
    print(f"         → LLM writes 'turn left into the bedroom' → TURN_ANCHOR_RE matches → anchor%→~16-18%")
    print(f"  v164 PROBLEM: anchor%=6.1% (too low; GT=16.5%) — root cause: no direction in (→ room)")
    print(f"  v165: P_ANCHOR=0.03 + P_TURN_UNANCHORED=0.55 + 50%skip + P_continue=0.28 + FRESH")
    print(f"    Expected: anchor%≈16-18%(GT=16.5%), avg_turns≈1.0-1.3, walk_past_the≈2-3%")
    print(f"  GT: walk_straight=8.8%, go_straight=7.9%, avg_turns=0.59, turn-anchor=16.5%")
    print(f"  Expected: walk_straight~9%, go_straight~7%, avg_words≈25-27w, GT-match≥0.95")
