#!/usr/bin/env python3
"""
Gate 4 Visual v91 — v90 + remove _THROUGH_HALLWAY_RE from clean() (fix through_the + hallway)

v90 REMAINING GAPS:
  through_the: 21.9% (GT=27.2%) — stuck due to _THROUGH_HALLWAY_RE converting "through the
               hallway/corridor" → "down the hallway" in clean()
  hallway:     14.8% (GT=20.4%) — these converted phrases also lost from hallway% after
               dir filter; keeping them as "through the hallway" fixes BOTH metrics

v91 ROOT CAUSE ANALYSIS:
  _THROUGH_HALLWAY_RE in clean() converts "through the hallway/corridor" → "down the hallway".
  These are then filtered by hallway dir filter (P_suppress=0.31 → only 69% pass as hallway).
  Net: both through_the and hallway lose 4-7pp from these conversions.

  Calibration estimate (v34 analysis: 4.1% of instructions had "through the hallway" raw):
  - Keeping them as "through the hallway": through_the += 4.1%×(1-0.219) ≈ 3.2pp → ~25.1%
  - With P_THRU_MARKER=0.95, rate might be higher (~7%): → through_the ≈ 27.4%
  - hallway += same amount (also counted in hallway) → ~18-22%

v91 FIX — single assembly change (LLM prompt IDENTICAL to v84/v86/v88/v90):
  Remove `_THROUGH_HALLWAY_RE.sub(...)` from clean() entirely.
  "through the hallway/corridor" stays as-is → counts in both through_the AND hallway.
  _THROUGH_ROOM_RE (v86-relaxed) does NOT include hallway — safe.
  Hallway dir filter does NOT match "through the hallway" (only down/along) — safe.
  Seed: 0x9145_5300. Fresh P2 checkpoint.

Phase 2 checkpoint: outputs/gate4_v91_phase2_checkpoint.json (FRESH — seed changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v91.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v91.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v91_phase2_checkpoint.json"  # FRESH — seed changed
...
async def main():
    ...
    print(f"  [v91 targets: through_the ~25-27%, toward_the ~4%, hallway ~18-22%, continue ~10%]")
