#!/usr/bin/env python3
"""
Gate 4 Visual v97 — Fix v96 word count: ensure sufficient instruction length while keeping room-entry improvements.

ROOT CAUSE OF SR REGRESSION (v62-v94 vs v24):
  v24 (GT-match=0.934): SR=40.24% — best achieved with generated instructions
  v62-v94 (GT-match=0.953-0.992): SR=33-35% — WORSE despite better GT-match score

  Root cause analysis (345 episodes where v24 wins, v62 fails):
    has_turn:     v24=90.7%  vs v62=52.2%  → v24 has MORE explicit turn instructions
    walk_forward: v24=4.3%   vs v62=92.5%  → v62 replaced everything with "walk forward"
    room_name:    v24=77.4%  vs v62=51.0%  → v24 mentions rooms more naturally
    hallway:      v24=58.8%  vs v62=19.1%  → v24 uses hallway naturally at transitions
    through_the:  v24=0.9%   vs v62=24.6%  → INJECTED through_the HURTS SR
    walk_past:    v24=0.3%   vs v62=10.7%  → INJECTED walk_past HURTS SR

  Three compounding suppressions in v47-v94 caused SR regression:
    1. build_route_desc line 1175: "turn left" (hidden rm) — LLM can't mention room
    2. Prompt line 1207: "ONLY if final destination" — bans room-entry for mid-route turns
    3. Prompt line 1213: ROOM NAMES BAN — bans "walk into the hallway" etc explicitly

  Assembly injections added to match GT phrase frequencies also hurt SR:
    v92: through_the injection (+5pp) → 27.1% (GT=27.2%) but v24_wins have 0.9% → HURTS
    v94: hallway injection (+5.5pp) → 20.3% (GT=20.4%) but was already fine naturally
    Result: GT-match=0.992 but SR ≈ 33-35% (worse than v24 with GT-match=0.934, SR=40.24%)

v95 PRIMARY FIX:
  1. build_route_desc: Include room name in route — "turn left (→ hallway)" instead of "turn left"
     Also remove double-sampling for rm turns (they always need context)
  2. Prompt line 1207: "naturally mention the room — 'walk into the hallway'" (restore v24 rule)
  3. Prompt line 1213: Replace ban with permission — allow room-entry at transitions
  4. anchor_note: Update for room transitions — tell LLM to use room-entry language
  5. Remove through_the assembly injection (v92) — let LLM generate naturally via [thru:] markers
  6. Remove hallway assembly injection (v94) — let LLM generate naturally via (→ room) route

  Expected: turn rate ≈ 75-86% (closer to v24=86%), room mentions ≈ 65-75% (v24=68%)
  through_the stays at ~22% from [thru:] markers (lower than GT 27.2% but no injection)
  hallway stays at ~17-20% from natural LLM generation
  SR target: ≥ 40% (v24 level), potentially 45-55% with better stop phrases + route notation

Phase 2 checkpoint: outputs/gate4_v95_phase2_checkpoint.json (NEW — fresh LLM calls needed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v95.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v97.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v97_phase2_checkpoint.json"  # NEW — fresh P2 for room-entry + distance fixes
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v95: Restore room name in route notation — "turn left (→ hallway)".
                # v49-v94 hid room name ("turn left") + banned room-entry in prompt →
                #   LLM wrote "walk forward" for every room transition → SR crashed vs v24.
                # v24 root cause analysis: 90.7% turn rate, 77.4% room_name for winning eps.
                # Restore: include room name so LLM can write "walk into the hallway".
                # Remove double-sampling (line 1172) — rm turns always need room context.
...
- When a turn has (→ room) after the arrow: write the turn FIRST ("turn left"), then a SEPARATE brief phrase for the room entry ("walk into the hallway", "enter the bedroom"). Format: "turn left. Walk into the hallway." or "turn left, then enter the kitchen." Do NOT combine into one phrase like "turn left into the hallway" — keep turn and room entry as separate clauses.
...
- ROOM TRANSITIONS: When a turn is marked (→ room), mention the room as a SEPARATE clause after the turn: "turn left. Walk into the hallway." or "turn right, then enter the bedroom." Keep it brief (3-5 words for the room phrase). Mention each room name AT MOST ONCE across the entire instruction — do not repeat hallway, bedroom, etc. If multiple turns enter the same room, only describe it the first time.
- DISTANCE: When the route shows a 'straight Xm' segment — NEVER write metric numbers and ALWAYS describe the movement (do NOT skip it). Use varied natural language: short (≤3m)="a few steps" or "take a couple steps" or "a short distance"; medium (4–7m)="continue forward" or "keep going" or "a bit further"; long (≥8m)="walk for a while" or "keep going for a while" or "continue along the way". Use "walk forward" sparingly — prefer "continue", "keep going", "head forward". R2R annotators describe movement between turns.
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    if n_bracketed > 0 and n_room_trans > 0:
        # v96: both landmarks AND room transitions — use landmarks for turn anchors, room entry as separate clause
        anchor_note = (f"Route has {n_bracketed} bracketed landmark(s) and {n_room_trans} "
                       f"room transition(s) marked (→ room) — use landmarks naturally for turn directions; "
                       f"add brief separate room-entry phrase after turns: 'walk into the [room]'. "
                       f"Use each room name AT MOST ONCE.")
    elif n_bracketed > 0:
        anchor_note = f"Route has {n_bracketed} bracketed landmark(s) — use them naturally."
    elif n_room_trans > 0:
        # v96: room-entry as separate clause from turn (avoids "turn left into the hallway" combined form)
        anchor_note = (f"Route has {n_room_trans} room transition(s) marked (→ room) — "
                       f"after each turn, add a brief separate room-entry phrase: 'walk into the [room]' "
                       f"or 'enter the [room]'. Use each room name AT MOST ONCE in the instruction.")
    else:
...
async def main():
    ...
    print(f"  [v97 targets: room-entry separate clause, word count fixed (~26-27w), SR > v95/v96]")
