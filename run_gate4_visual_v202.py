#!/usr/bin/env python3
"""
Gate 4 Visual v202 -- FIX16b+FIX18+FIX19: walk_to reduction, sentence compression, continue reduction.

BASE: v201 (FIX1-17b — walk_past=33.7%, toward=14.2%, continue=17.7%, walk_to=10.0%)

MOTIVATION (from deep research agent findings):
1. walk_to=10.0% (GT=5.1%) is the #1 remaining gap — FIX8's 25% walk_to injection is the root cause
2. 3-sentence=51.9% (GT=29.3%) structural mismatch — FIX18 sentence merging addresses this
3. continue=17.7% (GT=8.2%) — FIX19 converts some Continue straight→Go straight (40%)
Research estimates: FIX16b +1-2pp SR, FIX18 +1-1.5pp SR, FIX19 +0.5-1pp SR
Combined predicted: +2.5-4.5pp SR uplift from v201

NEW IN v202:
FIX16b: FIX8 Walk_to Injection Reduction.
  PROBLEM: FIX16 (v199) used 51/24/25 FIX8 distribution → 25% walk_to injection
           causes walk_to to increase from 7.3% (v197/v198) to 10.0% (v201).
           GT walk_to=5.1%. walk_to excess directly causes intermediate landmark stops (CMA bug).
  FIX: Reduce FIX8 walk_to injection: 51/24/25 → 60/30/10
       P=0.60: "walk past the {lm}" (was 51%, +9%)
       P=0.30: "walk toward the {lm}" (was 24%, +6%)
       P=0.10: "walk to the {lm}" (was 25%, -15%) — FIX7 converts 90% of these
  EXPECTED: walk_to ~8% (GT=5.1%, -2pp from v201=10.0%)
            walk_past ~34.5% (barely increases, +0.7pp from redistribution)
  Seed: random.Random(eid ^ 0xF1F2_F3F4)

FIX18: 3-Sentence → 2-Sentence Compression.
  PROBLEM: v201 3-sentence=51.9% (GT=29.3%), 1-sentence=2.7% (GT=20.0%).
           LLM generates 3 sentences even for simple paths, creating unnecessary navigation steps.
           CMA follows step-by-step; excess sentences = potential confusion at sentence boundaries.
  FIX: Merge S1+S2 in 3-sentence instructions at P=0.40 using "and" connector:
       "[NavSentence1]. [NavSentence2]. [StopSentence]."
       → "[NavSentence1] and [NavSentence2lowercase]. [StopSentence]."
       Guard conditions:
       - Both S1, S2 start with navigation verb (Walk/Turn/Go/Enter/Continue/Pass/Head/Exit)
       - S1 does NOT already contain " and "
       - S2 does NOT start with Stop/Wait/You/This/Once/When/If/Now/Then/Finally
       - S3 starts with Stop/Wait (confirmed stop sentence)
       - Merged length ≤ 45 words
  EXPECTED: 3-sentence 51.9%→~41%, 2-sentence 35.1%→~51%

FIX19: Continue Excess Reduction.
  PROBLEM: continue=17.7% (GT=8.2%), 9.5pp excess from LLM's cont_rng restoration.
  FIX: Convert 35% of "Continue straight" → "Go straight"
       Convert 25% of "Continue walking" → "Walk"
  EXPECTED: continue ~15-16% (GT=8.2%, partial improvement)
  Seed: random.Random(eid ^ 0xC1D2_E3F4)

INHERITED FROM v201: FIX1-17b (FIX7 5-phrase, FIX8 diversification, surface quality)
ASSEMBLY-ONLY: Reuse outputs/gate4_v202_phase2_checkpoint.json (copy of v190 P2).

Expected v202:
  walk_to: ~8% (GT=5.1%) — −2pp from v201 [FIX16b]
  walk_past: ~34.5% (GT=8.7%) — +0.7pp from FIX16b redistribution
  3-sentence: ~41% (GT=29%) — −11pp [FIX18]
  continue: ~15-16% (GT=8.2%) — −2pp [FIX19]
  Predicted SR: 63-70%+ (research estimate: +2.5-4.5pp from v201 baseline of 58-65%)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v202_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v202.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v202.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v202_phase2_checkpoint.json"  # v202: assembly-only, copy of v190 P2 (FIX16b+18+19)
...
def replace_go_straight_with_landmarks(text: str, phase_c_descs: Dict[str, str], rng=None) -> str:
    ...
    def _replace_gs(m):
        ...
        if lm_idx[0] < len(landmarks):
            ...
            if rng is not None:
                r = rng.random()
                # v202 FIX16b: Reduce walk_to injection 25%→10%, redistribute to walk_past/walk_toward
                # Research finding: walk_to excess (10.0% vs GT=5.1%) from 25% injection is #1 gap
                # New distribution: 60% walk_past / 30% walk_toward / 10% walk_to
                # walk_to residual: 10%×10% uncaptured = ~1% per FIX8 event (vs 25%×10%=2.5% before)
                if r < 0.60:
...
async def main():
    ...
    for ep in episodes:
        ...
        # v202 FIX18: 3-Sentence → 2-Sentence Compression for medium-length navigation sentences.
        # PROBLEM: v201 3-sentence=51.9% (GT=29.3%). Existing merger (P=0.60) uses tight word limits
        #          (s1≤12, total≤20) so many medium nav sentences aren't merged. FIX18 adds a second
        #          pass targeting 3-sentence instructions with relaxed length constraints.
        # PATTERN: [NavSent1]. [NavSent2]. [StopSent].
        #          → [NavSent1] and [NavSent2lowercase]. [StopSent].
        # Guard conditions (safe merge criteria from deep research):
        #   - Both S1, S2 start with navigation verb (Walk/Turn/Go/Enter/Continue/Pass/Head/Exit/Cross/Move)
        #   - S3 starts with Stop/Wait (confirmed stop sentence)
        #   - S1 does NOT already contain " and "
        #   - S2 does NOT start with Stop/Wait/You/This/Once/When/If
        #   - Merged S1+S2 length ≤ 45 words
        # Expected: 3-sentence 51.9%→~41% (40% of eligible 3-sentence instructions merged)
        ...
        if _fix18_rng.random() < 0.42:
            ...
            if len(_sents18) == 3:
                ...
                can_fix18 = (
                    ...
                    len((_s1.rstrip('.!?') + ' and ' + _s2).split()) <= 45
                )
                if can_fix18:
                    _s1_base = _s1.rstrip('.!?').rstrip()
                    _s2_lc = _s2[0].lower() + _s2[1:]
                    text = _s1_base + ' and ' + _s2_lc + ' ' + _s3

        # v202 FIX19: Continue Excess Reduction.
        # PROBLEM: continue=17.7% (GT=8.2%), 9.5pp excess. Excess from LLM cont_rng restoration.
        # FIX: Convert 35% of "Continue straight" → "Go straight"
        #      Convert 25% of "Continue walking" → "Walk"
        # These are the most common "continue" filler patterns that CMA doesn't need.
        _fix19_rng = random.Random(eid ^ 0xB2C3_D4E5)
        def _fix19_continue_straight(m):
            if _fix19_rng.random() < 0.35:
                cap = m.group(0)[0].isupper()
