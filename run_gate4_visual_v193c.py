#!/usr/bin/env python3
"""
Gate 4 Visual v193 -- AGGRESSIVE walk_to reduction via sentence-level post-processing.

CHANGES FROM v190:
  v190 actual stats: walk_to=44.8%(GT=5.1%), walk_past=11.3%(GT=8.7%), continue=17.7%(GT=10.4%),
    exit=13.6%(GT=18.0%), for_a_bit=0%(GT=0%)

  ROOT CAUSE of walk_to=44.8% (structural LLM artifact):
    LLM generates "walk to the X" for EVERY intermediate waypoint.
    v191 experiment: SYSTEM_INTRO prohibiting "walk to the X" reduced it to 33% but increased
    continue (28.1%) and walk_forward (23.7%) — unacceptable tradeoffs.
    v192 experiment: Expanding [pass:] eligibility made walk_to WORSE (63%) — LLM used the
    new [pass:] markers as "walk to the X" destinations.
    CONCLUSION: LLM prompt changes don't reliably reduce walk_to. Use REGEX post-processing.

FIX 7 (NEW): Sentence-level walk_to → walk_past conversion.
  Detect "walk to the [object]" that appears in NON-FINAL sentences.
  Non-final position → intermediate waypoint → convert to "walk past the [object]".
  Rule: If "walk to the X" appears in instruction AND is NOT the last navigation phrase
  before stop/wait/end-of-instruction → convert to "walk past the X" at P=0.70.
  Expected: walk_to: 44.8% → ~15-20%. walk_past: 11.3% → ~35-40%.
  NOTE: walk_past will overshoot GT=8.7%, but v24 (79.7% passing) got 40.24% SR → semantics matter!
  If walk_to=20% vs walk_to=45%, each walk_to occurrence may cause model to stop prematurely → SR loss.

FIX 7b (NEW): "walk to the [room]" → "enter the [room]" at P=0.60
  LLM sometimes generates "walk to the kitchen/bedroom" for room transitions.
  GT: "enter the kitchen" not "walk to the kitchen".
  Expected: exit improvement + walk_to -1-2pp.

INHERITED FROM v190: FIX 1-6 (all v190 post-processing fixes).
ASSEMBLY-ONLY: Reuse outputs/gate4_v193_phase2_checkpoint.json (copy of v180 P2).

Expected v193:
  walk_to: ~15-20% (massive reduction from 44.8%)
  walk_past: ~35-40% (overshoots GT=8.7% but v24-like semantic benefit expected)
  continue: ~17.7% (unchanged from v190)
  exit: ~14-15% (unchanged from v190 + FIX 7b room boost)
  for_a_bit: 0% (unchanged)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v193_phase2_checkpoint.json (COPY of v180 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v193.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v193c.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v193c_phase2_checkpoint.json"  # v193b: assembly-only, copy of v180 P2 (aggressive FIX7)
...
async def main():
    ...
    for ep in episodes:
        ...
        def _intermediate_walk_to_to_past(text_in):
            ...
            if len(sentences) <= 1:
                ...
                def _single_sent_conv(m):
                    if True:  # P=1.0: always convert non-final walk_to
                        cap = m.group(0)[0].isupper()
                        obj = m.group(1).strip()
                        return ('Walk past the ' if cap else 'walk past the ') + obj
                    return m.group(0)
        ...
        def _walk_to_room_to_enter(m):
            if True:  # P=1.0: always convert walk_to_room
                cap = m.group(0)[0].isupper()
