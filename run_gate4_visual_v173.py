...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v173.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v173_phase2_checkpoint.json"  # v173: FRESH — sofa→couch + towards allowed + vocab fixes
...
async def main():
    ...
    print('=== Gate 4 v173: STOP COLOR STRIP + sofa→couch + enter POSTPROC + towards ===')
    print(f"  v172 ANALYSIS: color_stop%=53.8% (was 66.5%) — improvement but still high")
    print(f"    ROOT CAUSE: Phase 1b2 goal-approach overrides stop_phrase with colored descriptions!")
    print(f"    goal_vis strip helped goal_full but Phase 1b2 'Stop near grey lounge chair' still shown in prompt")
    print(f"  FIX 1 (HIGHEST IMPACT): strip colors from stop_phrase AFTER apply_goal_approach_stop")
    print(f"         'Stop near the grey lounge chair' → 'Stop near the lounge chair'")
    print(f"         Expected: color_stop% drops from 53.8% → target <20%")
    print(f"  FIX 2: enter verb POST-PROCESSING substitution")
    print(f"         route-level notation failed (LLM writes 'walk into' regardless)")
    print(f"         'walk into the bedroom/kitchen/...' → 'enter the [room]' (30% of cases)")
    print(f"         Expected: enter% ≈ 5-8% (GT=17%, was 0.6% in v172)")
    print(f"  FIX 3: sofa→couch post-processing (GT=7.2% couch vs sofa=1.5%)")
    print(f"  FIX 4: 'towards' allowed in MOTION WORDS (GT=14%, was blocked by AVOID rule)")
    print(f"  INHERITS from v172: goal_vis strip, enter route notation, 1-3 sent rule, ANTI-COLOR")
    print(f"  v173: Expected color_stop%<20%, enter%≈5-8%, couch≈GT, towards≈GT=14%")
