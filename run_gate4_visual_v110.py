#!/usr/bin/env python3
"""
Gate 4 Visual v110 — Suppress hallway room labels in route + GT-style vocabulary continued.

v109 ACTUAL RESULTS:
  - passing_the: 24.3% (down from 58.8% but still 24x GT's 1.1%) ← LLM ignores "walk past" rule
  - walk_past:   3.2% (expected ~12% but LLM reverts to "passing the")
  - continue:    15.0% (down from 45.2% — good improvement, near GT's 10.4%)
  - hallway:     39.9% (STILL high vs GT's 20.5%) ← route still emits "(→ hallway)"
  - into_hallway: 30.2% (STILL high vs GT's 4.4%) ← persistent v106 revert issue
  - word_count:  25.8w (GT=26.8w, good)

ROOT CAUSE OF HALLWAY OVERUSE:
  v103-v105 suppressed ALL hallway room labels → hallway=23.4%, into_hall=4.2% (near GT).
  v106 RESTORED hallway labels (first per episode) because v24 had hallway=51.8%.
  But GT only has hallway=20.5% and into_hall=4.4% — closer to v103-v105 behavior.
  Fix: revert to v103-v105 approach: suppress ALL (→ hallway) route labels.

v110 CHANGES FROM v109:
  1. HALLWAY ROUTE SUPPRESSION (key fix):
     - Change route builder: NEVER emit "(→ hallway)" label in route descriptor
     - Just "turn {direction}" for hallway transitions (no room label)
     - Expected: into_hallway drops from 30.2% → ~4% (matching GT=4.4%)
     - Expected: hallway drops from 39.9% → ~20% (matching GT=20.5%)

  2. RETAIN all v109 changes:
     - P_PASS_MARKER=0.12, P_TURN_UNANCHORED=0.28, CONTINUE_MOTION_RE active
     - "walk past the X" instruction (LLM may still generate "passing the" but less)

  Expected vocabulary targets:
    passing_the: 24.3% → ~20%  (still above GT but lower)
    walk_past:   3.2% → ~5%    (some improvement)
    continue:    15.0% → ~12%  (near GT=10.4%)
    turn_left:   ~24%  (maintained)
    hallway:     39.9% → ~22%  (matching GT=20.5%)
    into_hallway: 30.2% → ~5%  (matching GT=4.4%)
    SR target: 45-60%+ (better hallway alignment, continued GT-vocab)

Phase 2 checkpoint: outputs/gate4_v110_phase2_checkpoint.json (NEW -- fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v110.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v110.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v110_phase2_checkpoint.json"  # v110 — hallway route suppression
...
- ROOM TRANSITIONS: When you see (→ room) in the route, briefly describe entering: "walk into the bedroom", "walk into the kitchen", "walk into the bathroom". Do NOT use "enter the" — always use "walk into the". Do NOT write "turn left" or "turn right" for room entries. Use the room name once, briefly (3-5 words). The route will NEVER show "→ hallway" — if you see no (→ room) marker, write "walk straight" NOT any room name.
- HALLWAY: NEVER say "hallway" or "corridor" in your instructions — the route will not show hallway transitions. Write "walk straight" or just give turn directions instead of mentioning hallway rooms.
