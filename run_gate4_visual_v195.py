#!/usr/bin/env python3
"""
Gate 4 Visual v195 -- FIX9: Reduce 'continue walking' (major continue contributor).

INHERITED FROM v194: FIX 1-8 (all v194 fixes including ROOT CAUSE FIX8).

FIX 9 (NEW): "continue walking" → "walk" conversion.
  Raw P2 has 331 "continue walking" occurrences (biggest continue source).
  Assembly reduces to 17.7% (from raw P2's 33.8%), but "continue walking" remains.
  Fix: "continue walking" → "walk" at P=0.70 (keep natural flow, just remove redundant verb)
  Also: "continue moving" → "walk" at P=0.70 (similar pattern)
  Expected: continue: 17.7% → ~12-13% (GT=10.4%)

FIX 9b (NEW): "continue [direction/preposition]" normalization.
  "continue across" → "walk across" at P=0.70
  "continue up" → "walk up" at P=0.70
  "continue down" → "walk down" at P=0.70
  "continue through" → "walk through" at P=0.70 (already handled somewhat by assembly)
  "continue along" → "walk along" at P=0.70
  "continue towards/toward" → "walk towards/toward" at P=0.70
  Expected: additional -2pp continue reduction

Expected v195:
  walk_to: ~7.3% (unchanged from v194=7.3%)
  walk_past: ~54.2% (unchanged from v194)
  continue: ~12-13% (from v194=17.7%, GT=10.4%)
  go_straight: ~12-13% (slightly up from conversions)
  Predicted SR: 61-69% (additional continue reduction on top of v194)

Phase 2 checkpoint: outputs/gate4_v195_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v195.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v195.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v195_phase2_checkpoint.json"  # v195: assembly-only, copy of v190 P2
