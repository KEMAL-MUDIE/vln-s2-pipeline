#!/usr/bin/env python3
"""
Gate 4 Visual v88 — v86 + P_TOWARD 0.20→0.25 (toward_the calibration only)

v87 FAILURE ANALYSIS:
  v87 added ROOM NAMES relaxation ("walk down/along the hallway") → hallway 14.4%→27.4% (GT=20.4%).
  v87 added STYLE "Walk X and continue Y" multi-clause examples → continue 5.4%→18.4% (GT=10.4%).
  v87 hallway language competed with [thru:] markers → through_the 22.2%→17.2%.
  ONLY v87 success: P_TOWARD=0.25 → toward_the=3.9% (GT=3.8%) ← PERFECT.
  Lesson: Assembly post-processing only; never change LLM prompt for frequency tuning.

v88 FIXES — minimal delta from v86 (STABLE base):
  1. P_TOWARD: 0.20 → 0.25 (from v87 — toward_the hit exactly GT 3.8%)
  2. All LLM prompt content: IDENTICAL to v86/v84 (no ROOM NAMES changes, no STYLE changes)
  3. P_THRU_MARKER: stays at 0.90 (v86 level)
  4. Hallway assembly suppression: stays at P=0.48 (v86 level)
  Seed: 0x8845_5300. Fresh P2 checkpoint.

Phase 2 checkpoint: outputs/gate4_v88_phase2_checkpoint.json (FRESH — seed changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v88.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v88.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v88_phase2_checkpoint.json"  # FRESH — seed changed
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8845_5300)  # v84: new seed
...
async def main():
    ...
    for ep in episodes:
        ...
        if "hallway" in text.lower():
            ...
            if _HALLWAY_ENTRY_RE.search(text):
                ...
                entry_rng = random.Random(eid ^ 0x8845_E300)
            ...
            if _HALLWAY_DIR_RE.search(text):
                ep_rng = random.Random(eid ^ 0x8845_5300)
        ...
        if _WALK_TO_THE_RE.search(text):
            toward_rng = random.Random(eid ^ 0x8845_A300)
    ...
    print(f"  [v88 targets: through_the ~22%+, toward_the ~3.8%, hallway ~14-16%, into_hall ~4%]")
