#!/usr/bin/env python3
"""
Gate 4 Visual v109 — GT-STYLE vocabulary shift: "walk past the X" instead of "passing the X".

KEY DISCOVERY (v108 analysis):
  - v24 (best generated): passing_the=79.7%, walk_past=0.3%, continue=57%, turn=51.6%, hallway=51.8%
  - GT (63.89% SR):       passing_the=1.1%,  walk_past=10.5%, continue=10.4%, turn=24.1%, hallway=20.5%
  - v108 (generated):     passing_the=58.8%, walk_past=0.0%, continue=45.2%, turn=?, hallway=43.9%

CRITICAL INSIGHT:
  GT achieves 63.89% SR with only 1.1% "passing the" and 10.5% "walk past the".
  v24 achieves only 40.24% SR with 79.7% "passing the".
  The model (InternVLA-N1) was trained on GT-style R2R instructions.
  GT-style vocabulary (imperative "walk past the X", fewer turns, less hallway)
  should work better because it matches the model's training distribution.

v109 CHANGES FROM v108:
  1. VOCABULARY SHIFT — "passing the X" → "walk past the X":
     - Change [pass:] route marker to produce "walk past the X" (imperative, GT style)
     - Reduce P_PASS_MARKER: 0.80 → 0.12 (target ~10% walk_past, matching GT=10.5%)
     - Remove MOTION LANDMARKS rule (was causing spontaneous "passing the" overuse)
     - Expected: walk_past rises from 0% → ~10%; passing_the drops from 58% → ~0%

  2. TURN FREQUENCY — match GT (24.1% vs v24's 51.6%):
     - Reduce P_TURN_UNANCHORED: 0.90 → 0.28 (target ~24% turn rate)
     - Keep angle threshold at 30° (same as v108)
     - Expected: turn_left ~25%, turn_right ~25% (matching GT)

  3. CONTINUE SUPPRESSION — match GT (10.4% vs v108's 45.2%):
     - Restore _CONTINUE_MOTION_RE: "Continue straight/forward" → "Walk straight/forward"
     - This matches GT style where "walk" is used instead of "continue"
     - Expected: continue drops from 45% → ~10%

  4. HALLWAY SUPPRESSION:
     - Add hallway to SYSTEM_INTRO: "Avoid using 'hallway' unless clearly a narrow corridor"
     - Expected: hallway drops from 43.9% → ~20% (matching GT)

  Expected vocabulary targets (GT-matched):
    passing_the: 58.8% → ~0%   (GT=1.1%)
    walk_past:   0.0% → ~10%   (GT=10.5%)
    continue:    45.2% → ~10%  (GT=10.4%)
    turn_left:   51.6% → ~24%  (GT=24.1%)
    hallway:     43.9% → ~20%  (GT=20.5%)
    SR target: 45-60%+ (GT-vocabulary alignment for model trained on GT instructions)

Phase 2 checkpoint: outputs/gate4_v109_phase2_checkpoint.json (NEW -- fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v109.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v109.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v109_phase2_checkpoint.json"  # v109 — GT-style vocabulary: "walk past the X"
...
P_PASS_MARKER = 0.12   # v109: GT-style — "walk past the X" at ~10% of episodes (GT=10.5%)
...
- When route has [pass: description]: the robot moves alongside an object — write "walk past the X" (imperative form, 3-5 words). Example: "walk past the brown armchair", "walk past the dining table". Do NOT write "passing the" or "passing by" — write ONLY "walk past the X". Keep it very brief.
...
- HALLWAY: Do NOT say "hallway" unless the route shows "→ hallway" or you can clearly see it is a narrow connecting corridor. Do NOT invent hallway labels for open spaces or rooms. If unsure, write "walk straight" instead.
- DISTANCE: When the route shows a 'straight Xm' segment — NEVER write metric numbers. Use natural language: "a few steps" (≤2m), "walk straight" (3–5m), "walk straight for a while" (6–9m), "walk straight for a long while" (≥10m). R2R annotators never write "3 meters" or "5m" — keep it conversational.
- STYLE: Prefer "walk straight" and "go straight" over "walk forward". Use "walk" not "continue" — "walk straight" not "continue straight". Keep instructions clean and turn-focused.
...
async def main():
    ...
    print('=== Gate 4 v109: GT-style vocabulary — "walk past the X" at 10%, reduced turns to 24%, suppress continue ===')
