#!/usr/bin/env python3
"""
Gate 4 Visual v98 — v95 + only fix: separate "turn" from room-entry clause (keeps v95 word count).
...
Phase 2 checkpoint: outputs/gate4_v98_phase2_checkpoint.json (NEW — fresh LLM calls needed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v98.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v98.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v98_phase2_checkpoint.json"  # NEW — fresh P2 for room-entry prompt
...
- When a turn has (→ room) after the arrow: write the turn direction FIRST ("turn left"), then a SEPARATE new sentence for the room entry: "Walk into the hallway." or "Enter the bedroom." Do NOT combine into one phrase — write "Turn left. Walk into the hallway." NOT "Turn left into the hallway." Use brief room-entry language (3-5 words).
...
- ROOM TRANSITIONS: When a turn is marked (→ room), write the turn direction then a SEPARATE brief sentence: "Turn left. Walk into the hallway." or "Turn right. Enter the bedroom." Do NOT use the combined "turn left into the hallway" form — they must be separate clauses. Use each room name AT MOST ONCE across the entire instruction. After entering a room, continue without repeating the room name.
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    if n_bracketed > 0 and n_room_trans > 0:
        ...
        anchor_note = (f"Route has {n_bracketed} bracketed landmark(s) and {n_room_trans} "
                       ...
                       f"then write a SEPARATE sentence for room entry after turns: 'Walk into the [room].' "
                       f"Do NOT combine. Use each room name AT MOST ONCE.")
    elif n_bracketed > 0:
        ...
        anchor_note = (f"Route has {n_room_trans} room transition(s) marked (→ room) — "
                       f"after each turn, write a SEPARATE brief sentence for room entry: "
                       f"'Walk into the [room].' or 'Enter the [room].' "
                       f"Do NOT combine with turn direction. Use each room name AT MOST ONCE.")
    else:
...
async def main():
    ...
    print(f"  [v98 targets: v95 word count preserved, separate clause room-entry (no combined turn-into)]")
