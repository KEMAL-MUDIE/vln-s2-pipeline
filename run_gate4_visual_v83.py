#!/usr/bin/env python3
"""
Gate 4 Visual v83 — v82 + relax continue suppression

v82 ISSUE (post-analysis):
  continue%: 1.2% (GT=10.4%) — massively underused.
  Root cause: _CONTINUE_MOTION_RE converts ALL directional continues to walk:
    "Continue through/into/along/toward/past/down/up/forward/straight/ahead" → "Walk [dir]"
  This removes GT-style phrases like "continue toward the kitchen", "continue through the arch",
  "continue up the stairs" which appear naturally in R2R annotations.
  GT uses "continue" in phrases like: "continue forward to the end", "continue into the bedroom",
  "continue toward the X" — these are continuation-of-motion, not standalone motion commands.

v83 FIX:
  1. Relax _CONTINUE_MOTION_RE: only suppress bare motion-only continues:
     "Continue forward/straight/ahead/walking/moving" → "Walk forward/straight/ahead"
     ALLOW: "Continue through/into/along/toward/past/down/up/across/in" (GT-style)
  2. Expected: continue% rises from 1.2% toward 5-8%, toward% from 0.9% toward 2-3%.
  3. Keep all v82 improvements: hallway prob suppression, clean() fixes, distance rule.
  4. Seed: 0x8345_5300. P_PASS_MARKER=0.10 (unchanged).

Phase 2 checkpoint: outputs/gate4_v83_phase2_checkpoint.json (FRESH — continue regex changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v83.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v83.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v83_phase2_checkpoint.json"  # FRESH — continue regex changed
...
def apply_goal_approach_stop(stop_phrase: str, stop_type: str, goal_desc: str,
    ...
    if stop_type == "stop":
        new_phrase = rng.choice([f"Stop near {goal_desc_clean}.",
                                  f"Stop at {goal_desc_clean}.",
                                  f"Stop in front of {goal_desc_clean}."])
    else:
        new_phrase = rng.choice([f"Wait near {goal_desc_clean}.",
                                  f"Wait by {goal_desc_clean}."])
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8345_5300)  # v83: new seed
...
async def main():
    ...
    print('=== Gate 4 v83: v82 + relax continue suppression ===')
    print(f"  KEY FIX: _CONTINUE_MOTION_RE relaxed — only suppress forward/straight/ahead/walking/moving")
    print(f"  ALLOW: through/into/along/toward/past/down/up/across (GT-style continuation phrases)")
    print(f"  Expected: continue% rises from 1.2% toward 5-8% (GT=10.4%)")
    print(f"  Expected: toward% rises from 0.9% toward 2-3% (GT=3.8%)")
    print(f"  RETAIN: all v82 fixes (hallway prob suppress, garbling fix, period fix, distance rule)")
    print(f"  P_PASS_MARKER={P_PASS_MARKER} (unchanged from v80)")
    print(f"  Seed: 0x8345_5300")
    ...
    for ep in episodes:
        ...
        if "hallway" in text.lower():
            ...
            if _HALLWAY_DIR_RE.search(text):
                ep_rng = random.Random(eid ^ 0x8345_5300)
                if ep_rng.random() > 0.36:  # suppress 64% → allow 36% → target ~20% (GT=20.4%)
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v75=0.993, v74=0.990)")
    print(f"  [v76 target: same GT-match ~0.990+, walk-past ~10% (was 16.9%), wooden <4%]")
