#!/usr/bin/env python3
"""
Gate 4 Visual v29 — Material-Adjective Filter + "passing the" Post-Processing + GT-match Fix

IMPROVEMENTS OVER v28:
  1. POST-PROCESS "passing the X" → "walk past the X" in clean():
     LLM ignores the rule and writes "passing the X" 27.4% of the time (GT=5.1%).
     Post-processing converts "passing through the X" → "through the X",
     and "passing the X" → "walk past the X".
  2. MATERIAL-ADJECTIVE FILTER for midpoint descriptions:
     Vision LLM generates "wooden floor", "wooden door" constantly (39.9% vs GT 3.3%).
     Strip material adjectives (wooden, hardwood, marble, granite, carpeted, etc.)
     from midpoint descriptions before building route descriptions for Phase 2.
  3. FIX GT-MATCH METRIC:
     has_turn_anchor() used WALK_PAST_RE which matched "walk past the X" as turn anchors.
     Fixed: has_turn_anchor() now uses ONLY TURN_ANCHOR_RE (not WALK_PAST_RE).
     Corrected GT-match for v28 was ~0.956 — now the metric reflects reality.
  4. All v28 innovations retained:
     - Generic-object filter: skip [pass:] for sofa/table/chair/wall etc.
     - [thru: X] → "through the X" for doorways (~27% target)
     - [pass: X] → "walk past the X" for distinctive objects only
     - Turn threshold 70deg, straight threshold 2m
     - Anti-loop, path-budget, door landmarks

  GT analysis reference:
    "through the":   GT=27.2%, v28=33.2%  (slightly above GT — acceptable)
    "walk past the": GT=8.7%,  v28=26.2%  → v29 target: ~15-20%
    "passing the":   GT=5.1%,  v28=27.4%  → v29 target: <5% (post-processing fix)
    "wooden":        GT=3.3%,  v28=39.9%  → v29 target: <10% (material filter)
    "continue":      GT=10.4%, v28=8.9%   (on target)
    GT-match:        broken v28=0.529, corrected=0.956 → v29: fixed metric

  Phase 2 checkpoint: outputs/gate4_v29_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v29.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v29.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v29_phase2_checkpoint.json"
...
def is_generic_pass_object(desc: str) -> bool:
    """v28: Return True if the pass midpoint object is too generic to be a useful landmark.
    Generic objects (sofa, table, chair, wall) → skip [pass:] marker.
    Distinctive objects (fireplace, piano, art, stairs) → keep [pass:] marker.
    """
    if not desc:
        return True
    # If it has distinctive keywords, keep it even if also has generic ones
    if _DISTINCTIVE_OBJECTS_RE.search(desc):
        return False
    # If purely generic, skip
    if _GENERIC_OBJECTS_RE.search(desc):
        return True
    return False
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None):
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if seg_midpoint and turn_label in seg_midpoint:
                ...
                if action == "thru" or not is_generic_pass_object(mid_desc):
    ...
    # v28: last segment midpoint — classify; skip [pass:] if generic
    # v29: also strip material adjectives
    if seg_midpoint and "goal" in seg_midpoint:
        mid_desc = strip_material_adjectives(seg_midpoint["goal"])
        action = classify_pass_action(mid_desc)
        if action == "thru" or not is_generic_pass_object(mid_desc):
            parts.append(f"[{action}: {mid_desc}]")
...
async def main():
    ...
    print("=== Gate 4 v29: Material Filter + 'passing the' Post-Processing + GT-match Fix ===")
    ...
    print(f"  v29 key changes: (1) post-process 'passing the X' → 'walk past the X',"
          f" (2) strip material adjectives (wooden/marble/carpeted) from midpoints,"
          f" (3) fix GT-match metric (only TURN_ANCHOR_RE, not WALK_PAST_RE)")
    print()
    ...
    print(f"  turn-anchor%:  {100*anchor_n/n:.1f}%  (GT=16.5%) ← v29 FIXED metric (TURN_ANCHOR_RE only)")
    print(f"  dup-stop:      {dup_stop} (target: 0)")
    print(f"  walk past the: {100*past_n/n:.1f}%  (GT={100*gt_past_n/gt_n:.1f}%) ← KEY METRIC")
    print(f"  through the:   {100*through_n/n:.1f}%  (GT=27.2%) ← KEY METRIC")
    print(f"  walk past the: {100*walkpast_n/n:.1f}%  (GT=8.7%) ← KEY METRIC (v28=26.2%)")
    print(f"  passing the:   {100*passing_n/n:.1f}%  (GT=5.1%) ← KEY METRIC (v28=27.4%, target <5%)")
    print(f"  continue:      {100*continue_n/n:.1f}%  (GT=10.4%)")
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← KEY METRIC (v28=39.9%, target <10%)")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v28-corrected=0.956, v26=0.955)"
          f"  [v29: metric fixed, should match v26-level again]")
    ...
    print(f"  [Note: v29 strips material adjectives (wooden/marble/etc.) from instructions]")
