#!/usr/bin/env python3
"""
Gate 4 Visual v149 — Fix three assembly bugs that create degenerate 'destination' fallbacks.

v148 ACTUAL: through_the=27.08%, hallway=20.83%, into_hallway=4.46%, continue=10.66%, walk_past=8.48%
             go_straight=0.00% ✓, avg_words=27.9w, vocab_gap=1.08pp → pred_SR=63.23%

v148 PROBLEMS (found from EU6/TbH scene regression analysis):
  1. remove_loops BUG: truncates at seen[ng] (first occurrence position) instead of i (second).
     When loop starts at position 0-2, truncation → empty string → ". stop_phrase" → quality_ok
     rejects (starts_with_period) → 'destination' fallback. 13+ episodes affected.
  2. quality_ok doesn't account for stop_phrase words: after _MID_STOP_FRAG_RE strips "and stop X"
     from LLM text, instruction may be <8 words even though enforce_stop_phrase will add 5-7 more.
     quality_ok fails → 'destination' fallback. ~15 episodes affected (e.g. ep 621 EU6Fwq7SyZv).
  3. Destination fallback uses extracted sv (e.g. "Enter", "Make") → grammatically broken output
     "Enter to the destination." instead of "Walk to the destination."

v149 FIXES:
  1. remove_loops: truncate at i (second occurrence) — keeps first good navigation segment
  2. quality_ok(text, stop_phrase): count stop_phrase words toward minimum word threshold
  3. Destination fallback: always use "Walk" (not sv) for safe fallback verb
  4. Reuse v145 Phase 2 checkpoint (instant reassembly, no new LLM calls)

Phase 1 checkpoint: outputs/gate4_v143_phase1_checkpoint.json (same as v143–v148)
Phase 2 checkpoint: outputs/gate4_v145_phase2_checkpoint.json (REUSED from v145–v148)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v149.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v149.json.gz"
...
async def main():
    ...
    print('=== Gate 4 v149: Fix 3 assembly bugs (remove_loops direction, quality_ok stop_phrase, dest verb) ===')
