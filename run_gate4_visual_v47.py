#!/usr/bin/env python3
"""
Gate 4 Visual v47 — Fix "toward the" spike + transit hallway overuse

ROOT CAUSES FIXED (vs v45):
  1. SYSTEM_INTRO line: "or walk toward [goal]" caused LLM to substitute "toward the X" for
     suppressed turns → toward_the spiked from 16.9% (v44) to 26.5% (v45) vs GT=3.8%.
     FIX: Remove "walk toward [goal]" from turn suppression rule → "walk forward" only.

  2. anchor_note for room transitions: explicitly told LLM "Use 'turn left and walk into the X'"
     → LLM faithfully generated "walk into the hallway" 17.6% vs GT=4.4%.
     FIX: Reword anchor note to prohibit "walk into the [room]".

  3. build_route_desc shows "turn right → hallway" for transit turns:
     → LLM reads room name and generates "walk into the hallway".
     FIX: Remove transit room names from route (show "turn right" only, no → rm).

  4. ROOM NAMES rule not explicit enough about "walk toward the hallway":
     FIX: Add explicit prohibition of "walk toward the [room]".

EXPECTED vs v45:
  toward_the:    26.5% → ~4% (GT=3.8%)
  into_hallway:  17.6% → ~4-6% (GT=4.4%)
  hallway total: 45.1% → ~22-28% (GT=20.4%)
  through_the:   26.8% → ~27% (maintained)
  walkpast:       9.6% → ~9% (maintained)
  GT-match:      0.980 → ~0.975-0.980 (maintained)

Phase 2 checkpoint: outputs/gate4_v47_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v47.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v47.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v47_phase2_checkpoint.json"
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v47: hide transit room names — "turn right → hallway" caused LLM to generate
                # "walk into the hallway" 17.6% vs GT=4.4%. Route shows plain turn instead.
...
async def main():
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v45=0.92) ← v44 KEY FIX")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v45=26.5%) ← v47 KEY FIX")
    print(f"  hallway:       {100*hallway_n/n:.1f}%  (GT=20.4%, v45=45.1%) ← v47 KEY FIX")
    print(f"  into hallway:  {100*into_hall_n/n:.1f}%  (GT=4.4%, v45=17.6%) ← v47 KEY FIX")
    print(f"  spatial:")
