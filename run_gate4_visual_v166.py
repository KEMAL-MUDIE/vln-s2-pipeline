#!/usr/bin/env python3
"""
Gate 4 Visual v166 — Fix v165's anchor% overcorrection (49.1% → target 16.5%).

ROOT CAUSE ANALYSIS (v165 → v166):
  v165 FAILED: anchor%=49.1% (GT=16.5%), GT-match=0.342 — massive overcorrection.
  Root cause: non-hallway room transitions ALWAYS got direction:
    "if direction: parts.append(f'turn {direction} (→ {rm})')"
  → LLM writes "turn left into the bedroom" for EVERY room transition → anchor%=49.1%

  Fix: Add probability gate P_dir=0.20 → only 20% of non-hallway transitions get direction:
    "if direction and rng.random() < P_DIR_ROOM_TRANS:"
  → Expected anchor%: room anchors≈8-10% + hallway≈3% + furniture≈3% ≈ 14-16%

  Also: P_TURN_UNANCHORED 0.55 → 0.30 to reduce avg_turns (v165=1.28 vs GT=0.59).

KEY METRICS:
  v165 anchor%=49.1% (GT=16.5%) — always-direction MASSIVELY overcorrects
  v164 anchor%=6.1%  (GT=16.5%) — no direction undercorrects
  v166 target: anchor%≈16-18%, avg_turns≈0.7-0.9, GT-match≥0.95

v166 DESIGN:
  1. Furniture filter in [pass:] — kept from v165 (prevents armchair/sofa confusion)
  2. Room transition direction probability P_DIR_ROOM_TRANS=0.20 (not always)
     "turn left (→ bedroom)" only 20% of non-hallway transitions → anchor%≈16-18%
  3. P_TURN_UNANCHORED: 0.55 → 0.30 (reduce excess turns: v165=1.28, GT=0.59)
  4. P_ANCHOR_EPISODE: 0.03 (unchanged — furniture anchors at GT level)
  5. FRESH Phase 2 checkpoint

Expected: anchor%≈16-18% (GT=16.5%), avg_turns≈0.7-0.9, avg_words≈24-26w
GT-match estimate: ~0.97-0.99 (anchor gap ≈ 1.5%, stop/wait already perfect)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v166_phase2_checkpoint.json (FRESH — route prob changes)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v166.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v166.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v166: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v166_phase2_checkpoint.json"  # v166: FRESH — prob gate on non-hallway room transitions
...
P_DIR_ROOM_TRANS  = 0.20  # v166: 20% of non-hallway room transitions get "turn left (→ bedroom)"
                           # v165=1.0 (always) → anchor%=49.1% (massively overcorrected)
                           # v166=0.20 → expected anchor%≈14-17% (target GT=16.5%)
...
async def main():
    ...
    print('=== Gate 4 v166: Fix v165 anchor% overcorrection (49.1%→target 16.5%) ===')
    print(f"  v165 FAILED: anchor%=49.1% — always-direction on non-hallway transitions OVERCORRECTED")
    print(f"  FIX: P_DIR_ROOM_TRANS=0.20 (20% prob) for non-hallway room transitions")
    print(f"       Only 20% get 'turn left (→ bedroom)' → expected anchor%≈14-17% (GT=16.5%)")
    print(f"  FIX 2 (kept): Furniture filtered from [pass:] — prevents 'walk past armchair' confusion")
    print(f"  P_TURN_UNANCHORED: 0.55→0.30 — reduce avg_turns (v165=1.28, GT=0.59)")
    print(f"  v166: P_ANCHOR=0.03 + P_DIR=0.20 + P_TURN_UNANCHORED=0.30 + P_continue=0.28 + FRESH")
    print(f"    Expected: anchor%≈16-18%(GT=16.5%), avg_turns≈0.7-0.9, GT-match≥0.97")
