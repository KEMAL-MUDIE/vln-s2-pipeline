#!/usr/bin/env python3
"""
Gate 4 Visual v141 — FRESH Phase 2 generation fixing "go straight" + "take a few steps" overuse.

=== EXTENDED VOCABULARY ANALYSIS (discovered v139) ===
Phrase divergences NOT in 5-metric model:
  go straight    : v139=81.35% vs GT=7.94%  (+73.4pp CRITICAL)
  walk into the  : v139=38.99% vs GT=6.25%  (+32.7pp CRITICAL)
  take a few steps: v139=28.82% vs GT=0.22% (+28.6pp CRITICAL)
  walk through the: v139=17.94% vs GT=9.62% (+8.3pp)
  walk to the    : v139=14.68% vs GT=5.06%  (+9.6pp)
  walk down the  : v139=2.61%  vs GT=8.54%  (-5.9pp underuse)
  toward the     : v139=1.63%  vs GT=3.75%  (-2.1pp underuse)

Root causes:
  "go straight": Phase 2 prompt says "Prefer go straight/walk straight" + _ws_variants 50% → go straight
  "take a few steps": DISTANCE rule says "≤2m → take a few steps" → triggers for every short segment
  "walk into the": "(→ room)" rule generates "walk into the [room]" for every room entry

v141 STRATEGY: FRESH Phase 2 generation with updated prompt to fix top-2 divergences.

=== PHASE 2 PROMPT CHANGES ===
1. MOTION WORDS: Remove "go straight"/"walk straight" preference; add explicit AVOID.
   Use "walk forward" SPARINGLY (≤1 per instruction) or describe by destination.
2. DISTANCE: Remove "take a few steps"/"go straight"/"walk straight" from distance guidance.
   New rule: (≤2m) omit or use "walk forward"; (>2m) use "walk to the [destination]" or "walk forward"

=== POST-PROCESSING CHANGES ===
Same "go straight" elimination as v140 (safety net), but convert to "walk forward" instead of
"walk a bit"/"walk ahead" (both were OOD; "walk forward" is GT=6.09%, less OOD).

=== _ws_variants CHANGES ===
Remove "go straight ahead" and "go straight" from variants; replace with "walk forward" (25%).
New variants for "walk straight": ["walk straight ahead", "walk forward", "walk forward", "walk forward"]
→ 75% "walk forward" (triggers continue restoration), 25% "walk straight ahead" (triggers restoration).
→ This INCREASES continue restoration rate from 50% to 100% of "walk straight" occurrences.
→ May increase continue%; adjust P_CONT from 0.655 → 0.50 to compensate.

=== CALIBRATION ===
continue: currently 10.4-10.5% with P_CONT=0.655.
With new _ws_variants (100% restoration eligible vs 50%): rate doubles → ~20%.
To keep continue≈10.4%: P_CONT must drop by ~50% → P_CONT=0.33.
But this is imprecise. Better: use P_CONT=0.40 (safe estimate, measure actual).

5-metric impact (expected):
  through_the: ~27.2% (GT! — unchanged since thru mechanics untouched)
  hallway:     ~20.4% (GT! — unchanged since hallway annotation untouched)
  into_hallway: ~4.4% (GT! — unchanged since into_hallway annotation untouched)
  continue:    ~10.4% (GT! — P_CONT=0.40 compensates for doubled restoration rate)
  walk_past:   ~8.6%  (P_PASS=0.82 unchanged)

Extended vocab impact (expected):
  go straight:    81.35% → ~5-10% (LLM avoids it, residual safety net post-proc catches remainder)
  take a few steps: 28.82% → ~2-5% (removed from DISTANCE rule, LLM uses less)
  walk forward:   11.69% → ~30-40% (replaces go straight/walk straight, still overuse vs GT=6.09%)

Phase 2 checkpoint: FRESH (gate4_v141_phase2_checkpoint.json) — prompt changes require new LLM calls.
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v141.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v141_phase2_checkpoint.json"  # v141: FRESH generation (prompt changes require new LLM calls)
...
- MOTION WORDS: AVOID "go straight", "walk straight", "take a few steps" — real annotators rarely write these. Describe motion by destination: "walk to the sofa", "walk down the hallway", "walk past the chair". Use "walk forward" ONLY as a last resort when no landmark is visible, and at most ONCE per instruction. Do NOT use "continue", "proceed", "go forward", "keep going", or "head toward".
- DISTANCE: When route shows 'straight Xm': (≤3m) omit — do NOT write any motion; just continue to the next action; (3–8m) write "walk forward" (ONLY if no destination landmark); (>8m) write "walk forward to the [next landmark]". NEVER write "take a few steps", "go straight", "walk straight", or "walk straight ahead".
...
def clean(raw, sv):
    ...
    # v141: Remove ALL "go straight" from _ws_variants. Use only restoration-eligible alternatives.
    # v126 had: 25% "walk straight ahead", 25% "go straight ahead", 25% "go straight", 25% "walk forward"
    # → 50% restoration-eligible (walk straight ahead, walk forward) × P=0.655 = 33% continue.
    # v141: 25% "walk straight ahead", 75% "walk forward" → 100% restoration-eligible × P=0.40 = 40% continue.
    # P_CONT adjusted 0.655→0.40 to keep continue≈10.4%.
    _ws_variants = [
        (r'\bwalk straight for a long while\b', ['walk forward a long way', 'walk forward']),
        (r'\bwalk straight for a while\b',      ['walk forward for a bit', 'walk forward']),
        (r'\bwalk straight a few\b',             ['walk forward a few', 'walk a few steps further']),
        (r'\bwalk straight ahead\b',             ['walk forward']),  # prevent double-hit; all → walk forward
        (r'\bwalk straight\b',                   ['walk straight ahead', 'walk forward', 'walk forward', 'walk forward']),
    ]
...
async def main():
    ...
    print('=== Gate 4 v141: FRESH gen — "go straight"+"take a few steps" elimination via updated Phase 2 prompt ===')
    print(f"  v136 actual: through_the=27.1%, hallway=20.1%, into_hallway=4.4%, continue=10.3%, walk_past=8.4%")
    print(f"  v138 actual: through_the=27.1%, hallway=20.3%, into_hallway=4.5%, continue=10.5%, walk_past=8.4%")
    print(f"  v138 lesson: P_PASS only works with FRESH generation; cached reuse doesn't change walk_past")
    print(f"  v140 actual: through_the=27.6%, hallway=20.6%, walk_past=8.6%, continue=10.4%, go_straight→walk_ahead/walk_a_bit")
    print(f"  v140 problem: walk_ahead=62.64% (GT=0.16%), walk_a_bit=48.83% (GT=0.05%) — new OOD replacements!")
    print(f"  v141: FRESH gen — prompt changes to AVOID go_straight+take_a_few_steps; P_CONT=0.40, _ws_variants no go_straight")
    ...
    for ep in episodes:
        ...
        # v141: P=0.40 (reduced from 0.655) — _ws_variants now 100% restoration-eligible vs 50% previously
        # With 100% of "walk straight" → "walk forward" (via changed _ws_variants), restoration rate doubles.
        # P=0.40 compensates: 0.655 × 50% ≈ 0.327 ≈ 0.40 × ~82% (conservative estimate, measure actual).
        if cont_rng.random() < 0.40 and _WALK_FWD_CAP_RE.search(text):
            text = _WALK_FWD_CAP_RE.sub(r'Continue \1', text, count=1)

        # v141: Safety net — convert residual "go straight" → "walk forward" (GT=6.09%, more natural).
        # Runs AFTER continue restoration so "walk forward" here is NOT further restored to "Continue forward".
        # "Walk forward" (sentence-start capital) would match _WALK_FWD_CAP_RE — but restoration already ran.
        text = re.sub(r'\bGo straight ahead\b', 'Walk forward', text)
        text = re.sub(r'\bgo straight ahead\b', 'walk forward', text)
        text = re.sub(r'\bGo straight\b', 'Walk forward', text)
        text = re.sub(r'\bgo straight\b', 'walk forward', text)
        text = re.sub(r'\bWalk straight ahead\b', 'Walk forward', text)
        text = re.sub(r'\bwalk straight ahead\b', 'walk forward', text)
