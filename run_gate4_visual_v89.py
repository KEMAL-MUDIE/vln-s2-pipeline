#!/usr/bin/env python3
"""
Gate 4 Visual v89 — v88 + calibrated assembly fixes for through/hallway/continue

v88 REMAINING GAPS (stable base for v89):
  through_the:  21.6% (GT=27.2%) — need +5.6pp
  hallway:      14.2% (GT=20.4%) — need +6.2pp
  continue:     5.8%  (GT=10.4%) — need +4.6pp
  into_hallway: 0.5%  (GT=4.4%)  — max achievable ~2.5% (LLM generates too few)

v89 FIXES — assembly post-processing ONLY (LLM prompt IDENTICAL to v84/v86/v88):
  1. P_THRU_MARKER: 0.90 → 0.95 → more [thru:] markers → more "through the" phrases.
     Expected: 21.6% → ~24-25%.
  2. Hallway dir allow: ep_rng.random() > 0.48 → > 0.31 (calibrated).
     Pre-filter=14.2%/0.48=29.6%; target=20.4%; P_pass=20.4/29.6=0.69; P_suppress=0.31.
     Expected: 14.2% → ~20%.
  3. into_hallway: remove 80% suppression → allow 100%.
     Max achievable: ~2.5% (LLM generates ~2.5% pre-filter at P_allow=1.0).
  4. Continue probabilistic promotion: "Walk forward/straight/ahead" → "Continue ..." in P=0.35 eps.
     Expected: 5.8% + 0.35×(18.5-5.8)% ≈ 5.8%+4.4% = 10.2% (GT=10.4%).
     Only sentence-start "Walk" (capital), first occurrence only. Seed: 0x8945_C300.
  Seed: 0x8945_5300. Fresh P2 checkpoint.

Phase 2 checkpoint: outputs/gate4_v89_phase2_checkpoint.json (FRESH — seed changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v89.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v89.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v89_phase2_checkpoint.json"  # FRESH — seed changed
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8945_5300)  # v84: new seed
...
async def main():
    ...
    for ep in episodes:
        ...
        if "hallway" in text.lower():
            ...
            if _HALLWAY_DIR_RE.search(text):
                ep_rng = random.Random(eid ^ 0x8945_5300)
                if ep_rng.random() > 0.31:  # v89: suppress 31% → allow 69% → target ~20% (GT=20.4%)
        ...
        if _WALK_TO_THE_RE.search(text):
            toward_rng = random.Random(eid ^ 0x8945_A300)
        ...
        cont_rng = random.Random(eid ^ 0x8945_C300)
        if cont_rng.random() < 0.35 and _WALK_FWD_CAP_RE.search(text):
    ...
    print(f"  [v89 targets: through_the ~24-25%, toward_the ~3.8%, hallway ~20%, continue ~10%]")
