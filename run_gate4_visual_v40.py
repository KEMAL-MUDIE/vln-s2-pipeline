#!/usr/bin/env python3
"""
Gate 4 Visual v40 — Fix fragment sentences + truncated stop phrases
...
v40 FIXES:
  ...
  Phase 2 checkpoint: outputs/gate4_v40_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v40.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v40.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v40_phase2_checkpoint.json"
...
def clean(raw, sv):
    ...
    for s in sents:
        ...
        # v40: skip fragment sentences (preposition-only, no subject/verb)
        if _FRAGMENT_SENT_RE.match(s):
            continue
    ...
    result = " ".join(s for s in cleaned_sents[:3] if s and s not in ('.',)).strip()
...
async def main():
    ...
    print('=== Gate 4 v40: Fix dup-stop=62 ("and stop at X" mid-instruction → "and walk to X") ===')
    ...
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v40 should be <1%")
    ...
    print(f"  [Note: v40: v39 + fragment sentence removal + truncated landmark attr fix]")
