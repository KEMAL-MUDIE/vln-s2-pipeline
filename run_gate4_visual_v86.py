#!/usr/bin/env python3
"""
Gate 4 Visual v86 — v84 + assembly-level toward/hallway/through calibration

v85 FAILURE ANALYSIS:
  v85 added "walk toward the X" to LLM prompt → toward_the 0.7% → 15.1% (GT=3.8%).
  LLM used "toward" for [thru:] markers → through_the collapsed 21.7% → 10.0%.
  Lesson: DO NOT change LLM prompt for spatial preposition tuning — use assembly instead.

v84 REMAINING GAPS (stable base for v86):
  toward the:   0.7%  (GT=3.8%)  — need +3.1pp
  through the:  21.7% (GT=27.2%) — need +5.5pp
  hallway:      13.8% (GT=20.4%) — need +6.6pp

v86 FIXES (assembly post-processing ONLY — LLM prompt unchanged from v84):
  1. "toward the" via assembly: After clean(), randomly convert "walk to the X" →
     "walk toward the X" with P_TOWARD=0.20. Only non-hallway/non-room targets.
     Expected: ~3-4% toward_the. Deterministic via episode seed.
  2. "through the" via P_THRU_MARKER: 0.85 → 0.90 (small increase).
     Also relax _THROUGH_ROOM_RE: allow "through the kitchen/office/study/lobby/foyer"
     (GT uses these in 7.9% of cases — room-entry via kitchen doorway etc.)
  3. "hallway" via assembly: Allow probability 36% → 52%.
  4. "into hallway" via assembly: P=0.20 allow (was always suppressed).
  Seed: 0x8645_5300. Fresh P2 checkpoint.

Phase 2 checkpoint: outputs/gate4_v86_phase2_checkpoint.json (FRESH — seed changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v86.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v86.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v86_phase2_checkpoint.json"  # FRESH — seed changed
...
P_THRU_MARKER = 0.90   # v86: increased from 0.85 → 0.90 to push through_the toward GT 27.2%
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8645_5300)  # v84: new seed
...
async def main():
    ...
    for ep in episodes:
        ...
        if "hallway" in text.lower():
            ...
            if _HALLWAY_ENTRY_RE.search(text):
                ...
                entry_rng = random.Random(eid ^ 0x8645_E300)
            ...
            if _HALLWAY_DIR_RE.search(text):
                ep_rng = random.Random(eid ^ 0x8645_5300)
        ...
        if _WALK_TO_THE_RE.search(text):
            toward_rng = random.Random(eid ^ 0x8645_A300)
            if toward_rng.random() < 0.20:
    ...
    print(f"  [v86 targets: through_the ~24%+, toward_the ~3-4%, hallway ~18-20%, into_hall ~4%]")
