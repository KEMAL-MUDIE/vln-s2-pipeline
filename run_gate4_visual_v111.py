#!/usr/bin/env python3
"""
Gate 4 Visual v111 — POST-PROCESS "passing the X" → "walk past the X" (GT-style fix).

v110 ACTUAL RESULTS:
  - passing_the: 24.2% (STILL 24x GT's 1.1%) ← LLM ignores "walk past" instruction
  - walk_past:   0.0%  (v106-v110 clean() was converting "walk past the" → "passing the"!)
  - continue:    14.5% (near GT=10.4%, good)
  - hallway:     6.4%  (GT=20.4%) ← suppression worked but OVER-suppressed
  - into_hallway: 1.6%  (GT=4.4%) ← FIXED! was 30.2%
  - word_count:  25.3w (GT=26.8w, close)

ROOT CAUSE OF "passing the" DOMINANCE (CRITICAL BUG FIXED):
  v106-v110's clean() had: "walk past the X" → "passing the X" (WRONG direction!)
  This UNDID the LLM's correct "walk past the" output from [pass:] markers.
  Fix: REMOVE that conversion. ADD reverse: "passing the X" → "walk past the X"
  Now: LLM generates "walk past the" from [pass:] markers → stays "walk past the" ✓
       LLM spontaneously writes "passing the X" → post-processed to "walk past the X" ✓

v111 CHANGES FROM v110:
  1. REVERSE "passing the" CONVERSION (critical fix):
     REMOVED: re.sub(r'\bwalk past the\b', 'passing the', ...) — WRONG DIRECTION
     ADDED:   re.sub(r'\bpassing the\b', 'walk past the', ...) — CORRECT DIRECTION
     Also normalizes: "go past the" / "passed the" → "walk past the"
     After: _GENERIC_WPT_OBJ_RE still suppresses "walk past the [generic]" → "walk straight"
     Expected: passing_the: 24.2% → ~1% (GT=1.1%)
               walk_past:   0.0% → ~10-12% (GT=8.7%)

  2. RETAIN all v110 changes (hallway suppression, P_PASS_MARKER=0.12, etc.)

  Expected vocabulary targets:
    passing_the:  24.2% → ~1%   (GT=1.1%) ← MAIN TARGET
    walk_past:    0.0%  → ~10%  (GT=8.7%) ← MAIN TARGET
    continue:     14.5% → ~12%  (near GT=10.4%)
    into_hallway: 1.6%  → ~2-4% (GT=4.4%, currently slightly under)
    SR target: 55-65%+ (GT-vocabulary alignment achieved)

Phase 2 checkpoint: outputs/gate4_v111_phase2_checkpoint.json (NEW -- fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v111.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v111.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v111_phase2_checkpoint.json"  # v111 — "passing the" → "walk past the" post-processing
...
def clean(raw, sv):
    ...
    # v111: GT-style: convert ALL "passing the X" → "walk past the X" (imperative, GT=10.5%)
    # v106-v110 was converting "walk past the X" → "passing the X" — WRONG DIRECTION!
    # GT uses "walk past the X" (10.5%), NOT "passing the X" (1.1%). Reverse the conversion.
    ...
    # After converting all "passing the" to "walk past the", suppress generic objects.
