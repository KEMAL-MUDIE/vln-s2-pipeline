#!/usr/bin/env python3
"""
Gate 4 Visual v22 — Intermediate Waypoint Walking Landmarks

IMPROVEMENT OVER v20:
  v20 used the 2nd sentence of turn descriptions to add [X ahead] for ~17% of
  anchored turns. But intermediate landmarks (what the agent passes BETWEEN
  waypoints) were still missing for most episodes.

  GT instructions reference objects the agent PASSES WHILE WALKING through
  segments. These confirm the agent is on the right path.

THE FIX (v22): RENDERED INTERMEDIATE GT WAYPOINTS
  Phase 1b: render_midpoints.py + phase1_midpoints.py render ALL non-keyframe
    GT waypoints (7125 frames) and get visual descriptions via vLLM.
    Checkpoint: outputs/gate4_v22_midpoint_p1_checkpoint.json (84.6% useful)

  Phase 1 — REUSED from v19 (keyframes: start, turns, goal).
    Checkpoint: outputs/gate4_v19_phase1_checkpoint.json

  Phase 2 — Instruction generation WITH midpoint walking landmarks:
    For each episode, pick the BEST SEGMENT midpoint (last segment preferred).
    Add [pass: X] in route before the segment's endpoint.
    [pass: X] rule: "robot passes X while walking — write 'passing the X'."
    Checkpoint: outputs/gate4_v22_phase2_checkpoint.json
    Output: outputs/datasets/val_unseen_generated_gemma_visual_v22.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v22.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v19_phase1_checkpoint.json"  # reuse v19 Phase 1!
P1_MID_CKPT   = ROOT / "outputs" / "gate4_v22_midpoint_p1_checkpoint.json"  # v22 midpoints
P2_CKPT       = ROOT / "outputs" / "gate4_v22_phase2_checkpoint.json"
...
def get_segment_midpoint(ep, midpoints_ckpt: Dict) -> Dict[str, str]:
    """
    For an episode, find the best single midpoint landmark across all segments.

    Strategy: prefer the LAST SEGMENT (just before goal) for maximum goal approach
    guidance. Fall back to first available segment.

    Returns {endpoint_label: desc} with at most 1 entry.
    endpoint_label is "goal" for last segment, or "turn_N" for earlier segments.
    """
    eid = ep["episode_id"]
    eid_str = str(eid)
    ...
    segments = []  # list of (b_label, list_of_(mid_idx, desc))
    for i in range(len(keyframes) - 1):
        ...
        if intermediates:
            segments.append((b_label, intermediates))

    if not segments:
        return {}

    # Prefer the last segment (before goal) for final approach guidance
    last_label, last_mids = segments[-1]
    # Use first valid midpoint in the chosen segment
    best_desc = last_mids[0][1]
    return {last_label: best_desc}
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None):
    ...
    seg_midpoint: {endpoint_label: desc} — at most 1 entry, injected before that endpoint.
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            ahead = td.get("ahead","")
            # v22: insert [pass: X] before this turn if this is the midpoint endpoint
    ...
    # v22: [pass: X] for last segment (before goal)
...
- When route has [pass: object] before a turn or the goal: the robot walks past that object — write ONLY "passing the X" or "continuing past the X" (3-5 words). FORBIDDEN: "walk past", "go past", "head past". Use ONLY gerund form "passing the X". Use ONLY ONCE per instruction
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    if n_pass > 0:
        anchor_note += (f" Route has [pass: X] marker(s) — write 'passing the X' (gerund form only;"
...
async def main():
    ...
    print("=== Gate 4 v22: Intermediate Waypoint Walking Landmarks ===")
    ...
    print(f"  Phase 1b: midpoints checkpoint (7125 frames, 84.6% useful)")
    print(f"  Phase 2 concurrency: {args.concurrency_p2}")
    print(f"  v22 key change: rendered intermediate GT waypoints → [pass: X] in route")
    print()
    ...
    for ep in episodes:
        ...
        # v22: best segment midpoint for this episode
        seg_midpoint = get_segment_midpoint(ep, midpoints_ckpt)
    ...
    print(f"  passing+walk:  {100*passing_n/n:.1f}% ← v22 midpoint landmark usage")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v19=0.965, v18=0.958, v17=0.948)")
