#!/usr/bin/env python3
"""
Gate 4 Visual v192 -- NEW Phase 2 LLM generation.
KEY FIX: "walk past the X" for intermediate objects (GT=8.7%). NOT "walk to the X" (GT only for final dest).
→ [object ahead] markers → "walk past the X". Expanded [pass:] eligibility (table/bed/couch back in).

CHANGES FROM v190:
  v190 actual stats: walk_to=44.8%(GT=5.1%), walk_past=11.3%(GT=8.7%), continue=17.7%(GT=10.4%),
    exit=13.6%(GT=18.0%), go_straight=11.3%(GT=7.9%), for_a_bit=0%(GT=0%)

  ROOT CAUSE of walk_to=44.8%:
    (A) SYSTEM_INTRO line for → [object ahead]: "add a BRIEF 'walk to the X' phrase" — WRONG.
        GT uses "walk past the X" (8.7%) for intermediate objects, not "walk to the X" (5.1% = final only).
    (B) anchor_note for → [object ahead]: "walk to the X" or "walk past the X" — allows "walk to"
    (C) [pass:] blacklist removes table/bed/couch — but GT uses these in "walk past the table/bed/couch" (top 3-6!)

  FIX A (NEW Phase 2): SYSTEM_INTRO → [object ahead]: "walk past the X" (not "walk to the X")
  FIX B (NEW Phase 2): anchor_note → [object ahead]: "walk past the X" only (remove "walk to the X")
  FIX C (STRUCTURAL): Remove table/bed/couch from [pass:] structural blacklist
    GT "walk past the X" top objects: stairs, kitchen, TABLE(24), BED(18), dining, COUCH(11), sink, piano
    Current blacklist wrongly excludes all three. Re-enable them for [pass:] eligibility.

FIX 1 (INHERITED): "walk to the X for a bit/while/moment" -> "walk past the X" at P=0.75
FIX 2 (INHERITED): "walk to the X and/comma turn [dir]" -> "turn [dir] at the X" at P=0.80
FIX 3 (INHERITED): Remove remaining "for a bit/while/moment" filler (GT=0%).
FIX 4 (INHERITED): "walk out of the X" -> "exit the X" at P=0.70 (+1.4pp exit).
FIX 5 (INHERITED): "leave the [room]" -> "exit the [room]" at P=0.40 (+1.5pp exit).
FIX 6 (INHERITED): walk_ahead->continue P=0.45 (continue: 17.7%->~16%).

Expected v192:
  walk_to: ~25-30% (still high but -15-20pp from 44.8% via [pass:]+→[obj] expansion)
  walk_past: ~18-22% (GT=8.7%... may overshoot, but semantic benefit > surface match)
  for_a_bit: 0% (GT=0%)
  exit: ~13.6% (GT=18.0%, inherited)
  continue: ~16-17% (GT=10.4%, inherited)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v192_phase2_checkpoint.json (NEW LLM generation)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v192.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v192.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v192_phase2_checkpoint.json"  # v192: NEW LLM generation -- walk_past for intermediate
...
_STRUCTURAL_OBJECTS_RE = re.compile(
    ...
    # v192: RESTORED table/bed/couch — GT "walk past the X" uses these as top waypoints:
    #   stairs(38), kitchen(28), table(24), bed(18), dining(12), couch(11), sink(9), piano(9)
    r'sofa|loveseat|sectional|settee|'  # "sofa" kept out (GT uses "couch"); couch/table/bed restored
    r'armchair|armchairs?|recliner|stool|bench|ottoman|'
    r'coffee\s+table|end\s+table|side\s+table|'  # generic sub-types kept out; "table" restored
    r'desk|counter|countertop|'
    r'mattress|headboard|'  # sub-types kept out; "bed" restored
...
- When a turn has → [object ahead] after the arrow: describe moving PAST that object — write "walk past the X" (3-5 words). FORBIDDEN: "walk to the X", "go to the X" for intermediate objects. Example: → [couch] → "walk past the couch and turn right".
...
- CRITICAL: NEVER write "walk to the X" for intermediate objects in the middle of the path. "walk to the X" is ONLY for the FINAL stop destination. For objects you pass along the way: use "walk past the X".
- MOTION WORDS: Use natural walking verbs: "walk", "go". Chain movements naturally: "turn left into the kitchen". For intermediate landmarks: "walk past the X". For final destination: "walk to the X" then "stop near the Y". AVOID "proceed", "go forward", "walk forward", "keep going", "head toward". Use "towards" (not "toward") when indicating destination direction. Use "continue" ONLY when no new visual landmark is present — prefer "walk" or "go" as primary verbs.
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    if n_ahead > 0:
        anchor_note += (f" Route also has {n_ahead} [X ahead] marker(s) — after turning at"
                        f" [landmark], write 'walk past the X' (3-5 words max). NEVER 'walk to the X' for these.")
...
async def main():
    ...
    print('=== Gate 4 v192: walk_past for intermediate objects + expanded [pass:] eligibility ===')
    ...
    print(f"  v192 NEW FIXES (on top of v190):")
    print(f"    FIX_A: SYSTEM_INTRO + anchor_note: walk_past_the for intermediate objects (NOT walk_to)")
    print(f"    FIX_B: [pass:] eligibility expanded — table/bed/couch RESTORED (GT top waypoints!)")
    print(f"  v190 INHERITED FIXES:")
    ...
    print(f"  v192: [pass:] blacklist UPDATED — table/bed/couch RESTORED (GT uses these!); sofa/armchair/desk/lamp remain blocked")
