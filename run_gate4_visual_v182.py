#!/usr/bin/env python3
"""
Gate 4 Visual v182 -- FUNDAMENTAL FIX: remove go_straight→walk_ahead, restore go_straight as fallback.

DEEP RESEARCH (v24 analysis vs v181):
  v24 SR=40.24% (BEST EVER): go_straight=23% in output (kept as-is, NOT converted to walk_ahead)
  v181 SR=?: walk_ahead=22%, walk_forward=30% — both wrong (GT=6% each)

  KEY INSIGHT: go_straight→walk_ahead conversion (introduced after v24) is ROOT CAUSE of overuse.
  v24 KEPT go_straight as-is, and the model handled 23% go_straight fine → SR=40.24%
  Our conversions redistribute go_straight → walk_ahead/walk_forward, BOTH of which are wrong:
    - walk_ahead=22% vs GT=6% (3.7×)
    - walk_forward=30% vs GT=6% (5×)
    - go_straight=0% vs GT=7.9% (missing!)

  GT distribution: go_straight=7.9%, walk_ahead=6.1%, walk_forward=6.1% = 20.1% total straight-motion
  v181 distribution: go_straight=0%, walk_ahead=22%, walk_forward=30% = 52% total straight-motion
  52% vs 20% → 2.6× MORE explicit straight-motion than GT (overcorrected!)

FUNDAMENTAL FIX: Remove go_straight→walk_ahead conversion; keep go_straight as-is.
  v24 evidence: go_straight=23% AND SR=40.24% — model understands go_straight fine
  The conversion was introduced to "match GT's walk_ahead=6.1%" but actually HURT us
  because it multiplied the explicit straight-motion problem

FIX 1: REMOVE line: `text = re.sub(r'\bgo straight\b', 'walk ahead', text)` (v176 addition)
  go_straight remains as-is from 50% skip episodes → go_straight≈12-15% in output
  Expected: walk_ahead dramatically drops from 22%→~10% (LLM-generated + walk_straight conversion)
  walk_forward also drops since the fallback in replace_go_straight... changes

FIX 2: Change replace_go_straight_with_landmarks fallback: "walk forward" → "go straight"
  Currently: if no landmark → "walk forward" (adds to walk_forward)
  Fix: if no landmark → "go straight" (keeps vocabulary in GT-known range)
  Expected: walk_forward drops from 30%→~12% (only LLM-generated walk_forward survives)
  go_straight total ≈ 12% (50% skip × avg occurrence) + 12% (50% run fallback) = 20-24%

FIX 3: Remove standalone walk_ahead→walk_forward (v181 FIX1 — no longer needed)
  With walk_ahead now only 10%, redistribution to walk_forward is unnecessary
  Removing to simplify and avoid the walk_forward spike

INHERITED from v181 (rest):
  - go_start P=0.70 (toward GT=18.5%)
  - enter P=0.75 for walk_to (incremental improvement)
  - stop-phrase merger P=0.45 (calibrated toward GT 2-sent=42%)
  - walk_ahead/fwd prep reduction (v177/v178)
  - sentence merger s1≤12, combined≤20, P_MERGE=0.75 (v178)
  - stop_dir P=0.06 (v178)

Expected v182:
  avg_words ≈ 27.0w (go_straight is 2w same as walk_ahead/walk_forward)
  go_straight ≈ 15-20% (re-introduced, matching v24 behavior)
  walk_ahead ≈ 10% (only LLM-generated + walk_straight conversion)
  walk_forward ≈ 12% (only LLM-generated)
  Total straight-motion ≈ 37-42% (still high vs GT=20%, but much better than v181's 52%)
  2-sent ≈ 43-47%
  go_start ≈ 14-17%
  enter ≈ 10%

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v180_phase2_checkpoint.json (REUSE v180 — assembly-only changes)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v182.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v182.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v182_phase2_checkpoint.json"  # v182: FRESH (use v180 P2 results)
...
async def main():
    ...
    print('=== Gate 4 v182: FUNDAMENTAL FIX — remove go_straight→walk_ahead, restore go_straight ===')
    print(f"  v181 stats: walk_ahead=22%, walk_forward=30%, go_straight=0% (GT: 6%/6%/8%)")
    print(f"  ROOT CAUSE: go_straight→walk_ahead conversion multiplied straight-motion overuse")
    print(f"  v24 evidence: go_straight=23% → SR=40.24% — model handles go_straight fine!")
    print(f"  v182 FIXES:")
    print(f"    FIX1: REMOVE go_straight→walk_ahead conversion → go_straight≈15-20%")
    print(f"    FIX2: replace_go_straight fallback 'walk forward'→'go straight'")
    print(f"    FIX3: REMOVE standalone walk_ahead→walk_forward (v181 FIX1 — not needed)")
    print(f"  INHERITED: go_start P=0.70, enter P=0.75, stop-phrase merger P=0.45")
    print(f"  Expected: go_straight≈15-20%, walk_ahead≈10%, walk_forward≈12%, avg_words≈27.0")
    ...
    for ep in episodes:
        ...
        # v182: REMOVED standalone walk_ahead→walk_forward (was v181 FIX1)
        # No longer needed since walk_ahead comes only from LLM + walk_straight conversion → ~10%
        # The walk_ahead→walk_forward redistribution in v181 just moved the problem, not fixed it
        ...
        # v180: P=0.75 → 2-sent=50% (8pp above GT=42%); v181: P=0.45 → ~43% (near GT)
        # Calibration: stop-phrase merger added ~17pp at P=0.75 → P=0.45 contributes ~10pp → 2-sent≈43%
        _stop_merge_rng = random.Random(eid ^ 0xF0E1_D2C3)
        if _stop_merge_rng.random() < 0.45:
