#!/usr/bin/env python3
"""
Gate 4 Visual v181 -- walk_ahead reduction + 2-sent calibration + go_start boost.

DEEP RESEARCH FINDINGS (v180 vs GT analysis):
  v180 stats: walk_ahead=33.0% (GT=6.1%), 2-sent=50.0% (GT=42%), go_start=12.6% (GT=18.5%)
  walk_ahead remains massively overused: go_straight→walk_ahead conversion creates ~33%
  2-sent=50% slightly above GT=42% — stop-phrase merger P=0.75 too aggressive
  go_start=12.6% still below GT=18.5% — P=0.50 not sufficient

ROOT CAUSE: walk_ahead comes mainly from:
  (1) go_straight → walk_ahead post-proc conversion (large driver)
  (2) walk_straight → walk_ahead post-proc conversion
  (3) LLM-generated "walk ahead" directly
  v178 prep reduction already converted "walk ahead [prep]" → "walk [prep]"
  Remaining walk_ahead=33% is STANDALONE (no following preposition)

FIX 1: standalone walk_ahead → walk_forward REDUCTION (P=0.40)
  "walk ahead" (no following to/past/through/etc.) → "walk forward" at 40% per match
  Expected: walk_ahead 33%→~20% (P=0.40 reduces standalone by 40%)
  walk_forward increases slightly — still in GT-friendly range
  Word count: neutral (both 2w)

FIX 2: stop-phrase merger P = 0.75→0.45
  v180: P=0.75 → 2-sent=50% (8pp above GT=42%)
  Reducing to P=0.45 should bring 2-sent back toward 42-44%
  Calibration: stop-phrase merger added ~17pp to 2-sent vs v179 (no merger)
  P=0.45 contribution: ~17pp × (0.45/0.75) = ~10pp → 2-sent ≈ 33% + 10% = 43%

FIX 3: go_start P = 0.50→0.70 (toward GT=18.5%)
  v180: P=0.50 → go_start=12.6% (GT=18.5%)
  P=0.70 expected: 12.6% × (0.70/0.50) = 17.6% ≈ GT=18.5%

FIX 4: enter P for walk_to = 0.60→0.75
  v180: P=0.60 → enter=8.5% (GT=17%)
  P=0.75 expected: ~10-11% enter (incremental improvement toward GT)

INHERITED from v180:
  - walk_ahead/fwd prep reduction (v177/v178)
  - stop_dir P=0.06 (v178)
  - sentence merger s1≤12, combined≤20, P_MERGE=0.75 (v178)
  - enter via route notation P=0.40 (v172)
  - walk_into_room→enter conversion P=0.30 (v173)
  - walk_to→enter P=0.75 (FIX 4 above)
  - go_start P=0.70 (FIX 3 above)

Expected v181:
  avg_words ≈ 27.0w (neutral; walk_ahead→walk_forward word-neutral)
  2-sent ≈ 42-44% (GT=42% — stop-phrase merger calibrated)
  walk_ahead ≈ 20% (down from 33%)
  walk_forward ≈ 20% (up from ~8%)
  go_start ≈ 17-18% (GT=18.5%)
  enter ≈ 10-11%

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v180_phase2_checkpoint.json (REUSE v180 — only assembly changes)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v181.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v181.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v181_phase2_checkpoint.json"  # v181: FRESH (assembly-only changes — use v180 P2 results)
...
async def main():
    ...
    print('=== Gate 4 v181: walk_ahead reduction + 2-sent calibration + go_start boost ===')
    print(f"  v180 stats: walk_ahead=33.0% (GT=6.1%), 2-sent=50.0% (GT=42%), go_start=12.6% (GT=18.5%)")
    print(f"  v181 FIXES:")
    print(f"    FIX1: standalone walk_ahead → walk_forward P=0.40 → walk_ahead 33%→~20%")
    print(f"    FIX2: stop-phrase merger P=0.75→0.45 → 2-sent 50%→~43% (GT=42%)")
    print(f"    FIX3: go_start P=0.50→0.70 → go_start 12.6%→~17-18% (GT=18.5%)")
    print(f"    FIX4: enter P for walk_to: 0.60→0.75 → enter ~10-11%")
    print(f"  INHERITED: walk_ahead/fwd prep reduction, stop_dir P=0.06, sentence merger s1≤12/total≤20")
    print(f"  Expected: avg_words≈27.0, 2-sent≈42-44%, walk_ahead≈20%, go_start≈17-18%")
    ...
    for ep in episodes:
        ...
        # v181: standalone walk_ahead → walk_forward REDUCTION (P=0.40)
        # walk_ahead=33.0% (GT=6.1%); v178 prep reduction converted "walk ahead [prep]"→"walk [prep]"
        # Remaining walk_ahead=33% is standalone (no following prep) — convert 40% → walk_forward
        # walk_ahead and walk_forward are both 2w → word-count neutral
        _walk_ahead_standalone_re = re.compile(
            r'\bwalk ahead(?!\s+(to|past|through|into|toward|towards|along|up|down))\b',
            re.IGNORECASE
        )
        _wa_standalone_rng = random.Random(eid ^ 0xA5B6_C7D8)
        def _replace_standalone_walk_ahead(m):
            if _wa_standalone_rng.random() < 0.40:
                cap = m.group(0)[0].isupper()
                return 'Walk forward' if cap else 'walk forward'
            return m.group(0)
        text = _walk_ahead_standalone_re.sub(_replace_standalone_walk_ahead, text)
