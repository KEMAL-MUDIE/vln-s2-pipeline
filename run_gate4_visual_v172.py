...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v172.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v172_phase2_checkpoint.json"  # v172: FRESH — goal_vis color strip + enter route + 2-sent max
        ...
        if t == "straight":
            ...
            if lm_clean and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    ...
                    use_enter = not add_dir and rng.random() < 0.40  # v172: 40% of no-dir transitions use "enter"
                    if add_dir:
                        if use_door:
...
- SENTENCE STRUCTURE: Write 1-3 sentences maximum. Chain consecutive actions with "and" or comma+then: "turn left and walk through the hallway", "walk to the sofa, then turn right". Chain the STOP: "walk to the X and stop near the Y." NEVER write one action per sentence. Real R2R instructions: "Exit the room and turn left. Walk to the sofa and stop there." (2 sentences, NOT 4).
...
async def main():
    ...
    print('=== Gate 4 v172: GOAL_VIS COLOR STRIP + ENTER ROUTE + 2-SENT MAX ===')
    print(f"  v171 MEASURED: anchor%=24.3%, avg_turns=1.09, sent(3)=64.5%, sent(4)=16.7%")
    print(f"  v171 sent chaining: 4-sent 30%→16.7% (good), but 3-sent stayed high (64.5%→64.5%)")
    print(f"  v171 enter%=1.3% (LLM IGNORED rule — needs route-level fix)")
    print(f"  FIX 1 (HIGHEST IMPACT): goal_vis color stripping")
    print(f"         69.5% of stop phrases had color words (GT=10.9%)")
    print(f"         Strip colors from goal_vis BEFORE building prompt → LLM sees color-free goal")
    print(f"         Expected: color_stop% GT=10.9%, target <20% (was 69.5%)")
    print(f"  FIX 2: 'enter room' ROUTE NOTATION (40% of no-dir transitions)")
    print(f"         v171 rule ignored (LLM wrote 'into the room' anyway)")
    print(f"         Route says 'enter bedroom' → LLM MUST write 'enter the bedroom' (literal)")
    print(f"         Expected: enter%=5-8% (GT=5.4%, was 1.3%)")
    print(f"  FIX 3: Tighter sentence rule '1-3 max' + real example in prompt")
    print(f"  KEPT: P_ANCHOR=0.06, P_HALLWAY=30%, P_TURN_UNANCHORED=1.0, ANTI-COLOR, nav-verb fixes")
    print(f"  v172: Expected color_stop%<20%, enter%≈5-8%, anchor%≈16-18%, GT-match≥0.93")
