#!/usr/bin/env python3
"""
Gate 4 Visual v199 -- FIX16: FIX8 Diversification (adversarially-validated walk_past reduction).

BASE: v198 (FIX1-15 — walk_to=7.3%, walk_past=49.6%, walk_toward=18.1%, GT-match=0.855)

MOTIVATION:
Deep research + adversarial verification identified FIX8 as injecting walk_past unconditionally
for all "go straight" replacements. Replace 100% walk_past injection with a probabilistic mix.
This is the ONLY walk_past change explicitly approved by the adversarial verification pass.

NEW IN v199:
FIX16: FIX8 Diversification — replace_go_straight_with_landmarks output diversification.
  PROBLEM: FIX8 injects "walk past the {lm}" 100% of the time for all "go straight"
           replacements → contributes ~15.9pp to walk_past (v198=49.6% vs GT=8.7%).
           Assembly-introduced walk_past at catastrophic scale.
  FIX: Probabilistic replacement using per-episode seeded RNG:
       P=0.51: "walk past the {lm}"   (GT-dominant intermediate waypoint phrase)
       P=0.24: "walk toward the {lm}" (adds directional variety, GT-present)
       P=0.25: "walk to the {lm}"     (FIX7 converts 90% → walk_past/walk_toward mix)
  EXPECTED: walk_past ≈44% (−5.6pp from v198=49.6%)
            walk_toward ≈23% (FIX8 adds +5.2pp via 24% direct + 25%×90%×40%)
            walk_to ≈7.7% (FIX8 injects 25%×10% uncaptured = +0.4pp, trivial)
  ADVERSARIAL STATUS: Validated safe. Option D from 8-agent deep research synthesis.
  Seed: random.Random(eid ^ 0xF1F2_F3F4) — unique from _gs_skip_rng (eid ^ 0x60_5700)

INHERITED FROM v198: FIX1-15 (OOV, walk-and, stop_cap, turn_diversity, surface quality)
INHERITED FROM v197: FIX1-10+FIX10b (walk_to=7.3%, walk_past=49.6%, walk_toward=18.1%)
ASSEMBLY-ONLY: Reuse outputs/gate4_v199_phase2_checkpoint.json (copy of v190 P2).

Expected v199:
  walk_to: ~7.7% (GT=5.1%) — +0.4pp from FIX8 residual walk_to (trivial)
  walk_past: ~44% (GT=10.5%) — −5.6pp from FIX8 diversification [FIX16]
  walk_toward: ~23% (GT=4.9%) — +5.2pp from FIX8 diversification [FIX16]
  All v198 surface fixes (FIX11-15) inherited unchanged.
  Predicted SR: 63-72% (conservative +1-3pp from v198, synergy with OOV/stop/turn fixes)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v199_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v199.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v199.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v199_phase2_checkpoint.json"  # v199: assembly-only, copy of v190 P2 (FIX16)
