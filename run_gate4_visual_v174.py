#!/usr/bin/env python3
"""
Gate 4 Visual v174 -- On-direction door injection + sentence merger + walk_forward reduction.

KEY IMPROVEMENTS over v173:
  v173: color_stop%=9.1%(GT~10.9%)✓, enter=6.7%(GT=5.4%)✓, couch=8.2%(GT=7.2%)✓
  v173 REMAINING GAPS:
    on_left:  0.1% vs GT=12.9%  (HUGE GAP — doors/doorways qualified with direction)
    on_right: 0.0% vs GT=13.3%
    sentence: 3-sent=63.2% vs GT=31.0% (GT peaks at 2-sent=33.8%)
    walk_forward: 25.2% vs GT=6.1% (LLM ignores AVOID rule)
    avg_words: 24.6 vs GT=26.8 (-2.2w)

FIX 1: "door on the left/right" INJECTION (P_DOOR_DIR=0.50)
  GT pattern: "through the door on the left" (24x), "through the doorway on the right" (18x)
  GT: 23.8% of episodes have "on the left/right" — all for door disambiguation at choice points
  v174: When use_door=True and direction is known: P_DOOR_DIR=0.50 → inject "on the {direction}"
  Route: "turn left (door→ bedroom)" → "turn left (door on the left → bedroom)" [50% of cases]
  Route: "(door→ bedroom)" → "(door on the left → bedroom)" when direction known [50% of cases]
  Expected: on_left≈10%, on_right≈10%, matching GT's 23.8% combined

FIX 2: SENTENCE MERGER POST-PROCESSING
  GT: 2-sent=33.8% peak, 1-sent=18.9%, 3-sent=31.0%
  v173: 3-sent=63.2% dominant — model trained on GT expects shorter chained instructions
  Post-process: If S1 (≤10 words, ends with ".") + S2 (starts with action verb):
    "Turn left. Enter the bedroom." → "Turn left and enter the bedroom."
  Expected: 2-sent rises from 18.9% to 35%+, 3-sent drops from 63.2% to 40%

FIX 3: walk_forward/go_straight REDUCTION
  walk_forward=25.2% vs GT=6.1% — LLM ignores AVOID rule → post-processing
  "walk forward to/past/through/into" → "walk to/past/through/into"
  "walk forward" (standalone) → "continue"
  "go straight to/past/through" → "walk to/past/through"
  Expected: walk_forward drops from 25.2% to ~6%, go_straight drops from 25% to ~8%
  6. FRESH Phase 2 checkpoint

Expected: anchor%~16.5% (GT=16.5%), avg_turns~0.7-0.9, GT-match>=0.97

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v168_phase2_checkpoint.json (FRESH)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v168.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v174.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v167: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v174_phase2_checkpoint.json"  # v174: FRESH — on-direction + sentence merger + walk_forward reduction
...
- SENTENCE STRUCTURE: PREFER 2 sentences. Chain ALL consecutive actions with "and": "turn left and enter the bedroom", "walk to the couch and stop". Chain the STOP too: "walk forward and stop near the table." NEVER write one action per sentence. If you have 3 actions, use 2 sentences by chaining 2 of them: "Turn left and walk through the hallway. Stop by the table." NOT "Turn left. Walk through the hallway. Stop by the table." Real R2R: "Exit the bedroom and turn left. Walk to the couch and stop there." (2 sentences, NOT 4).
...
- DISTANCE: The route shows 'straight Xm' entries — NEVER write metric numbers or "take a few steps". NEVER use "walk forward", "go forward", "go straight", "walk straight" — these are NOT in GT. For short segments (≤2m): skip them or chain adjacent movements (e.g. "turn left into the kitchen"). For medium segments (3-8m): use "walk" or "continue" briefly — one phrase. For long segments (>8m): use "continue" or "walk" toward the next landmark — one phrase, NO "straight".
...
def replace_go_straight_with_landmarks(text: str, phase_c_descs: Dict[str, str]) -> str:
    ...
    def _replace_gs(m):
        ...
        if lm_idx[0] < len(landmarks):
            ...
            replacement = "walk forward"  # v156: changed from 'walk straight' to reduce walk_straight%
...
async def main():
    ...
    print('=== Gate 4 v174: ON-DIRECTION DOORS + SENTENCE MERGER + walk_forward REDUCTION ===')
    print(f"  v173 MEASURED: color_stop%=9.1%(GT~10.9%)✓, enter=6.7%(GT=5.4%)✓")
    print(f"    couch=8.2%(GT=7.2%)✓, towards=11%(GT=12.4%)✓")
    print(f"  v173 REMAINING GAPS:")
    print(f"    on_left=0.1% vs GT=12.9% — 90% of GT on-dir usage is for door disambiguation")
    print(f"    sentence: 3-sent=63.2% vs GT peak=2-sent=33.8%")
    print(f"    walk_forward=25.2% vs GT=6.1% (LLM ignores AVOID rule)")
    print(f"  FIX 1: 'door on the left/right' injection (P_DOOR_DIR={P_DOOR_DIR})")
    print(f"         Route: 'turn left (door on the left → bedroom)' → LLM writes 'through the door on the left'")
    print(f"         GT: 24x 'door on the left', 18x 'doorway on the right' in val_unseen")
    print(f"         Expected: on_left≈10%, on_right≈10% → combined≈20% (GT=23.8%)")
    print(f"  FIX 2: SENTENCE MERGER post-processing")
    print(f"         'Turn left. Enter the bedroom.' → 'Turn left and enter the bedroom.'")
    print(f"         Merges when S1≤9w, S2 starts action verb, combined≤15w, S1 not stop")
    print(f"         Expected: 2-sent rises from 18.9% to 35%+, 3-sent drops from 63.2% to 40%")
    print(f"  FIX 3: walk_forward/go_straight REDUCTION post-processing")
    print(f"         'walk forward to/past/through' → 'walk to/past/through'")
    print(f"         'walk forward' alone → 'continue'")
    print(f"         'go straight' alone → 'continue'")
    print(f"         Expected: walk_forward drops from 25.2% to ~6%, go_straight ≈ 8%")
    print(f"  INHERITS v173: color strip, sofa→couch, enter postproc, towards allowed, goal_vis strip")
    print(f"  GT: walk_straight=8.8%, go_straight=7.9%, avg_turns=0.59, turn-anchor=16.5%")
    print(f"  Expected: go_straight~8%, walk_forward~6%, on_left≈10%, on_right≈10%, 2-sent dominant")
    ...
    for ep in episodes:
        ...
        # v174: walk_forward/go_straight REDUCTION
        # GT: walk_forward=6.1% but v173=25.2% — LLM ignores AVOID rule → post-process
        # "walk forward to/past/through/into/towards" → "walk to/past/through/into/towards"
        text = re.sub(r'\bwalk forward (to|past|through|into|towards|toward|along)\b',
                      lambda m: f'walk {m.group(1)}', text, flags=re.IGNORECASE)
        # "go straight to/past/through/into/towards" → "walk to/past/through/into/towards"
        text = re.sub(r'\bgo straight (to|past|through|into|towards|toward|along)\b',
                      lambda m: f'walk {m.group(1)}', text, flags=re.IGNORECASE)
        # "walk straight to/past/through/into" → "walk to/past/through/into"
        text = re.sub(r'\bwalk straight (to|past|through|into|towards|toward|along)\b',
                      lambda m: f'walk {m.group(1)}', text, flags=re.IGNORECASE)
        # "walk forward" alone (not followed by navigational preposition) → "continue"
        text = re.sub(r'\bwalk forward\b(?!\s+(?:to|past|through|into|towards|toward|and|along|around))',
                      'continue', text, flags=re.IGNORECASE)
        # "go straight" alone → "continue" (LLM uses this for long featureless segments)
        text = re.sub(r'\bgo straight\b(?!\s+(?:to|past|through|into|towards|toward|and|along|ahead))',
                      'continue', text, flags=re.IGNORECASE)

        # v174: SENTENCE MERGER — merge consecutive short action sentences to get 2-sent dominant
        # GT: 2-sent=33.8% peak. v173: 3-sent=63.2% dominant. Model trained on GT expects shorter.
        # "Turn left. Enter the bedroom." → "Turn left and enter the bedroom." (1 sentence)
        ...
        def _merge_sentences(t):
            ...
            while i < len(sents):
                ...
                if i + 1 < len(sents):
                    ...
                    # Merge conditions: s1 short, s2 starts action verb, combined ≤16w, s1 doesn't stop
                    can_merge = (
                        s1.rstrip().endswith('.') and
                        not re.search(r'\b(stop|wait)\b', s1, re.I) and
                        s1_words <= 9 and
                        (s1_words + s2_words) <= 15 and
                        bool(_ACTION_VERBS_RE.match(s2)) and
                        not re.search(r'\band\s+\w+[.,]?\s*$', s1, re.I)  # s1 already ends with "and X"
                    )
                    if can_merge:
                        s1_base = s1.rstrip('.').rstrip()
                        s2_lc = s2[0].lower() + s2[1:]  # lowercase first char of s2
