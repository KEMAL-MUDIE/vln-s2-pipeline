#!/usr/bin/env python3
"""
Gate 4 Visual v156 — Fix walk_straight fallback paths + fix avg_turns + introduce go_straight.

v155 ACTUAL: GT-match=0.962✓, toward_the=1.1%✓, take_few=0.2%✓, turn-anchor=18.1%✓
  Remaining gaps:
  1. walk_straight=17.1% (GT=8.8%): assembly fallback paths still inject "walk straight":
     - Line 1672: _cap_go_forward → 'walk straight' for 2nd+ go_forward
     - Line 1686: _suppress_extra_walk_into → 'walk straight' for 2nd+ walk_into
     - Line 1696: _suppress_extra_walk_past → 'walk straight' for 2nd+ walk_past
     - Line 1813: replace_go_straight_with_landmarks fallback → 'walk straight' when no landmarks
     - Line 1655: _ws_variants diversity includes 'walk straight ahead' (25% of walk_straight hits)
  2. avg_turns=0.90 (GT=0.59): P_TURN_UNANCHORED=0.28 too high
  3. go_straight=0.0% (GT=7.9%): replace_go_straight_with_landmarks converts ALL go_straight

v156 CHANGES from v155:
  1. Lines 1672/1686/1696: 'walk straight'→'go straight' so step4 (replace_go_straight_with_landmarks)
     uses landmark substitution or walk_forward fallback instead of walk_straight.
  2. Line 1813: replace_go_straight fallback 'walk straight'→'walk forward' (avoids walk_straight).
  3. Line 1655 _ws_variants: remove 'walk straight ahead' from alternatives (→'walk ahead' instead).
  4. P_TURN_UNANCHORED: 0.28→0.18 (reduce avg_turns from 0.90 toward GT=0.59; ratio=0.59/0.90=0.656).
  5. go_straight injection: 8% of episodes skip replace_go_straight_with_landmarks (GT=7.9%).

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C)
Phase 2 checkpoint: outputs/gate4_v156_phase2_checkpoint.json (FRESH)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v156.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v156.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v156: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v156_phase2_checkpoint.json"  # v156: FRESH — walk_straight fallback fix + avg_turns fix
...
P_TURN_UNANCHORED = 0.18  # v156: 0.28→0.18 to reduce avg_turns from 0.90 toward GT=0.59 (ratio=0.59/0.90=0.656)
...
def clean(raw, sv):
    ...
    _ws_variants = [
        ...
        # v156: removed 'walk straight ahead' from alts (still counted as walk_straight); use 'walk ahead' instead
        (r'\bwalk straight\b',                   ['walk ahead', 'go straight ahead', 'go straight', 'walk forward']),
    ]
...
async def main():
    ...
    print('=== Gate 4 v156: Fix walk_straight fallback paths + avg_turns + introduce go_straight ===')
    print(f"  v155 METRICS: GT-match=0.962✓, toward_the=1.1%✓, take_few=0.2%✓, turn-anchor=18.1%✓")
    print(f"  v155 GAPS: walk_straight=17.1%(GT=8.8%), avg_turns=0.90(GT=0.59), go_straight=0.0%(GT=7.9%)")
    print(f"  v156 FIX 1: _cap_go_forward/walk_into/walk_past fallbacks → 'go straight' (not 'walk straight')")
    print(f"  v156 FIX 2: replace_go_straight fallback → 'walk forward' (not 'walk straight')")
    print(f"  v156 FIX 3: _ws_variants removes 'walk straight ahead' → 'walk ahead' instead")
    print(f"  v156 FIX 4: P_TURN_UNANCHORED 0.28→0.18 (reduce avg_turns 0.90→~0.59)")
    print(f"  v156 FIX 5: 8% episodes skip replace_go_straight_with_landmarks (go_straight→~8%, GT=7.9%)")
    print(f"  GT: walk_straight=8.8%, go_straight=7.9%, avg_turns=0.59, turn-anchor=16.5%")
    print(f"  Expected: walk_straight~9%, go_straight~8%, avg_turns~0.60, GT-match≥0.96")
    ...
    print(f"  v105: HIGH-TURN — P_TURN_UNANCHORED=0.18 (v156 reduced from 0.28)")
    ...
    for ep in episodes:
        ...
        # v156: 8% of episodes skip replacement → allows "go straight" to survive (GT go_straight=7.9%).
        # Seed derived from eid to be deterministic per episode.
        _gs_skip_rng = random.Random(eid ^ 0x60_5700)
        if _gs_skip_rng.random() >= 0.08:
    ...
    print(f"  go_straight:   {100*go_straight_n/n:.1f}%  (GT=7.9%, v155=0.0%) ← v156 target ~8%")
    print(f"  walk_straight: {100*wlk_straight_n/n:.1f}%  (GT=8.8%, v155=17.1%) ← v156 target ~9%")
    ...
    print(f"  [v156 targets: walk_straight~9%(GT=8.8%), go_straight~8%(GT=7.9%), avg_turns~0.60(GT=0.59), GT-match≥0.96]")
