#!/usr/bin/env python3
"""
Gate 4 Visual v23 — Dual-Segment Intermediate Waypoint Walking Landmarks

IMPROVEMENT OVER v22:
  v22 used the LAST segment midpoint only (before goal).
  v23 uses BOTH the FIRST and LAST segment midpoints (when both exist).

  Result: 88.6% coverage (vs v22 68.7%) and 300 episodes get 2 "passing" phrases.

  Phase 1b checkpoint REUSED from v22: gate4_v22_midpoint_p1_checkpoint.json
  Phase 1 checkpoint REUSED from v19: gate4_v19_phase1_checkpoint.json

  Phase 2 — Instruction generation with dual midpoint walking landmarks:
    For each episode, pick FIRST and LAST segment midpoints (up to 2 total).
    Add [pass: X] in route before each chosen segment's endpoint.
    [pass: X] rule: "robot passes X while walking — write 'passing the X'."
    Allow up to 2 'passing the X' phrases per instruction.
    Checkpoint: outputs/gate4_v23_phase2_checkpoint.json
    Output: outputs/datasets/val_unseen_generated_gemma_visual_v23.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v23.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v23_phase2_checkpoint.json"
...
async def main():
    ...
    print("=== Gate 4 v23: Dual-Segment Intermediate Waypoint Walking Landmarks ===")
    ...
    print(f"  v23 key change: first AND last segment midpoints → up to 2 [pass: X] per episode")
    print()
