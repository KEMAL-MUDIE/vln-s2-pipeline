#!/usr/bin/env python3
"""
Gate 4 Visual v57 — v56 base + walk_past reduction via P_PASS_MARKER reduction

v56 RESULTS (base):
  GT-match: 0.958, toward_the: 3.3% (GT=3.8% — near perfect!), through_the: 28.0% (GT=27.2%)
  walk_past: 13.6% (GT=8.7%), hallway: 26.0% (GT=20.4%), avg_words: 26.2

REMAINING GAPS (v56 vs GT):
  walk_past:  13.6% vs GT=8.7% — 4.9pp over, main source: [pass:] markers (P_PASS_MARKER=0.50)
  hallway:    26.0% vs GT=20.4% — 5.6pp over
  avg_turns:  0.71 vs GT=0.59  — medium gap

v57 ROOT CAUSE ANALYSIS (walk_past):
  [pass:] markers drive walk_past. P_PASS_MARKER=0.50 → ~50% of episodes with pass-type midpoints
  get a [pass: X] marker → LLM writes "walk past the X".
  GT walk_past=8.7% suggests only ~30% of episodes should have [pass:] markers.
  Fix: Reduce P_PASS_MARKER from 0.50 → 0.30 (40% reduction).
  Math: 13.6% × (0.30/0.50) ≈ 8.2% → close to GT=8.7%
  (Note: relationship not perfectly linear since → rule also contributes, but good approximation)

v57 FIX:
  - P_PASS_MARKER: 0.50 → 0.30
  - All other settings identical to v56 (same → rule examples, same post-processing)
  - P_PASS_MARKER change alters route prompts passed to Phase 2 LLM → fresh generation needed
  Expected: walk_past 13.6% → ~9-11% (close to GT=8.7%), GT-match unchanged ~0.958

Phase 2 checkpoint: outputs/gate4_v57_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v57.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v57.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v57_phase2_checkpoint.json"
...
P_PASS_MARKER = 0.30   # v57: further reduced (was 0.50) → target ~9-10% walk-past (GT=8.7%)
...
async def main():
    ...
    print('=== Gate 4 v57: walk_past fix — P_PASS_MARKER reduced 0.50→0.30 (target GT=8.7%) ===')
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v56=0.71) ← v57: P_PASS_MARKER=0.30")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v56=3.3%, v54fix=10.1%) ← KEY METRIC")
