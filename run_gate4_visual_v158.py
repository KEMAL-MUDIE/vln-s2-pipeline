#!/usr/bin/env python3
"""
Gate 4 Visual v158 — Disable replace_go_straight_with_landmarks + reduce avg_turns further.

SR HYPOTHESIS: v24 (go_straight=23%) got 40.24% SR (BEST non-GT), while all calibrated versions
  (v130+) get only ~34% SR despite better GT-match. Hypothesis: converting "go straight" →
  "walk to the [landmark]" hurts SR. v24 kept simple navigation commands ("go straight",
  "turn left into the hallway") instead of complex visual-landmark references.

v157 ACTUAL: GT-match=0.974✓, walk_straight=9.0%≈GT✓, go_straight=6.8%≈GT, avg_turns=0.80

v158 CHANGES from v157:
  1. replace_go_straight_with_landmarks DISABLED (100% skip) — main SR experiment
     Expected: go_straight rises from 6.8% → ~40-45% (raw LLM go_straight rate)
     This tests if keeping simple "go straight" commands improves SR like v24 did.
  2. P_TURN_UNANCHORED: 0.12→0.09 (avg_turns 0.80→~0.60 ≈ GT=0.59)
     Scaling: 0.80*(0.09/0.12)=0.60 → should reach ~0.60 (GT=0.59) ✓

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C)
Phase 2 checkpoint: outputs/gate4_v158_phase2_checkpoint.json (FRESH: new P_TURN_UNANCHORED)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v158.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v158.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v158: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v158_phase2_checkpoint.json"  # v158: FRESH — disabled replace_go_straight + lower avg_turns
...
async def main():
    ...
    print('=== Gate 4 v158: Disable replace_go_straight_with_landmarks + reduce avg_turns ===')
    print(f"  v157 METRICS: GT-match=0.974✓, walk_straight=9.0%✓, go_straight=6.8%, avg_turns=0.80")
    print(f"  v157 GAPS: go_straight=6.8%(GT=7.9%), avg_turns=0.80(GT=0.59), through_the=33.7%(GT=27.2%)")
    print(f"  v158 CHANGE 1: replace_go_straight_with_landmarks DISABLED (100% skip)")
    print(f"    Expected go_straight: ~40-45% (raw LLM rate, vs v157=6.8%, GT=7.9%)")
    print(f"    SR HYPOTHESIS: v24 go_straight=23% got SR=40.24% (BEST). More 'go straight' = better SR?")
    print(f"  v158 CHANGE 2: P_TURN_UNANCHORED 0.12→0.09 (avg_turns 0.80→~0.60 ≈ GT=0.59)")
    ...
    for ep in episodes:
        ...
        # v158: replace_go_straight_with_landmarks FULLY DISABLED (100% skip).
        # SR HYPOTHESIS: "go straight" → "walk to the landmark" conversion may HURT SR.
        # v24 (go_straight=23%) got SR=40.24% (BEST non-GT). All calibrated versions (v130+)
        # that convert "go straight" → "walk to X" get only ~34% SR.
        # DISABLED: replace_go_straight_with_landmarks → go_straight stays ~40-45% raw rate.
        # If this improves SR, it validates the hypothesis. v157 skip=15% → go_straight=6.8%.
        # (Previously: v156 skip=8%→3.9%, v157 skip=15%→6.8%, v158 skip=100%→~40-45%)
        pass  # replace_go_straight_with_landmarks disabled in v158
    ...
    print(f"  go_straight:   {100*go_straight_n/n:.1f}%  (GT=7.9%, v157=6.8%) ← v158: DISABLED replace_gs → raw ~40-45% expected")
    print(f"  walk_straight: {100*wlk_straight_n/n:.1f}%  (GT=8.8%, v157=9.0%) ← v158: unchanged from v157")
