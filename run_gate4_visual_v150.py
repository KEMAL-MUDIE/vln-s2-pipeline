#!/usr/bin/env python3
"""
Gate 4 Visual v150 — Direction-aware Phase C auto-annotator + all v149 bug fixes.

v149 ACTUAL: 3 degenerate 'destination' episodes (vs 42 in v148). pred_SR=62.65% (vocab_gap=2.04pp)
             go_straight=0.00% ✓, all v149 assembly bugs fixed.

v150 IMPROVEMENT:
  - Direction-aware Phase 1 auto-annotator: VLM descriptions now include LEFT/RIGHT turn context.
    "Turn left at the brown leather sofa. Ahead, a living area with grey TV opens up."
    vs old generic: "Turn at the large wooden pillar."
  - Fresh Phase 2 generation using new v150 Phase 1 descriptions as LLM context.
  - The LLM now generates instructions with explicit turn directions and more specific landmarks.
  - All v149 bug fixes preserved: remove_loops, quality_ok stop_phrase, Walk fallback verb.

Phase 1 checkpoint: outputs/gate4_v150_phase1_checkpoint.json (NEW: direction-aware, 3192 frames)
Phase 2 checkpoint: outputs/gate4_v150_phase2_checkpoint.json (FRESH generation with v150 P1 context)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v150.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v150.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v150_phase1_checkpoint.json"  # v150: direction-aware Phase C auto-annotator
...
P2_CKPT       = ROOT / "outputs" / "gate4_v150_phase2_checkpoint.json"  # v150: FRESH generation with direction-aware P1 context
...
async def main():
    ...
    print('=== Gate 4 v150: Direction-aware Phase C (LEFT/RIGHT turn prompts) + v149 bug fixes + fresh Phase 2 ===')
