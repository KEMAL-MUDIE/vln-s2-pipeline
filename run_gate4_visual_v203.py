#!/usr/bin/env python3
"""
Gate 4 Visual v203 -- FIX20+FIX21+FIX22+FIX23+FIX19b: walk_past reduction, 3→2 sentence fix, walk_to mop-up.

BASE: v202 (FIX16b+FIX18+FIX19 — walk_past=35.1%, toward=16.5%, continue=17.0%, walk_to=8.5%, GT-match=0.851)

DEEP RESEARCH FINDINGS:
1. walk_past=35.1% (GT=8.7%) — 4x excess. FIX8 at 60% + FIX17b at 35% are main sources.
   FIX8 contributes ~9.3pp walk_past, FIX7 contributes ~25pp walk_past.
   Fix: FIX8 25/15/30/20/10, FIX17b 15/15/30/25/15. Expected walk_past drop: -8pp → 27%.
2. 3-sentence=50.6% (GT=29.3%) — FIX18 only fixed 1.3pp because 92.2% of S1 have "and".
   Root cause: "' and ' not in s1" blocked everything. Fix: use ", then " connector.
   71.2% qualify with FIX22 → -15pp 3-sentence (50.6%→35.5%, GT=29.3%).
3. walk_to=8.5%, 96 occurrences in non-final sentences uncaught by FIX7.
   Root cause: FIX7 regex fails on hyphened objects and prepositional phrases.
   Fix (FIX23): Broad mop-up: simple \bwalk to the\b match, keeps object intact.
4. continue=17.0% (GT=10.4%) — FIX19 only -0.7pp. Need more patterns covered.
   Fix (FIX19b): Add Continue past→Go past(50%), Continue down/through→Walk(30%).

NEW IN v203:
FIX20: FIX8 Redistribution (go_straight→landmark replacement).
  v202: 60/30/10 (walk_past/walk_toward/walk_to)
  v203: 25/15/30/20/10 (walk_past/walk_toward/go_past/pass_the/walk_to)
  Expected: walk_past −5.4pp. go_past +4pp. pass_the +3pp.
  Seed: random.Random(eid ^ 0xF1F2_F3F4) [same seed]

FIX21: FIX7 Output Redistribution (walk_to→landmark conversion phrases).
  v202 FIX17b: 35/24/23/12/6 (walk_past/walk_toward/go_past/pass_the/head_toward)
  v203 FIX21:  15/15/30/25/15 (walk_past/walk_toward/go_past/pass_the/head_toward)
  Expected: walk_past −2.9pp. go_past +3pp. pass_the +4pp.

FIX22: Relaxed FIX18 — ", then" Connector.
  Root cause: 92.2% of 3-sentence S1 have "and" → s1_and check blocks 92%.
  Fix: Remove s1_and check, use ", then " connector, relax length to 50 words.
  71.2% qualify (was 5.2%). At P=0.42: 3-sentence 50.6%→35.5% (GT=29.3%).

FIX23: Broad Walk_to Mop-up in Non-Final Sentences.
  Root cause: FIX7 misses hyphened objects, prepositional phrases → 96 unconverted.
  Fix: Simple \bwalk to the\b replacement in non-final sentences (after FIX7 runs).
  Expected: walk_to −3 to −4pp (8.5%→4.5-5.5%, approaching GT=5.1%).
  Distribution: 15/15/30/25/15 same as FIX21.
  Seed: random.Random(eid ^ 0xC3D4_E5F6)

FIX19b: More Aggressive Continue Reduction.
  "Continue straight" → "Go straight" P=0.50 (was 35%)
  "Continue walking" → "Walk" P=0.40 (was 25%)
  NEW: "Continue past the X" → "Go past the X" P=0.50
  NEW: "Continue [down/along/through/toward/to]" → "Walk [...]" P=0.30
  Expected: continue −2 to −4pp (17.0%→13-15%, GT=10.4%).

INHERITED FROM v202: FIX1-17b, FIX16b, FIX18 structure, FIX19
ASSEMBLY-ONLY: Reuse outputs/gate4_v203_phase2_checkpoint.json (copy of v190 P2).

Expected v203:
  walk_past: ~26-28% (GT=8.7%) — −8pp from v202=35.1% [FIX20+FIX21]
  walk_to: ~4.5-5.5% (GT=5.1%) — −3 to −4pp from v202=8.5% [FIX23]
  go_past: ~10-14% (GT=2.8%) [FIX20+FIX21]
  pass_the: ~8-12% (GT=5.3%) [FIX20+FIX21]
  3-sentence: ~35.5% (GT=29.3%) — −15pp from v202=50.6% [FIX22]
  continue: ~13-15% (GT=10.4%) — −2 to −4pp [FIX19b]
  Predicted SR: 65-74% (central ~69%) — walk_past at 27% meets research target of ~20-25%

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v203_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v203.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v203.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v203_phase2_checkpoint.json"  # v202: assembly-only, copy of v190 P2 (FIX16b+18+19)
...
def replace_go_straight_with_landmarks(text: str, phase_c_descs: Dict[str, str], rng=None) -> str:
    ...
    def _replace_gs(m):
        ...
        if lm_idx[0] < len(landmarks):
            ...
            if rng is not None:
                r = rng.random()
                # v203 FIX20: Radical walk_past reduction in FIX8 — add go_past and pass_the.
                # v202: 60/30/10 (walk_past/walk_toward/walk_to) → walk_past ~9.3pp contribution.
                # v203: 25/15/30/20/10 (walk_past/walk_toward/go_past/pass_the/walk_to).
                # go_past(GT=2.8%) and pass_the(GT=5.3%) are correct "passing" semantics for intermediates.
                # Expected: walk_past FIX8 contribution drops 9.3pp→3.9pp (−5.4pp).
                if r < 0.25:
                    replacement = f"walk past the {lm}"
                elif r < 0.40:
                    replacement = f"walk toward the {lm}"
                elif r < 0.70:
                    replacement = f"go past the {lm}"
                elif r < 0.90:
                    replacement = f"pass the {lm}"
                else:
                    replacement = f"walk to the {lm}"  # FIX7/FIX23 converts 90%
            else:
...
async def main():
    ...
    for ep in episodes:
        ...
        def _pick_passing_replacement(cap, obj):
            # v203 FIX21: Reduce walk_past/walk_toward, boost go_past/pass_the/head_toward.
            # v202 FIX17b: walk_past(35%)/walk_toward(24%)/go_past(23%)/pass_the(12%)/head_toward(6%)
            # v203 FIX21:  walk_past(15%)/walk_toward(15%)/go_past(30%)/pass_the(25%)/head_toward(15%)
            # Rationale: go_past=2.8% and pass_the=5.3% are GT vocab with correct "passing" semantics.
            # Expected: walk_past FIX7 contribution drops ~25pp→10.7pp (−2.9pp in overall walk_past).
        ...
        def _intermediate_walk_to_to_past(text_in):
            ...
            for i, sent in enumerate(sentences):
                ...
                if is_last or has_stop:
                    ...
                    # v203 FIX23: Broad mop-up — catch remaining "walk to the" in this non-final sent.
                    # Root cause: _WTO_IN_SENT_RE misses: hyphened objects (walk-in closet),
                    # prepositional phrases (cabinet in the bathroom), clauses (room until you reach).
                    # FIX: Simple \bwalk to the\b match, just replaces the verb phrase prefix.
                    # Distribution: same as FIX21 (15/15/30/25/15) for vocabulary consistency.
                    _v203_fix23_rng = random.Random(eid ^ 0xC3D4_E5F6)
                    def _conv_wt_broad(m):
                        if _v203_fix23_rng.random() < 0.90:
                            _c = m.group(0)[0].isupper()
                            _r = _v197_fix10_rng.random()
                            if _r < 0.15: return 'Walk past the' if _c else 'walk past the'
                            elif _r < 0.30: return 'Walk toward the' if _c else 'walk toward the'
                            elif _r < 0.60: return 'Go past the' if _c else 'go past the'
                            elif _r < 0.85: return 'Pass the' if _c else 'pass the'
                            else: return 'Head toward the' if _c else 'head toward the'
                        return m.group(0)
                    sent = re.sub(r'\bwalk to the\b', _conv_wt_broad, sent, flags=re.IGNORECASE)
                    result_sents.append(sent)
        ...
        # v203 FIX22: Relaxed 3→2 Sentence Compression (FIX18 fix).
        # ROOT CAUSE of FIX18 failure: 92.2% of 3-sentence S1 contain "and" (LLM generates compound S1).
        # "' and ' not in _s1.lower()" condition blocked 92.2% → only 5.2% qualified → 2.2% merged.
        # FIX: (1) Remove the s1_and check entirely.
        #      (2) Use ", then " connector instead of " and " (avoids double-and in merged sentence).
        #      (3) Relax word limit to 50 (compound S1 is naturally longer).
        # Result: 71.2% qualify → 29.9% of 3-sentence merged at P=0.42 → 3-sent 50.6%→35.5%.
        # Example: "[Walk past X and turn left]. [Walk to Y]. [Stop.]"
        #       → "[Walk past X and turn left, then walk to Y. Stop.]" (natural GT style)
        _fix18_rng = random.Random(eid ^ 0xA1B2_C3D4)
        if _fix18_rng.random() < 0.42:
            _sents18 = [s.strip() for s in re.split(r'(?<=[.!?])\s+', text.strip()) if s.strip()]
            if len(_sents18) == 3:
                _s1, _s2, _s3 = _sents18
                _NAV_START_RE = re.compile(
                    r'^(Walk|Turn|Go|Enter|Continue|Pass|Head|Exit|Cross|Move)\b', re.IGNORECASE
                )
                _NO_MERGE_S2_RE = re.compile(
                    r'^(Stop|Wait|You|This|Once|When|If|Now|Then|Finally)\b', re.IGNORECASE
                )
                _STOP_S3_RE = re.compile(r'^(Stop|Wait|End|Pause)\b', re.IGNORECASE)
                can_fix18 = (
                    _NAV_START_RE.match(_s1) and
                    _NAV_START_RE.match(_s2) and
                    _STOP_S3_RE.match(_s3) and
                    not _NO_MERGE_S2_RE.match(_s2) and
                    not re.search(r'\b(stop|wait)\b', _s1, re.IGNORECASE) and
                    not re.search(r'\b(stop|wait)\b', _s2, re.IGNORECASE) and
                    len((_s1.rstrip('.!?') + ', then ' + _s2).split()) <= 50
                )
                if can_fix18:
                    _s1_base = _s1.rstrip('.!?').rstrip()
                    _s2_lc = _s2[0].lower() + _s2[1:]
                    text = _s1_base + ', then ' + _s2_lc + ' ' + _s3

        # v203 FIX19b: More Aggressive Continue Reduction (extends v202 FIX19).
        # v202 FIX19 achieved only -0.7pp continue (17.7%→17.0%). GT=10.4%.
        # Increase existing rates + add new patterns: Continue past→Go past, Continue [dir]→Walk [dir].
        # "Continue past the X" is especially useful: reduces continue AND adds go_past vocabulary.
        _fix19_rng = random.Random(eid ^ 0xB2C3_D4E5)
        def _fix19_continue_straight(m):
            if _fix19_rng.random() < 0.50:  # was 35%
                cap = m.group(0)[0].isupper()
                return 'Go straight' if cap else 'go straight'
            return m.group(0)
        text = re.sub(r'\bContinue straight\b', _fix19_continue_straight, text, flags=re.IGNORECASE)
        def _fix19_continue_walking(m):
            if _fix19_rng.random() < 0.40:  # was 25%
                cap = m.group(0)[0].isupper()
                return 'Walk' if cap else 'walk'
            return m.group(0)
        text = re.sub(r'\bContinue walking\b', _fix19_continue_walking, text, flags=re.IGNORECASE)
        # NEW FIX19b: Continue past the X → Go past the X (reduces continue AND increases go_past)
        def _fix19_continue_past(m):
            if _fix19_rng.random() < 0.50:
                cap = m.group(0)[0].isupper()
                return 'Go past' if cap else 'go past'
            return m.group(0)
        text = re.sub(r'\bContinue past\b', _fix19_continue_past, text, flags=re.IGNORECASE)
        # NEW FIX19b: Continue [direction verb] → Walk [direction verb]
        def _fix19_continue_dir(m):
            if _fix19_rng.random() < 0.30:
                cap = m.group(0)[0].isupper()
                verb = m.group(1)
                return ('Walk ' if cap else 'walk ') + verb
            return m.group(0)
        text = re.sub(
            r'\bContinue (down|up|along|through|toward|towards|to\b)',
            _fix19_continue_dir, text, flags=re.IGNORECASE
        )
