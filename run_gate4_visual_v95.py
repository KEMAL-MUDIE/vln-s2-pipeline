#!/usr/bin/env python3
"""
Gate 4 Visual v95 — PRIMARY FIX: restore room-entry language, remove all assembly injections.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v95.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v95_phase2_checkpoint.json"  # NEW — fresh P2 for room-entry prompt
...
- When a turn has (→ room) after the arrow: naturally mention the room — "walk into the hallway", "turn left into the bedroom", "continue into the kitchen". Use brief room-entry language (3-5 words). Do NOT write "walk through the hallway" — write "walk into the hallway" or "enter the hallway" instead.
...
- ROOM TRANSITIONS: When a turn is marked (→ room), use room-entry language briefly — "walk into the hallway", "enter the bedroom", "continue into the kitchen" (3-5 words). Use this ONLY at turns marked (→ room) — do not invent room transitions. Use each room name at most once. After entering a room, describe movement within it without repeating the room name.
...
def build_prompt(ep, gt_instr, examples, lm_info, primitives, sel_turns,
    ...
    if n_bracketed > 0 and n_room_trans > 0:
        ...
        anchor_note = (f"Route has {n_bracketed} bracketed landmark(s) and {n_room_trans} "
                       f"room transition(s) marked (→ room) — use landmarks naturally for turns, "
                       f"and mention each room entry briefly: 'walk into the [room]'.")
    elif n_bracketed > 0:
        ...
        anchor_note = (f"Route has {n_room_trans} room transition(s) marked (→ room) — "
                       f"mention each room entry naturally: 'walk into the [room]', "
                       f"'turn and enter the [room]'. Keep it brief (3-5 words).")
    else:
...
async def main():
    ...
    print(f"  [v95 targets: room-entry language restored, no injections, SR > 40% (v24 baseline)]")
