#!/usr/bin/env python3
"""
Gate 4 Visual v153 — Fix v152 visual narrative confusion + walk_straight spike + toward_the overuse.

v152 ACTUAL: GT-match=0.845, turn-anchor=24.0%(GT=16.5%), avg_turns=1.13(GT=0.59), toward_the=32.0%(GT=3.8%)
  Root causes in v152:
  1. Visual narrative 2-sentence → 1st "Turn left at X." for unanchored turns confused Phase 2 into anchoring
     extra turns (turn-anchor 24% vs 16.5%, avg_turns 1.13 vs 0.59)
  2. 2nd sentence "Ahead, a hallway leads..." caused "toward the hallway" → toward_the=32% (GT=3.8%)
  3. DISTANCE medium rule fired as "walk straight" not "go straight" → walk_straight=23.6% (GT=8.8%)

v153 CHANGES from v152:
  1. build_visual_narrative: smart sentence selection for unanchored turns:
     - "Turn [dir] through [opening]..." → show 1st sentence only (GT-style, no object anchor noise)
     - "Turn [dir] at [object]..." → show 2nd sentence only (room context only, strip misleading anchor prefix)
     - Other → show 1st sentence (v151 behavior)
     This eliminates the "Turn left at sofa" → Phase 2 extra anchor cascade.
  2. DISTANCE medium (3-8m): use "walk" or "continue" (not "walk straight") — stays near GT=8.8%.
  3. TURN LANDMARKS rule: simplified — just say prefer doorway/room phrasing; remove "toward" suggestion.
  4. REUSE v152 Phase C checkpoint (room-context descriptions are high quality).

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C)
Phase 2 checkpoint: outputs/gate4_v153_phase2_checkpoint.json (FRESH)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v153.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v153.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v153: REUSE v152 room-context Phase C (no re-annotation)
...
P2_CKPT       = ROOT / "outputs" / "gate4_v153_phase2_checkpoint.json"  # v153: FRESH — smart visual narrative + tuned DISTANCE
...
- TURN LANDMARKS: When the visual context describes a doorway or opening, write the turn as "turn left/right through the doorway into the [room]" (GT style). When it describes entering a room, write "turn left/right into the [room]". These doorway/room phrasings are preferred over object anchors.
...
def build_visual_narrative(lm_info: Dict, sel_turns: List[Dict], vision_map: Dict,
    ...
    for i, sel_turn in enumerate(sel_turns):
        ...
        if desc:
            # v153: smart sentence selection based on Phase C description type
            sents = re.split(r'(?<=[.!?])\s+', desc)
            _first = sents[0] if sents else ""
            _second = sents[1] if len(sents) > 1 else ""
            # "Turn [dir] through/past [opening]..." → GT-style anchor, show 1st sentence only
            _thru_re = re.compile(r'^Turn\s+(?:left|right)\s+(?:through|past)\b', re.I)
            # "Turn [dir] at [object]..." → object-first description, show 2nd sentence (room context) only
            _at_re   = re.compile(r'^Turn\s+(?:left|right)\s+at\b', re.I)
            if _thru_re.match(_first):
                chosen = _first  # GT-style doorway phrase — pass through to Phase 2
            elif _at_re.match(_first) and _second:
                chosen = _second  # strip misleading object anchor; show only room context
            else:
                chosen = _first  # fallback: first sentence (v151 behavior)
...
async def main():
    ...
    print('=== Gate 4 v153: Smart visual narrative + fix walk_straight/toward_the/turn-anchor cascade ===')
    print(f"  v152 METRICS: GT-match=0.845, turn-anchor=24.0%(GT=16.5%), avg_turns=1.13(GT=0.59)")
    print(f"  v152 issues: toward_the=32.0%(GT=3.8%), walk_straight=23.6%(GT=8.8%), turn-anchor overflow")
    print(f"  v24 ACTUAL: 40.24% SR. take_few=0.3%, go_straight=23%, hallway=51.8%")
    print(f"  GT: take_few=0.2%, go_straight=7.9%, walk_straight=8.8%, hallway=20.5%")
    print(f"  v153 FIX 1: smart visual narrative — 'Turn at obj.' → show 2nd sentence (room context only)")
    print(f"  v153 FIX 2: 'Turn through opening.' → show 1st sentence (GT-style, keep doorway phrase)")
    print(f"  v153 FIX 3: DISTANCE medium (3-8m) → 'walk'/'continue' (not 'walk straight')")
    print(f"  v153 FIX 4: TURN LANDMARKS rule simplified (no 'toward' suggestion)")
    print(f"  Expected: turn-anchor~16%, avg_turns~0.6, toward_the<10%, walk_straight~5%, GT-match>0.95")
    ...
    print(f"  Phase 1: REUSE v152 room-context Phase C (doorway/hallway/room anchors, 3192 descriptions)")
    ...
    print(f"  [v153 targets: turn-anchor~16.5%(GT), avg_turns~0.6, toward_the<10%, walk_straight<10%, GT-match>0.95, SR target ≥42%]")
