#!/usr/bin/env python3
"""
Gate 4 Visual v143 — FRESH Phase 1 from auto-annotator Phase C (new visual descriptions).

v142 ACTUAL: through_the=27.8%, hallway=20.6%, into_hallway=4.4%, continue=11.1%, walk_past=8.5%,
             avg_words=24.9w, vocab_gap=1.7pp → pred_SR=62.85%
v142 EXTENDED: go_straight=0% ✓, but walk_forward=88.69% (GT=6.09%, 14.6x overuse — post-proc artifact)

v143 STRATEGY: Test whether fresh Phase 1 descriptions (from auto-annotator vision) improve SR.
  - Phase 1 checkpoint: NEW (gate4_v143_phase1_checkpoint.json from auto_annotator Phase C)
  - Phase 2: FRESH generation (P_ params same as v139, Phase 2 prompt unchanged)
  - No go_straight post-proc on fresh generation — let the Phase 2 prompt run naturally
  - This gives clean A/B test: same Phase 2 rules + new Phase 1 descriptions

A/B test vs v139:
  - v139: v19 Phase 1 checkpoint (older descriptions) + same Phase 2 → actual SR to be determined
  - v143: auto-annotator Phase 1 (new, more specific descriptions) + same Phase 2 → actual SR?
  - If v143 actual SR > v139 actual SR: better Phase 1 descriptions directly improve SR
  - If similar: Phase 1 quality doesn't significantly affect SR (vocabulary distribution dominates)

Expected v143:
  - 5-metric: ~same as v139 (Phase 2 rules unchanged → through_the≈27%, hallway≈20%, etc.)
  - vocab_gap: ~0.8-1pp → pred_SR≈63.3%
  - go_straight: ~81% (same as v139, fresh Phase 2 generates go_straight naturally)
  - Actual SR: potentially higher if better visual grounding helps model navigation

Phase 1 checkpoint: outputs/gate4_v143_phase1_checkpoint.json (from auto_annotator Phase C output)
Phase 2 checkpoint: outputs/gate4_v143_phase2_checkpoint.json (NEW — fresh LLM calls)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v143.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v143.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v143_phase1_checkpoint.json"  # NEW: auto-annotator Phase C descriptions
...
P2_CKPT       = ROOT / "outputs" / "gate4_v143_phase2_checkpoint.json"  # v143: FRESH generation (new Phase 1)
...
async def main():
    ...
    print('=== Gate 4 v143: NEW Phase 1 from auto-annotator Phase C (A/B test vs v139 old Phase 1) ===')
