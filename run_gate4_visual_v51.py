#!/usr/bin/env python3
"""
Gate 4 Visual v51 — v49 base + increased word-count budgets (+4 words all path_len>=5)

v49 RESULTS (base):
  GT-match: 0.959, hallway: 23.1%, through_the: 31.8%, toward_the: 8.6%, turns: 0.71
  avg_words: 23.1 vs GT=26.8 (-3.7w gap)

v50 TESTED: Recalibrated P_THRU/P_PASS markers — minimal effect (~0.3% change in through_the).
  Root cause: LLM generates "through the doorway" independently of [thru:] markers.

v51 FIX: Increase LENGTH budget hints by +4 words for path_len>=5.
  Deficit measured: path_len=5: -4.1w (20.4 vs GT=24.5)
                    path_len=6: -2.7w (23.9 vs GT=26.6)
                    path_len>=7: -4.4w (26.1 vs GT=30.5)
  New budgets:
    path_len==5: 24-32 words (was 20-28)
    path_len==6: 27-35 words (was 23-31)
    path_len>=7: 31-40 words (was 27-36)
  path_len<=4 unchanged (avg=16.4, GT=16.1 — already fine)

All v49 improvements retained (hidden rm + rm turn sampling + hallway/toward_the suppression).

Phase 2 checkpoint: outputs/gate4_v51_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v51.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v51.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v51_phase2_checkpoint.json"
...
async def main():
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v49=0.71, v47=0.99) ← v51: v49+wordfix")
