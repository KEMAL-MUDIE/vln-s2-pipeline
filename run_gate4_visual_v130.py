#!/usr/bin/env python3
"""
Gate 4 Visual v130 — Recalibration: use v129 measured compliance to hit all 4 GT targets simultaneously.

v129 ACTUAL results: through_the=26.3% (GT=27.2%), hallway=17.6% (GT=20.4%), into_hallway=2.1% (GT=4.4%).
                     continue=11.5% (GT=10.4%), avg_words=25.4w (GT=26.8w).

v129 ANALYSIS:
  - (→ hallway) compliance = 2.1% / (7.5%×69.3%) = 40.4% (was predicted 85% — LLM outputs "walk to the hallway"
    or "walk through the hallway" instead of "walk into the hallway" ~60% of the time).
  - (hall→) compliance = (17.6 - 14.77 - 2.1) / (5.15%×69.3%) = 0.73/3.57 = 20.4% (confused by (→ hallway)
    coexistence — LLM generates "into" when it should generate "to").
  - through_the = 26.3%; from hallway: 26.3 - 12.47 = 13.83pp → actual compliance = 13.83/28.4 = 48.7%
    (was assumed 52% — slightly lower; use 48.7% for v130 calibration).
  - continue: 11.5% (1.1pp above GT=10.4% — overshoot; reduce P 0.70 → 0.63).

v130 CALIBRATION (using v129 measured compliance values):
  Non-hallway through_the (stable from v128 extended Stage 1 cleaning): 12.47pp
  (through hallway) LLM compliance: 48.7% (measured from v129: 13.83/28.4%)
  (→ hallway) LLM compliance: 40.4% (measured from v129)

Fix 1 — P_THRU_HALL: 41% → 44% (fix through_the: 26.3% → 27.3%)
         Math: 44% × 69.3% × 48.7% = 14.86pp from hallway
         Total through_the: 14.86 + 12.47 = 27.33% ≈ GT=27.2% ✓

Fix 2 — (→ hallway) P: 7.5% → 16% absolute (fix into_hallway: 2.1% → 4.5%)
         Math: 16% × 69.3% × 40.4% = 4.48% ≈ GT=4.4% ✓
         Draw: 0.44 < draw < 0.60 → (→ hallway)

Fix 3 — (hall→) P: 5.15% → 2% absolute (fine-tune hallway total → 20.4%)
         Total annotated hallway episodes: (44+16+2)% × 69.3% = 43.0% of all episodes
         hallway yield = 47.5% per annotated episode (empirical from v127/v128/v129 avg)
         Total hallway: 47.5% × 43.0% = 20.4% ≈ GT=20.4% ✓
         Draw: 0.60 < draw < 0.62 → (hall→)

Fix 4 — Continue restoration P: 0.70 → 0.63 (fix continue: 11.5% → ~10.4%)
         Math: 0.63/0.70 × 11.5% = 10.35% ≈ GT=10.4% ✓

Fix 5 — Stage 1 + Stage 2 suppression: UNCHANGED (v128 extended pattern keeps non-hallway through_the=12.47pp).

Expected v130: through_the≈27.3%, hallway≈20.4%, into_hallway≈4.5%, continue≈10.4%, avg_words≈25.4w.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v130.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v130_phase2_checkpoint.json"  # v130 — recalibrated from v129 measured compliance
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    ...
                    #
                    # v130 calibration (using v129 measured compliance):
                    #   (through hallway) actual compliance = 48.7% (v129: 13.83pp / 28.4% annotated)
                    #   (→ hallway) actual compliance = 40.4% (v129: 2.1% / (7.5%×69.3%))
                    #   hallway yield per annotated episode = 47.5% (avg of v127/v128/v129: 48.9/47.4/47.4%)
                    #
                    #   P_THRU=44%:        44% × 69.3% × 48.7% = 14.86pp through_the AND hallway
                    #   non-hallway through_the: 12.47pp (stable, extended Stage 1)
                    #   total through_the: 14.86 + 12.47 = 27.33% ≈ GT=27.2% ✓
                    #
                    #   (→ hallway) P=16% abs → draw 0.44–0.60
                    #   into_hallway: 16% × 69.3% × 40.4% = 4.48% ≈ GT=4.4% ✓
                    #
                    #   (hall→) P=2% abs → draw 0.60–0.62
                    #   Total annotated: (44+16+2)% × 69.3% = 43.0% of all eps
                    #   hallway: 47.5% × 43.0% = 20.4% ≈ GT=20.4% ✓
                    draw = rng.random()
                    if draw < 0.44:        # P_THRU_HALL=44% — write "walk through the hallway"
                        if direction:
                            parts.append(f"turn {direction} (through hallway)")
                        else:
                            parts.append("(through hallway)")
                    elif draw < 0.60:      # (→ hallway) P=16% abs → into_hallway≈4.48%≈GT ✓
                        # "(→ hallway)" notation: LLM writes "walk into the hallway"
                        if direction:
                            parts.append(f"turn {direction} (→ hallway)")
                        else:
                            parts.append("(→ hallway)")
                    elif draw < 0.62:      # (hall→) P=2% abs → fine-tune hallway total
...
async def main():
    ...
    print('=== Gate 4 v130: Re-calibration from v129 measured compliance ===')
    print(f"  v128 actual: through_the=24.0%, hallway=19.0%, into_hallway=2.6%, continue=11.2%, avg_words=25.3w")
    print(f"  Fix 1: P_THRU_HALL 41%→44% → through_the: 44%×69.3%×48.7%=14.86pp + 12.47pp = 27.33% ≈ GT=27.2%")
    print(f"  Fix 2: (→ hallway) P=16% abs → into_hallway: 16%×69.3%×40.4%=4.48% ≈ GT=4.4%")
    print(f"         P=7.5% abs → into_hallway: 7.5%×69.3%×85%=4.42pp ≈ GT=4.4%")
    print(f"  Fix 3: (hall→) P=2% abs; total annotated=43%; hallway: 47.5%×43%=20.4% ≈ GT=20.4%")
    print(f"  Fix 4: Continue P=0.70→0.63 → continue: 0.63/0.70×11.5%=10.35% ≈ GT=10.4%")
    ...
    for ep in episodes:
        ...
        # v130: P 0.70 → 0.63 (reduce continue: 11.5% → ~10.4%).
        # Math: 0.63/0.70 × 11.5% = 10.35% ≈ GT=10.4% ✓
        if cont_rng.random() < 0.63 and _WALK_FWD_CAP_RE.search(text):
