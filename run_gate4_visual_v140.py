#!/usr/bin/env python3
"""
Gate 4 Visual v140 — "go straight" elimination via post-processing (reuses v139 P2 checkpoint).

v139 ACTUAL: through_the=27.5%(+0.3pp), hallway=20.6%(+0.2pp), into_hallway=4.5%(+0.1pp),
             continue=10.5%(+0.1pp), walk_past=8.6%(-0.1pp), avg_words=25.7w
             vocab_gap=0.8pp → pred_SR=63.40%

v139 CRITICAL FINDING: "go straight" bigram: v136=4.91% vs GT=0.35% (4.56pp gap).
  Root causes:
    (a) Phase 2 prompt says "Prefer 'walk straight', 'go straight'" → LLM writes it directly
    (b) _ws_variants: "walk straight" → 25% "go straight ahead", 25% "go straight" (50% total)
    (c) Line ~1698: "continue forward"/"continue straight" → "go straight"
    (d) "keep going" → "go straight"
  This gap is NOT in the 5-metric model → actual SR may be lower than pred_SR due to OOD phrase.

v140 STRATEGY: Post-processing only (reuse v139 P2 checkpoint — no new LLM calls).
  Add "go straight" elimination AFTER continue restoration (so it does NOT affect continue%):
    "go straight ahead" → "walk ahead"
    "go straight" → "walk a bit"
    "walk straight ahead" → "walk ahead" (secondary cleanup)
  All other parameters: UNCHANGED from v139.

  Why AFTER continue restoration:
    Continue restoration at P=0.655 converts "Walk forward/straight/ahead" → "Continue forward/straight/ahead".
    "go straight" is NOT matched by restoration (no "Walk" prefix), so it's safe to convert after.
    "walk a bit" and "walk ahead" do NOT trigger restoration (no "Walk\s+(forward|straight|ahead)" match
    for "walk a bit"; "Walk ahead" DOES match restoration but that's fine — restoration already ran).

Expected v140:
  - "go straight" bigram: 4.91% → ~0% (most converted, tiny residual from novel LLM phrases)
  - 5-metric: same as v139 (through_the≈27.5%, hallway≈20.6%, walk_past≈8.6%, continue≈10.5%)
  - vocab_gap (5-metric): ≈0.8pp → pred_SR≈63.40% (unchanged by 5-metric model)
  - actual SR potential: +0.5-2% bonus if "go straight" was hurting (OOD phrase for GT-trained model)

Fix 1 — Phase 2 checkpoint: REUSE v139 (gate4_v139_phase2_checkpoint.json)
Fix 2 — Post-proc "go straight" elimination: added AFTER continue restoration (line ~2090)
Fix 3 — All P_ parameters: UNCHANGED from v139

Expected v139: through_the≈27.1%, hallway≈20.4%, into_hallway≈4.4%, continue≈10.3%, walk_past≈8.75%.
5-metric vocab_gap: |0.1|+|0|+|0|+|0.1|+|0.05| = 0.25pp → pred SR = 63.89-0.61×0.25 ≈ 63.7%
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v140.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v139_phase2_checkpoint.json"  # v140: REUSE v139 checkpoint (go_straight fix is post-proc only)
...
async def main():
    ...
    print('=== Gate 4 v140: "go straight" elimination (post-proc only, reuse v139 P2 checkpoint) ===')
    ...
    print(f"  v139 actual: through_the=27.5%, hallway=20.6%, walk_past=8.6%, continue=10.5%, go_straight=4.91%(GT=0.35%)")
    print(f"  v140: REUSE v139 P2 checkpoint + post-proc 'go straight'→'walk a bit'/'walk ahead' (AFTER cont restoration)")
    ...
    for ep in episodes:
        ...
        # v140: Eliminate "go straight" overuse — GT=0.35% vs v136=4.91% (4.56pp gap, OOD for GT-trained model).
        # Must run AFTER continue restoration (above) to avoid interference with continue% metric.
        # "go straight" is NOT matched by _WALK_FWD_CAP_RE, so restoration already skipped it → safe to convert.
        text = re.sub(r'\bGo straight ahead\b', 'Walk ahead', text)
        text = re.sub(r'\bgo straight ahead\b', 'walk ahead', text)
        text = re.sub(r'\bGo straight\b', 'Walk a bit', text)
        text = re.sub(r'\bgo straight\b', 'walk a bit', text)
        text = re.sub(r'\bWalk straight ahead\b', 'Walk ahead', text)
        text = re.sub(r'\bwalk straight ahead\b', 'walk ahead', text)
