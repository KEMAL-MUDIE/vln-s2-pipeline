#!/usr/bin/env python3
"""
Gate 4 Visual v197 -- FIX10: Diversify FIX7 replacement (walk_past → walk_toward at P=0.40).

BASE: v194 (FIX1-8 — root cause fix, walk_to=7.3% ≈ GT=5.1%)

NEW IN v197:
FIX 10: FIX7 replacement diversification.
  PROBLEM: FIX7 converts 100% of non-final walk_to → walk_past → spikes walk_past from
           11.3% (GT orig) to 54.2% (43pp above GT=10.5%).
  ANALYSIS: v194 walk_past×2 in 10.2% of episodes (cap runs before FIX7 — ordering bug).
            GT vocabulary includes walk_toward (4.9%) which we completely miss (v194=0%).
            walk_toward is directional (CMA approaches landmark then acts) — GT-valid.
  FIX: In FIX7 replacement, use "walk toward the X" with P=0.40 instead of always walk_past.
       This diversifies vocabulary and reduces walk_past.
  Expected impact:
    walk_past: 54.2% → ~43% (FIX8 still uses walk_past for assembly injection)
    walk_toward: 0% → ~11% (GT=4.9%, still somewhat high but adds GT vocabulary)
    walk_to: unchanged at 7.3% (FIX7 still converts at same P=0.90)
  Predicted SR: 60-68% (vocabulary better balanced than v194)

ALSO: Second walk_past cap after FIX7/FIX7b to fix the ordering bug.
  walk_past×2 episodes: 10.2% in v194 → reduced to near 0%.

ROOT CAUSE ANALYSIS (inherited from v194):
  v190 actual: walk_to=44.8%(GT=5.1%), walk_past=11.3%(GT=8.7%)
  FIX8 identified: replace_go_straight_with_landmarks() was injecting "walk to the {lm}" for 29.5% of episodes.
  FIX8 changed: "walk to the {lm}" → "walk past the {lm}" (correct GT intermediate waypoint vocab).

FIX 7 (from v193b): Sentence-level walk_to → walk_past/walk_toward for NON-FINAL sentences (P=0.90).
FIX 7b (from v193b): "walk to the [room]" → "enter the [room]" at P=0.80.
INHERITED FROM v190/v193b/v194: FIX 1-8.
ASSEMBLY-ONLY: Reuse outputs/gate4_v197_phase2_checkpoint.json (copy of v190 P2).

Expected v197:
  walk_to: ~7% (GT=5.1%) — same as v194, ROOT CAUSE FIXED
  walk_past: ~43% (GT=10.5%) — reduced from v194=54.2%
  walk_toward: ~11% (GT=4.9%) — NEW vocabulary added
  continue: ~17.7% (unchanged)
  for_a_bit: 0% (unchanged)
  Predicted SR: 60-68%+ (vocabulary better distributed)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v197_phase2_checkpoint.json (COPY of v190 P2 — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v197.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v197.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v197_phase2_checkpoint.json"  # v197: assembly-only, copy of v190 P2 (FIX10)
