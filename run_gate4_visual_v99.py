#!/usr/bin/env python3
"""
Gate 4 Visual v99 — MAJOR: remove turn direction from room transitions. GT:3.2% vs v95:41.4%.
...
Phase 2 checkpoint: outputs/gate4_v99_phase2_checkpoint.json (NEW — fresh LLM calls needed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v99.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v99.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v99_phase2_checkpoint.json"  # NEW — fresh P2 for room-entry prompt
...
- When you see (→ room) in the route: write "walk into the [room]" or "enter the [room]" or "exit to the [room]" — naturally describe entering that room. Do NOT write "turn left" or "turn right" for these transitions — just describe entering: "walk into the hallway", "enter the bedroom", "go into the kitchen". Keep it brief (3-5 words).
...
- ROOM TRANSITIONS: When you see (→ room) in the route, simply describe entering: "walk into the hallway", "enter the bedroom", "go into the kitchen". Do NOT write "turn left" or "turn right" here — the robot knows to head toward the room. Use each room name AT MOST ONCE. Brief (3-5 words) room-entry is enough.
...
async def main():
    ...
    print(f"  [v99 targets: rm route has no turn dir → walk into X; eps_with_turn ~41% (GT), hallway turn into ~3%]")
