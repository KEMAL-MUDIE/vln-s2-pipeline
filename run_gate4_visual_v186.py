#!/usr/bin/env python3
"""
Gate 4 Visual v186 -- expand through→enter regex + soft walk_ahead→go_straight at P=0.20.

CHANGES FROM v185:
  v185 final stats (PYTHONHASHSEED=42): through_the=35.2%, enter=15.2%, walk_ahead=11.6%, 2-sent=42.7%
  GT reference:                         through_the=27.2%, enter=17%,   walk_ahead=6.1%, 2-sent=42%

  Analysis of remaining through_the=35.2%: top sources are
    - "through the hallway" 192×, "through the door on" 168×, "through the doorway" 84×
    - "through the room" 37×, "through the house" 27×, "through the area" 13×
  Current regex excludes: "room", "house", "area" — only "bedroom/kitchen/hallway/doorway" etc. matched.

FIX 1: Expand through→enter regex to include "room", "area", "foyer"
  "walk through the room" → "enter the room" ✓ (GT-style)
  "walk through the foyer" → "enter the foyer" ✓ (GT-style)
  Expected: ~37+13=50 more match opportunities → through_the↓ ~1.5pp

FIX 2: soft walk_ahead → go_straight at P=0.20
  walk_ahead=11.6% (GT=6.1%) — 5.5pp above GT.
  Convert 20% of "walk ahead" to "go straight" (like v24's style).
  Expected: walk_ahead 11.6%→9.3%, go_straight 23.4%→25.7% (slightly above v24's 23%)

INHERITED from v185 (all):
  - through→enter P=0.25 (v185 level)
  - walk_forward→go_straight P=0.30
  - P_STOP_MERGE=0.30
  - skip rate 25%, P_MERGE=0.60
  - PYTHONHASHSEED=42 (deterministic hash)
  - REMOVED go_straight→walk_ahead

Expected v186:
  through_the ≈ 33-34% (from 35.2%)
  enter ≈ 16% (slight increase from regex expansion)
  walk_ahead ≈ 9.3% (from 11.6%)
  go_straight ≈ 25.7% (from 23.4%)
  2-sent ≈ 42.7% (same)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v186_phase2_checkpoint.json (REUSE v180 P2 results — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v186.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v186.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v186_phase2_checkpoint.json"  # v186: FRESH (use v180 P2 results)
...
async def main():
    ...
    print(f"  INHERITED FROM v185: through→enter P=0.25, walk_fwd→go_straight P=0.30, P_STOP_MERGE=0.30")
