#!/usr/bin/env python3
"""
Gate 4 Visual v148 — Fix hallway injection and trailing 'ahead' in landmark replacements.

v147 ACTUAL: through_the=27.08%, hallway=22.84%, into_hallway=4.40%, continue=10.17%, walk_past=8.54%
             go_straight=0.0% ✓, avg_words=28.7w, vocab_gap=2.96pp → pred_SR=62.09%

v147 PROBLEMS:
  1. hallway=22.84% (GT=20.4%, +2.44pp) — Phase C descriptions sometimes contain "hallway"
     which gets injected as "walk to the hallway" → inflates hallway%
  2. Trailing "ahead": "go straight ahead" → regex replaces only "go straight"
     → leaves "walk to the kitchen island ahead" (should be "walk to the kitchen island")

v148 FIXES:
  1. Filter hallway/corridor from landmarks: if extracted text contains "hallway" or "corridor",
     use "walk straight" fallback → preserves hallway% calibration
  2. Fix regex: r'\bgo straight(?:\s+ahead)?\b' → consumes trailing "ahead"
  3. Reuse v145 Phase 2 checkpoint (instant reassembly)

Phase 1 checkpoint: outputs/gate4_v143_phase1_checkpoint.json (same as v143–v147)
Phase 2 checkpoint: outputs/gate4_v145_phase2_checkpoint.json (REUSED from v145–v147)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v148.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v148.json.gz"
...
def replace_go_straight_with_landmarks(text: str, phase_c_descs: Dict[str, str]) -> str:
    """v148: Replace 'go straight[ahead]' with Phase C landmarks or 'walk straight' fallback.
...
async def main():
    ...
    print('=== Gate 4 v148: Fix hallway injection + trailing ahead in go_straight landmark replacements ===')
