#!/usr/bin/env python3
"""
Gate 4 Visual v82 — v81 clean() fixes + hallway correction

v81 ISSUE (post-generation):
  hallway%: 56.1% (GT=20.4%) — massive overuse caused by two mistakes:
    1. DISTANCE rule said "use 'walk down the hallway' for ≥8m" → LLM applies universally
    2. Relaxed assembly suppression (_HALLWAY_ENTRY_RE: only blocked into/toward, not down/along)
  GT analysis: hallway uses are 20.3% MID-ROUTE, 0.2% stop phrases.
  The v80 stop_phrase condition was wrong — it over-suppressed valid mid-route uses.

v82 FIX:
  1. Revert ROOM NAMES to v80 ban: remove "walk down the hallway" from allowed phrases.
  2. Remove DISTANCE rule mention of "walk down the hallway" (the primary cause of 56.1%).
  3. Keep "walk forward for a while" as the recommended phrase for ≥6m segments.
  4. Assembly: restore v80 _MIDROUTE_HALL_RE with corrected condition:
     Instead of keying on stop_phrase (wrong — GT hallway is 99.8% mid-route),
     use episode_rng to allow ~35% of hallway episodes through (target ~20% hallway%).
     Rationale: v81 has 56.1% hallway, GT=20.4%; 20.4/56.1 ≈ 0.36 → allow P=0.36.
  5. Keep v81 clean() fixes: garbling fix (_FWDOBJ_FIX_RE) + leading period strip.
  6. Seed: 0x8245_5300. P_PASS_MARKER=0.10 (unchanged).

Phase 2 checkpoint: outputs/gate4_v82_phase2_checkpoint.json (FRESH — prompt changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v82.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v82.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v82_phase2_checkpoint.json"  # FRESH — prompt changed
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x8245_5300)  # v82: new seed
...
async def main():
    ...
    print('=== Gate 4 v82: v81 clean() fixes + hallway probabilistic suppression ===')
    print(f"  KEY FIX: Hallway prompt ban restored (v80 style: ban 'walk down the hallway')")
    print(f"  KEY FIX: DISTANCE rule updated ('walk forward for a while' for ≥6m, no hallway)")
    print(f"  KEY FIX: Assembly hallway suppression: room-entry always; directional P=0.64 suppress")
    print(f"  KEY FIX: Target hallway% ~20% (0.36 × v81=56.1%) — GT=20.4%")
    print(f"  RETAIN: 'walk forward art' garbling fix (_FWDOBJ_FIX_RE)")
    print(f"  RETAIN: leading period strip in clean()")
    print(f"  P_PASS_MARKER={P_PASS_MARKER} (unchanged from v80)")
    print(f"  Seed: 0x8245_5300")
    ...
    for ep in episodes:
        ...
        if "hallway" in text.lower():
            ...
            if _HALLWAY_DIR_RE.search(text):
                ep_rng = random.Random(eid ^ 0x8245_5300)
