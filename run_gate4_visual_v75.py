#!/usr/bin/env python3
"""
Gate 4 Visual v75 — v74 (ranked-priority turns + goal-approach stops) + v75 midpoints

v74 BASELINE:
  GT-match: 0.990, avg_words: 26.5 (GT=26.8)
  Turn landmarks: ranked-priority (40% artwork, 11.5% door/doorframe)
  Stop phrases: 74.9% vision-grounded (goal-approach last midpoint frame)
  Midpoints (v22): door=18.5%, artwork=4.3%

v75 FIX:
  Replace v22 midpoint checkpoint with v75 ranked-priority midpoints.
  - phase1_midpoints_v75.py: same ranked-priority prompt as v73 turns
  - Door: 3.1% (was 18.5%), Artwork: 42.4% (was 4.3%)
  - improve=1094, regress=0 vs v22
  - "walk past the oil painting" > "through the wooden door"
  Also carries forward v74's goal-approach stop overrides (603 episodes improved).

Phase 2 checkpoint: outputs/gate4_v75_phase2_checkpoint.json (FRESH — midpoints changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v75.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v75.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v75_phase2_checkpoint.json"  # FRESH — midpoints changed
...
def apply_goal_approach_stop(stop_phrase: str, stop_type: str, goal_desc: str,
    ...
    goal_desc_clean = goal_desc.strip()
...
async def main():
    ...
    print('=== Gate 4 v75: v74 (ranked turns + goal stops) + v75 midpoints (door 3.1%, artwork 42.4%) ===')
    ...
    print(f"  Phase 1b2: v74 goal-approach stop descriptions (603 eps improved)")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v73=0.990, v74=0.990)")
    print(f"  [v75 target: same GT-match ~0.990, better midpoint descriptions → higher SR]")
