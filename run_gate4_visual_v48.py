#!/usr/bin/env python3
"""
Gate 4 Visual v48 — Fix turn regression from v47 by sampling rm turns + restore → rm

v47 RESULTS:
  hallway:      22.6% (GT=20.4%) ← NEAR-PERFECT from 45.1%
  through_the:  28.0% (GT=27.2%) ← maintained
  toward_the:    9.0% (GT=3.8%)  ← improved from 26.5%
  into_hallway:  1.3% (GT=4.4%)  ← improved from 17.6%
  avg_turns:     0.99 (GT=0.59)  ← REGRESSION from v45=0.92
  GT-match:     0.958 (v45=0.980) ← regression

ROOT CAUSE OF TURN REGRESSION (v47):
  v45 rm turns: "turn right → hallway" → LLM wrote "walk toward the hallway" (implicit turn)
  v47 rm turns: "turn right" (no rm) → LLM wrote "turn right" explicitly → turns went up

v48 FIX: Apply P_TURN_UNANCHORED sampling to rm turns too + restore → rm for context.
  - rm turns sampled at P_TURN_UNANCHORED=0.55 (was always-included before)
  - For kept rm turns: restore "turn {direction} → {rm}" (spatial context helps GT-match)
  - All v47 prompt improvements retained (no "walk toward [goal]", stronger ROOM NAMES)

EXPECTED vs v47:
  avg_turns:    0.99 → ~0.82-0.87 (rm turns sampled 45% filtered)
  hallway:      22.6% → ~28-33% (kept → rm for 55% of rm turns)
  through_the:  28.0% → ~27% (maintained)
  toward_the:    9.0% → ~7-9% (maintained, LLM gets some rm context again)
  GT-match:     0.958 → ~0.970-0.975 (improved with lower turns)

Phase 2 checkpoint: outputs/gate4_v48_phase2_checkpoint.json
Output: outputs/datasets/val_unseen_generated_gemma_visual_v48.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v48.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v48_phase2_checkpoint.json"
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                # v48: Apply P_TURN_UNANCHORED sampling to rm turns too.
                # v47 always included rm turns (bypass sampling) which caused avg_turns=0.99.
                # Restore → rm in route for kept turns (spatial context improves GT-match).
...
async def main():
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v47=0.99, v45=0.92) ← v48 KEY FIX")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v45=26.5%, v47=9.0%) ← KEY METRIC")
    print(f"  hallway:       {100*hallway_n/n:.1f}%  (GT=20.4%, v45=45.1%, v47=22.6%) ← KEY METRIC")
    print(f"  into hallway:  {100*into_hall_n/n:.1f}%  (GT=4.4%, v45=17.6%, v47=1.3%) ← KEY METRIC")
    print(f"  spatial:")
