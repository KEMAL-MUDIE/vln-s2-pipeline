#!/usr/bin/env python3
"""
Gate 4 Visual v177 -- walk_forward reduction + stop-area on_dir + merger tightening.

DEEP RESEARCH FINDINGS (from GT analysis):
  GT on_the_left/right contexts (374 total):
    - door: 48.7% → covered by v174+ door_on injection (4.6% each)
    - stop_area: 17.6% → NEW in v177 (P=0.20 stop-area on_dir injection)
    - turn: 7.0%, stair: 6.7%, furniture: 4.3%, other: 7.8%
  GT walk_forward=6.2% but v176 has 28.4% — LLM ignores AVOID rule completely
    "walk forward [to/past/through/into/toward/along]" → "walk [prep]" (removes forward -1w)
    Expected: walk_fwd 28.4% → ~14-16%

FIX 1: walk_forward PREPOSITION REDUCTION (post-processing)
  "walk forward [to/past/through/into/toward/towards/along/up/down]" → "walk [prep]"
  Example: "walk forward into the bedroom" → "walk into the bedroom" (-1w)
  Example: "walk forward to the end" → "walk to the end" (-1w)
  Preserves "walk forward" standalone (no preposition) — GT uses this 6.2%
  Expected: walk_fwd drops from 28.4% to ~14% (-14%) → aligns better with GT=6.2%
  Word count impact: ~14% episodes × -1w = -0.14w avg (negligible loss)

FIX 2: STOP-AREA on_left/right INJECTION (P=0.20)
  GT: 17.6% of on_left/right contexts are stop-area ("stop next to the table on the right")
  v177: When stop_type=="stop", P=0.20 → inject "on the [left/right]" into stop phrase
  "Stop near the couch." → "Stop near the couch on the left." (+3w to 20% → +0.6w avg)
  Combined with door_on (4.6%) → total on_left/right ≈ 14-15% (GT=18.4%)
  Uses deterministic seed per episode to avoid randomness

FIX 3: INCREASE P_MERGE to 0.75 (was 0.60 in v176)
  v176: P=0.60 → 2-sent=29.9%, 3-sent=53.5%
  GT: 2-sent=33.8%, 3-sent=31.0%
  v177: P=0.75 → 2-sent≈35%, 3-sent≈48% (closer to GT)
  Word count impact: +0.15w avg (more "and" tokens from merges)

Expected v177:
  avg_words ≈ 27.3w (26.8-27.5 range: -0.14 walk_fwd + +0.6 stop_dir + +0.15 merger)
  2-sent ≈ 35%, on_left ≈ 9-10%, on_right ≈ 9-10%
  walk_fwd ≈ 14-16%, walk_ahead ≈ 38%, walk_to ≈ 40%
  in_room ≈ 12% (from stop_room P=0.15 + stop_dir P=0.20)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v177_phase2_checkpoint.json (FRESH)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v177.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v177.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v177_phase2_checkpoint.json"  # v177: FRESH — walk_fwd reduction, stop-area on_dir, P_MERGE=0.75
...
async def main():
    ...
    print('=== Gate 4 v177: walk_fwd reduction + stop-area on_dir + P_MERGE=0.75 ===')
    print(f"  DEEP RESEARCH: GT on_left/right contexts: door=48.7%, stop=17.6%, turn=7%, stair=6.7%")
    print(f"    v176 walk_fwd=28.4% vs GT=6.2% — LLM ignores AVOID rule")
    print(f"  v177 FIXES:")
    print(f"    FIX1: walk_forward [prep] → walk [prep] (removes redundant forward -1w)")
    print(f"      Expected: walk_fwd 28.4% → ~14-16%")
    print(f"    FIX2: Stop-area on_left/right P=0.20 → 'on the left/right' after stop obj")
    print(f"      Expected: on_left/right +10% → total ~14% (GT=18.4%)")
    print(f"    FIX3: P_MERGE=0.75 (was 0.60) → 2-sent≈35% (GT=33.8%)")
    print(f"  KEEPS ALL v176: avg_words=27.0, in_room=10.7%, door_on=9.1%, stop_room P=0.15")
    print(f"  Expected: avg_words≈27.3, 2-sent≈35%, on_left≈9%, on_right≈9%")
    ...
    for ep in episodes:
        ...
        # v177: STOP-AREA on_left/right INJECTION (P=0.20)
        # GT: 17.6% of on_left/right contexts are stop-area ("stop next to the table on the right")
        # Combined with door_on (4.6%) → total on_left/right ≈ 14% (GT=18.4%)
        # "Stop near the couch." → "Stop near the couch on the left." (+3w, P=0.20 of stop eps)
        _stop_dir_rng = random.Random(eid ^ 0xC1D2_E3F4)
        if (stop_type == "stop" and _stop_dir_rng.random() < 0.20
        ...
        # v174/v177: SENTENCE MERGER — merge consecutive short action sentences toward GT 2-sent style
        # GT: 2-sent=33.8% peak, v173: 3-sent=63.2% dominant.
        # v176: P_MERGE=0.60 → 2-sent=29.9%; v177: P_MERGE=0.75 → ~35% (closer to GT)
        _P_MERGE = 0.75
