#!/usr/bin/env python3
"""
Gate 4 Visual v33 — Suppress LLM-Hallucinated "walk past the [generic]"

IMPROVEMENTS OVER v32:
  v32 achieved "through the"=32.3% (down from v31's 39.5%) but "walk past the"
  increased slightly from 18.7% to 21.2%. Root cause: the LLM hallucinates
  "walk past the table/wall/sofa" without a [pass:] marker in the route description.

  Analysis of v32 instructions:
    - 390/1839 (21.2%) have "walk past the"
    - 69 have GENERIC objects (table, wall, sofa, etc.) — pure LLM hallucination
    - 321 have distinctive objects from [pass:] markers — valid landmarks

  This version adds post-generation suppression of generic WPT objects:
    - New `_GENERIC_WPT_RE` regex: matches "walk past the [non-distinctive]"
    - In clean(): replace matched generic WPT with "continue forward"
    - Covers: table/wall/sofa/chair/bed/cabinet/door/hallway/lounge/shelf/plant/column

  All v32 innovations retained:
    - No goal-segment midpoints (both [thru:] and [pass:] suppressed)
    - Strict whitelist for [pass:] turn midpoints (_DISTINCTIVE_OBJECTS_RE)
    - Material filter for goal_lm and midpoints
    - Post-process "passing the" → "walk past the" (0% pure passing the)
    - Fixed GT-match metric (TURN_ANCHOR_RE only)

  Expected:
    "through the":   ~32-35% (unchanged from v32)
    "walk past the": ~17-19% (slight improvement, eliminating 69 generic episodes)
    GT-match:        ≥ 0.960 (unchanged)

  GT analysis reference (v32 actuals):
    "through the":   GT=27.2%, v32=32.3%
    "walk past the": GT=8.7%,  v32=21.2%  → v33 target: ~17-19%
    "wooden":        GT=3.3%,  v32=0.1%   ✓
    GT-match:        v32=0.961 ✓

  Phase 2 checkpoint: outputs/gate4_v33_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v33.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v33.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v33_phase2_checkpoint.json"
...
_GENERIC_WPT_OBJ_RE = re.compile(
    ...
    r'(?:(?:grey|white|beige|black|brown|dark|light|'  # optional color/size/descriptor adjectives
    ...
    r'long|short|kitchen|dining|living|coffee|'
    r'console|display|storage|filing|end)\s+)*'
    ...
    r'plant|potted\s+plant|bush|tree|'
    r'shelf|shelv(?:es|ing)|shelving\s+unit|rack|'
    r'window(?!\s+seat)|curtain|blind|'
    r'counter|countertop|island|'
    r'rug|mat|carpet(?!\s+runner))\b',
    re.IGNORECASE
)
...
async def main():
    ...
    print('=== Gate 4 v33: Suppress LLM-Hallucinated Generic "walk past the" ===')
    ...
    print('  v33 key change: post-process "walk past the [generic]" → "continue forward" in clean()'
          f" v31 had through-the=39.5% (GT=27.2%) due to goal [thru:]. Target: ~32-35%.")
    print()
    ...
    print(f"  walk past the: {100*walkpast_n/n:.1f}%  (GT=8.7%) ← KEY METRIC (v31=18.7%)")
    print(f"  passing the:   {100*passing_n/n:.1f}%  (GT=5.1%) ← combined; pure passing the=0%")
    print(f"  continue:      {100*continue_n/n:.1f}%  (GT=10.4%)")
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v33 should be <1%")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v32=0.961, v31=0.952)")
    ...
    print(f"  [Note: v33 suppresses generic 'walk past the [table/wall/sofa/etc.]' → 'continue forward']")
