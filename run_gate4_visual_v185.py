#!/usr/bin/env python3
"""
Gate 4 Visual v185 -- increase through→enter P 0.15→0.25 for deeper through_the reduction.

CHANGES FROM v184:
  v184 final stats: through_the=36.5%, enter=15.0%, walk_forward=9.5%, 2-sent=43.4%
  GT reference:     through_the=27.2%, enter=17%,   walk_forward=6.1%, 2-sent=42%

  KEY FINDING: through_the still 9.3pp above GT after v184's P=0.15 conversion (reduced only 3.4pp).
  Root cause: many "walk through the X" phrases use room types not in regex, or prepositions not matched.
  Solution: increase P from 0.15→0.25 (67% increase in conversion rate → ~5.5pp more reduction)

FIX: Increase through→enter P: 0.15 → 0.25
  Expected: through_the 36.5%→~31%, enter 15.0%→~17% (≈ GT=17%!)

INHERITED from v184 (all):
  - through→enter P=0.15 (now 0.25 — this version's main change)
  - walk_forward→go_straight P=0.30
  - P_STOP_MERGE=0.30
  - skip rate 25%, P_MERGE=0.60
  - REMOVED go_straight→walk_ahead (v182 fix)
  - go_start P=0.70, enter P=0.75 for walk_to
  - walk_ahead/fwd prep reduction (v177/v178)

NOTE: walk_ahead non-determinism root cause identified:
  _ws_variants uses hash(result) which varies per Python process (PYTHONHASHSEED non-deterministic).
  "walk straight" → one of ['walk straight', 'go straight ahead', 'go straight', 'walk forward'] randomly.
  If 'walk straight' is chosen → walk_straight→walk_ahead runs → inflates walk_ahead.
  The 9.7%→13.3% walk_ahead difference between v183 and v184 is this hash noise, not FIX1/FIX2/FIX3.
  Fix in v185: add PYTHONHASHSEED=0 to make hash() deterministic. (Or fix in assembler.py)

Expected v185:
  avg_words ≈ 27.4w (same)
  through_the ≈ 31% (from 36.5% — toward GT=27.2%)
  enter ≈ 17% (from 15.0% — matching GT exactly!)
  walk_forward ≈ 9.5% (same as v184)
  2-sent ≈ 43.4% (same as v184)

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v185_phase2_checkpoint.json (REUSE v180 P2 results — assembly-only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v185.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v185.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v185_phase2_checkpoint.json"  # v185: FRESH (use v180 P2 results)
...
async def main():
    ...
    print('=== Gate 4 v185: increase through→enter P 0.15→0.25 for deeper through_the reduction ===')
    print(f"  v184 stats: through_the=36.5%, enter=15.0%, walk_forward=9.5%, 2-sent=43.4%")
    print(f"  GT reference: through_the=27.2%, enter=17%, walk_forward=6.1%, 2-sent=42%")
    print(f"  v185 CHANGES:")
    print(f"    FIX: through→enter P 0.15→0.25 → through_the:36.5%→~31%, enter:15.0%→~17%!")
    print(f"  INHERITED FROM v184: walk_fwd→go_straight P=0.30, P_STOP_MERGE=0.30, skip=25%, P_MERGE=0.60")
