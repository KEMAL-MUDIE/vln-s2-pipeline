#!/usr/bin/env python3
"""
Gate 4 Visual v58 — v56 checkpoint + surgical walk_past post-processing

v56 RESULTS (base):
  GT-match: 0.958, toward_the: 3.3%, through_the: 28.0%, walk_past: 13.6% (GT=8.7%), avg_words: 26.2

KEY FINDING from v57 (P_PASS_MARKER 0.50→0.30):
  walk_past barely changed: 13.6%→13.9%. Same pattern as P_THRU_MARKER lesson from v50.
  LLM generates "walk past the X" INDEPENDENTLY of [pass:] markers, from visual context.
  P_PASS_MARKER reduction does NOT control walk_past frequency — markers aren't the driver.

v58 ROOT CAUSE ANALYSIS (walk_past):
  Same as through_the before v54fix: LLM hallucinated "through the doorway" without [thru:] markers.
  Here: LLM hallucinated "walk past the X" without [pass:] markers in the route.
  Fix: Surgical post-processing — identify episodes with NO pass-type midpoints → any "walk past"
  in those episodes is definitely hallucinated → replace with "walk to the X" (neutral approach)
  Episodes with pass-type midpoints: KEEP "walk past the X" (legitimate per-context)

v58 FIX:
  - Reuse v56 Phase 2 checkpoint (RULES unchanged — no LLM re-generation needed)
  - New post-processing: check _has_pass_midpoint(eid) from midpoints_ckpt + classify_pass_action()
    - If NO pass-type midpoints AND instruction has "walk past the X": replace with "walk to the X"
    - If HAS pass-type midpoints: leave "walk past the X" as-is (may be legitimate)
  - Retain v54's doorway post-processing (unchanged)
  Expected: walk_past 13.6% → ~10-11% (GT=8.7%), GT-match maintained ~0.958

Phase 2 checkpoint: outputs/gate4_v56_phase2_checkpoint.json (REUSE — post-processing only)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v58.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v58.json.gz"
...
async def main():
    ...
    print('=== Gate 4 v58: walk_past surgical fix — remove hallucinated walk_past for no-pass-midpoint eps ===')
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v56=0.71) ← v58: walk_past surgical post-proc")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v56=3.3%) ← KEY METRIC")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v56=0.958, v54fix=0.961, v53=0.961)")
