#!/usr/bin/env python3
"""
Gate 4 Visual v36 — Eliminate LLM "proceed" Overuse + Fix Malformed Short Instructions

CRITICAL DISCOVERY FROM v35 ANALYSIS:
  v35 "proceed" frequency = 43.8% vs GT = 0.8% ← MASSIVE distribution shift!
  The LLM uses "proceed" constantly (proceed forward/straight/to/past/through) while
  GT annotators almost never use it (0.8%). Model trained on GT will behave poorly
  when encountering "proceed" at 43x the expected rate.

  Also: ~325/1839 (18%) instructions are <20w (GT <20w = 27%),
  with some malformed/truncated at 6-9w due to clean() 3-sentence cap.

IMPROVEMENTS OVER v35:
  v35 (GT-match=0.961): through_the=27.2% (exact GT!), walk_past=10.7%, proceed=43.8%.

  v36 changes:
    1. Post-process ALL "proceed X" → GT-style alternatives:
       "Proceed forward/straight/ahead" → deleted (if motion-only, no destination)
       "Proceed to/toward/towards the X" → "Walk to/toward the X"
       "Proceed past the X" → "Walk past the X"
       "Proceed through the X" → "Walk through the X" / "Go through the X"
       "Proceed into the X" → "Walk into the X"
       "Proceed down/up the X" → "Walk down/up the X"
       "Proceed along the X" → "Walk along the X"
       Effect: "proceed" 43.8% → <2% (matching GT 0.8%)

    2. Fix consecutive duplicate motion phrases:
       "continue forward. Walk forward." → "continue forward."
       "walk forward and proceed forward" → "walk forward"
       Effect: reduces "continue" from 18.1% → closer to GT 10.4%

    3. Fix minimum length check: instructions <12w get regenerated or padded
       (prevents malformed "Take a. Stop at X." artifacts)

  All v35 innovations retained:
    - through_the hallway→down, [room]→into fix
    - P_PASS=0.50, P_THRU=0.85 midpoint sampling
    - Generic WPT suppression + through-hallway suppression
    - Material filter, GT-match metric

  Expected:
    "proceed":       <2% (from 43.8%, GT=0.8%)
    "continue":      ~12% (from 18.1%, GT=10.4%)
    "through the":   ~27% (keep v35 fix)
    "walk past the": ~9-11%
    GT-match:        ≥ 0.961 (v35=0.961)
    avg_words:       ≥ 24 (may increase as "walk X" replaces "proceed X")

  GT analysis reference (v35 actuals):
    "proceed":       GT=0.8%,  v35=43.8%  → v36 target: <2%
    "continue":      GT=10.4%, v35=18.1%  → v36 target: ~12%
    "through the":   GT=27.2%, v35=27.2%  ← KEEP EXACT MATCH ✓
    "walk past the": GT=8.7%,  v35=10.7%  → v36 target: ~9-10%
    GT-match:        v35=0.961 ✓

  Phase 2 checkpoint: outputs/gate4_v36_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v36.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v36.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v36_phase2_checkpoint.json"
            ...
            if seg_midpoint and turn_label in seg_midpoint:
                ...
                if action == "thru":
                    ...
                    if random.random() < P_PASS_MARKER:  # 60% → ~8.4% walk-past (GT=8.7%)
                        parts.append(f"[{action}: {mid_desc}]")
            # v27: Skip un-anchored turns angle < 70° (was 55° in v26).
            # GT total turns 48.9% vs our 88.2%. Need to cut ~40% of turns.
...
- When route has [thru: description]: the robot passes through an architectural opening — write "through the X" (2-4 words). Example: "through the doorway", "through the arched entrance". Do NOT write "passing through" or "passing the" — write "through the X" only.
...
- STYLE: Avoid starting sentences with "Continue straight" — use action verbs instead ("Walk forward", "Proceed"). Avoid overusing "wooden" for generic surfaces (floor, wall, beam) — mention material only for distinctive objects (marble fireplace, glass door are fine).
...
def clean(raw, sv):
    ...
    # v33: suppress LLM-hallucinated "walk past the [generic]" → "continue forward"
    # The LLM introduces "walk past the table/wall/sofa" without a [pass:] marker.
    # After converting "passing the" → "walk past the" above, apply this as the final step.
    raw = _GENERIC_WPT_OBJ_RE.sub('continue forward', raw)
    ...
    raw = re.sub(r'\s+', ' ', raw).strip()
    # Expand to 4 sentences (was 3) since we're now removing filler — v36
    sents = re.split(r'(?<=[.!?])\s+', raw)
    result = " ".join(sents[:4]).strip()
...
async def main():
    ...
    print('=== Gate 4 v36: Eliminate "proceed" Overuse (43.8%→<2%) + Fix Malformed Instructions ===')
    ...
    print(f"  KEY FIX: 'proceed to/past/through/into' → 'walk X', 'Proceed forward' → deleted")
    print(f"  KEY FIX: duplicate motion phrases collapsed, fragment artifacts removed")
    print(f"  KEY FIX: sentence cap 3→4 to recover content lost from filler removal")
    print()
    ...
    print(f"  through the:   {100*through_n/n:.1f}%  (GT=27.2%) ← KEY METRIC (v35=27.2%)")
    print(f"  walk past the: {100*walkpast_n/n:.1f}%  (GT=8.7%) ← KEY METRIC (v35=10.7%)")
    print(f"  passing the:   {100*passing_n/n:.1f}%  (GT=5.1%) ← combined; pure passing the=0%")
    print(f"  continue:      {100*continue_n/n:.1f}%  (GT=10.4%) ← v35=18.1%")
    print(f"  proceed:       {100*proceed_n/n:.1f}%  (GT=0.8%) ← v35=43.8% KEY FIX")
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v36 should be <1%")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v35=0.961, v34=0.968)")
    ...
    print(f"  [Note: v36: eliminated 'proceed' overuse, fixed malformed frags, 4-sentence cap]")
