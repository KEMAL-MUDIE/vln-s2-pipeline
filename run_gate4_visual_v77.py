#!/usr/bin/env python3
"""
Gate 4 Visual v77 — v76 + seeded pass-marker decisions → hallucination suppression

v76 ISSUE:
  "walk past the": 14.8% (GT=8.7%) — still ~2x too high.
  Root cause: LLM hallucinates "walk past the X" without route markers.
  v58 filter (replace hallucinated walk-past) is disabled because v75 midpoints
  (42.4% artwork = pass type) make _has_pass_midpoint() True for all episodes.

v77 FIX:
  1. Seed build_route_desc() pass/thru marker decisions with episode RNG
     → route decisions are DETERMINISTIC and can be recomputed at assembly time
  2. At assembly, recompute which episodes had actual [pass:] in their route
  3. If NO [pass:] markers in route → any "walk past the X" is hallucinated
     → replace with "walk to the X" (v58 logic, now properly scoped)
  4. Also add more generic nouns to _GENERIC_WPT_OBJ_RE:
     armchair/armchairs, chairs (plural), area (adjective), folding/sideboard

Phase 2 checkpoint: outputs/gate4_v77_phase2_checkpoint.json (FRESH — route determinism changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v77.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v77.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v77_phase2_checkpoint.json"  # FRESH — route determinism changed
...
P_PASS_MARKER = 0.30   # v76: reduced from 0.50 → target ~10% walk-past (GT=8.7%); v75=16.9% OOD
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x7745_5300)  # v77: episode-specific seed for pass decisions
