#!/usr/bin/env python3
"""
Gate 4 Visual v117 — Doorway on room transitions: "(door-> room)" notation for 60% of transitions.
Fixes through_the (4%->~25%), hallway (9%->~18%), continue (22%->~10%).
Root cause v116: midpoints_ckpt only 3.1% door-type -> P_THRU_MARKER barely helps.
Room transitions ALWAYS involve doorways in Habitat -> inject "(door-> room)" notation.
Ban "Continue" in SYSTEM_INTRO. Allow "through the hallway" for hallway transitions.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v117.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v117_phase2_checkpoint.json"  # v117 — doorway notation on room transitions
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            if lm and rt and "doorway" in rt.lower():
                ...
                if is_hallway:
                    # v117: Allow hallway with doorway notation 40% of the time
                    if rng.random() < 0.40:
                        if direction:
                    ...
                    # v117: 60% chance of doorway notation for non-hallway room transitions
                    if rng.random() < 0.60:
...
- When route has [pass: description]: the robot moves alongside an object — write "walk past the X" (imperative form, 3-5 words). Example: "walk past the brown armchair". Do NOT write "passing the" — write ONLY "walk past the X". Keep it very brief.
