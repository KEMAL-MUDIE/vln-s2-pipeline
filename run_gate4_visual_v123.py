#!/usr/bin/env python3
"""
Gate 4 Visual v123 — Remove "head toward" and "walk down the hallway" from SYSTEM_INTRO rules.
v122 results: toward_the=17.1% (GT=3.8%), hallway=41.5% (GT=20.4%) — still regressed.
Root cause confirmed: MOTION WORDS rule says "head toward" → LLM writes "head toward X" everywhere.
                     DISTANCE rule says "walk down the hallway" → LLM adds hallway mentions.
(→ room) frequency (100%) was correctly reverted in v122 — not the cause of v121/v122 regression.

Fix: Remove "head toward" from MOTION WORDS, remove "walk down the hallway" from DISTANCE.
Use simpler DISTANCE rule without hallway-triggering phrases.
Keep all v122 improvements: _ws_variants, keep_going→go_straight, [pass: X] P=0.70.

Expected v123: toward_the~5%, hallway~33%, go_forward~15%, keep_going~2%, walk_past~8%.
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v123.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v123_phase2_checkpoint.json"  # v123 — remove "head toward" + "walk down the hallway" triggers
...
async def main():
    ...
    print(f"  v123: remove 'head toward' from MOTION WORDS, remove 'walk down the hallway' from DISTANCE")
    print(f"  v123: keep v122 motion-word improvements (_ws_variants, keep_going→go_straight)")
