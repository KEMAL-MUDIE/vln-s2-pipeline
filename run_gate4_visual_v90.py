#!/usr/bin/env python3
"""
Gate 4 Visual v90 — v89 with corrected hallway/continue assembly math

v89 BUG ANALYSIS:
  1. Hallway threshold WRONG DIRECTION: > 0.31 suppresses 69% (allows 31%), not 69%.
     Correct: `> 0.69: suppress` → P_suppress=0.31, P_allow=0.69.
     v89 result: hallway=14.8% (barely changed from v88's 14.2%; should have been ~20%).
  2. Continue P=0.35 overshot: "Walk forward" (caps) appears in ~62% of all episodes.
     0.35×62% = 21.7pp extra continue → 5.8%+21.7% = 27.5% (GT=10.4%).
     Correct P: (10.4-5.8)/(62%×(1-0.058)) = 4.6/58.4% ≈ 0.079 → use P=0.08.
     Expected: 5.8% + 0.08×58.4% = 5.8%+4.7% = 10.5% (close to GT).

v90 FIXES — assembly post-processing ONLY (LLM prompt IDENTICAL to v84/v86/v88):
  1. Hallway dir allow: `> 0.31` → `> 0.69` (suppress 31%, allow 69%).
     P_allow=0.69, pre-filter=29.6% → target=0.69×29.6%=20.4% ≈ GT=20.4%. Calibrated.
  2. Continue: P=0.35 → P=0.08 ("Walk forward" → "Continue forward" in 8% of episodes).
  3. P_THRU_MARKER stays 0.95 (through_the: 22.9% — needs further investigation).
  4. into_hallway: stays always allow (max ~0.5%; LLM generates too few entry phrases).
  Seed: 0x9045_5300. Fresh P2 checkpoint.

Phase 2 checkpoint: outputs/gate4_v90_phase2_checkpoint.json (FRESH — seed changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v90.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v90.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v90_phase2_checkpoint.json"  # FRESH — seed changed
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    rng = random.Random(eid ^ 0x9045_5300)  # v84: new seed
...
async def main():
    ...
    for ep in episodes:
        ...
        if "hallway" in text.lower():
            ...
            if _HALLWAY_DIR_RE.search(text):
                ep_rng = random.Random(eid ^ 0x9045_5300)
        ...
        if _WALK_TO_THE_RE.search(text):
            toward_rng = random.Random(eid ^ 0x9045_A300)
        ...
        cont_rng = random.Random(eid ^ 0x9045_C300)
    ...
    print(f"  [v90 targets: through_the ~23%, toward_the ~4%, hallway ~20%, continue ~10%]")
