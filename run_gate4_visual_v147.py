#!/usr/bin/env python3
"""
Gate 4 Visual v147 — Fix v146 bug: go_straight replacement AFTER continue restoration.

v146 ACTUAL: through_the=27.9%, hallway=20.1%, into_hallway=4.4%, continue=15.39%, walk_past=8.37%
             avg_words=28.4w, vocab_gap=6.33pp → pred_SR=60.03%
             go_straight=0.0% ✓ (fixed!) BUT continue=15.39% (GT=10.4%) — too high

v146 BUG: replace_go_straight_with_landmarks() ran BEFORE continue restoration.
  "Go straight" → "Walk straight" (fallback) → continue restoration fires on "Walk straight"
  → many "Walk straight" → "Continue straight" → continue inflated 9.68% → 15.39%

v147 FIX: Move replace_go_straight_with_landmarks() call to AFTER continue restoration.
  Continue restoration first fires on existing LLM "Walk forward/straight/ahead" outputs (rare).
  Then go_straight replacement runs: "Go straight" → "Walk to X" or "Walk straight".
  New "Walk straight" from replacement is NOT eligible for continue (restoration already ran).
  Expected: continue~9.7% (same as v145), go_straight~0-3%

  - Phase 1: SAME as v143 (gate4_v143_phase1_checkpoint.json)
  - Phase 2: REUSE v145 Phase 2 checkpoint (no new generation needed)
  - SAME improved extract_landmark_brief() as v146
  - SAME "walk straight" fallback as v146
  - FIXED ordering: go_straight replacement AFTER continue restoration

Phase 1 checkpoint: outputs/gate4_v143_phase1_checkpoint.json (same as v143/v145/v146)
Phase 2 checkpoint: outputs/gate4_v145_phase2_checkpoint.json (REUSED from v145/v146)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v147.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v147.json.gz"
...
async def main():
    ...
    print('=== Gate 4 v147: go_straight replacement AFTER continue restoration (fix v146 ordering bug) ===')
