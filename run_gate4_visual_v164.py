#!/usr/bin/env python3
"""
Gate 4 Visual v164 — Reduce Phase C furniture anchors to GT level + more unanchored turns.

ROOT CAUSE ANALYSIS (v24 vs v157-v163):
  GT anchored_turn=3.2%  (turn left AT the [furniture]) — almost never used by human annotators
  v157 anchored_turn=13.3% — Phase C generates 4× too many furniture anchors
  v24 anchored_turn=16.2% but avg_turns=1.53, unanchored=60.4% — MANY explicit unanchored turns
  Agent was trained on R2R (GT-style): knows unanchored turns, knows rooms, NOT furniture anchors
  Phase C "turn left at the grey sofa" is out-of-distribution → agent may navigate TOWARD sofa

KEY METRICS:
  v24 (SR=40.24%): avg_turns=1.53, anchored=16.2%, unanchored=60.4%, walk_into=52.6%, hallway=51.8%
  GT  (SR=63.30%): avg_turns=0.59, anchored=3.2%,  unanchored=27.0%, walk_into=12.2%
  v163 (pending):  avg_turns=0.83, anchored=?%,    unanchored=?%

v164 DESIGN:
  1. P_ANCHOR_EPISODE: 0.18 → 0.03 → anchored_turn≈3% (match GT's 3.2%)
     Phase C furniture anchors are harmful — agent treats "grey sofa" as navigation target
  2. P_TURN_UNANCHORED: 0.09 → 0.28 → more explicit unanchored turns (avg_turns≈1.0-1.2)
     More turn-by-turn guidance like v24, but using room context not furniture
  3. FRESH Phase 2 checkpoint (gate4_v164_phase2_checkpoint.json) — anchor rate changes route notation
  4. replace_go_straight_with_landmarks: 50% skip (balanced; fewer anchor → fewer "walk to X")
  5. P_continue: 0.28 (same as v163 — controlled continue)

Expected: avg_turns≈1.0-1.2, anchored_turn≈3%, go_straight≈10-15%, continue≈15%

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE)
Phase 2 checkpoint: outputs/gate4_v164_phase2_checkpoint.json (FRESH — anchor rate changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v164.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v164.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v164: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v164_phase2_checkpoint.json"  # v164: FRESH — P_ANCHOR changed (0.18→0.03), must regenerate
...
P_TURN_UNANCHORED = 0.28  # v164: 0.09→0.28 → more unanchored turns (avg_turns≈1.0-1.2, like v24=1.53 style)
...
async def main():
    ...
    print('=== Gate 4 v164: Phase C anchors 0.18→0.03(GT=3.2%), unanchored turns↑(P=0.28), FRESH ckpt ===')
    ...
    print(f"  v164: P_ANCHOR=0.03(GT=3.2%) + P_TURN_UNANCHORED=0.28 + 50%skip + P_continue=0.28 + FRESH")
