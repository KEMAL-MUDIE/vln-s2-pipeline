#!/usr/bin/env python3
"""
Gate 4 Visual v61 — P_TURN_UNANCHORED 0.42 + stronger hallway prompt rule

v60 RESULTS (base):
  GT-match: 0.952, toward_the: 2.1%, through_the: 28.9%, walk_past: 14.8%, hallway: 25.1%
  avg_turns: 0.63 (GT=0.59) — improved but still 0.04 over

v61 TWO FIXES:
  1. P_TURN_UNANCHORED: 0.45 → 0.42 (target avg_turns ≈ 0.59 matching GT exactly)
  2. Stronger ROOM NAMES rule in prompt — prohibit ALL room name usage mid-route:
     - Current rule listed "walk into/toward the hallway" but LLM still generates
       "walk through/down/along the hallway", "enter the hallway"
     - New rule: NEVER use hallway/bedroom/kitchen/living room/bathroom mid-route
     - Exception: final Stop/Wait phrase only if goal IS that room

Math (avg_turns):
  v60: P=0.45, avg_turns=0.63
  v61: P=0.42 → avg_turns ≈ 0.63 × (0.42/0.45) ≈ 0.59 (GT=0.59) — perfect!
  eps_with_turn: 50.1% × (0.42/0.45) ≈ 46.8% (GT=41.3%)

Expected hallway:
  v60: 25.1% (GT=20.4%), stronger prompt rule → ~20-22%
  Combined with v59 surgical post-processing → ~20-21%

Phase 2 checkpoint: outputs/gate4_v61_phase2_checkpoint.json (FRESH — P_TURN_UNANCHORED changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v61.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v61.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v61_phase2_checkpoint.json"  # FRESH — P_TURN_UNANCHORED changed!
...
- ROOM NAMES: NEVER use the words "hallway", "corridor", "bedroom", "kitchen", "living room", "dining room", or "bathroom" anywhere in the instruction EXCEPT in the final "Stop" or "Wait" sentence if that room IS the goal listed in "Goal:". Mid-route: do NOT write "walk into the hallway", "walk through the hallway", "walk down the hallway", "go through the hallway", "enter the hallway", "head down the hallway", or ANY other phrase with a room name. Write "walk forward" instead.
...
async def main():
    ...
    print('=== Gate 4 v61: P_TURN_UNANCHORED 0.42 + stronger hallway rule (NEVER mid-route) ===')
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v60=0.63) ← v61: P_TURN_UNANCHORED 0.42")
    ...
    print(f"  toward the:    {100*toward_n/n:.1f}%  (GT=3.8%, v60=2.1%) ← KEY METRIC")
    print(f"  hallway:       {100*hallway_n/n:.1f}%  (GT=20.4%, v60=25.1%) ← KEY METRIC")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v60=0.952, v59=0.958, v54fix=0.961)")
