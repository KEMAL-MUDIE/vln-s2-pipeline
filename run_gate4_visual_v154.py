#!/usr/bin/env python3
"""
Gate 4 Visual v154 — Remove "toward the X" everywhere + extract scene from Phase C for visual context.

v153 ACTUAL: GT-match=0.855, turn-anchor=23.5%, toward_the=37.0%(GT=3.8%), walk_straight=18.4%
  Root causes in v153:
  1. toward_the=37%: "walk toward the X" still in MOTION WORDS + "→ [object ahead]" rule.
     v150 had toward_the=0.9% before v151 added "walk toward the X" → it's the EXACT cause.
  2. turn-anchor=23.5% still: Phase C "Turn through [opening]" in visual context treated as anchor directive.
     v153 only showed 1st sentence for these, but "Turn left through the plain doorway into the hallway."
     IS a turn instruction — Phase 2 still anchors the turn.
  3. walk_straight=18.4% still: DISTANCE medium removed "walk straight" but still high.
     Root: assembly post-proc replaces some phrases with "walk straight". Need to check.

v154 CHANGES from v153:
  1. Remove "toward the X" completely from MOTION WORDS (use only "walk to the X").
  2. Change "→ [object ahead]" rule: use only "walk to the X" or "walk past the X" — no "toward".
  3. build_visual_narrative: extract_scene_from_desc() converts Phase C turn directives to neutral:
     - "Turn left through the plain doorway into the hallway." → "A plain doorway leads into the hallway."
     - "Turn right at the brown sofa. Ahead, a hallway leads..." → "Ahead, a hallway leads..."
     This completely eliminates the turn-directive confusion in visual context.
  4. DISTANCE medium: "walk" only (same as v153).
  5. Reuse v152 Phase C checkpoint.

Phase 1 checkpoint: outputs/gate4_v152_phase1_checkpoint.json (REUSE: room-context Phase C)
Phase 2 checkpoint: outputs/gate4_v154_phase2_checkpoint.json (FRESH)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v154.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v154.json.gz"
P1_CKPT       = ROOT / "outputs" / "gate4_v152_phase1_checkpoint.json"  # v154: REUSE v152 room-context Phase C
...
P2_CKPT       = ROOT / "outputs" / "gate4_v154_phase2_checkpoint.json"  # v154: FRESH — remove toward + scene extraction
...
- TURN LANDMARKS: When visual context describes a doorway or opening, write the turn as "turn left/right through the doorway into the [room]" (GT style). When it describes a room, write "turn left/right into the [room]". Prefer doorway/room phrasings over object anchors.
...
async def main():
    ...
    print('=== Gate 4 v154: Remove toward_the everywhere + extract_scene_from_desc for visual context ===')
    print(f"  v153 METRICS: GT-match=0.855, turn-anchor=23.5%, toward_the=37.0%(GT=3.8%), walk_straight=18.4%")
    print(f"  v150 BASELINE: toward_the=0.9% — v151 added 'walk toward the X' → jumped to 27.5%→37%")
    print(f"  v24 ACTUAL: 40.24% SR. take_few=0.3%, go_straight=23%, hallway=51.8%")
    print(f"  GT: take_few=0.2%, go_straight=7.9%, toward_the=3.8%, hallway=20.5%")
    print(f"  v154 FIX 1: Remove 'walk toward the X' from MOTION WORDS (use only 'walk to the X')")
    print(f"  v154 FIX 2: Remove 'toward' from '→ [object ahead]' rule (use only 'walk to the X')")
    print(f"  v154 FIX 3: extract_scene_from_desc() — converts 'Turn left through doorway into hall.'")
    print(f"              → 'A doorway leads into the hallway.' (no turn directive confusion for Phase 2)")
    print(f"  v154 FIX 4: DISTANCE medium same as v153 (walk/continue)")
    print(f"  Expected: toward_the<5%(GT=3.8%), turn-anchor~17%, walk_straight~8%, GT-match>0.95")
    ...
    print(f"  Phase 1: REUSE v152 room-context Phase C (room-context, 3192 descs, used as neutral scene)")
    ...
    print(f"  [v154 targets: toward_the<5%(GT=3.8%), turn-anchor~17%(GT=16.5%), walk_straight<10%, GT-match>0.95, SR target ≥42%]")
