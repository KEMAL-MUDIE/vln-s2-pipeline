#!/usr/bin/env python3
"""
Gate 4 Visual v76 — v75 + walk-past calibration + material strip fix

v75 ISSUES:
  GT-match: 0.993, avg_words: 25.6 (GT=26.8)
  "walk past the": 16.9% (GT=8.7%) — 2x OOD! New artwork midpoints all "pass" type.
  "wooden": 8.2% (GT=3.3%) — goal-approach stop phrases bypass material stripper.

v76 FIXES:
  1. P_PASS_MARKER: 0.30 (was 0.50) → targets ~10% walk-past-the (GT=8.7%)
  2. Apply strip_material_adjectives() to goal-approach stop desc before building phrase
  3. Fix _GENERIC_WPT_OBJ_RE: add wall(?:s)?, light switch, outlet to generic list

Everything else identical to v75 (same midpoints, same approach-view landmarks, same stops).

Phase 2 checkpoint: outputs/gate4_v76_phase2_checkpoint.json (FRESH — P_PASS_MARKER changed)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v76.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v76.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v76_phase2_checkpoint.json"  # FRESH — P_PASS_MARKER changed
