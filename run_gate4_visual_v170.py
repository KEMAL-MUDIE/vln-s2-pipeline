#!/usr/bin/env python3
"""
Gate 4 Visual v167 -- Calibrate anchor% to GT=16.5% + fix "[Turn right ahead]" bug.

ROOT CAUSE ANALYSIS (v166 -> v167):
  v166: anchor%=23.2% (GT=16.5%), GT-match=0.8607. Still overcorrected.
  Calibration math:
    v164 (P_DIR=0.00): anchor%=6.1%  (hallway+furniture baseline)
    v166 (P_DIR=0.20): anchor%=23.2% (non-hallway adds +17.1pp)
    v167 target: anchor%=16.5% -> need non-hallway to add +10.4pp
    Scale: P_DIR_new = 0.20 x (10.4/17.1) = 0.20 x 0.609 = 0.12

  FIX 2: "[Turn right ahead]" bug
    Phase C descriptions sometimes include nav verbs ("Turn right through doorway")
    extract_ahead_noun extracts "Turn right" as the ahead noun
    Route gets "-> [Turn right ahead]" -> LLM writes "walk to the Turn right" (WRONG!)
    Fix: Filter ahead labels starting with navigation verbs (turn/go/walk/move/proceed)

KEY METRICS:
  v164 anchor%=6.1%  -- no P_DIR (baseline: hallway+furniture only)
  v166 anchor%=23.2% -- P_DIR=0.20 (overcorrected)
  v167 anchor%~16.5% -- P_DIR=0.12 (calibrated to GT)
  GT   anchor%=16.5% -- target

v167 DESIGN:
  1. Furniture filter in [pass:] -- kept from v165 (prevents armchair/sofa confusion)
  2. P_DIR_ROOM_TRANS=0.12 (calibrated from v166 measurement: 6.1+10.4=16.5%)
  3. P_TURN_UNANCHORED=0.30 -- kept from v166 (same turn density)
  4. P_ANCHOR_EPISODE=0.03 -- furniture anchors at GT level
  5. Filter nav-verb ahead labels: if ahead starts with turn/go/walk/move/proceed -> ""
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v170.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v170_phase2_checkpoint.json"  # v170: FRESH — P_ANCHOR=0.08 + no-color system prompt + positional landmarks
...
P_ANCHOR_EPISODE = 0.08   # v170: recalibrated from v169=0.13→anchor%=22.5%. Target 16.5%.
                           # Calibration: v169(0.13)→22.5%, need -6pp → 0.08 estimated → anchor%≈16-17%
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None,
    ...
    for p in primitives:
        ...
        if t == "straight":
            ...
            # v170: Positional landmark injection for hallway turns (GT prefers structural refs)
            # GT top landmarks: "end of" (98), "stairs" (45), "entrance" (29) — not furniture!
            # If no visual landmark but in hallway room → inject "end of the hallway"
            # This gives the agent a reliable structural reference that's always at hallway turns
...
async def main():
    ...
    print('=== Gate 4 v170: anchor% CALIBRATED + NO-COLOR PROMPT + POSITIONAL LANDMARKS ===')
    print(f"  v169 MEASURED: anchor%=22.5% (too high, GT=16.5%), avg_turns=1.08, doorway=21.9%(GT!)")
    print(f"  v169 GOOD: avg_turns improved 0.64→1.08, doorway matched GT=20.6%")
    print(f"  FIX 1: P_ANCHOR_EPISODE=0.08 (was 0.13→22.5%; 0.08 estimated→~16.5%)")
    print(f"  FIX 2: ANTI-COLOR system prompt (GT color=15%, v169=32%+ — LLM still adding colors!)")
    print(f"         'Do NOT add color words (white, brown, dark, grey)' in RULES + STOP PHRASE")
    print(f"  FIX 3: POSITIONAL LANDMARKS (GT top: 'end of hallway'×98, 'stairs'×45, 'entrance'×29)")
    print(f"         Hallway turns without landmark → inject 'end of the hallway'")
    print(f"         Post-stairs turns without landmark → inject 'top/bottom of the stairs'")
    print(f"  KEPT: P_TURN_UNANCHORED=1.0 (all turns), (door→room)=45%, nav-verb fixes")
    print(f"  v170: Expected anchor%≈16-17%, avg_turns≈1.0-1.1, color%≈10-15%, door%≈25-30%")
