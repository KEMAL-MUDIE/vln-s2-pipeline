#!/usr/bin/env python3
"""
Gate3-Informed Gemma Annotation v5 — GT-aligned init_turn threshold at 90°.

Key insight from GT analysis:
- GT annotators skip explicit init_turn in ~50% of episodes even for 90°+ angles
- Model (trained on GT) handles implicit initial orientation through visual context
- v4 threshold=45° still gives explicit turns for ~76% episodes (GT: 44%)
- Setting threshold=90° aligns better: ~55% of episodes will have NO explicit turn

Key improvements over v4:
- init_turn threshold: 90° (vs v4: 45°)
  → For 30-90° angles: NO "Initial facing" in context (GT often skips these)
  → For 90-150°: "Initial facing: sharp X" (GT uses ~47% of the time for these)
  → For 150+°: "Initial facing: AROUND" (GT uses ~47% for these)
- Expected pct_zero_explicit: ~40-50% (GT=56%, v4=24.3%)
- Expected avg_explicit_turns: ~0.55-0.65 (GT=0.66, v4=0.76)
- All other v4 improvements retained: room-sequence, priority turns, triplet diversity

Output: outputs/datasets/val_unseen_gate3_gemma_v5.json.gz
Checkpoint: outputs/gate3_gemma_v5_checkpoint.json
"""
import asyncio
import gzip
import json
...
OUTPUT_PATH = ROOT / "outputs" / "datasets" / "val_unseen_gate3_gemma_v5.json.gz"
CHECKPOINT_PATH = ROOT / "outputs" / "gate3_gemma_v5_checkpoint.json"
...
# GT analysis: annotators skip explicit turns for 45-90° episodes ~55% of the time.
# At 90°: agent can visually correct ≤90° orientation using door/hallway cues.
# At 90°+: sharp/around turns are explicitly needed.
...
def _is_key_turn_v5(pf_turn: dict, prim: dict, prev_rooms: set) -> Tuple[bool, int]:
    """
...
def build_context_v5(episode_id: int, ep: dict, perframe: dict, landmark: dict) -> Optional[dict]:
    """v5 context builder: init_turn threshold=90°, room-sequence, priority turns."""
    ...
    # Initial facing — only include for angles >= 90° (GT skips ~55% even at 90°+).
    # For 30-90°: agent can visually correct orientation; GT annotators often skip.
    # For 90°+: sharp/around turns that agent cannot resolve visually.
...
async def main():
    ...
    print("=" * 70)
    print("Gate3-Informed Gemma Annotation v5 — val_unseen")
    print("v5: init_turn threshold=90°, triplet temperature diversity, priority turns")
    print("=" * 70)
    ...
    for ep in episodes:
        ...
        if not perframe:
            missing_gate3 += 1
        ctx = build_context_v5(int(eid), ep, perframe, landmark)
        if ctx:
    ...
    dataset = {
        ...
        "_generation_meta": {
            ...
            "avg_words": avg_words,
            "v5_changes": "init_turn threshold=90deg (GT-aligned), triplet diversity, priority key-turns",
        },
    }
    save_dataset(dataset, OUTPUT_PATH)

    NVME_PATH = Path("/mnt/nvme0/vln_habitat/habitat_data/datasets/vln/mp3d/r2r/v1/val_unseen/val_unseen_gate3_gemma_v5.json.gz")
