#!/usr/bin/env python3
"""
Gate 4 Visual v31 — Strict [pass:] Whitelist + No [pass:] in Goal Segment

IMPROVEMENTS OVER v30:
  1. STRICT WHITELIST for [pass:] midpoints:
     Old: is_generic_pass_object() returns True only if in generic list, else False.
          Problem: "potted plant", "support beam", "large vase" aren't in EITHER list
          → passes filter → "walk past the potted plant" (useless landmark).
     New: is_generic_pass_object() returns False ONLY if in distinctive list.
          Everything else is filtered (exclusive whitelist, not inclusive blacklist).
          Expected: eliminates spurious landmarks not in distinctive list.
  2. NO [pass:] FOR GOAL SEGMENT:
     Goal-approach segment ([seg_midpoint["goal"]]) only emits [thru:] midpoints.
     [pass:] markers right before stop phrases may confuse VLN model.
     Analysis: 268/495 v30 "walk past the" instances were in last 2 sentences.
     GT: 100/160 walk-past-the are before stop (62.5%) so some is expected,
     but we're generating 2.68x more than GT in that position.
     Fix: goal segment only allows [thru:], never [pass:].
  3. All v30 innovations retained:
     - Material filter for goal_lm before choose_stop() (wooden=0.1%)
     - Post-process "passing the X" → "walk past the X" (0% pure passing the)
     - Material adjective filter in clean() and build_route_desc()
     - Fixed GT-match metric (TURN_ANCHOR_RE only)
     - Turn threshold 70deg, straight threshold 2m
     - Anti-loop, path-budget, door landmarks

  GT analysis reference (v30 actuals):
    "through the":   GT=27.2%, v30=33.3%  (slightly above GT)
    "walk past the": GT=8.7%,  v30=26.9%  → v31 target: ~10-13%
    "wooden":        GT=3.3%,  v30=0.1%   ✓ (essentially eliminated)
    "continue":      GT=10.4%, v30=9.7%   ✓
    GT-match:        v30=0.947 ✓

  Phase 2 checkpoint: outputs/gate4_v31_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v31.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v31.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v31_phase2_checkpoint.json"
...
def build_route_desc(primitives, sel_turns, seg_midpoint: Optional[Dict[str, str]] = None):
    ...
    # v31: goal segment ONLY emits [thru:] (not [pass:]).
    # [pass:] right before stop phrase may confuse VLN model (should I walk past or stop?).
    # Analysis: 268/495 v30 "walk past the" were in last 2 sentences (before stop/wait).
    # GT has 100/160 before stop, but we were generating 2.68x more → filter goal [pass:].
    if seg_midpoint and "goal" in seg_midpoint:
        mid_desc = strip_material_adjectives(seg_midpoint["goal"])
        action = classify_pass_action(mid_desc)
        if action == "thru":  # v31: only [thru:] allowed for goal segment
            parts.append(f"[{action}: {mid_desc}]")
...
async def main():
    ...
    print("=== Gate 4 v31: Strict [pass:] Whitelist + No [pass:] in Goal Segment ===")
    ...
    print(f"  v31 key changes: (1) strict whitelist for [pass:] (only in _DISTINCTIVE_OBJECTS_RE),"
          f" (2) goal segment: only [thru:] allowed, never [pass:]."
          f" Predicts walk-past-the: 10-13% (was 26.9% v30, GT=8.7%)")
    print()
    ...
    print(f"  walk past the: {100*walkpast_n/n:.1f}%  (GT=8.7%) ← KEY METRIC (v30=26.9%)")
    print(f"  passing the:   {100*passing_n/n:.1f}%  (GT=5.1%) ← combined; pure passing the=0%")
    print(f"  continue:      {100*continue_n/n:.1f}%  (GT=10.4%)")
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v31 should be <1%")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v30=0.947, v26=0.955)")
    ...
    print(f"  [Note: v31 strict [pass:] whitelist + no [pass:] for goal segment]")
