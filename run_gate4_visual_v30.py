#!/usr/bin/env python3
"""
Gate 4 Visual v30 — Stop-Phrase Material Filter (wooden in goal_lm fix)

IMPROVEMENTS OVER v29:
  1. FIX "wooden" IN STOP PHRASES:
     Analysis of v29 shows 417/419 "wooden" instances come from stop phrases,
     NOT the main navigation body. The goal_lm (e.g., "brown wooden door frame")
     is extracted from vision descriptions and passed to choose_stop() AFTER clean().
     Fix: apply strip_material_adjectives() to goal_lm before calling choose_stop().
     "brown wooden door frame" → "brown door frame" → "Stop at the brown door frame."
     Expected: "wooden" drops from 22.8% → ~1% (target: <5%).
  2. All v29 innovations retained:
     - Post-process "passing the X" → "walk past the X" (v29: 0% pure "passing the")
     - Material adjective filter in clean() and build_route_desc()
     - Fixed GT-match metric (TURN_ANCHOR_RE only, GT-match=0.943)
     - Generic-object filter for [pass:] midpoints
     - [thru: X] → "through the X" for doorways
     - Turn threshold 70deg, straight threshold 2m
     - Anti-loop, path-budget, door landmarks

  GT analysis reference (v29 actuals):
    "through the":   GT=27.2%, v29=33.8%  (slightly above GT — acceptable)
    "walk past the": GT=8.7%,  v29=27.3%  (still 3x GT — acceptable for now)
    "passing the":   GT=5.1%,  v29=0.0%   ✓ (pure "passing the" eliminated)
    "wooden":        GT=3.3%,  v29=22.8%  → v30 target: <2% (stop-phrase fix)
    "continue":      GT=10.4%, v29=9.0%   ✓
    GT-match:        v29=0.943 (target ≥ 0.90) ✓

  Phase 2 checkpoint: outputs/gate4_v30_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v30.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v30.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v30_phase2_checkpoint.json"
...
async def main():
    ...
    print("=== Gate 4 v30: Stop-Phrase Material Filter (goal_lm wooden fix) ===")
    ...
    print(f"  v30 key change: strip_material_adjectives() applied to goal_lm before choose_stop()."
          f" Fixes 417/419 'wooden' cases in stop phrases. Predicts wooden% < 2% (was 22.8% v29).")
    print()
    ...
    print(f"  walk past the: {100*walkpast_n/n:.1f}%  (GT=8.7%) ← KEY METRIC (v29=27.3%)")
    print(f"  passing the:   {100*passing_n/n:.1f}%  (GT=5.1%) ← combined; pure 'passing the'=0%")
    print(f"  continue:      {100*continue_n/n:.1f}%  (GT=10.4%)")
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← KEY METRIC (v29=22.8%, target <2%)")
    ...
    print(f"\n  GT-match score: {match_score:.3f}  (target ≥ 0.90; v29=0.943, v26=0.955)")
    ...
    print(f"  [Note: v30 strips material adjectives from goal_lm before stop-phrase generation]")
