#!/usr/bin/env python3
"""
Gate 4 Visual v46 — Stronger room suppression: eliminate hallway/living room as waypoints

v39 RESULTS:
  GT-match: 0.968, through_the: 27.0%, walk_past: 10.5%, continue: 9.1%, proceed: 1.7%
  dup-stop: 0 (FIXED)
  avg_words: 22.9 (GT=26.8)

NEW ISSUES FOUND IN v39:
  1. Fragment sentences (74 eps): "into the living room." / "through the doorway."
     - LLM splits navigation at periods, leaving incomplete preposition-only sentences
     - Fix: remove sentences starting with prepositions/conjunctions without a verb
  2. Truncated stop phrases (50 eps): "Stop in front of the white shelf holding decorative."
     - LLM starts describing landmark attributes but leaves noun incomplete
     - Examples: "holding decorative." / "featuring two dark." / "beside a brown."
     - Fix: strip trailing dangling adj/preposition phrase from stop landmark label

v40 FIXES:
  1. `_FRAGMENT_SENT_RE`: detect and remove sentences that are pure preposition phrases
     (start with into/through/and/or/but/at/in/via + noun phrase, no main verb)
  2. `_TRUNCATED_ATTR_RE`: in stop phrases, strip "holding/featuring/with/beside a [adj]."
     that ends without a proper noun → truncate to core landmark

  All v39 innovations retained:
    - dup-stop fix (and stop/wait fragment deletion from ALL sentences)
    - proceed elimination (43.8%->1.7%)
    - WPT substitution "walk forward" (continue 18.5%->9.1%)
    - through_the hallway->down, [room]->into
    - P_PASS=0.50, P_THRU=0.85
    - Generic WPT suppression, material filter, GT-match metric
    - Fragment removal, duplicate motion collapse, 3-sentence cap

  Expected:
    dup-stop:        0 (maintained)
    GT-match:        ~0.968 (maintained)
    fragment_sents:  0 (was 74)
    truncated_stops: 0 (was 50)

  Phase 2 checkpoint: outputs/gate4_v46_phase2_checkpoint.json
  Output: outputs/datasets/val_unseen_generated_gemma_visual_v46.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v46.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v46_phase2_checkpoint.json"
...
- CRITICAL: Do NOT add "turn left" or "turn right" unless the route explicitly shows "turn left" or "turn right". If the route has no turn, do NOT write one. Describe the movement as "walk forward", "walk straight", or "walk toward [goal]" instead.
...
- ROOM NAMES: NEVER write "walk into the hallway", "walk into the living room", "walk toward the hallway", "enter the hallway", or any phrase that names a room as a waypoint. Write "walk forward" or "walk straight" instead. ONLY name a room (hallway, living room, bedroom, kitchen, bathroom) when it IS the explicitly labeled FINAL destination in the "Goal: X" line. If the route uses (→ room) notation mid-route, ignore the room name and write "walk forward".
...
async def main():
    ...
    print('=== Gate 4 v46: Stronger room suppression — no hallway/room as waypoints ===')
    ...
    print(f"  avg_turns/ep:  {avg_turns:.2f}  (GT=0.59, v43=1.03) ← v44 KEY FIX")
    ...
    print(f"  wooden:        {100*wooden_n/n:.1f}%  (GT=3.3%) ← v46 should be <1%")
    ...
    print(f"  [Note: v46: fresh P2 + stronger room suppression (hallway/room as waypoints) — v45 hallway went UP to 45.1% due to turn→room substitution")
