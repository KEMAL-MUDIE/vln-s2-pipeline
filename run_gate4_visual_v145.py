#!/usr/bin/env python3
"""
Gate 4 Visual v145 — Post-process go_straight → Phase C landmark replacements (safe approach).
...
v143 EXTENDED: go_straight=80.91% (GT=7.94%, +72.97pp!) — structural DISTANCE rule issue

v144 ATTEMPT: Phase C landmark injection + prompt changes → FAILED
  - toward_the=38.61% (GT=3.75%) — "walk toward the X" caused 10× overshoot
  - continue=26.43% (GT=10.40%) — LLM used "continue" as connector between landmark steps

v145 STRATEGY: Post-processing only — no prompt changes, same calibrated Phase 2 rules as v143.
  - Phase 1: SAME as v143 (gate4_v143_phase1_checkpoint.json from auto_annotator Phase C)
  - Phase 2: FRESH generation with IDENTICAL prompt to v143 (no rule changes at all)
  - POST-PROCESS: After Phase 2 generates instruction, replace "go straight" → "walk to the [landmark]"
    using Phase C destination labels in route order (turn_1, turn_2, ..., goal)
  - Safe: no prompt changes → 5-metric calibration unchanged

Expected v145:
  - 5-metric: same as v143 (through_the≈27.4%, hallway≈20.8%, etc.)
  - go_straight: 80.91% → ~15-30% (only when Phase C landmarks run out)
  - toward_the: ~1% (unchanged from v143)
  - vocab_gap: ~0.8pp → pred_SR≈63.4%

Phase 1 checkpoint: outputs/gate4_v143_phase1_checkpoint.json (same as v143)
Phase 2 checkpoint: outputs/gate4_v145_phase2_checkpoint.json (FRESH generation)
Output: outputs/datasets/val_unseen_generated_gemma_visual_v145.json.gz
"""
import asyncio
import base64
import gzip
...
OUTPUT        = ROOT / "outputs" / "datasets" / "val_unseen_generated_gemma_visual_v145.json.gz"
...
P2_CKPT       = ROOT / "outputs" / "gate4_v145_phase2_checkpoint.json"  # v145: FRESH generation (post-proc go_straight fix)
...
def extract_landmark_brief(desc: str) -> Optional[str]:
    ...
    # Goal: "The grey lounge chair positioned..." → "grey lounge chair"
...
def replace_go_straight_with_landmarks(text: str, phase_c_descs: Dict[str, str]) -> str:
    """v145: Replace 'go straight' occurrences with Phase C landmark alternatives.
    Uses turn_1, turn_2, ..., goal in route order. Safe: no prompt changes needed.
    """
    if not phase_c_descs or not text:
        return text
    # Build ordered list of destination landmarks from Phase C
    landmarks = []
    for i in range(1, 15):  # support up to 14 turns
        label = f"turn_{i}"
        if label in phase_c_descs:
            lm = extract_landmark_brief(phase_c_descs[label])
            if lm:
                landmarks.append(lm)
    goal_desc = phase_c_descs.get("goal", "")
    goal_lm = extract_landmark_brief(goal_desc)
    if goal_lm:
        landmarks.append(goal_lm)
    if not landmarks:
        return text
    # Replace "go straight" / "Go straight" occurrences in order
    lm_idx = [0]
    def _replace_gs(m):
        ...
        return m.group(0)  # no more landmarks → keep original "go straight"
    return re.sub(r'\bgo straight\b', _replace_gs, text, flags=re.IGNORECASE)
...
async def main():
    ...
    print('=== Gate 4 v145: Post-process go_straight → Phase C landmarks (safe, no prompt changes) ===')
    ...
    for ep in episodes:
        ...
        # v145: Post-process go_straight → Phase C landmark alternatives (safe, no prompt changes)
